...
suite('Language Server Download Channels', () => {
    ...
    function getPackageInfo(channel) {
        ...
        const instance = new class extends classToCreate {
            ...
            get storageAccount() { return this.azureBlobStorageAccount; }
            get storageContainer() { return this.azureBlobStorageContainer; }
        }();
        return [instance.storageAccount, instance.storageContainer];
    }
    test('Stable', () => {
        chai_1.expect(getPackageInfo(languageServerPackageRepository_1.LanguageServerDownloadChannel.stable)).to.be.deep.equal(['https://pvsc.blob.core.windows.net', 'python-language-server-stable']);
    });
    test('Beta', () => {
        chai_1.expect(getPackageInfo(languageServerPackageRepository_1.LanguageServerDownloadChannel.beta)).to.be.deep.equal(['https://pvsc.blob.core.windows.net', 'python-language-server-beta']);
    });
    test('Daily', () => {
        chai_1.expect(getPackageInfo(languageServerPackageRepository_1.LanguageServerDownloadChannel.daily)).to.be.deep.equal(['https://pvsc.blob.core.windows.net', 'python-language-server-daily']);
    });
});
