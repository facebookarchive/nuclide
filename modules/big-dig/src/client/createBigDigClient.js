...
exports.default = (() => {
  var _ref = (0, _asyncToGenerator.default)(function* (config) {
    ...
    if (!config.ignoreIntransientErrors) {
      nuclideSocket.onIntransientError(function (error) {
        return nuclideSocket.close();
      });
    }

    const client = new (_BigDigClient || _load_BigDigClient()).BigDigClient(nuclideSocket, nuclideSocket.getHeartbeat());
    try {
      // Make sure we're able to make the initial connection
      yield nuclideSocket.testConnection();
      return client;
    } catch (error) {
      client.close();
      throw error;
