...
class ManagedPackage {
  ...
  async verifyInstallation(ssh) {
    ...
    try {
      ...
      const manifest = (0, (_Manifest || _load_Manifest()).deserializeManifest)((await sftp.readFile(manifestFile)));
  ...
  async _saveManifest(version, sftp) {
    ...
    const current = (0, (_Manifest || _load_Manifest()).createManifest)(version, this._manifestPath(), installPath, currentFileTree);
  ...
  async _extractFromStdin(archive, extract, ssh) {
    ...
    let output = '';
    stdout.subscribe(data => output += data);
    const [, { code }] = await Promise.all([this._transferViaStream(archive, stdio), result]);
    if (code !== 0) {
