...
(0, _globals().describe)('ThriftClientManager', () => {
  ...
  (0, _globals().it)('create thrift client Case 1: failed to start server', async () => {
    ...
    await (0, _globals().expect)(manager.createThriftClient(mockedServiceName)).rejects.toThrow('failed to start server');
  });
  (0, _globals().it)('create thrift client Case 2: failed to create tunnel', async () => {
    ...
    manager = new (_ThriftClientManager().ThriftClientManager)(mockedTransport, mockedTunnelManager);
    await (0, _globals().expect)(manager.createThriftClient(mockedServiceName)).rejects.toThrow(mockedFailureMessage);
  });
  ...
  (0, _globals().it)('reuse existing tunnel', async () => {
    ...
    mockClientServerCommunication(clientMessage, serverMessage);
    await manager.createThriftClient(mockedServiceName); // create second client
  ...
  (0, _globals().it)('close ThriftClientManager instance', async () => {
    ...
    await manager.close(); // due to async

    (0, _globals().expect)(callServer.mock.calls.length).toBeGreaterThanOrEqual(4);
    (0, _globals().expect)(callServer.mock.calls.length).toBeLessThanOrEqual(6);
  });
});
