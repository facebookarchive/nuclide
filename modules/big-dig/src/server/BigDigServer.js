...
class BigDigServer {
  ...
  _onWebSocketConnection(ws, req) {
    ...
    if (headerClientId != null) {
      this._logger.info(`received clientId ${headerClientId} in header, not waiting for first clientId message`);
      this._handleClientId(ws, headerClientId);
    } else {
      this._logger.info('did not receive clientId in header, will wait for first message');
