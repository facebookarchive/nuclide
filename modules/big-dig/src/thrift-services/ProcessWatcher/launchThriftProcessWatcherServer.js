...
import os from 'os';
import {ThriftProcessWatcherServer} from './ThriftProcessWatcherServer';
...
async function main() {
  ...
  const server = new ThriftProcessWatcherServer(portOrPath);
...
log4js.configure({
  appenders: [
    {
      type: 'file',
      filename: nuclideUri.join(
        os.tmpdir(),
        'big-dig-service-process-watcher.log',
      ),
    },
    {
      type: 'stderr',
