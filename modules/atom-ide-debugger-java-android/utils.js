...
async function resolveConfiguration(configuration) {
  ...
  const adbServiceUri = config.adbServiceUri != null ? config.adbServiceUri : targetUri;
  ...
  if (debugMode === 'launch') {
    ...
    , adbServiceUri, service || null, activity || null, intent || null /* intent and action are the same */
    ...
    pid = (await (0, (_AndroidJavaDebuggerHelpers || _load_AndroidJavaDebuggerHelpers()).launchAndroidServiceOrActivityAndGetPid)(selectedProcessPid, adbServiceUri, null, null, null, device, selectedProcess.name)).pid;
  }
  ...
  const attachPortTargetConfig = await (0, (_AndroidJavaDebuggerHelpers || _load_AndroidJavaDebuggerHelpers()).getAdbAttachPortTargetInfo)((0, (_nullthrows || _load_nullthrows()).default)(device), adbServiceUri, targetUri, (0, (_nullthrows || _load_nullthrows()).default)(pid), subscriptions);
