...
function _load_Breakpoints() {
  return _Breakpoints = require('./Breakpoints');
}

var _Breakpoints2;

function _load_Breakpoints2() {
  return _Breakpoints2 = _interopRequireDefault(require('./Breakpoints'));
}
...
class SourceBreakpoints {
  ...
  _cacheBreakpointsInConfiguration(path, addRemove) {
    ...
    addRemove.addBreakpoints.forEach((breakpoint, index) => {
      ...
      const newBreakpoint = new (_Breakpoints || _load_Breakpoints()).Breakpoint(null, path, line, breakpoint.condition, false);
  ...
  async _addRemoveBreakpointsViaProxy(path, addRemove) {
    ...
    if (failure != null) {
      throw new Error(`Failed adding new source breakpoints ${(0, (_MITypes || _load_MITypes()).toCommandError)(failure).msg}`);
    }
    ...
    results.forEach((response, index) => {
      ...
      const breakpoint = new (_Breakpoints || _load_Breakpoints()).Breakpoint(parseInt(bkpt.number, 10), path, line, addRemove.addBreakpoints[index].condition, bkpt.pending == null);
