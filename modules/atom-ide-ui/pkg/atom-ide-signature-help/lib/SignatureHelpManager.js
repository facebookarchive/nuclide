...
const SIGNATURE_TIMEOUT = 2500;class SignatureHelpManager {constructor() {this._commands = new _rxjsBundlesRxMinJs.Subject();
  ...
  _signatureHelpTriggers(editor) {
    ...
    .let((0, (_observable || _load_observable()).fastDebounce)(0)).
    filter(edit => {
  ...
  _getSignatureStream(editor) {
    ...
    _rxjsBundlesRxMinJs.Observable.defer(() => _rxjsBundlesRxMinJs.Observable.of(editor.getCursorBufferPosition())),
    // Further cursor changes will be debounced.
    (0, (_textEditor || _load_textEditor()).getCursorPositions)(editor).let((0, (_observable || _load_observable()).fastDebounce)(CURSOR_DEBOUNCE_TIME))).
    ...
    .takeUntil(
    _rxjsBundlesRxMinJs.Observable.fromEvent(editor.getElement(), 'keydown').filter(
    evt => evt.keyCode === 27)),
  ...
  point)
  {
    ...
    _rxjsBundlesRxMinJs.Observable.from(this._providerRegistry.getAllProvidersForEditor(editor))).

    concatMap(provider => {
      return provider.getSignatureHelp(editor, point).catch(err => {
        const editorPath = editor.getPath() || '<untitled editor>';
        (0, (_log4js || _load_log4js()).getLogger)('atom-ide-signature-help').error(
    ...
    timeoutWith(SIGNATURE_TIMEOUT, _rxjsBundlesRxMinJs.Observable.of(null)).
    take(1).
    defaultIfEmpty(null);
  }
  ...
  signatureHelp)
  {
    return _rxjsBundlesRxMinJs.Observable.defer(() => {
      ...
      return (
        ...
        let(
        (0, (_observable || _load_observable()).completingSwitchMap)(point => {
          return _rxjsBundlesRxMinJs.Observable.create(() => {
            const disposable = datatipService.createPinnedDataTip(
            (0, (_getSignatureDatatip || _load_getSignatureDatatip()).default)(signatureHelp, point),
            editor,
            {
