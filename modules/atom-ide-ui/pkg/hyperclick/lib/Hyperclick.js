'use strict';Object.defineProperty(exports, "__esModule", { value: true });var _asyncToGenerator = _interopRequireDefault(require('async-to-generator'));var _HyperclickForTextEditor;
...
function _load_HyperclickForTextEditor() {return _HyperclickForTextEditor = _interopRequireDefault(require('./HyperclickForTextEditor'));}var _SuggestionList;
...
/**
  ...
  observeTextEditor(textEditor) {
    const hyperclickForTextEditor = new (_HyperclickForTextEditor || _load_HyperclickForTextEditor()).default(
    textEditor,
    this);
  ...
  {var _this = this;return (0, _asyncToGenerator.default)(function* () {
      for (const provider of _this._providers.getAllProvidersForEditor(
      textEditor))
      {
        let result;
        if (provider.getSuggestion) {
          ...
          const { wordMatch, range } = match;if (!
          provider.getSuggestionForWord) {throw new Error('Invariant violation: "provider.getSuggestionForWord"');}
          // eslint-disable-next-line no-await-in-loop
          result = yield provider.getSuggestionForWord(
          textEditor,
          wordMatch[0],
          range);
