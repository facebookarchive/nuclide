...
class Activation {
  ...
  constructor(state) {
    ...
    const removedHostnames = (0, (_event || _load_event()).observableFromSubscribeFunction)(atom.project.onDidChangePaths.bind(atom.project)).map(paths => new Set(paths.filter((_nuclideUri || _load_nuclideUri()).default.isRemote).map((_nuclideUri || _load_nuclideUri()).default.getHostname))).let((0, (_observable || _load_observable()).diffSets)()).flatMap(diff => _rxjsBundlesRxMinJs.Observable.from(diff.removed));

    this._disposables = new (_UniversalDisposable || _load_UniversalDisposable()).default(this._layoutManager, this._service, this._uiModel, this._breakpointManager, removedHostnames.subscribe(hostname => {
    ...
    atom.contextMenu.add({
      ...
      'atom-text-editor': [{ type: 'separator' }, {
        ...
        submenu: [{
          ...
          shouldDisplay: event => this._service.getDebuggerMode() === (_constants || _load_constants()).DebuggerMode.PAUSED
        }]
      }, { type: 'separator' }]
