...
function _load_Tabs() {
  return _Tabs = _interopRequireDefault(require('../../../../../nuclide-commons-ui/Tabs'));
}
...
class DebuggerLaunchAttachUI extends _react.Component {
  ...
  _filterProviders(key) {
    ...
    _rxjsBundlesRxMinJs.Observable.merge(...(this.props.providers.get(key) || []).map(provider => _rxjsBundlesRxMinJs.Observable.fromPromise(this._getProviderIfEnabled(provider)))).filter(provider => provider != null).map(provider => {
      if (!(provider != null)) {
  ...
  render() {
    ...
    return _react.createElement(
      ...
      (0, (_AtomServiceContainer || _load_AtomServiceContainer()).isNuclideEnvironment)() ? _react.createElement(
        'h1',
