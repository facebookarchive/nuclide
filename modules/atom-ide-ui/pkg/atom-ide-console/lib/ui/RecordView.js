...
function _load_Ansi() {
  return _Ansi = _interopRequireDefault(require('../../../../../nuclide-commons-ui/Ansi'));
}

var _TextRenderer;

function _load_TextRenderer() {
  return _TextRenderer = require('../../../../../nuclide-commons-ui/TextRenderer');
}
...
function _load_parseText() {
  return _parseText = _interopRequireDefault(require('../parseText'));
}
...
const AnsiRenderSegment = ({ key, style, content }) => _react.createElement(
  'span',
  { key: key, style: style, className: 'nuclide-console-default-text-colors' },
...
class RecordView extends _react.Component {
  ...
  _renderContent(displayableRecord) {
    ...
    if (record.kind === 'request') {
      ...
      return _react.createElement(
        'pre',
        null,
        (0, (_parseText || _load_parseText()).default)(text)
      );
    }
  }
  ...
  render() {
    ...
    const classNames = (0, (_classnames || _load_classnames()).default)('console-record', `level-${level || 'log'}`, {
