...
class Activation {
  ...
  registerOpenerAndCommand() {
    let lastMouseEvent;
    return new (_UniversalDisposable || _load_UniversalDisposable()).default(atom.commands.add('atom-text-editor', 'find-references:activate', async event => {
      openViewModel(tryCreateView((await this._getProviderData((_ContextMenu || _load_ContextMenu()).default.isEventFromContextMenu(event) ? lastMouseEvent : null))));
    }),
  ...
  async _getProviderData(event) {
    ...
    const resultPromise = (0, (_promise || _load_promise()).asyncFind)(supported.map(provider => provider.findReferences(editor, point).catch(err => {
    ...
    if (busySignalService != null) {
      ...
      return busySignalService.reportBusyWhile(`Finding references for ${displayPath}:${point.row}:${point.column}`, () => resultPromise, { revealTooltip: true });
    }
