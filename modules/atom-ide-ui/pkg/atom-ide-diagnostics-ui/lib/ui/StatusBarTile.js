...
class StatusBarTile {
  ...
  consumeDiagnosticUpdates(diagnosticUpdater, isStaleMessageEnabledStream) {
    ...
    this._subscriptions.add((0, _event().observableFromSubscribeFunction)(diagnosticUpdater.observeMessages).let((0, _observable().fastDebounce)(RENDER_DEBOUNCE_TIME)).combineLatest(isStaleMessageEnabledStream).map(([diagnostics, isStaleMessageEnabled]) => diagnostics.map(diagnostic => {
