...
class StatusBarTile {
  ...
  consumeDiagnosticUpdates(diagnosticUpdater, isStaleMessageEnabledSteam) {
    ...
    this._subscriptions.add((0, _event().observableFromSubscribeFunction)(diagnosticUpdater.observeMessages).let((0, _observable().fastDebounce)(RENDER_DEBOUNCE_TIME)).combineLatest(isStaleMessageEnabledSteam).map(([diagnostics, isStaleMessageEnabled]) => diagnostics.map(diagnostic => {
