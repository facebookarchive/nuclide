...

{
  ...
  constructor(props) {
    ...
    // Memoize `_getRows()`
    _initialiseProps.call(this);this._getRows = (0, (_memoizeUntilChanged || _load_memoizeUntilChanged()).default)(
    this._getRows,
  ...
  render() {
    ...
    if (sortedRows.length > MAX_RESULTS_COUNT) {
      ...
      maxResultsMessage =
      _react.createElement('div', { className: 'highlight-warning diagnostics-ui-table-message' }, 'Max results (',
        MAX_RESULTS_COUNT, ') reached. Fix diagnostics or show only diagnostics for the current file to view more.');
...
function getIconName(classification) {
  ...
  severity !== 'Hint')) {throw new Error('Invariant violation: "severity !== \'Hint\'"');}
  switch (severity) {
    case 'Warning':
...
function DirComponent(props) {
  return (
    ...
    _react.createElement('div', { className: 'nuclide-diagnostics-ui-dir-cell-contents' }, '\u200E',
...
function FilenameComponent(props) {
  ...
  if (locationInFile == null) {
    ...
    return _react.createElement('span', null, DASH);
  }
  const { basename, line } = locationInFile;
  return (
    _react.createElement('span', null,
      basename,
      _react.createElement('span', { className: 'nuclide-diagnostics-ui-line-number' }, ':', line)));
...
function LineNumberComponent(props) {
  ...
  return _react.createElement('span', null, line == null ? DASH : line);
}
