...
function _applyUpdateToEditor(editor, update, diagnosticUpdater, blockDecorationContainer, openedMessageIds, setOpenMessageIds) {
  ...
  for (const [row, messages] of rowToMessage.entries()) {
    ...
    const {
      item,
      dispose
    } = createGutterItem(editor, messages, diagnosticUpdater, gutter, openedMessageIds, setOpenMessageIds, gutterMarker);
...
function createGutterItem(editor, messages, diagnosticUpdater, gutter, openedMessageIds, setOpenMessageIds, gutterMarker) {
