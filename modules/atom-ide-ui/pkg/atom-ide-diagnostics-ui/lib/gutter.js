...
const HIGHLIGHT_CSS = 'diagnostics-gutter-ui-highlight';const HIGHLIGHT_CSS_LEVELS = { Error: 'diagnostics-gutter-ui-highlight-error', Warning: 'diagnostics-gutter-ui-highlight-warning', Info: 'diagnostics-gutter-ui-highlight-info', Hint: '' };const GUTTER_CSS_GROUPS = { review: 'diagnostics-gutter-ui-gutter-review', errors: 'diagnostics-gutter-ui-gutter-error', warnings: 'diagnostics-gutter-ui-gutter-warning', info: 'diagnostics-gutter-ui-gutter-info', action: 'diagnostics-gutter-ui-gutter-action', hidden: '' };const editorToMarkers = new WeakMap();const itemToEditor = new WeakMap();function applyUpdateToEditor(editor, update, diagnosticUpdater) {let gutter = editor.gutterWithName(GUTTER_ID);if (!gutter) {// TODO(jessicalin): Determine an appropriate priority so that the gutter:
    ...
    gutter = editor.addGutter({ name: GUTTER_ID, visible: false, // Priority is -200 by default and 0 is the line number
      priority: -1000 });}let marker;let markers = editorToMarkers.get(editor); // TODO: Consider a more efficient strategy that does not blindly destroy all of the
  // existing markers.
  if (markers) {for (marker of markers) {marker.destroy();
    }
    markers.clear();
  } else {
    markers = new Set();
  ...
  for (const message of update.messages) {
    ...
    (0, (_range || _load_range()).wordAtPosition)(editor, message.range.start) :
    null;
    const range = wordRange != null ? wordRange.range : message.range;

    const highlightCssClass = (0, (_classnames || _load_classnames()).default)(
    HIGHLIGHT_CSS,
    ...
    if (range) {
      ...
      for (let line = range.start.row; line <= range.end.row; line++) {
        ...
        new _atom.Range([line, 0], [line + 1, 0]));
        ...
        new _atom.Range([line, start], [line, end]));
...
gutter)
{
  ...
  const spawnPopup = () => {
    return _rxjsBundlesRxMinJs.Observable.create(observer => {
      ...
      return () => {
        _reactDom.default.unmountComponentAtNode(popupElement);if (!(
        popupElement.parentNode != null)) {throw new Error('Invariant violation: "popupElement.parentNode != null"');}
        popupElement.parentNode.removeChild(popupElement);
      };
    });
  };

  const hoverSubscription = _rxjsBundlesRxMinJs.Observable.fromEvent(item, 'mouseenter').
  exhaustMap(event => {
    return spawnPopup().
    let(
    (0, (_observable || _load_observable()).completingSwitchMap)(popupElement => {
      const innerPopupElement = popupElement.firstChild;if (!(
      innerPopupElement instanceof HTMLElement)) {throw new Error('Invariant violation: "innerPopupElement instanceof HTMLElement"');}

      // Events which should cause the popup to close.
      return _rxjsBundlesRxMinJs.Observable.merge(
      (0, (_aim || _load_aim()).hoveringOrAiming)(item, innerPopupElement),
...
gutter)
{
  ...
  const editor = itemToEditor.get(item);if (!(
  ...
  const BoundPopup = (0, (_bindObservableAsProps || _load_bindObservableAsProps()).bindObservableAsProps)(
  (0, (_event || _load_event()).observableFromSubscribeFunction)(cb =>
  ...
  const popupElement = hostElement.firstElementChild;if (!(
  popupElement instanceof HTMLElement)) {throw new Error('Invariant violation: "popupElement instanceof HTMLElement"');}
