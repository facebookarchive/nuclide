...
class Activation {
  ...
  _getGlobalViewStates() {
    if (this._globalViewStates == null) {
      ...
      const diagnosticsStream = updaters.switchMap(updater => updater == null ? _RxMin.Observable.of([]) : (0, _event().observableFromSubscribeFunction)(updater.observeMessages)).withLatestFrom(this._getIsStaleMessageEnabledSteam()).map(([diagnostics, isStaleMessageEnabled]) => diagnostics.filter(d => d.type !== 'Hint').map(diagnostic => {
...
function getEditorDiagnosticUpdates(editor, diagnosticUpdater, isStaleMessageEnabledSteam) {
  return (0, _event().observableFromSubscribeFunction)(editor.onDidChangePath.bind(editor)).startWith(editor.getPath()).switchMap(filePath => filePath != null ? (0, _event().observableFromSubscribeFunction)(cb => diagnosticUpdater.observeFileMessages(filePath, cb)) : _RxMin.Observable.empty()).withLatestFrom(isStaleMessageEnabledSteam).map(([diagnosticMessages, isStaleMessageEnabled]) => {
