...
function StatusBarTileComponent(props) {
  ...
  if (props.waitingForUser) {
    element = _react.createElement((_Icon || _load_Icon()).Icon, { className: 'atom-ide-busy-signal-status-bar', icon: 'unverified' });
  } else if (props.waitingForComputer) {
    element = _react.createElement('div', { className: 'atom-ide-busy-signal-status-bar loading-spinner-tiny' });
  } else {
    element = null;
  }
