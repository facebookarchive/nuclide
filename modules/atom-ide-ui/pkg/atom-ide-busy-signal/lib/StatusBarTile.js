...
function StatusBarTileComponent(props) {
  ...
  if (props.waitingForUser) {
    element = _react.default.createElement((_Icon || _load_Icon()).Icon, { className: 'atom-ide-busy-signal-status-bar', icon: 'unverified' });
  } else if (props.waitingForComputer) {
    element = _react.default.createElement('div', { className: 'atom-ide-busy-signal-status-bar loading-spinner-tiny' });
  } else {
    element = null;
  }
...
class StatusBarTile {
  ...
  constructor(statusBar, messageStream) {
    ...
    this._disposables = new (_UniversalDisposable || _load_UniversalDisposable()).default(messageStream.subscribe(messages => this._handleMessages(messages)));
  }
  ...
  _handleMessages(messages) {
    ...
    _reactDom.default.render(_react.default.createElement(StatusBarTileComponent, props), this._item);
    if (this._tooltip != null) {
