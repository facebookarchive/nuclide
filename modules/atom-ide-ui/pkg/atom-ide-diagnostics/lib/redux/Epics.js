...
function fetchCodeActions(actions, store) {
  ...
  return actions.ofType((_Actions || _load_Actions()).FETCH_CODE_ACTIONS).distinctUntilChanged((x, y) => {
    if (!(x.type === (_Actions || _load_Actions()).FETCH_CODE_ACTIONS)) {
      throw new Error('Invariant violation: "x.type === Actions.FETCH_CODE_ACTIONS"');
    }

    if (!(y.type === (_Actions || _load_Actions()).FETCH_CODE_ACTIONS)) {
      throw new Error('Invariant violation: "y.type === Actions.FETCH_CODE_ACTIONS"');
    }

    return x.payload.editor === y.payload.editor && (0, (_collection || _load_collection()).arrayEqual)(x.payload.messages, y.payload.messages);
  }).switchMap(action => {
