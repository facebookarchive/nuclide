...
class Activation {
  ...
  _observeActivePaneItemAndMarkMessagesStale() {
    return this._gatekeeperServices.switchMap(gatekeeperService => {
      ...
      return (0, _event().observableFromSubscribeFunction)(atom.workspace.observeActivePaneItem.bind(atom.workspace)).map(editor => (0, _textEditor().isValidTextEditor)(editor) ? editor : null).filter(Boolean).mergeMap(editor => {
        return (0, _event().observableFromSubscribeFunction)(editor.onDidChange.bind(editor)).map(() => editor.getPath());
      });
    }).subscribe(filePath => {
