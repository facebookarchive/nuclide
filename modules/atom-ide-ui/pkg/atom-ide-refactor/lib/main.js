...
class Activation {
  constructor() {
    ...
    'nuclide-refactorizer:refactorize-from-context-menu', () => {
    ...
    }), atom.commands.add('atom-text-editor', 'nuclide-refactorizer:inline-rename', event => {
      const editor = atom.workspace.getActiveTextEditor();
      ...
      this._store.dispatch(Actions().displayInlineRename(editor, provider, selectedText, mountPosition, position));
    }), atom.contextMenu.add({
      'atom-text-editor': [{
        label: 'Rename',
