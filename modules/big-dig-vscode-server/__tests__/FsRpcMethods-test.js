...
describe('FsRpcMethods', () => {
  let rpc: FsRpcMethods;
  let FsRpcMethods: FsRpcMethods;
  ...
  describe('fs/write', () => {
    ...
    beforeEach(async () => {
      rpc = new FsRpcMethods((undefined: any));
    });
    ...
    it('create & overwrite', async () => {
      ...
      expect(call0[1]).toEqual(Buffer.from('abc123', BUFFER_ENCODING));
      // eslint-disable-next-line no-bitwise
      expect(call0[2]).toEqual({flags: O_WRONLY | O_TRUNC | O_CREAT});
    });
    ...
    it('create & !overwrite', async () => {
      ...
      expect(call0[1]).toEqual(Buffer.from('abc123', BUFFER_ENCODING));
      // eslint-disable-next-line no-bitwise
      expect(call0[2]).toEqual({flags: O_WRONLY | O_TRUNC | O_CREAT | O_EXCL});
    });
    ...
    it('!create & overwrite', async () => {
      ...
      expect(call0[1]).toEqual(Buffer.from('abc123', BUFFER_ENCODING));
      // eslint-disable-next-line no-bitwise
      expect(call0[2]).toEqual({flags: O_WRONLY | O_TRUNC});
    });
  ...
  describe('fs/move', () => {
    ...
    it('move; !overwrite', async () => {
      ...
      expect(call0[2]).toEqual({clobber: false});
    });
    ...
    it('move; overwrite', async () => {
      ...
      expect(call0[0]).toBe('foo');
      expect(call0[1]).toBe('abc');
      expect(call0[2]).toEqual({clobber: true});
    });
  });
  ...
  describe('fs/delete', () => {
    ...
    it('nonrecursive - dir', async () => {
      ...
      const rm = rpc._delete({path: 'foo', recursive: false});
      expect(rm).resolves.toEqual({});
      await rm;
  ...
  describe('fs/copy', () => {
    ...
    it('overwrite fail', async () => {
      ...
      expect(cp).rejects.toBeInstanceOf(RpcMethodError);
      expect(cp).rejects.toHaveProperty('parameters', {code: 'EEXIST'});
      try {
        await cp;
      } catch (error) {}
      expect(fsPromise.exists.mock.calls.length).toBe(1);
  ...
  describe('fs/watch', () => {
    let watchmanSub: Emitter;
    let onChangeEventRegistered: Deferred<void>;
    let watchmanClient: {
      watchDirectoryRecursive: JestMockFn<
        [string, ?string, ?WatchmanSubscriptionOptions],
        Promise<WatchmanSubscription>,
      >,
      unwatch: JestMockFn<[string], Promise<void>>,
    };
    ...
    beforeEach(async () => {
      onChangeEventRegistered = new Deferred();
      watchmanSub = new class extends Emitter {
        on(event: string, listener): this {
          super.on(event, listener);
          // We want to wait for this event to be registered before emitting
          // changes.
          if (event === 'change') {
      ...
      watchmanClient.watchDirectoryRecursive.mockReturnValueOnce(
        Promise.resolve(((watchmanSub: any): WatchmanSubscription)),
      );
      rpc = new FsRpcMethods((watchmanClient: any));
    });
    ...
    it('empty - basic', async () => {
      const changes = [];
      const changesSub = rpc
        ._watch({path: 'foo', recursive: true, exclude: []})
        .subscribe(c => changes.push(c));
      await onChangeEventRegistered.promise;
      // Are basic watch parameters correct?
      ...
      expect(watchmanClient.unwatch.mock.calls.length).toBe(0);
      // Is `unwatch` called when unsubscribed?
      await changesSub.unsubscribe();
      expect(watchmanClient.unwatch.mock.calls.length).toBe(1);
      expect(watchmanClient.unwatch.mock.calls[0][0]).toEqual(
        'big-dig-filewatcher-foo',
      );
      // empty:
    ...
    it('empty - non-recursive', async () => {
      rpc
        ._watch({path: 'foo', recursive: false, exclude: []})
        .subscribe(c => {});
      await onChangeEventRegistered.promise;
      // Watch expression should exclude recursive files:
      const call0 = watchmanClient.watchDirectoryRecursive.mock.calls[0];
      expect(call0[2]).toBeDefined();
      const excls = (call0[2]: any).expression[1].slice(1);
      expect(call0[2]).toEqual({expression: ['not', ['anyof', ...excls]]});
      expect(excls).toEqual([['dirname', '', ['depth', 'ge', 2]]]);
    });

    it('exclude', async () => {
      rpc
        ._watch({path: 'foo', recursive: true, exclude: ['**/.hg/**/*', 'bar']})
        .subscribe(c => {});
      await onChangeEventRegistered.promise;
      // Watch expression should have excludes:
      const call0 = watchmanClient.watchDirectoryRecursive.mock.calls[0];
      expect(call0[2]).toBeDefined();
      const excls = (call0[2]: any).expression[1].slice(1);
      expect(call0[2]).toEqual({expression: ['not', ['anyof', ...excls]]});
    ...
    it('non-recursive & exclude', async () => {
      rpc
        ...
        .subscribe(c => {});
      await onChangeEventRegistered.promise;
      // Watch expression should have excludes (and exclude recursive results):
      const call0 = watchmanClient.watchDirectoryRecursive.mock.calls[0];
      expect(call0[2]).toBeDefined();
      const excls = (call0[2]: any).expression[1].slice(1);
      expect(call0[2]).toEqual({expression: ['not', ['anyof', ...excls]]});
    ...
    it('changes', async () => {
      const changesPromise = rpc
        ._watch({path: 'abc', recursive: true, exclude: []})
        .take(4)
        .toArray()
        .toPromise();
      await onChangeEventRegistered.promise;
      // Create some watch results:
      watchmanSub.emit('change', [{name: 'aaa', exists: false, new: false}]);
      watchmanSub.emit('change', [{name: 'bbb', exists: false, new: true}]);
      watchmanSub.emit('change', [
        {name: 'ccc', exists: true, new: false},
        {name: 'ddd', exists: false, new: false},
      ]);
      watchmanSub.emit('change', [{name: 'eee', exists: true, new: true}]);
      // Check encoding of watch results:
      const changes = await changesPromise;
      expect(changes.length).toBe(4);
      expect(changes).toContainEqual([{path: 'aaa', type: 'd'}]);
      expect(changes).toContainEqual([{path: 'bbb', type: 'd'}]);
      expect(changes).toContainEqual([
        {path: 'ccc', type: 'u'},
        {path: 'ddd', type: 'd'},
      ]);
      expect(changes).toContainEqual([{path: 'eee', type: 'a'}]);
    });
  });
});
