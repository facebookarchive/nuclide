...
describe('throttle', () => {
  it('handles basic test', () => {
    ...
    const delay = new _RxMin.Subject();
  ...
  it('emits trailing values when the source is a synchronously emitting cold observable', async () => {
    const source = _RxMin.Observable.of(1, 2, 3, 4).concat(_RxMin.Observable.never());
  ...
  it('calls delay based on emitted value', async () => {
    ...
    const source = new _RxMin.Observable.of(delayInput, delayInput);

    function delay(val) {
      return _RxMin.Observable.timer(val);
    }
  ...
  it('emits the leading item immeditately by default', () => {
    ...
    const spy = jest.fn();
    source.pipe((0, _observable().throttle)(() => _RxMin.Observable.never())).subscribe(spy);
  ...
  it('subscribes to the source once per subscription', () => {
    ...
    source.pipe((0, _observable().throttle)(() => _RxMin.Observable.of(null))).subscribe();
    expect(spy.mock.calls.length).toBe(1);
  });
  it('supports promises', () => {
    const source = _RxMin.Observable.of(1, 2);
