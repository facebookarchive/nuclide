...
function loadConnectionProfiles() {
  ...
  const profiles = [];
  const connectionIdToPorts = new Map();
  ...
  for (const profile of configurationProfiles) {
    ...
    if (existingPorts != null && existingPorts !== parsedProfile.ports) {
      throw new Error('You have multiple connection profiles to the same hostname ' + `but with different ports for ${connectionId}. ` + 'Please edit settings.json to disambiguate and then reload VS Code.');
    }
