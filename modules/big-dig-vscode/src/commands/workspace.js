...
async function pickRemoteWorkspaceFolder(prompt, options) {
  const items = Array.prototype.concat(...(0, _remote().getFilesystems)().map(fs => fs.getWorkspaceFolders().map(folder => ({
    label: folder.name,
  ...
  if (items.length === 1 && (options === null || options === void 0 ? void 0 : options.skipUIForSingleWorkspace) === true) {
