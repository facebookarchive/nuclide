...
exports.createDevZip = createDevZip;
...
function init() {
  if (_dev().__DEV__) {
    (0, _devWorker().startDevPackagerWorker)(devZip, devZipVersion); // The above code essentially runs the following in another process. If it
...
async function createDevZip() {
  ...
  deltaPkgData.then(data => logger.info(`Server package delta is ready (${data.length} bytes)`)); // TODO(siegbell): Cannot update an existing zip: https://github.com/cthackers/adm-zip/issues/64
  // // Update the cached zip:
  // const fullPkgData = zutil
  //   .addFilesToZip(zip, files)
  //   .then(() => zutil.zipToBuffer(zip));

  const fullPkgData = (0, _production().createProductionZip)(archive, devVersion, files).then(({
    data
  }) => data); // Assume this will be used to update an installation
