...
function _unavailable() {
  const data = require("./commands/unavailable");
...
function loadCommands(context) {
  ...
  registerCommand('big-dig.addFolderToWorkspace', file => (0, _workspace().addFolderToWorkspace)(context, file)); // Add an on demand instance to the workspace.

  registerCommand('big-dig.addOnDemandWWWToWorkspace', () => (0, _unavailable().unavailable)(context)); // UI to select an active remote connection and shut down its server.
...
function startMultiplexingFilesystems() {
  ...
  return {
    disposable,
    fileSystemProvider: fsMultiplexer
  };
}