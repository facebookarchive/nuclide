...
async function makeConnection(profile) {
  ...
  try {
    return await vscode().window.withProgress(progressConfig, connectOrReconnect.bind(null, profile, canceller));
  } finally {
    canceller.dispose();
  }
}

async function connectOrReconnect(profile, canceller, progress) {
  const {
    address,
    deployServer,
  ...
  const checkServer = deployServer.validateServerVersion != null ? deployServer.validateServerVersion : true;
  let connectionWrapper = await doReconnect(connectionId, address, checkServer, progress);

  if (connectionWrapper == null) {
    connectionWrapper = await doConnect(connectionId, address, autoUpdate, canceller, authMethodPromise, privateKeyPromise, username, server, ports, progress);
  }

  return connectionWrapper;
}

async function doConnect(connectionId, address, autoUpdate, canceller, authMethodPromise, privateKeyPromise, username, server, ports, progress) {
...
async function doReconnect(connectionId, address, validateServerVersion, progress) {
  try {
    ...
    if (credentials != null) {
      ...
      const compatible = validateServerVersion ? await validateCurrentServer(conn) : true;
