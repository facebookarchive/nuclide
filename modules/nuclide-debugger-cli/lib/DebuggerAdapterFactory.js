...
function _load_constants() {return _constants = require('nuclide-debugger-common/constants');}var _debuggerRegistry;
...
class DebuggerAdapterFactory {constructor() {this.
    _vspServersByTargetType = new Map([
    [
    (_constants || _load_constants()).VsAdapterTypes.PYTHON,
    {
      key: 'python',
      type: 'python',
    ...
    (_constants || _load_constants()).VsAdapterTypes.NATIVE_GDB,
    {
      key: 'native_gdb',
      type: 'mi',
      customArguments: new Map() }]]);this.




    _targetTypeByFileExtension = new Map([
    ['.py', (_constants || _load_constants()).VsAdapterTypes.PYTHON],
    ['.js', (_constants || _load_constants()).VsAdapterTypes.NODE]]);this.





    _excludeOptions = new Set([
    'args',
    'console',
    'diagnosticLogging',
    'externalConsole',
    ...
    'restart',
    'trace',
    'verboseDiagnosticLogging']);this.




    _includeOptions = new Set(['address', 'port']);} // These are options which are either managed by the debugger or don't
  ...
  // if they exist
  adapterFromArguments(args) {const node = args.usenode == null ? 'node' : args.usenode;let adapter;
    if (args.attach) {
  ...
  showContextSensitiveHelp(args) {
    ...
    const adapter = this._vspServersByTargetType.get(targetType);if (!(

    adapter != null)) {throw new Error(
