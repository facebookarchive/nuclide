...
function _load_vscodeDebugprotocol() {return _vscodeDebugprotocol = _interopRequireWildcard(require('vscode-debugprotocol'));}var _idx;




function _load_idx() {return _idx = _interopRequireDefault(require('idx'));}var _Thread;
function _load_Thread() {return _Thread = _interopRequireDefault(require('./Thread'));}function _interopRequireWildcard(obj) {if (obj && obj.__esModule) {return obj;} else {var newObj = {};if (obj != null) {for (var key in obj) {if (Object.prototype.hasOwnProperty.call(obj, key)) newObj[key] = obj[key];}}newObj.default = obj;return newObj;}}function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}
...
class BackTraceCommand {
  ...
  constructor(con, debug) {this.name = 'backtrace';this.helpText = '[frame] Displays the call stack of the active thread. With optional frame index, sets the current frame for variable display.';this.detailedHelpText = `
backtrace [frame]
...
* The 'print' command will evaluate expression in terms of the variables that are
  ...
  `;this._console = con;this._debugger = debug;}execute(args) {var _this = this;return (0, _asyncToGenerator.default)(function* () {const activeThread = _this._debugger.getActiveThread();if (args.length > 1) {throw Error("'backtrace' takes at most one argument -- the index of the frame to select");}const frameArg = args[0];if (frameArg != null) {yield _this._setSelectedStackFrame(activeThread, frameArg);return;}

      const frames = yield _this._debugger.getStackTrace(
      activeThread.id(),
  ...
  selectedFrame)
  {
    frames.forEach((frame, index) => {var _ref, _ref2;
