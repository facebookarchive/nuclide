...
function _load_collection() {return _collection = require('nuclide-commons/collection');}var _Breakpoint;

function _load_Breakpoint() {return _Breakpoint = _interopRequireDefault(require('./Breakpoint'));}var _SourceBreakpoint;
function _load_SourceBreakpoint() {return _SourceBreakpoint = _interopRequireDefault(require('./SourceBreakpoint'));}function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}

class BreakpointCollection {constructor() {this.


    _breakpoints = new Map();this.
    _nextIndex = 1;} // $TODO function breakpoints when we have an adapter that supports them
  ...
  getAllEnabledBreakpointsByPath() {
    ...
    (0, (_collection || _load_collection()).arrayCompact)(Array.from(this._breakpoints.values()).map(_ => _.path)));
  ...
  getBreakpointById(id) {
    const breakpoint = Array.from(this._breakpoints.values()).find(
    _ => _.id === id);
