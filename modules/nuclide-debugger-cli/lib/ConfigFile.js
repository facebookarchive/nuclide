...
function _load_yargs() {return _yargs = _interopRequireDefault(require('yargs'));}var _collection;
...
class ConfigFile {
  ...
  constructor() {
    ...
    (_nuclideUri || _load_nuclideUri()).default.join(_os.default.homedir(), '.fbdbg', 'config.json')];

    // $FlowFixMe Flow doesn't understand Object.assign
    this._config = configFiles.
    filter(fname => _fs.default.existsSync(fname)).
    reduce((agg, fname) => {
      try {
