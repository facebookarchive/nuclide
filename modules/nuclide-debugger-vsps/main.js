...
const _adapters = new Map([['node', {
  ...
  root: (_nuclideUri || _load_nuclideUri()).default.join(__dirname, 'fb-native-debugger-gdb-vsp/src')
}], ['hhvm', {
  executable: {
    command: 'node',
  ...
  executable: {
    command: 'node',
    args: [(_nuclideUri || _load_nuclideUri()).default.join(__dirname, '../../pkg/nuclide-debugger-php-rpc/lib/vscode/vscode-debugger-entry.js')]
  },
  root: (_nuclideUri || _load_nuclideUri()).default.join(__dirname, '../../pkg/nuclide-debugger-php-rpc')
}]]); /**
