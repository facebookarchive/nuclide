'use strict';Object.defineProperty(exports, "__esModule", { value: true });exports.DebugBridge = exports.DEFAULT_ADB_PORT = undefined;
...
function _load_process() {return _process = require('../../../nuclide-commons/process');} /**
                                                                                           ...
                                                                                           * 
                                                                                           * @format
                                                                                           */const DEFAULT_ADB_PORT = exports.DEFAULT_ADB_PORT = 5037;const ADB_TIMEOUT = 5000; // 5s
class DebugBridge {
  ...
  runShortCommand(...command) {
    ...
    (0, (_process || _load_process()).runCommand)(config.path, this.getDeviceArgs().concat(command)));
  ...
  runLongCommand(...command) {
    // TODO(T17463635)
    return this.constructor.configObs.switchMap(config =>
    (0, (_process || _load_process()).observeProcess)(config.path, this.getDeviceArgs().concat(command), {
      killTreeWhenDone: true,
      /* TODO(T17353599) */isExitError: () => false }).
    catch(error => _rxjsBundlesRxMinJs.Observable.of({ kind: 'error', error })));
    // TODO(T17463635)
  }
  ...
  static getDevices(options) {
    ...
    return this.configObs.switchMap(config => {
      ...
      ...ports.map(port =>
      (0, (_process || _load_process()).runCommand)(config.path, ['-P', String(port), 'devices']).map(
      stdout => this._parseDevicesCommandOutput(stdout, port)))) :



      _rxjsBundlesRxMinJs.Observable.concat(
      (0, (_process || _load_process()).runCommand)(config.path, ['devices']).map(stdout =>
  ...
  static killServer() {
    ...
    switchMap(config => (0, (_process || _load_process()).runCommand)(config.path, ['kill-server'])).
    mapTo(undefined).
    toPromise();
  }
  ...
  static getVersion() {
    return this.configObs.
    switchMap(config => (0, (_process || _load_process()).runCommand)(config.path, ['version'])).
    map(versionString => {
