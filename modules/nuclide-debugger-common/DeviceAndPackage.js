...
function _load_utils() {
  return _utils = require('../nuclide-adb/lib/utils');
}
...
class DeviceAndPackage extends _react.Component {
  ...
  async _refreshPackageList(device) {
    if (device != null) {
      const packages = (_expected || _load_expected()).Expect.value((await (0, (_utils || _load_utils()).getAdbServiceByNuclideUri)(this.props.targetUri).getInstalledPackages(device)).sort());
      this.setState({
        packages
      });
    } else {
