...
function _load_AtomServiceContainer() {
  return _AtomServiceContainer = require('atom-ide-ui/pkg/atom-ide-debugger/lib/AtomServiceContainer');
}
...
class SelectableFilterableProcessTable extends _react.Component {
  ...
  componentDidMount() {
    this._disposables.add(_rxjsBundlesRxMinJs.Observable.interval(PROCESS_UPDATES_INTERVAL_MS).startWith(0).flatMap(_ => (0, (_AtomServiceContainer || _load_AtomServiceContainer()).getVSCodeDebuggerAdapterServiceByNuclideUri)(this.props.targetUri).getProcessTree()).subscribe(this._updateList));
  }
