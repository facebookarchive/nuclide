...
class ServiceAtomCommands {
  ...
  getClientConnections(hostname) {
    return (0, _asyncToGenerator.default)(function* () {
      const clientConnections = yield Promise.all(CommandServer._connections.map(function (commandServer) {
        return commandServer._atomCommands.getClientConnections(hostname);
      }));
