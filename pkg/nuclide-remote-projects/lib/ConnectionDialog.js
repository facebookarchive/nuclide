...
let ConnectionDialog = (_temp = _class = class ConnectionDialog extends _reactForAtom.React.Component {
  ...
  ok() {
    ...
    if (mode === REQUEST_CONNECTION_DETAILS) {
      ...
      const server = _connectionDetailsFor.server;
      const cwd = _connectionDetailsFor.cwd;
      const remoteServerCommand = _connectionDetailsFor.remoteServerCommand;
      const sshPort = _connectionDetailsFor.sshPort;
      const pathToPrivateKey = _connectionDetailsFor.pathToPrivateKey;
      const authMethod = _connectionDetailsFor.authMethod;
      const password = _connectionDetailsFor.password;
      const displayTitle = _connectionDetailsFor.displayTitle;
  ...
  getFormFields() {
    ...
    const server = _connectionDetailsFor2.server;
    const cwd = _connectionDetailsFor2.cwd;
    const remoteServerCommand = _connectionDetailsFor2.remoteServerCommand;
    const sshPort = _connectionDetailsFor2.sshPort;
    const pathToPrivateKey = _connectionDetailsFor2.pathToPrivateKey;
    const authMethod = _connectionDetailsFor2.authMethod;
    const displayTitle = _connectionDetailsFor2.displayTitle;
