...
var createEditorForNuclide = _asyncToGenerator(function* (uri) {
  try {
    ...
    try {
      buffer = yield (0, (_commonsAtomLoadingNotification2 || _commonsAtomLoadingNotification()).default)((0, (_commonsAtomTextEditor2 || _commonsAtomTextEditor()).loadBufferForUri)(uri), 'Opening ' + (_commonsNodeNuclideUri2 || _commonsNodeNuclideUri()).default.nuclideUriToDisplayString(uri) + '...', 500);
    } /* delay */
    catch (err) {
...
var _RemoteDirectorySearcher2;

function _RemoteDirectorySearcher() {
  return _RemoteDirectorySearcher2 = _interopRequireDefault(require('./RemoteDirectorySearcher'));
}

var _RemoteDirectoryProvider2;

function _RemoteDirectoryProvider() {
  return _RemoteDirectoryProvider2 = _interopRequireDefault(require('./RemoteDirectoryProvider'));
}

var _RemoteProjectsController2;

function _RemoteProjectsController() {
  return _RemoteProjectsController2 = _interopRequireDefault(require('./RemoteProjectsController'));
}
...
function addRemoteFolderToProject(connection) {
  ...
  function checkClosedProject() {
    ...
    var confirmServerActionOnLastProject = (_commonsAtomFeatureConfig2 || _commonsAtomFeatureConfig()).default.get('nuclide-remote-projects.confirmServerActionOnLastProject');
    (0, (_assert2 || _assert()).default)(typeof confirmServerActionOnLastProject === 'boolean');

    var shutdownServerAfterDisconnection = (_commonsAtomFeatureConfig2 || _commonsAtomFeatureConfig()).default.get('nuclide-remote-projects.shutdownServerAfterDisconnection');
    (0, (_assert2 || _assert()).default)(typeof shutdownServerAfterDisconnection === 'boolean');
...
function closeOpenFilesForRemoteProject(connection) {
  ...
  for (var openInstance of openInstances) {
    ...
    if (connection.isOnlyConnection() || (_commonsNodeNuclideUri2 || _commonsNodeNuclideUri()).default.contains(connection.getUriForInitialWorkingDirectory(), uri)) {
...
function deleteDummyRemoteRootDirectories() {
  for (var directory of atom.project.getDirectories()) {
    if ((_commonsNodeNuclideUri2 || _commonsNodeNuclideUri()).default.isRemote(directory.getPath()) && !(_nuclideRemoteConnection2 || _nuclideRemoteConnection()).RemoteDirectory.isRemoteDirectory(directory)) {
...
function activate(state) {
  ...
  controller = new (_RemoteProjectsController2 || _RemoteProjectsController()).default();
...
function createRemoteDirectoryProvider() {
  return new (_RemoteDirectoryProvider2 || _RemoteDirectoryProvider()).default();
}

function createRemoteDirectorySearcher() {
  return new (_RemoteDirectorySearcher2 || _RemoteDirectorySearcher()).default(function (dir) {
    var service = (0, (_nuclideRemoteConnection2 || _nuclideRemoteConnection()).getServiceByNuclideUri)('GrepService', dir.getPath());
