...
class ConnectFlow {
  ...
  _updateState(nextState, saveProfiles = true) {
    const prevState = this._model.state;

    this._model.setState(nextState); // If the connection profiles changed, save them to the config. The `saveProfiles` option allows
