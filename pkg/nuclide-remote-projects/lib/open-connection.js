 ...
 * 
 * @format
 */function openConnectionDialog(dialogOptions) {return new Promise(resolve => {(0, (_showModal || _load_showModal()).default)(({ dismiss }) => {const StatefulModal = (0, (_bindObservableAsProps || _load_bindObservableAsProps()).bindObservableAsProps)(createPropsStream({ dismiss, onConnected: resolve, dialogOptions }), (_RemoteProjectConnectionModal || _load_RemoteProjectConnectionModal()).default);return _react.createElement(StatefulModal, null);}, { shouldDismissOnClickOutsideModal: () => false });});
}
...
function createPropsStream({ dismiss, onConnected, dialogOptions }) {
  ...
  ...(0, (_connectionProfileUtils || _load_connectionProfileUtils()).getSavedConnectionProfiles)()];
  ...
  const staticProps = {
    ...
    onConnect: (() => {var _ref = (0, _asyncToGenerator.default)(function* (connection, config) {
      ...
      });return function onConnect(_x, _x2) {return _ref.apply(this, arguments);};})(),
    onCancel: () => {
      onConnected(null);
      dismiss();
    },
  ...
  model.toObservable().map(state => Object.assign({}, state, staticProps));

  const savedProfilesStream = (0, (_event || _load_event()).observableFromSubscribeFunction)((_connectionProfileUtils || _load_connectionProfileUtils()).onSavedConnectionProfilesDidChange).

  map(() => (0, (_connectionProfileUtils || _load_connectionProfileUtils()).getSavedConnectionProfiles)());
