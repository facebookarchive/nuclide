...
function _load_Model() {
  return _Model = _interopRequireDefault(require('../../../modules/nuclide-commons/Model'));
}

var _showModal;

function _load_showModal() {
  return _showModal = _interopRequireDefault(require('../../../modules/nuclide-commons-ui/showModal'));
}
...
function createPropsStream({ dismiss, onConnected, dialogOptions }) {
  ...
  const staticProps = {
    ...
    onConnect: async (connection, config) => {
      ...
      if (project) {
        (_ProjectManager || _load_ProjectManager()).default.addRecentProject(project, connection.getConfig().host);
      }
