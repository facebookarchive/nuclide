...
function openConnectionDialog(dialogOptions) {
  return new Promise(resolve => {
    (0, (_showModal || _load_showModal()).default)(dismiss => {
      ...
      return _react.default.createElement(StatefulModal, null);
...
function createPropsStream({ dismiss, onConnected, dialogOptions }) {
  ...
  const staticProps = {
    ...
    onProfileCreated(newProfile) {
      const connectionProfiles = [...states.getValue().connectionProfiles, newProfile];
      updateState({
        connectionProfiles,
  ...
  function updateState(nextState, saveProfiles = true) {
    ...
    states.next(Object.assign({}, prevState, nextState));
  ...
  const states = new _rxjsBundlesRxMinJs.BehaviorSubject({
    screen: 'connect',
    selectedProfileIndex: 0,
    connectionProfiles: initialConnectionProfiles
  });

  const props = states.map(state => Object.assign({}, state, staticProps));
