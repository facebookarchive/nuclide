...
class RemoteProjectsService {
  ...
  async createRemoteConnection(remoteProjectConfig) {
    ...
    const connection = await (_nuclideRemoteConnection || _load_nuclideRemoteConnection()).RemoteConnection.reconnect(host, cwd, displayTitle, promptReconnectOnFailure);
    if (connection != null) {
      return connection;
    }
