...
function notifyConnectedRemoteFile(fileUri) {
  atom.notifications.addInfo('The connection to the server: <code>' + (0, (_nuclideRemoteUri2 || _nuclideRemoteUri()).getHostname)(fileUri) + '</code> is healthy.');
}

function notifyDisconnectedRemoteFile(fileUri) {
  atom.notifications.addError('The connection to the server: <code>' + (0, (_nuclideRemoteUri2 || _nuclideRemoteUri()).getHostname)(fileUri) + '</code> is lost,\n    retrying in the background!');
}
