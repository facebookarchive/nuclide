...
function notifyConnectedRemoteFile(fileUri) {
  atom.notifications.addInfo('The connection to the server: <code>' + (_nuclideRemoteUri2 || _nuclideRemoteUri()).default.getHostname(fileUri) + '</code> is healthy.');
}

function notifyDisconnectedRemoteFile(fileUri) {
  atom.notifications.addError('The connection to the server: <code>' + (_nuclideRemoteUri2 || _nuclideRemoteUri()).default.getHostname(fileUri) + '</code> is lost,\n    retrying in the background!');
}
