...
function notifyConnectedRemoteFile(fileUri) {
  atom.notifications.addInfo('The connection to the server: <code>' + (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.getHostname(fileUri) + '</code> is healthy.');
}

function notifyDisconnectedRemoteFile(fileUri) {
  atom.notifications.addError('The connection to the server: <code>' + (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.getHostname(fileUri) + '</code> is lost,\n    retrying in the background!');
}
...
function notifySshHandshakeError(errorType, error, config) {
  ...
  var createTimeoutDetail = function createTimeoutDetail() {
