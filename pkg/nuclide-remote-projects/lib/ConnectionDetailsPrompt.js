...
function _load_nullthrows() {return _nullthrows = _interopRequireDefault(require('nullthrows'));}var _ConnectionDetailsForm;
function _load_ConnectionDetailsForm() {return _ConnectionDetailsForm = _interopRequireDefault(require('./ConnectionDetailsForm'));}var _connectionProfileUtils;
function _load_connectionProfileUtils() {return _connectionProfileUtils = require('./connection-profile-utils');}var _HR;

function _load_HR() {return _HR = require('../../../modules/nuclide-commons-ui/HR');}var _MutableListSelector;
function _load_MutableListSelector() {return _MutableListSelector = require('../../nuclide-ui/MutableListSelector');}
...

{
  ...
  constructor(props) {
    ...
    };this._settingFormFieldsLock = false;this.state = { IPs: null, shouldDisplayTooltipWarning: false };}componentDidMount() {if (this.props.connectionProfiles) {this.setState({ IPs: (0, (_connectionProfileUtils || _load_connectionProfileUtils()).getIPsForHosts)((0, (_connectionProfileUtils || _load_connectionProfileUtils()).getUniqueHostsForProfiles)(this.props.connectionProfiles)) });}this._checkForHostCollisions();}componentDidUpdate(prevProps, prevState) {// Manually update the contents of an existing `ConnectionDetailsForm`, because it contains
    ...
    prevProps.connectionProfiles != null && this.props.connectionProfiles != null && prevProps.connectionProfiles.length !== this.props.connectionProfiles.length) {const existingConnectionDetailsForm = this._connectionDetailsForm;if (existingConnectionDetailsForm) {// Setting values in the ConnectionDetailsForm fires change events. However, this is a
        ...
        this._settingFormFieldsLock = true;existingConnectionDetailsForm.setFormFields( // $FlowFixMe
        this.getPrefilledConnectionParams());existingConnectionDetailsForm.clearPassword();this._settingFormFieldsLock = false;existingConnectionDetailsForm.focus();}}if (prevProps.connectionProfiles !== this.props.connectionProfiles && this.props.connectionProfiles) {this.setState({ IPs: (0, (_connectionProfileUtils || _load_connectionProfileUtils()).getIPsForHosts)((0, (_connectionProfileUtils || _load_connectionProfileUtils()).getUniqueHostsForProfiles)(this.props.connectionProfiles)) });}this._checkForHostCollisions();}focus() {(0, (_nullthrows || _load_nullthrows()).default)(this._connectionDetailsForm).focus();}getFormFields() {return (0, (_nullthrows || _load_nullthrows()).default)(this._connectionDetailsForm).getFormFields();}getPrefilledConnectionParams() {// If there are profiles, pre-fill the form with the information from the specified selected
    // profile.
    if (this.props.connectionProfiles != null && this.props.connectionProfiles.length > 0 && this.props.selectedProfileIndex != null) {const selectedProfile = this.props.connectionProfiles[this.props.selectedProfileIndex];return selectedProfile.params;}}_checkForHostCollisions() {var _this = this;return (0, _asyncToGenerator.default)(function* () {if (_this.state.IPs) {const IPs = yield _this.state.IPs;if (IPs.length !== new Set(IPs).size) {if (!_this.state.shouldDisplayTooltipWarning) {_this.setState({ shouldDisplayTooltipWarning: true });}} else {
  ...
  render() {
    ...
    if (connectionProfiles == null || connectionProfiles.length === 0) {
      ...
      _react.createElement('div', { className: 'block select-list' },
        _react.createElement('ol', { className: 'list-group', style: { marginTop: 0 } },
    ...
    return (
      _react.createElement('div', { className: 'nuclide-remote-projects-connection-dialog' },
        _react.createElement('div', { className: 'nuclide-remote-projects-connection-profiles' },
          defaultConnectionProfileList,
          _react.createElement('h6', null, 'Profiles',
