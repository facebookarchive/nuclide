...
function applyActionMiddleware(actions, getState) {
  ...
  actions.filter(function (action) {
    ...
    return (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.from(projectDirectories).flatMap(function (directory) {
      ...
      var observable = (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.of({
        payload: {
          directory: directory,
      ...
      if (repository.getType() === 'hg') {
        (function () {
          ...
          observable = observable.concat((_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.merge((0, (_commonsNodeEvent2 || _commonsNodeEvent()).observableFromSubscribeFunction)(
    ...
    return (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.fromPromise(repository.async.checkoutReference(bookmark.bookmark, false)).ignoreElements().catch(function (error) {
    ...
    return renames.switchMap(function (action) {
      ...
      }).concat((_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.fromPromise(repositoryAsync.renameBookmark(bookmark.bookmark, nextName)).ignoreElements().catch(function (error) {
        atom.notifications.addWarning('Failed Renaming Bookmark', {
          detail: error,
          dismissable: true
        });
    ...
    return renames.switchMap(function (action) {
      ...
      return (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.of({
        ...
        type: (_ActionType2 || _ActionType()).SET_BOOKMARK_IS_LOADING
      }).concat((_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.fromPromise(repositoryAsync.deleteBookmark(bookmark.bookmark)).ignoreElements().catch(function (error) {
        atom.notifications.addWarning('Failed Deleting Bookmark', {
          detail: error,
          dismissable: true
        });
