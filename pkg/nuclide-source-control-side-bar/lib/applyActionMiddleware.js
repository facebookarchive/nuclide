...
function applyActionMiddleware(actions, getState) {
  ...
  actions.filter(function (action) {
    ...
    return (_rxjsBundlesRxUmdMinJs2 || _rxjsBundlesRxUmdMinJs()).Observable.from(projectDirectories).flatMap(function (directory) {
      ...
      var observable = (_rxjsBundlesRxUmdMinJs2 || _rxjsBundlesRxUmdMinJs()).Observable.of({
        payload: {
          directory: directory,
      ...
      if (repository.getType() === 'hg') {
        (function () {
          ...
          observable = observable.concat((_rxjsBundlesRxUmdMinJs2 || _rxjsBundlesRxUmdMinJs()).Observable.merge((0, (_commonsNodeEvent2 || _commonsNodeEvent()).observableFromSubscribeFunction)(
    ...
    return (_rxjsBundlesRxUmdMinJs2 || _rxjsBundlesRxUmdMinJs()).Observable.fromPromise(repository.async.checkoutReference(bookmark.bookmark, false)).ignoreElements().catch(function (error) {
    ...
    return renames.switchMap(function (action) {
      ...
      }).concat((_rxjsBundlesRxUmdMinJs2 || _rxjsBundlesRxUmdMinJs()).Observable.fromPromise(repositoryAsync.renameBookmark(bookmark.bookmark, nextName)).ignoreElements().catch(function (error) {
        atom.notifications.addWarning('Failed Renaming Bookmark', {
          detail: error,
          dismissable: true
        });
    ...
    return renames.switchMap(function (action) {
      ...
      return (_rxjsBundlesRxUmdMinJs2 || _rxjsBundlesRxUmdMinJs()).Observable.of({
        ...
        type: (_ActionType2 || _ActionType()).SET_BOOKMARK_IS_LOADING
      }).concat((_rxjsBundlesRxUmdMinJs2 || _rxjsBundlesRxUmdMinJs()).Observable.fromPromise(repositoryAsync.deleteBookmark(bookmark.bookmark)).ignoreElements().catch(function (error) {
        atom.notifications.addWarning('Failed Deleting Bookmark', {
          detail: error,
          dismissable: true
        });
