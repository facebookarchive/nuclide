...
var FlowTypeHintProvider = (function () {
  ...
  _createClass(FlowTypeHintProvider, [{
    ...
    value: _asyncToGenerator(function* (editor, position) {
      var enabled = (_commonsAtomFeatureConfig || _load_commonsAtomFeatureConfig()).default.get('nuclide-flow.enableTypeHints');
      if (!enabled) {
        return null;
      }
      ...
      var flowService = yield (0, (_FlowServiceFactory || _load_FlowServiceFactory()).getFlowServiceByNuclideUri)(filePath);
      (0, (_assert || _load_assert()).default)(flowService);

      var enableStructuredTypeHints = (_commonsAtomFeatureConfig || _load_commonsAtomFeatureConfig()).default.get('nuclide-flow.enableStructuredTypeHints');
      ...
      var word = (0, (_commonsAtomRange || _load_commonsAtomRange()).wordAtPosition)(editor, position, (_constants || _load_constants()).JAVASCRIPT_WORD_REGEX);
      var range = undefined;
      if (word) {
        range = word.range;
      } else {
        range = new (_atom || _load_atom()).Range(position, position);
      }
      var result = {
        hint: type,
        range: range
...
function jsonToTree(json) {
  ...
  switch (kind) {
    ...
    case FUNCTION:
      ...
      (0, (_assert || _load_assert()).default)(Array.isArray(paramNames));
