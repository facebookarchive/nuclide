...
var FlowServiceWatcher = (function () {
  function FlowServiceWatcher() {
    ...
    this._subscription = new (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Subscription();

    var serverStatusUpdates = (0, (_FlowServiceFactory || _load_FlowServiceFactory()).getServerStatusUpdates)();
...
function handleNotInstalled(pathToRoot) {
  if (!(_commonsAtomFeatureConfig || _load_commonsAtomFeatureConfig()).default.get(WARN_NOT_INSTALLED_CONFIG)) {
    return;
  }
  ...
  var notification = atom.notifications.addInfo(title, {
    ...
    buttons: [{
      ...
      onDidClick: function onDidClick() {
        notification.dismiss();
        (_commonsAtomFeatureConfig || _load_commonsAtomFeatureConfig()).default.set(WARN_NOT_INSTALLED_CONFIG, false);
      },
