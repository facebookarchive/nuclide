...
var FlowServiceWatcher = (function () {
  function FlowServiceWatcher() {
    ...
    this._subscription = new (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Subscription();
...
function handleNotInstalled(pathToRoot) {
  if (!(_commonsAtomFeatureConfig2 || _commonsAtomFeatureConfig()).default.get(WARN_NOT_INSTALLED_CONFIG)) {
    return;
  }
  ...
  var notification = atom.notifications.addInfo(title, {
    ...
    buttons: [{
      ...
      onDidClick: function onDidClick() {
        notification.dismiss();
        (_commonsAtomFeatureConfig2 || _commonsAtomFeatureConfig()).default.set(WARN_NOT_INSTALLED_CONFIG, false);
      },
