...
var FlowDiagnosticsProvider = (function () {
  function FlowDiagnosticsProvider(shouldRunOnTheFly, busySignalProvider) {
    ...
    var utilsOptions = {
      grammarScopes: new Set(_constants.JS_GRAMMARS),
    ...
    this._requestSerializer = new _nuclideCommons.promises.RequestSerializer();
  ...
  _createDecoratedClass(FlowDiagnosticsProvider, [{
    ...
    value: function _receivedNewUpdateSubscriber(callback) {
      ...
      if (activeTextEditor) {
        var matchesGrammar = _constants.JS_GRAMMARS.indexOf(activeTextEditor.getGrammar().scopeName) !== -1;
        if (matchesGrammar) {
