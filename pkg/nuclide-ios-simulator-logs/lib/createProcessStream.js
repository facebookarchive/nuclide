...
function createProcessStream() {
  ...
  var simctlOutput$ = (0, (_commonsNodeProcess || _load_commonsNodeProcess()).observeProcess)(spawnSimctlList).map(function (event) {
    if (event.kind === 'error') {
      throw event.error;
    }
    return event;
    ...
    (0, (_assert || _load_assert()).default)(typeof event.data === 'string');
    return event.data;
  }).reduce(function (acc, next) {
    return acc + next;
  }, '').map(function (rawJson) {
    (0, (_assert || _load_assert()).default)(typeof rawJson === 'string');
...
function spawnSimctlList() {
  return (0, (_commonsNodeProcess || _load_commonsNodeProcess()).safeSpawn)('xcrun', ['simctl', 'list', '--json']);
}

function tailDeviceLogs(udid) {
  var logDir = (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.join((_os || _load_os()).default.homedir(), 'Library', 'Logs', 'CoreSimulator', udid, 'asl');
  return (0, (_commonsNodeProcess || _load_commonsNodeProcess()).safeSpawn)((_commonsAtomFeatureConfig || _load_commonsAtomFeatureConfig()).default.get('nuclide-ios-simulator-logs.pathToSyslog'), ['-w', '-F', 'xml', '-d', logDir]);
}