...
function _load_parseMessageText() {
  return _parseMessageText = require('../lib/parseMessageText');
}
...
describe('parseMessageText', () => {
  ...
  const parsed = lines.map((_parseMessageText || _load_parseMessageText()).parseMessageText);

  it('parses tags', () => {
    expect(parsed.map(message => message.tags)).toEqual([['tid:com.facebook.react.JavaScript'], ['tid:com.facebook.react.JavaScript'], ['tid:com.facebook.react.JavaScript'], ['tid:com.facebook.react.JavaScript'], ['tid:com.facebook.react.JavaScript'], ['tid:com.facebook.react.JavaScript'], ['core.react', 'tid:main'], ['tid:com.facebook.react.JavaScript']]);
  });

  it('parses levels', () => {
    expect(parsed.map(message => message.level)).toEqual(['info', 'warning', 'error', 'info', 'info', 'info', 'debug', 'info']);
  });

  it('parses text', () => {
    expect(parsed.map(message => message.text)).toEqual([
    /* eslint-disable max-len */
    'This is info', 'This is warn', 'This is error', 'Running application "Something" with appParams: {"rootTag":1,"initialProps":{}}. __DEV__ === true, development-level warning are ON, performance optimizations are OFF', '[Something] Checking for update.', '[Something] App is up to date.', '[RCTModuleData.m:103] The module RCTDiskCacheStore is returning nil from its constructor. You may need to instantiate it yourself and pass it into the bridge.', 'This\nis\ninfo']
    /* eslint-enable max-len */
    );
  });
  ...
  it('ends the tag list when it encounters a tag with brackets in it', () => {
    ...
    const { tags, text } = (0, (_parseMessageText || _load_parseMessageText()).parseMessageText)(line);
