...
function _load_createMessageStream() {
  return _createMessageStream = require('../lib/createMessageStream');
}
...
beforeEach(() => {
  (_featureConfig || _load_featureConfig()).default.set('nuclide-ios-simulator-logs.senderBlacklist', []);
}); /**
...
describe('createMessageStream', () => {
  it('splits the output by record', async () => {
    ...
    jest.spyOn((_featureConfig || _load_featureConfig()).default, 'observeAsStream').mockImplementation(name => name === 'nuclide-ios-simulator-logs.whitelistedTags' ? _rxjsBundlesRxMinJs.Observable.of('.*') : original(name));
    await (async () => {
      const output = _rxjsBundlesRxMinJs.Observable.from(OUTPUT_LINES);
      const messages = await (0, (_createMessageStream || _load_createMessageStream()).createMessageStream)(output).map(message => message.text).toArray().toPromise();
  ...
  it('only includes messages with whitelisted tags', async () => {
    await (async () => {
      const original = (_featureConfig || _load_featureConfig()).default.observeAsStream.bind((_featureConfig || _load_featureConfig()).default);
      jest.spyOn((_featureConfig || _load_featureConfig()).default, 'observeAsStream').mockImplementation(name => name === 'nuclide-ios-simulator-logs.whitelistedTags' ? _rxjsBundlesRxMinJs.Observable.of('X|ExampleTag') : original(name));
      const output = _rxjsBundlesRxMinJs.Observable.from(OUTPUT_LINES);
      const messages = await (0, (_createMessageStream || _load_createMessageStream()).createMessageStream)(output).map(message => message.text).toArray().toPromise();
  ...
  it('shows an error (once) if the regular expression is invalid', async () => {
    ...
    await (async () => {
      const output = _rxjsBundlesRxMinJs.Observable.from(OUTPUT_LINES);
      await (0, (_createMessageStream || _load_createMessageStream()).createMessageStream)(output).toPromise();
