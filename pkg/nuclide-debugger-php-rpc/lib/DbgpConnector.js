...
var DbgpConnector = (function () {
  ...
  _createClass(DbgpConnector, [{
    ...
    value: function listen() {
      ...
      var server = (_net || _load_net()).default.createServer();

      server.on('close', function (socket) {
        return (_utils || _load_utils()).default.log('Closing port ' + _this._port);
      });
      server.listen(this._port, undefined, // Hostname.
      undefined, // Backlog -- the maximum length of the queue of pending connections.
      function () {
        return (_utils || _load_utils()).default.log('Listening on port ' + _this._port);
      });
