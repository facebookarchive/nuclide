...
class HhvmDebugSession extends (_vscodeDebugadapter || _load_vscodeDebugadapter()).LoggingDebugSession {
  ...
  _catchAsyncRequestError(response, fn) {
    fn().catch(error => {
      ...
      this.sendEvent(new (_vscodeDebugadapter || _load_vscodeDebugadapter()).OutputEvent(`HHVM Debugger ran into an error: \`${errorMessage}\``, 'nuclide_notification', { type: 'error' }));
