...
var Connection = (function () {
  function Connection(socket, onStatusCallback, onNotificationCallback, isDummyConnection) {
    ...
    var dbgpSocket = new (_DbgpSocket || _load_DbgpSocket()).DbgpSocket(socket);
    this._socket = dbgpSocket;
    this._dataCache = new (_DataCache || _load_DataCache()).DataCache(dbgpSocket);
    this._id = connectionCount++;
    this._status = (_DbgpSocket2 || _load_DbgpSocket2()).CONNECTION_STATUS.STARTING;
  ...
  _createClass(Connection, [{
    ...
    value: function _handleStatus(callback, newStatus) {
      ...
      switch (newStatus) {
        case (_DbgpSocket2 || _load_DbgpSocket2()).CONNECTION_STATUS.RUNNING:
          this._stopReason = null;
          break;
        case (_DbgpSocket2 || _load_DbgpSocket2()).CONNECTION_STATUS.BREAK:
          if (prevStatus === (_DbgpSocket2 || _load_DbgpSocket2()).CONNECTION_STATUS.BREAK_MESSAGE_RECEIVED) {
            this._stopReason = ASYNC_BREAK;
          } else if (prevStatus !== (_DbgpSocket2 || _load_DbgpSocket2()).CONNECTION_STATUS.BREAK) {
            // TODO(dbonafilia): investigate why we sometimes receive two BREAK_MESSAGES
            this._stopReason = BREAKPOINT;
          }
          break;
        case (_DbgpSocket2 || _load_DbgpSocket2()).CONNECTION_STATUS.DUMMY_IS_VIEWABLE:
          this._isDummyViewable = true;
          return;
        case (_DbgpSocket2 || _load_DbgpSocket2()).CONNECTION_STATUS.DUMMY_IS_HIDDEN:
          this._isDummyViewable = false;
          return;
      }
      if (newStatus === (_DbgpSocket2 || _load_DbgpSocket2()).CONNECTION_STATUS.BREAK_MESSAGE_RECEIVED && prevStatus !== (_DbgpSocket2 || _load_DbgpSocket2()).CONNECTION_STATUS.BREAK_MESSAGE_SENT) {
        return;
      }
    ...
    value: function _isInternalStatus(status) {
      return [(_DbgpSocket2 || _load_DbgpSocket2()).CONNECTION_STATUS.BREAK_MESSAGE_RECEIVED, (_DbgpSocket2 || _load_DbgpSocket2()).CONNECTION_STATUS.BREAK_MESSAGE_SENT, (_DbgpSocket2 || _load_DbgpSocket2()).CONNECTION_STATUS.DUMMY_IS_HIDDEN, (_DbgpSocket2 || _load_DbgpSocket2()).CONNECTION_STATUS.DUMMY_IS_VIEWABLE].some(function (internalStatus) {
    ...
    value: function isViewable() {
      if (this._isDummyConnection) {
        return this._status === (_DbgpSocket2 || _load_DbgpSocket2()).CONNECTION_STATUS.BREAK && this._isDummyViewable;
      } else {
        return this._status === (_DbgpSocket2 || _load_DbgpSocket2()).CONNECTION_STATUS.BREAK;
      }
    }
  }, {
    key: 'onNotification',
    ...
    value: function sendBreakCommand() {
      this._status = (_DbgpSocket2 || _load_DbgpSocket2()).CONNECTION_STATUS.BREAK_MESSAGE_SENT;
