...
var STATUS_ALL_CONNECTIONS_BREAK = 'all_connections_break';
exports.STATUS_ALL_CONNECTIONS_BREAK = STATUS_ALL_CONNECTIONS_BREAK;
var STATUS_USER_ASYNC_BREAK_SENT = 'async_break_sent';
...
var ConnectionMultiplexer = (function () {
  ...
  _createClass(ConnectionMultiplexer, [{
    ...
    value: _asyncToGenerator(function* (socket) {
      ...
      var dummyConnection = new (_Connection2 || _Connection()).Connection(socket, function (status, message) {
        switch (status) {
    ...
    value: _asyncToGenerator(function* (params) {
      ...
      if ((0, (_ConnectionUtils2 || _ConnectionUtils()).isDummyConnection)(message)) {
        ...
        yield this._handleNewConnection(socket);
      }
    })
  }, {
    key: '_handleNewConnection',
    value: _asyncToGenerator(function* (socket) {
      var connection = new (_Connection2 || _Connection()).Connection(socket, this._connectionOnStatus.bind(this), this._handleNotification.bind(this));
    ...
    value: function _shouldPauseAllConnections() {
      return this._status === STATUS_USER_ASYNC_BREAK_SENT || this._status === STATUS_ALL_CONNECTIONS_BREAK;
    }
  }, {
    ...
    value: function _updateStatus() {
      ...
      if (this._status === (_DbgpSocket2 || _DbgpSocket()).STATUS_BREAK || this._status === STATUS_ALL_CONNECTIONS_BREAK) {
    ...
    value: function _shouldEnableConnection(connection) {
      return connection.getStatus() === (_DbgpSocket2 || _DbgpSocket()).STATUS_BREAK && (
      // Only enable connection paused by async_break if user has explicityly issued an async_break.
      connection.stopReason() !== (_Connection4 || _Connection3()).ASYNC_BREAK || this._status === STATUS_USER_ASYNC_BREAK_SENT) && (
    ...
    value: function _pauseConnectionsIfNeeded() {
      if ((0, (_config2 || _config()).getConfig)().stopOneStopAll && this._status !== STATUS_USER_ASYNC_BREAK_SENT) {
        this._asyncBreak();
      }
    }
  }, {
    ...
    value: function _setBreakStatus() {
      this._setStatus(this._status === STATUS_USER_ASYNC_BREAK_SENT || (0, (_config2 || _config()).getConfig)().stopOneStopAll ? STATUS_ALL_CONNECTIONS_BREAK : (_DbgpSocket2 || _DbgpSocket()).STATUS_BREAK);
    }
  }, {
    key: '_setStatus',
    ...
    value: function _resumeBackgroundConnections() {
      for (var connection of this._connections) {
        if (connection !== this._enabledConnection && (connection.stopReason() === (_Connection4 || _Connection3()).ASYNC_BREAK || connection.getStatus() === (_DbgpSocket2 || _DbgpSocket()).STATUS_STARTING)) {
          connection.sendContinuationCommand((_DbgpSocket2 || _DbgpSocket()).COMMAND_RUN);
        }
      }
    }
    ...
    value: function _asyncBreak() {
      for (var connection of this._connections) {
        if (connection.getStatus() === (_DbgpSocket2 || _DbgpSocket()).STATUS_RUNNING) {
