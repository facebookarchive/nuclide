...
var ConnectionMultiplexer = (function () {
  function ConnectionMultiplexer(clientCallback) {
    ...
    this._connectionStatusEmitter = new (_events || _load_events()).default();
  ...
  _createClass(ConnectionMultiplexer, [{
    ...
    value: function onStatus(callback) {
      return (0, (_commonsNodeEvent || _load_commonsNodeEvent()).attachEvent)(this._connectionStatusEmitter, CONNECTION_MUX_STATUS_EVENT, callback);
    }
  }, {
    key: 'onNotification',
    value: function onNotification(callback) {
      return (0, (_commonsNodeEvent || _load_commonsNodeEvent()).attachEvent)(this._connectionStatusEmitter, CONNECTION_MUX_NOTIFICATION_EVENT, callback);
    }
  }, {
    key: 'listen',
    value: function listen() {
      ...
      if (launchScriptPath == null) {
        ...
        (0, (_assert || _load_assert()).default)(xdebugAttachPort !== xdebugLaunchingPort, 'xdebug ports are equal in attach mode');
      ...
      if (launchScriptPath != null) {
        this._launchedScriptProcessPromise = new Promise(function (resolve) {
          _this._launchedScriptProcess = (0, (_helpers || _load_helpers()).launchPhpScriptWithXDebugEnabled)(launchScriptPath, function (text) {
    ...
    value: function _setupConnector(port, disposeConnector) {
      var connector = new (_DbgpConnector || _load_DbgpConnector()).DbgpConnector(port);
    ...
    value: _asyncToGenerator(function* (socket, message) {
      var connection = new (_Connection || _load_Connection()).Connection(socket, this._connectionOnStatus.bind(this), this._handleNotification.bind(this), (0, (_ConnectionUtils || _load_ConnectionUtils()).isDummyConnection)(message));
    ...
    value: function _shouldEnableConnection(connection) {
      ...
      connection.getStopReason() !== (_Connection2 || _load_Connection2()).ASYNC_BREAK || this._status === MULTIPLEXER_STATUS.USER_ASYNC_BREAK_SENT) && (
    ...
    value: function _resumeBackgroundConnections() {
      for (var connection of this._connections.values()) {
        if (connection !== this._enabledConnection && (connection.getStopReason() === (_Connection2 || _load_Connection2()).ASYNC_BREAK || connection.getStatus() === (_DbgpSocket || _load_DbgpSocket()).CONNECTION_STATUS.STARTING)) {
          connection.sendContinuationCommand((_DbgpSocket || _load_DbgpSocket()).COMMAND_RUN);
        }
      }
    }
    ...
    value: _asyncToGenerator(function* () {
      if (this._onlyDummyRemains() && (this._attachConnector == null || this._launchConnector == null || (0, (_config || _load_config()).getConfig)().endDebugWhenNoRequests)) {
