...
let hgAsyncExecute = exports.hgAsyncExecute = (() => {
  var _ref = (0, _asyncToGenerator.default)(function* (args_, options_) {
    ...
    const args = _getHgExecParams.args;
    const options = _getHgExecParams.options;
...
const COMMIT_MESSAGE_STRIP_LINE = /^HG:.*(\n|$)/gm;function hgObserveExecution(args_, options_) {
  ...
  const args = _getHgExecParams2.args;
  const options = _getHgExecParams2.options;
...
function hgRunCommand(args_, options_) {
  ...
  const args = _getHgExecParams3.args;
  const options = _getHgExecParams3.options;
...
function getHgExecParams(args_, options_) {
  let args = args_;
  const options = _extends({}, options_, {
    env: _extends({}, (0, (_process || _load_process()).getOriginalEnvironment)(), {
