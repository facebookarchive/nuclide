...
'diff', 'log', 'show', 'status']);function hgObserveExecution(args_, options_) {
  ...
  return _rxjsBundlesRxMinJs.Observable.fromPromise(getHgExecParams(args_, options_)).switchMap(({ command, args, options }) => {
    return (0, (_process || _load_process()).observeProcess)('script', (0, (_process || _load_process()).createArgsForScriptCommand)(command, args), Object.assign({}, options, { killTreeOnComplete: true, /* TODO(T17353599) */isExitError: () => false })).catch(error => _rxjsBundlesRxMinJs.Observable.of({ kind: 'error', error })); // TODO(T17463635)
  });
}
