...
class HgService {
  ...
  getBlameAtHead(filePath) {
    ...
    return (0, _asyncToGenerator.default)(function* () {
      ...
      let revisionsArray;
      try {
        revisionsArray = yield (0, (_hgRevisionExpressionHelpers || _load_hgRevisionExpressionHelpers()).fetchRevisionsInfo)(uniqueRevisions.join('+'), _this9._workingDirectory, { hidden: true }).toPromise();
      } catch (e) {
        (0, (_nuclideLogging || _load_nuclideLogging()).getLogger)().error(`LocalHgServiceBase failed to fetch blame for file: ${ filePath }. Error: ${ e.stderr }`);
        throw e;
      }
