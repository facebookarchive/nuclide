  ...
  _createClass(HgService, [{
    ...
    value: _asyncToGenerator(function* (filePaths, options) {
      ...
      if (options && 'hgStatusOption' in options) {
        if (options.hgStatusOption === (_hgConstants || _load_hgConstants()).HgStatusOption.ONLY_IGNORED) {
          args.push('--ignored');
        } else if (options.hgStatusOption === (_hgConstants || _load_hgConstants()).HgStatusOption.ALL_STATUSES) {
          args.push('--all');
        }
      }
      ...
      statuses.forEach(function (status) {
        statusMap.set((_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.join(_this2._workingDirectory, status.path), status.status);
      });
      return statusMap;
    })
  }, {
    ...
    key: 'rebase',
    value: function rebase(destination) {
      var source = arguments.length <= 1 || arguments[1] === undefined ? (_hgConstants || _load_hgConstants()).HEAD_REVISION_EXPRESSION : arguments[1];

      var args = ['rebase', '-s', source, '-d', destination];
      var execOptions = {
        cwd: this._workingDirectory
      };
