...
subscribeToFilesCreateAndDelete = subscribeToFilesCreateAndDelete;var _log4js;function _load_log4js() {return _log4js = require('log4js');}var _fsPromise;function _load_fsPromise() {return _fsPromise = _interopRequireDefault(require('nuclide-commons/fsPromise'));}var _nuclideUri;function _load_nuclideUri() {return _nuclideUri = _interopRequireDefault(require('nuclide-commons/nuclideUri'));}var _rxjsBundlesRxMinJs = require('rxjs/bundles/Rx.min.js');function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };} /**
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    */function filesCreateOrDeleteToObserver(fileChanges) {const state = new Map();for (const fileChange of fileChanges) {if (fileChange.exists) {// Only emit create events if watchman says the file is new.
      ...
      if (fileChange.new) {state.set(fileChange.name, true);}} else {state.set(fileChange.name, false);}}if (state.size > 0) {return state;}return null;} /**
      ...
      fileNames.map(fileName => {
        ...
        (_fsPromise || _load_fsPromise()).default.exists(qualifiedFileName).then(exists => {
          if (exists) {
            ...
            (0, (_log4js || _load_log4js()).getLogger)('nuclide-hg-rpc').info(
