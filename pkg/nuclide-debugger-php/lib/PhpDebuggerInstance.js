...
function getConfig() {
  return (_commonsAtomFeatureConfig2 || _commonsAtomFeatureConfig()).default.get('nuclide-debugger-php');
}
...
var PhpDebuggerInstance = (function (_DebuggerInstance) {
  ...
  _createClass(PhpDebuggerInstance, [{
    ...
    value: _asyncToGenerator(function* () {
      ...
      var service = (0, (_nuclideRemoteConnection2 || _nuclideRemoteConnection()).getServiceByNuclideUri)('PhpDebuggerService', this.getTargetUri());
      (0, (_assert2 || _assert()).default)(service);
      var proxy = new service.PhpDebuggerService();
      this._disposables.add(proxy);
      this._proxy = proxy;
      this._observableManager = new (_ObservableManager2 || _ObservableManager()).ObservableManager(proxy.getNotificationObservable().refCount(), proxy.getServerMessageObservable().refCount(), proxy.getOutputWindowObservable().refCount().map(function (message) {
        var serverMessage = (0, (_ChromeMessageRemoting2 || _ChromeMessageRemoting()).translateMessageFromServer)((_commonsNodeNuclideUri2 || _commonsNodeNuclideUri()).default.getHostname(_this.getTargetUri()), message);
      ...
      var sessionConfig = {
        ...
        targetUri: (_commonsNodeNuclideUri2 || _commonsNodeNuclideUri()).default.getPath(this.getTargetUri()),
    ...
    value: function _sendServerMessageToChromeUi(message) {
      ...
      if (webSocket != null) {
        webSocket.send((0, (_ChromeMessageRemoting2 || _ChromeMessageRemoting()).translateMessageFromServer)((_commonsNodeNuclideUri2 || _commonsNodeNuclideUri()).default.getHostname(this.getTargetUri()), message));
      }
    }
  }, {
    key: '_endSession',
