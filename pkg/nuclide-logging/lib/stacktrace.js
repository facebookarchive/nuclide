...
function addPrepareStackTraceHook() {
  (_commonsNodeSingleton || _load_commonsNodeSingleton()).default.get(PREPARE_STACK_TRACE_HOOKED_KEY, function () {
...
function structuredStackTraceHook(error, frames) {
  ...
  error.stackTrace = frames.map(function (frame) {
    var scriptNameOrUrl = frame.getScriptNameOrSourceURL();
    var evalOrigin = scriptNameOrUrl == null ? null : frame.getEvalOrigin();
    return {
...
function defaultPrepareStackTrace(error, frames) {
  ...
  frames.forEach(function (frame) {
    formattedStackTrace += '\n    at ' + (0, (_commonsNodeString || _load_commonsNodeString()).maybeToString)(frame.toString());
  });
...
var __test__ = {
  ...
  resetPrepareStackTraceHooked: function resetPrepareStackTraceHooked() {
    (_commonsNodeSingleton || _load_commonsNodeSingleton()).default.clear(PREPARE_STACK_TRACE_HOOKED_KEY);
  }
};
