...
function _load_string() {return _string = require('../../../modules/nuclide-commons/string');}
...
// `--user-data-dir`.
const { ipcRenderer } = _electron.default;if (!(ipcRenderer != null)) {throw new Error('Invariant violation: "ipcRenderer != null"');}const STDOUT_FILTERS = [// Always shows:
/^window load time: \d+ms\n$/i];const STDERR_FILTERS = [// If the script takes ~1sec or more, and there's another Atom window open,
...
/^Could not connect to indexedDB Event { isTrusted: \[Getter] }\n$/i]; // eslint-disable-next-line no-unused-vars
const debugConsole = global.console; // https://github.com/nodejs/node/blob/v5.1.1/lib/console.js
const outputConsole = new _console.Console({ /* stdout */write(chunk) {if (!STDOUT_FILTERS.some(re => re.test(chunk))) {
...

{
  ...
  params)
  {
    ...
    try {
      ...
      atomGlobal.atomScriptMode = true;if (!(

      typeof process.env.FILE_ATOM_SCRIPT === 'string')) {throw new Error('Invariant violation: "typeof process.env.FILE_ATOM_SCRIPT === \'string\'"');}
      const fileAtomScript = process.env.FILE_ATOM_SCRIPT;if (!(

      typeof process.env.ARGS_ATOM_SCRIPT === 'string')) {throw new Error('Invariant violation: "typeof process.env.ARGS_ATOM_SCRIPT === \'string\'"');}
      ...
      const scriptArgs =
      argsAtomScript === "''" ? [] : (0, (_string || _load_string()).shellParse)(argsAtomScript);
    ...
    return exitCode;
  });function runTest(_x) {return _ref.apply(this, arguments);}return runTest;})();