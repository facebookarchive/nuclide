...
function _Completions() {
  const data = require("../src/Completions");
...
describe('getImportInformation', () => {
  it('Should correctly parse value imports', () => {
    const importInformation = (0, _Completions().getImportInformation)('import {something}');
  ...
  it('Should correctly parse default imports', () => {
    const importInformation = (0, _Completions().getImportInformation)('import TextDocuments ');
    expect(importInformation).toBeDefined();
    expect(importInformation && importInformation.ids.length).toBe(1);
  ...
  it('Should correctly parse type imports', () => {
    const importInformation = (0, _Completions().getImportInformation)('import  type {SomeType }');
    expect(importInformation).toBeDefined();
    expect(importInformation && importInformation.ids.length).toBe(1);
  ...
  it('Should correctly parse multiple values', () => {
    const importInformation = (0, _Completions().getImportInformation)('import {one,  two, three, four}');
    expect(importInformation).toBeDefined();
  ...
  it('Should correctly parse multiple types', () => {
    const importInformation = (0, _Completions().getImportInformation)('import type {One,  Two, Three, Four,}');
    expect(importInformation).toBeDefined();
    expect(importInformation && importInformation.ids.length).toBe(4);
    expect(importInformation && importInformation.ids[0]).toBe('One');
    expect(importInformation && importInformation.ids[1]).toBe('Two');
  ...
  it('Should handle importing type defaults', () => {
    const importInformation = (0, _Completions().getImportInformation)('import type TextDocuments ');
    expect(importInformation).toBeDefined();
    expect(importInformation && importInformation.ids.length).toBe(1);
    expect(importInformation && importInformation.ids[0]).toBe('TextDocuments');
  ...
  it('Should handle all valid Javascript identifiers', () => {
    const importInformation = (0, _Completions().getImportInformation)('import {WORKSPACE_VIEW_URI, $DOLLAR_SIGNS$}');
    expect(importInformation).toBeDefined();
  ...
  it('Should correctly parse incomplete default import', () => {
    const importInformation = (0, _Completions().getImportInformation)('import TextDoc');
    expect(importInformation).toBeDefined();
    expect(importInformation && importInformation.ids.length).toBe(1);
  ...
  it('Should correctly parse incomplete default type import', () => {
    const importInformation = (0, _Completions().getImportInformation)('import type TextDoc');
    expect(importInformation).toBeDefined();
    expect(importInformation && importInformation.ids.length).toBe(1);
    expect(importInformation && importInformation.ids[0]).toBe('TextDoc');
    expect(importInformation && importInformation.importType).toBe('defaultType');
  ...
  it('Should correctly parse incomplete named value import', () => {
    const importInformation = (0, _Completions().getImportInformation)('import {AutoImpor');
    expect(importInformation).toBeDefined();
    expect(importInformation && importInformation.ids.length).toBe(1);
  ...
  it('Should correctly parse incomplete named type import', () => {
    const importInformation = (0, _Completions().getImportInformation)('import type {AutoImpor');
    expect(importInformation).toBeDefined();
    expect(importInformation && importInformation.ids.length).toBe(1);
    expect(importInformation && importInformation.ids[0]).toBe('AutoImpor');
    expect(importInformation && importInformation.importType).toBe('namedType');
  ...
  it('Should correctly parse incomplete require', () => {
    const importInformation = (0, _Completions().getImportInformation)('const CSS');
    expect(importInformation).toBeDefined();
    expect(importInformation && importInformation.ids.length).toBe(1);
  ...
  it('Should not match const with leading whitespace', () => {
    let importInformation = (0, _Completions().getImportInformation)(' const CSS');
    expect(importInformation).toBeNull();
    importInformation = (0, _Completions().getImportInformation)('  const CSS');
    expect(importInformation).toBeNull();
    importInformation = (0, _Completions().getImportInformation)('      const CSS');
    expect(importInformation).toBeNull();
  });
  it('Should correctly parse destructured require statement', () => {
    const importInformation = (0, _Completions().getImportInformation)('const {someId, someOtherType}');
    expect(importInformation).toBeDefined();
    expect(importInformation && importInformation.ids.length).toBe(2);
  ...
  it('Should correctly parse incomplete destructured require statement', () => {
    const importInformation = (0, _Completions().getImportInformation)('const {some');
    expect(importInformation).toBeDefined();
    expect(importInformation && importInformation.ids.length).toBe(1);
...
describe('Completion Functions', () => {
  ...
  beforeEach(() => {
    autoImportsManager = new (_AutoImportsManager().AutoImportsManager)([]);
    importsFormatter = new (_ImportFormatter().ImportFormatter)(['/a/node_modules'], false);
  ...
  describe('provideFullImportCompletions', () => {
    it('returns completions in order', () => {
      ...
      const completions = (0, _Completions().provideFullImportCompletions)(importInformation, importsFormatter, autoImportsManager, '/a/b/test.js', '', 0); // 1) IDs need to be ordered by relevance.
  ...
  describe('provideImportFileCompletions', () => {
    it('provides exact matches', () => {
      ...
      const completions = (0, _Completions().provideImportFileCompletions)(importInformation, importsFormatter, autoImportsManager, '/a/b/test.js', '', 0);
      expect(completions.map(getCompletionText)).toEqual(["import {test, testAbove} from '../above';"]);
      importInformation.ids = ['test'];
      const singleCompletions = (0, _Completions().provideImportFileCompletions)(importInformation, importsFormatter, autoImportsManager, '/a/b/test.js', '', 0);
    ...
    it('does not complete within the same file', () => {
      ...
      const completions = (0, _Completions().provideImportFileCompletions)(importInformation, importsFormatter, autoImportsManager, '/a/b/test3.js', '', 0);
      expect(completions.map(getCompletionText)).toEqual([// Note the absence of test3.js.
