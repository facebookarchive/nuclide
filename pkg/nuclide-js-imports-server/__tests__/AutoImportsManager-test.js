...
describe('AutoImportsManager', () => {
  it('Can provide a basic missing value import', () => {
    ...
    const file2 = 'Foo();';
    const autoImportsManager = new (_AutoImportsManager || _load_AutoImportsManager()).AutoImportsManager([]);
    autoImportsManager.indexFile('file1.js', file1);
    const missingImports = autoImportsManager.findMissingImports('file2.js', file2);
  ...
  it('Does not provide imports if no import with that id is indexed.', () => {
    ...
    const autoImportsManager = new (_AutoImportsManager || _load_AutoImportsManager()).AutoImportsManager([]);
    const missingImports = autoImportsManager.findMissingImports('file1,js', file);
    expect(missingImports).toBeDefined();
  ...
  it('Can filter imports by range', () => {
    ...
    const exactRange = autoImportsManager.getSuggestedImportsForRange(fileName, {
