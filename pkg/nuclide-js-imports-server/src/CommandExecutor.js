'use strict';Object.defineProperty(exports, "__esModule", { value: true });exports.CommandExecutor = undefined;exports.
...
getEditsForImport = getEditsForImport;var _collection;function _load_collection() {return _collection = require('nuclide-commons/collection');}var _vscodeLanguageserver;function _load_vscodeLanguageserver() {return _vscodeLanguageserver = require('vscode-languageserver');}var _ImportFormatter;function _load_ImportFormatter() {return _ImportFormatter = require('./lib/ImportFormatter');}var _nuclideUri;function _load_nuclideUri() {return _nuclideUri = _interopRequireDefault(require('nuclide-commons/nuclideUri'));}var _AutoImportsManager;function _load_AutoImportsManager() {return _AutoImportsManager = require('./lib/AutoImportsManager');}var _simpleTextBuffer;function _load_simpleTextBuffer() {return _simpleTextBuffer = require('simple-text-buffer');}var _util;function _load_util() {return _util = require('./utils/util');}var _lspUtils;function _load_lspUtils() {return _lspUtils = require('../../nuclide-lsp-implementation-common/lsp-utils');}function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}class CommandExecutor {constructor(connection, autoImportsManager, importFormatter, documents) {this.connection = connection;this.autoImportsManager = autoImportsManager;this.importFormatter = importFormatter;this.documents = documents;}executeCommand(command, args) {switch (command) {case 'addImport':return this._addImport(args);default:command;throw new Error(`Unexpected command ${command}`);}}_addImport(args) {const [missingImport, fileMissingImport] = args;const ast = (0, (_AutoImportsManager || _load_AutoImportsManager()).parseFile)(this.documents.get((_nuclideUri || _load_nuclideUri()).default.nuclideUriToUri(fileMissingImport)).getText());if (ast == null || ast.program == null || ast.program.body == null) {// File could not be parsed. If this is reached, we shouldn't be applying
      // addImport anyways since the file must have changed from when we computed
      // the CodeAction.
      return;}const { body } = ast.program;const edits = getEditsForImport(this.importFormatter, fileMissingImport, missingImport, body);const lspUri = (_nuclideUri || _load_nuclideUri()).default.nuclideUriToUri(fileMissingImport); // Version 2.0 LSP
    const changes = {};changes[lspUri] = edits; // Version 3.0 LSP
    const documentChanges = [{ textDocument: { uri: lspUri, version: this.documents.get(lspUri).version }, edits }];this.connection.workspace.applyEdit({ changes, documentChanges });}getEditsForFixingAllImports(fileMissingImport) {const fileMissingImportUri = (_nuclideUri || _load_nuclideUri()).default.nuclideUriToUri(fileMissingImport);const ast = (0, (_AutoImportsManager || _load_AutoImportsManager()).parseFile)(this.documents.get(fileMissingImportUri).getText());if (ast == null || ast.program == null || ast.program.body == null) {// TODO(T24077432): Figure out when this happens and throw an error
      return [];}const { body } = ast.program;return (0, (_collection || _load_collection()).arrayFlatten)(this.autoImportsManager.findMissingImportsInAST(fileMissingImport, ast, false).map(({ filesWithExport, symbol }) => {if (filesWithExport.length === 0) {return undecidableImportEdits();}const missingImport = findClosestImport(symbol.id, fileMissingImport, filesWithExport);if (!missingImport) {return undecidableImportEdits();}return getEditsForImport(this.importFormatter, fileMissingImport, missingImport, body);}));}}exports.CommandExecutor = CommandExecutor; /**
...
function createEdit(
insertText,
{ row, column, indent, newLinesAfter, newLinesBefore })
{
  return {
...
function findSmallestByMeasure(
list,
measure)
{
