...
function addFileToIndex(root, fileRelative, hasteSettings, fileContents) {
  const file = (_nuclideUri || _load_nuclideUri()).default.join(root, fileRelative);
  return Promise.all([getExportsForFile(file, fileContents, hasteSettings), checkIfMain(file)]).then(([data, directoryForMainFile]) => {
...
function setupParentMessagesHandler(root, hasteSettings) {
  process.on('message', (() => {
    var _ref5 = (0, _asyncToGenerator.default)(function* (message) {
      ...
      try {
        const exportUpdate = yield addFileToIndex(root, (_nuclideUri || _load_nuclideUri()).default.relative(root, fileUri), hasteSettings, fileContents);
