...
let main = (() => {
  var _ref = (0, _asyncToGenerator.default)(function* () {
    ...
    const index = yield (0, (_fileIndex || _load_fileIndex()).getFileIndex)(root, hasteSettings);
    const newCache = new (_ExportCache || _load_ExportCache()).default({ root, hasteSettings });
...
function indexDirectory({ root, exportCache, jsFiles, mainFiles }, hasteSettings, maxWorkers = MAX_WORKERS) {
  ...
  return _rxjsBundlesRxMinJs.Observable.of(cachedUpdates).concat(workerMessages);
}

const getPackageJson = (0, (_lodash || _load_lodash()).default)((() => {
