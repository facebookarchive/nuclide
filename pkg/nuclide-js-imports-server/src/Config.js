...
getEslintEnvs = getEslintEnvs;exports.
...
getConfigFromFlow = getConfigFromFlow;var _nuclideUri;function _load_nuclideUri() {return _nuclideUri = _interopRequireDefault(require('nuclide-commons/nuclideUri'));}var _fs = _interopRequireDefault(require('fs'));var _globals;function _load_globals() {return _globals = _interopRequireDefault(require('globals'));}var _log4js;function _load_log4js() {return _log4js = require('log4js');}function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };} /**
  ...
  return JSON.stringify({ moduleDirs, hasteSettings: { isHaste: settings.isHaste, useNameReducers: settings.useNameReducers, nameReducers: settings.nameReducers.map(reducer => ({ regexp: reducer.regexp.toString(), replacement: reducer.replacement })), nameReducerBlacklist: settings.nameReducerBlacklist.map(String), nameReducerWhitelist: settings.nameReducerWhitelist.map(String) } });}function getEslintEnvs(root) {const eslintFile = (_nuclideUri || _load_nuclideUri()).default.join(root, '.eslintrc');const packageJsonFile = (_nuclideUri || _load_nuclideUri()).default.join(root, 'package.json');return eslintToEnvs(eslintFile) || packageJsonToEnvs(packageJsonFile) || ALL_ENVS;}function getConfigFromFlow(root) {let moduleDirs = [];let hasteSettings = { isHaste: false, useNameReducers: false, nameReducers: [], nameReducerWhitelist: [], nameReducerBlacklist: [] };try {const flowFile = (_nuclideUri || _load_nuclideUri()).default.join(root, '.flowconfig');const flowConfigContents = _fs.default.readFileSync(flowFile, 'utf8');moduleDirs = flowConfigToResolveDirnames(flowFile, flowConfigContents).concat(getYarnWorkspaces(root));hasteSettings = flowConfigToHasteSettings(root, flowConfigContents);
  } catch (error) {}
  return {
    moduleDirs,
    hasteSettings };
