...
function _load_idx() {return _idx = _interopRequireDefault(require('idx'));}var _vscodeLanguageserver;
function _load_vscodeLanguageserver() {return _vscodeLanguageserver = require('vscode-languageserver');}var _vscodeJsonrpc;

function _load_vscodeJsonrpc() {return _vscodeJsonrpc = require('vscode-jsonrpc');}var _log4js;
function _load_log4js() {return _log4js = require('log4js');}var _SafeStreamMessageReader;

function _load_SafeStreamMessageReader() {return _SafeStreamMessageReader = _interopRequireDefault(require('../../../modules/nuclide-commons/SafeStreamMessageReader'));}var _AutoImportsManager;
function _load_AutoImportsManager() {return _AutoImportsManager = require('./lib/AutoImportsManager');}var _TextDocuments;
function _load_TextDocuments() {return _TextDocuments = _interopRequireDefault(require('../../nuclide-lsp-implementation-common/TextDocuments'));}var _ImportFormatter;
function _load_ImportFormatter() {return _ImportFormatter = require('./lib/ImportFormatter');}var _Completions;
function _load_Completions() {return _Completions = require('./Completions');}var _Diagnostics;
function _load_Diagnostics() {return _Diagnostics = require('./Diagnostics');}var _Settings;
function _load_Settings() {return _Settings = require('./Settings');}var _CodeActions;
function _load_CodeActions() {return _CodeActions = require('./CodeActions');}var _CommandExecutor;
function _load_CommandExecutor() {return _CommandExecutor = require('./CommandExecutor');}var _initializeLogging;

function _load_initializeLogging() {return _initializeLogging = _interopRequireDefault(require('../logging/initializeLogging'));}var _Config;
function _load_Config() {return _Config = require('./Config');}var _nuclideUri;
function _load_nuclideUri() {return _nuclideUri = _interopRequireDefault(require('../../../modules/nuclide-commons/nuclideUri'));}var _constantsForClient;
function _load_constantsForClient() {return _constantsForClient = require('./utils/constantsForClient');}var _WorkspaceSymbols;
function _load_WorkspaceSymbols() {return _WorkspaceSymbols = require('./WorkspaceSymbols');}function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };} /**
...
let completion = new (_Completions || _load_Completions()).Completions(
documents,
autoImportsManager,
importFormatter);
...
let commandExecuter = new (_CommandExecutor || _load_CommandExecutor()).CommandExecutor(
connection,
autoImportsManager,
importFormatter,
documents);
...
connection.onInitialize(params => {
  ...
  importFormatter = new (_ImportFormatter || _load_ImportFormatter()).ImportFormatter(
  ...
  commandExecuter = new (_CommandExecutor || _load_CommandExecutor()).CommandExecutor(
  connection,
  autoImportsManager,
  importFormatter,
  documents);
...
textDocumentPosition => {
  const nuclideFormattedUri = (_nuclideUri || _load_nuclideUri()).default.uriToNuclideUri(
...
connection.onDocumentFormatting(params => {
  ...
  params.options.tabSize !== (_constantsForClient || _load_constantsForClient()).TAB_SIZE_SIGNIFYING_FIX_ALL_IMPORTS_FORMATTING ||
  fileUri == null ?
  [] :
...
function shouldProvideDiagnostics(params, root) {var _ref, _ref2;
  const diagnosticsWhitelist =
  ((_ref = params) != null ? (_ref2 = _ref.initializationOptions) != null ? _ref2.diagnosticsWhitelist : _ref2 : _ref) || [];
  return diagnosticsWhitelist.length !== 0 ?
  diagnosticsWhitelist.some(regex => root.match(new RegExp(regex))) :
  (_Settings || _load_Settings()).Settings.shouldProvideDiagnosticsDefault;
}

function shouldUseRequires(params, root) {var _ref3, _ref4;
  const requiresWhitelist =
  ((_ref3 = params) != null ? (_ref4 = _ref3.initializationOptions) != null ? _ref4.requiresWhitelist : _ref4 : _ref3) || [];
