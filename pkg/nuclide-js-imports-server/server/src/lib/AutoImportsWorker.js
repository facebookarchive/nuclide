...
let handleNodeModule = (() => {
  var _ref6 = (0, _asyncToGenerator.default)(function* (root, packageJsonFile) {
    ...
    try {
      ...
      const entryPoint = (_nuclideUri || _load_nuclideUri()).default.join((_nuclideUri || _load_nuclideUri()).default.dirname(packageJsonFile), packageJson.main || 'index.js');
      const fileName = entryPoint.endsWith('.js') ? entryPoint : `${entryPoint}.js`;

      const update = yield getExportsForFile((_nuclideUri || _load_nuclideUri()).default.join(root, fileName));
...
function indexDirectory(root, shouldIndexDefaultExportForEachFile) {
  ...
  files.filter(file => toIgnoreRegex.length === 0 || !(_nuclideUri || _load_nuclideUri()).default.join(root, file).match(toIgnoreRegex))).do(files => {
    ...
    shuffle(files);
    const numWorkers = Math.min(Math.max(1, Math.floor(files.length / MIN_FILES_PER_WORKER)), MAX_WORKERS);
