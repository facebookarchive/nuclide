...
var RepositoryStack = (function () {
  ...
  _createDecoratedClass(RepositoryStack, [{
    ...
    value: _asyncToGenerator(function* () {
      ...
      var revisionsState = yield this._fetchRevisionsState();
    ...
    value: _asyncToGenerator(function* () {
      ...
      var cachedRevisionsState = yield this.getCachedRevisionsState();
    ...
    value: _asyncToGenerator(function* () {
      var revisionsState = yield this.getCachedRevisionsState();
      switch (this._diffOption) {
        ...
        case (_constants2 || _constants()).DiffOption.COMPARE_COMMIT:
          if (revisionsState.compareCommitId == null || revisionsState.compareCommitId === revisionsState.headCommitId) {
            this._selectedFileChanges = this._dirtyFileChanges;
          } else {
        ...
        case (_constants2 || _constants()).DiffOption.LAST_COMMIT:
          ...
          if (headToForkBaseRevisions.length <= 1) {
            ...
            yield this._updateSelectedChangesToCommit(revisionsState, headToForkBaseRevisions[headToForkBaseRevisions.length - 2].id);
          }
          break;
      }
    ...
    value: _asyncToGenerator(function* (revisionsState, beforeCommitId) {
      var latestToOldesRevisions = revisionsState.headToForkBaseRevisions.slice().reverse();
    ...
    value: function _createRevisionsState(revisions) {
      var _hgConstants = (_nuclideHgRpc2 || _nuclideHgRpc()).hgConstants;

      var HEAD_COMMIT_TAG = _hgConstants.HEAD_COMMIT_TAG;
      var CommitPhase = _hgConstants.CommitPhase;

      var hashToRevisionInfo = new Map(revisions.map(function (revision) {
        return [revision.hash, revision];
      }));
      var headRevision = revisions.find(function (revision) {
    ...
    value: _asyncToGenerator(function* () {
      ...
      var revisions = undefined;
      try {
        revisions = yield this._repository.fetchSmartlogRevisions();
      } catch (error) {
    ...
    value: _asyncToGenerator(function* (revisions) {
      ...
      var revisionsFileHistory = yield Promise.all(revisions.map(_asyncToGenerator(function* (revision) {
        ...
        var changes = null;
        if (_this2._revisionIdToFileChanges.has(id)) {
          changes = _this2._revisionIdToFileChanges.get(id);
        } else {
          changes = yield _this2._repository.fetchFilesChangedAtRevision('' + id);
          if (changes == null) {
            throw new Error('Changes not available for revision: ' + id);
          }
          _this2._revisionIdToFileChanges.set(id, changes);
        }
        return changes;
      })));
