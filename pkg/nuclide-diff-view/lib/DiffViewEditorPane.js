...
var DiffViewEditorPane = (function (_React$Component) {
  ...
  _createClass(DiffViewEditorPane, [{
    ...
    value: function _setupDiffEditor() {
      ...
      var editorSubscriptions = this._editorSubscriptions = new (_commonsNodeUniversalDisposable2 || _commonsNodeUniversalDisposable()).default();
      this._subscriptions.add(editorSubscriptions);

      var editorDomElement = this.getEditorDomElement();
      this._diffViewEditor = new (_DiffViewEditor2 || _DiffViewEditor()).default(editorDomElement);
      ...
      if (this.props.onDidChangeScrollTop != null) {
        editorSubscriptions.add(editorDomElement.onDidChangeScrollTop(this.props.onDidChangeScrollTop));
      }

      this.props.onDidUpdateTextEditorElement();
      // TODO(most): Fix by listening to text editor rendering.
      editorSubscriptions.add((_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.interval(100).first().subscribe(function () {
