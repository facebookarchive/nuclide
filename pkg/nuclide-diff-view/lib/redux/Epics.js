...
function diffFileEpic(actions, store) {
  ...
  return actions.ofType((_ActionTypes || _load_ActionTypes()).DIFF_FILE).switchMap(action => {
    ...
    return _rxjsBundlesRxMinJs.Observable.merge(bufferChangeModifed, _rxjsBundlesRxMinJs.Observable.combineLatest(fetchHgDiff, _rxjsBundlesRxMinJs.Observable.merge(bufferReloads, bufferChanges)).debounceTime(20).switchMap(([hgDiff]) => {
      if (hgDiff == null) {
            ...
            case (_constants || _load_constants()).CommitMode.COMMIT:
              {
                ...
                })), _rxjsBundlesRxMinJs.Observable.fromPromise(activeRepository.getTemplateCommitMessage()).map(commitMessage => (_Actions || _load_Actions()).updateCommitState({
...
function commit(actions, store) {
  return actions.ofType((_ActionTypes || _load_ActionTypes()).COMMIT).switchMap(action => {
    ...
    })).do((_streamProcessToConsoleMessages || _load_streamProcessToConsoleMessages()).pipeProcessMessagesToConsole.bind(null, mode, publishUpdates)).ignoreElements(), _rxjsBundlesRxMinJs.Observable.of((_Actions || _load_Actions()).setViewMode((_constants || _load_constants()).DiffMode.BROWSE_MODE), (_Actions || _load_Actions()).updateCommitState((0, (_createEmptyAppState || _load_createEmptyAppState()).getEmptyCommitState)())), optionalPublishAction);
  });
}
