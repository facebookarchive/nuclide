...
function observeStatusChanges(repository) {
  return (0, (_commonsNodeEvent || _load_commonsNodeEvent()).observableFromSubscribeFunction)(repository.onDidChangeStatuses.bind(repository)).debounceTime(UPDATE_STATUS_DEBOUNCE_MS).map(function () {
    return null;
  }).startWith(null);
}
...
function addRepositoryEpic(actions, store) {
  return actions.ofType((_ActionTypes || _load_ActionTypes()).ADD_REPOSITORY).flatMap(function (action) {
    ...
    return observeStatusChanges(repository).map(function () {
      return (_Actions || _load_Actions()).updateDirtyFiles(repository, (0, (_utils || _load_utils()).getDirtyFileChanges)(repository));
...
function updateActiveRepositoryEpic(actions, store) {
  return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.combineLatest(actions.ofType((_ActionTypes || _load_ActionTypes()).UPDATE_ACTIVE_REPOSITORY), observeViewOpen(actions)).switchMap(function (_ref) {
    ...
    var selectedFileUpdates = (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.combineLatest(revisionChanges, diffOptionChanges, compareIdChanges, statusChanges, function (revisions, diffOption, compareId) {
      ...
      return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.concat(compareId != null && revisions.find(function (rev) {
        return rev.id === compareId;
      }) == null ? (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.of((_Actions || _load_Actions()).setCompareId(repository, null)) : (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.empty(), (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.of((_Actions || _load_Actions()).updateLoadingSelectedFiles(repository, true)), (0, (_utils || _load_utils()).getSelectedFileChanges)(repository, diffOption, revisions, compareId).catch(function (error) {
    ...
    var revisionStateUpdates = (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.combineLatest(revisionChanges, revisionStatusChanges).filter(function (_ref5) {
    ...
    return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.merge(selectedFileUpdates, revisionStateUpdates);
  });
}
...
function diffFileEpic(actions, store) {
  return actions.ofType((_ActionTypes || _load_ActionTypes()).DIFF_FILE).switchMap(function (action) {
    ...
    var fetchHgDiff = (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.combineLatest(revisionChanges, diffOptionChanges, compareIdChanges, function (revisions, diffOption, compareId) {
      ...
      var diffOption = _ref8.diffOption;
      var compareId = _ref8.compareId;

      // TODO(most): Add loading states.
      var headToForkBaseRevisions = (0, (_utils || _load_utils()).getHeadToForkBaseRevisions)(revisions);
      return (0, (_utils || _load_utils()).getHgDiff)(hgRepository, filePath, headToForkBaseRevisions, diffOption, compareId).catch(function (error) {
        (0, (_notifications || _load_notifications()).notifyInternalError)(error);
        return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.empty();
      });
    ...
    return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.merge(bufferChangeModifed, (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.combineLatest(fetchHgDiff, (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.merge(bufferReloads, bufferChanges)).map(function (_ref9) {
      var _ref92 = _slicedToArray(_ref9, 1);

      var _ref92$0 = _ref92[0];
      var committedContents = _ref92$0.committedContents;
      var revisionInfo = _ref92$0.revisionInfo;
      return (_Actions || _load_Actions()).updateFileDiff({
        filePath: filePath,
    ...
    }).takeUntil((_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.merge((0, (_commonsNodeEvent || _load_commonsNodeEvent()).observableFromSubscribeFunction)(buffer.onDidDestroy.bind(buffer)), deactiveRepsitory)).concat(clearActiveDiffObservable));
  });
}
