...
var DiffViewComponent = (function (_React$Component) {
  ...
  _createClass(DiffViewComponent, [{
    ...
    value: function _renderTree() {
      ...
      var rootPaths = activeRepository != null ? [activeRepository.getWorkingDirectory()] : [];
    ...
    value: function _renderEditors() {
      ...
      var isLoadingFileDiff = _state.isLoadingFileDiff;
    ...
    value: function _renderNavigation() {
      var diffSections = this.state.diffSections;

      var navigationPaneElement = this._getPaneElement(this._navigationPane);
      var oldEditorElement = this._oldEditorComponent.getEditorDomElement();
      var newEditorElement = this._newEditorComponent.getEditorDomElement();
      var diffViewHeight = Math.max(oldEditorElement.getScrollHeight(), newEditorElement.getScrollHeight(), 1);
      // Protect against zero scroll height while initializring editors.
    ...
    key: '_pixelRangeForDiffSection',
    value: function _pixelRangeForDiffSection(diffSection) {
      var status = diffSection.status;
      var lineNumber = diffSection.lineNumber;
      var lineCount = diffSection.lineCount;

      var textEditorElement = this._diffSectionStatusToEditorElement(status);
      var lineHeight = textEditorElement.getModel().getLineHeightInPixels();
      return {
    ...
    value: function render() {
      ...
      var selectedDiffSectionIndex = _state2.selectedDiffSectionIndex;
    ...
    value: function _onDidChangeScrollTop() {
      var editorElements = [this._oldEditorComponent.getEditorDomElement(), this._newEditorComponent.getEditorDomElement()];

      var elementsScrollCenter = editorElements.map(function (editorElement) {
        var scrollTop = editorElement.getScrollTop();
        return scrollTop + editorElement.clientHeight / 2;
      });

      var selectedDiffSectionIndex = -1;

      var diffSections = this.state.diffSections;

      // TODO(most): Pre-compute the positions of the diff sections.
      // Q: when to invalidate (line edits, UI elements & diff reloads, ..etc.)
      for (var sectionIndex = 0; sectionIndex < diffSections.length; sectionIndex++) {
        var _diffSections$sectionIndex = diffSections[sectionIndex];
        var _status = _diffSections$sectionIndex.status;
        var lineNumber = _diffSections$sectionIndex.lineNumber;

        var textEditorElement = this._diffSectionStatusToEditorElement(_status);
        var sectionPixelTop = textEditorElement.pixelPositionForBufferPosition([lineNumber, 0]).top;

        var sectionEditorIndex = editorElements.indexOf(textEditorElement);
        var sectionEditorScrollCenter = elementsScrollCenter[sectionEditorIndex];
    ...
    value: function _updateLineDiffState(fileState) {
      ...
      var isLoadingFileDiff = fileState.isLoadingFileDiff;
      ...
      var oldEditorElements = inlineComponents.filter(function (component) {
        return !component.inNewEditor;
      });
      var newEditorElements = inlineComponents.filter(function (component) {
        return component.inNewEditor;
      });
