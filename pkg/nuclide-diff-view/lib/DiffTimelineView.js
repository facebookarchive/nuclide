...
let DiffTimelineView = class DiffTimelineView extends _reactForAtom.React.Component {
  ...
  render() {
    ...
    const onSelectionChange = _props.onSelectionChange;

    var _diffModel$getState = diffModel.getState();

    const revisionsState = _diffModel$getState.revisionsState;

    if (revisionsState == null) {
      content = 'Revisions not loaded...';
    } else {
      const compareCommitId = revisionsState.compareCommitId;
      const revisionStatuses = revisionsState.revisionStatuses;
      const headCommitId = revisionsState.headCommitId;
      const headToForkBaseRevisions = revisionsState.headToForkBaseRevisions;
...
function RevisionsTimelineComponent(props) {
  ...
  const compareRevisionId = props.compareRevisionId;
  const revisionStatuses = props.revisionStatuses;
