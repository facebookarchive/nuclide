  ...
  .flatMap(function (decodedJSON) {
    ...
    switch (decodedJSON.type) {
      ...
      case 'phutil:err':
        ...
        break;
      case 'error':
        return (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.throw(new Error('Arc Error: ' + decodedJSON.message));
      default:
...
function getSelectedFileChanges(repository, diffOption, revisions, compareCommitId) {
  ...
  if (beforeCommitId == null) {
    return (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.throw(new Error('compareCommitId cannot be null!'));
  }
...
function getHgDiff(repository, filePath, headToForkBaseRevisions, diffOption, compareId) {
  ...
  if (headCommit == null) {
    return (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.throw(new Error('Cannot fetch hg diff for revisions without head'));
  }
  ...
  switch (diffOption) {
    ...
    case (_constants2 || _constants()).DiffOption.COMPARE_COMMIT:
      ...
      break;
    default:
      return (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.throw(new Error('Invalid Diff Option: ' + diffOption));
  }
  ...
  if (revisionInfo == null) {
    return (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.throw(new Error('Diff Viw Fetcher: revision with id ' + compareCommitId + ' not found'));
  }
...
function viewModeToDiffOption(viewMode) {
  switch (viewMode) {
    ...
    default:
      (0, (_nuclideLogging2 || _nuclideLogging()).getLogger)().error('Unrecognized diff view mode: ' + viewMode);
      return (_constants2 || _constants()).DiffOption.DIRTY;
  }
}
...
function createPhabricatorRevision(repository, publishUpdates, headCommitMessage, publishMessage, amended, isPrepareMode, lintExcuse) {
  ...
  (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.defer(function () {
    var stream = (0, (_nuclideRemoteConnection2 || _nuclideRemoteConnection()).getArcanistServiceByNuclideUri)(filePath).createPhabricatorRevision(filePath, isPrepareMode, lintExcuse).refCount();
