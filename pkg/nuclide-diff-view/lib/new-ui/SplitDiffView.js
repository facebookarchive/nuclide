...
class SplitDiffView {
  ...
  constructor(states, actionCreators) {
    ...
    const updateDiffSubscriptions = _rxjsBundlesRxMinJs.Observable.combineLatest(diffEditorsStream, diffStateStream).do(([diffEditors, fileDiff]) => {
      if (diffEditors == null) {
    ...
    const uiElementsUpdates = _rxjsBundlesRxMinJs.Observable.combineLatest(diffEditorsStream, uiElementStream).do(([diffEditors, fileDiff]) => {
      if (diffEditors == null) {
        // One or both editors were destroyed.
        return;
      }
    ...
    const diffLoadingIndicatorUpdates = diffEditorsStream.switchMap(diffEditors => {
      ...
      return states.map(({ isLoadingFileDiff }) => isLoadingFileDiff).distinctUntilChanged().do(isLoading => {
    ...
    const scrollToFirstChange = diffEditorsStream.switchMap(diffEditors => {
      ...
      .delay(SCROLL_FIRST_CHANGE_DELAY_MS).do(({ fileDiff: { navigationSections } }) => {
        try {
