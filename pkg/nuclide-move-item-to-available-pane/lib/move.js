...
moveUp = moveUp;exports.



moveDown = moveDown;exports.



moveRight = moveRight;exports.



moveLeft = moveLeft;var _nuclideAnalytics;function _load_nuclideAnalytics() {return _nuclideAnalytics = require('../../nuclide-analytics');} /**
                                                                                                                                              ...
                                                                                                                                              * 
                                                                                                                                              * @format
                                                                                                                                              */function moveUp(el) {doSplit(el, 'up', (pane, params) => pane.splitUp(params));}function moveDown(el) {doSplit(el, 'down', (pane, params) => pane.splitDown(params));}function moveRight(el) {doSplit(el, 'right', (pane, params) => pane.splitRight(params));}function moveLeft(el) {doSplit(el, 'left', (pane, params) => pane.splitLeft(params));}function doSplit(el, operation, splitOperation) {const pane = findNearestPane(el) || getCenter().getActivePane();
  if (pane == null) {
    return;
  }
...
operation)
{
  ...
  if (candidatePanes.length === 1) {
    ...
    const xAxisComparator = rect =>
    Math.abs(rect.left - activeRect.left);
    const yAxisComparator = rect =>
    ...
    candidatePanes.sort((pane1, pane2) => {
      ...
      const rect2 = paneToRect.get(pane2);if (!(
      rect1 != null)) {throw new Error('Invariant violation: "rect1 != null"');}if (!(
      rect2 != null)) {throw new Error('Invariant violation: "rect2 != null"');}
