...
function registerLocationFactoryEpic(actions, store) {
  return actions.ofType((_Actions || _load_Actions()).REGISTER_LOCATION_FACTORY).map(function (action) {
    (0, (_assert || _load_assert()).default)(action.type === (_Actions || _load_Actions()).REGISTER_LOCATION_FACTORY);
...
function createViewableEpic(actions, store) {
  return actions.ofType((_Actions || _load_Actions()).CREATE_VIEWABLE).switchMap(function (action) {
    (0, (_assert || _load_assert()).default)(action.type === (_Actions || _load_Actions()).CREATE_VIEWABLE);
    ...
    return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.of((_Actions || _load_Actions()).itemCreated(item, itemType));
  });
}
...
function trackActionsEpic(actions, store) {
  ...
  .map(function (action) {
    (0, (_assert || _load_assert()).default)(action.type === (_Actions || _load_Actions()).ITEM_CREATED);
    var itemType = action.payload.itemType;
...
function trackEpic(actions, store) {
  return actions.ofType((_Actions || _load_Actions()).TRACK).map(function (action) {
    (0, (_assert || _load_assert()).default)(action.type === (_Actions || _load_Actions()).TRACK);
...
function toggleItemVisibilityEpic(actions, store) {
  var toggleActions = actions.filter(function (action) {
    return action.type === (_Actions || _load_Actions()).TOGGLE_ITEM_VISIBILITY && !action.payload.immediate;
  });
  var nextTick = (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.create(function (observer) {
    process.nextTick(function () {
      observer.next();
    });
  });
  var missedActions = toggleActions.buffer(nextTick).take(1).concatAll();
  return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.concat(missedActions, toggleActions).map(function (action) {
    (0, (_assert || _load_assert()).default)(action.type === (_Actions || _load_Actions()).TOGGLE_ITEM_VISIBILITY);
...
function toggleItemVisibilityImmediatelyEpic(actions, store) {
  return actions.filter(function (action) {
    ...
    (0, (_assert || _load_assert()).default)(viewableFactory != null);
    ...
    if (itemsAndLocations.length === 0) {
      ...
      return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.of((_Actions || _load_Actions()).createViewable(itemType));
    }
    ...
    return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.from(itemsAndLocations.map(function (_ref4) {
      var item = _ref4.item;
...
function setItemVisibilityEpic(actions, store) {
  return actions.ofType((_Actions || _load_Actions()).SET_ITEM_VISIBILITY).do(function (action) {
    (0, (_assert || _load_assert()).default)(action.type === (_Actions || _load_Actions()).SET_ITEM_VISIBILITY);
    ...
    (0, (_assert || _load_assert()).default)(location != null);
    if (visible) {
      location.showItem(item);
    } else {
...
function unregisterViewableFactoryEpic(actions, store) {
  return actions.ofType((_Actions || _load_Actions()).UNREGISTER_VIEWABLE_FACTORY).do(function (action) {
    (0, (_assert || _load_assert()).default)(action.type === (_Actions || _load_Actions()).UNREGISTER_VIEWABLE_FACTORY);
...
function unregisterLocationEpic(actions, store) {
  return actions.ofType((_Actions || _load_Actions()).UNREGISTER_LOCATION).do(function (action) {
    (0, (_assert || _load_assert()).default)(action.type === (_Actions || _load_Actions()).UNREGISTER_LOCATION);
