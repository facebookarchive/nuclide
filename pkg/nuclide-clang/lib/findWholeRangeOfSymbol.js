...
function findWholeRangeOfSymbol(textEditor, text, textRange, spelling, extent) {
  if (!spelling || text === spelling) {
    ...
    var newRange = new (_atom || _load_atom()).Range(textRange.start, [textRange.end.row, textRange.end.column + 1]);
    return [newRange];
    ...
    var _ret = (function () {
      ...
      for (var selectorSegment of selectorSegments) {
        ...
        var rangeStart = rangeOfPreviousSegment ? rangeOfPreviousSegment.end : extent.start;
        var rangeToScan = new (_atom || _load_atom()).Range(rangeStart, extent.end);
