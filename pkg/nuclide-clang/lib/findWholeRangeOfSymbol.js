...
findWholeRangeOfSymbol;var _atom = require('atom'); // Matches something like: textA: or textA:textB:
const OBJC_SELECTOR_NAME_REGEX = /([^\s:]+:)+$/g; /**
                                                   ...
                                                   */ /**
                                                       ...
                                                       *  strict
                                                       * @format
                                                       */function findWholeRangeOfSymbol(textEditor, text, textRange, spelling, extent) {// flowlint-next-line sketchy-null-string:off
  if (!spelling || text === spelling) {return [textRange];} else if (text + ':' === spelling) {// Quick check for a common case, an Obj-C selector with one argument.
    const newRange = new _atom.Range(textRange.start, [textRange.end.row, textRange.end.column + 1]);return [newRange];} else if (spelling.match(OBJC_SELECTOR_NAME_REGEX)) {// Obj-C selector with multiple arguments, e.g. doFoo:withBar:
    ...
    const ranges = [];const selectorSegments = spelling.split(':');const iterator = ({ match, matchText, range, stop, replace }) => {if (!matchText) {return;}ranges.push(range);stop();};for (const selectorSegment of selectorSegments) {
