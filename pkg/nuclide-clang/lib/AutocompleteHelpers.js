'use strict';Object.defineProperty(exports, "__esModule", { value: true });exports.__test__ = undefined;var _asyncToGenerator = _interopRequireDefault(require('async-to-generator'));
...
var _atom = require('atom');var _fuzzaldrinPlus;
function _load_fuzzaldrinPlus() {return _fuzzaldrinPlus = _interopRequireDefault(require('fuzzaldrin-plus'));}var _AutocompleteCacher;
function _load_AutocompleteCacher() {return _AutocompleteCacher = _interopRequireDefault(require('../../commons-atom/AutocompleteCacher'));}var _collection;
function _load_collection() {return _collection = require('nuclide-commons/collection');}var _nuclideAnalytics;
function _load_nuclideAnalytics() {return _nuclideAnalytics = require('../../nuclide-analytics');}var _nuclideClangRpc;
function _load_nuclideClangRpc() {return _nuclideClangRpc = require('../../nuclide-clang-rpc');}var _libclang;
function _load_libclang() {return _libclang = require('./libclang');}function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };} /**
                                                                                                                                                                    ...
                                                                                                                                                                    * 
                                                                                                                                                                    * @format
                                                                                                                                                                    */const MAX_LINE_LENGTH = 120;const TAB_LENGTH = 2;const VALID_EMPTY_SUFFIX = /(->|\.|::|\()$/;const ClangCursorToAutocompletionTypes = Object.freeze({ STRUCT_DECL: 'class', UNION_DECL: 'class', CLASS_DECL: 'class',
  ENUM_DECL: 'class',
  FIELD_DECL: 'property',
...
function getCompletionBodyInline(completion) {
  ...
  const lastOptional = (0, (_collection || _load_collection()).arrayFindLastIndex)(chunks, chunk =>
  Boolean(chunk.isOptional && chunk.isPlaceHolder));

  if (lastOptional !== -1) {
    const lastNonOptional = (0, (_collection || _load_collection()).arrayFindLastIndex)(chunks, chunk =>
    Boolean(!chunk.isOptional && chunk.isPlaceHolder));
...
class AutocompleteHelpers {
  ...
  static _getAutocompleteSuggestions(
  request)
  {return (0, _asyncToGenerator.default)(function* () {
      const {
        editor,
      ...
      return data.map(function (completion) {
        ...
        if (completion.cursor_kind === 'OVERLOAD_CANDIDATE') {
          ...
          function (x) {return x.kind === 'CurrentParameter';});
        ...
        (_nuclideClangRpc || _load_nuclideClangRpc()).ClangCursorToDeclarationTypes[completion.cursor_kind] :
        null;
  ...
  }}exports.default = AutocompleteHelpers;AutocompleteHelpers._cacher = new (_AutocompleteCacher || _load_AutocompleteCacher()).default(AutocompleteHelpers._getAutocompleteSuggestions, { updateResults(_originalRequest, request, results) {// TODO: update the ranges once Clang LSP starts returning TextEdits.
    const { editor } = request;const prefix = getCompletionPrefix(editor); // We hit the results limit, so there may be unlisted results.
    // Needs to match the value in clang_server.py.
    if (results.length === 200) {return null;}return (_fuzzaldrinPlus || _load_fuzzaldrinPlus()).default.filter(results, prefix, { key: 'filterText' }).map(result => Object.assign({}, result, { replacementPrefix: prefix }));} });const __test__ = exports.__test__ = {
