...
function getCompletionBodyInline(completion) {
  ...
  var chunks = [].concat(_toConsumableArray(completion.chunks));

  // Merge everything between the last non-optional placeholder
  // and the last optional placeholder into one big optional.
  var lastOptional = (0, (_commonsNodeCollection || _load_commonsNodeCollection()).arrayFindLastIndex)(chunks, function (chunk) {
    return Boolean(chunk.isOptional && chunk.isPlaceHolder);
  });
  if (lastOptional !== -1) {
    var lastNonOptional = (0, (_commonsNodeCollection || _load_commonsNodeCollection()).arrayFindLastIndex)(chunks, function (chunk) {
      return Boolean(!chunk.isOptional && chunk.isPlaceHolder);
    });
    if (lastNonOptional !== -1 && lastNonOptional < lastOptional) {
      var mergedSpelling = '';
      for (var i = lastNonOptional + 1; i <= lastOptional; i++) {
...
function getCompletionPrefix(editor) {
  ...
  range.end = new (_atom || _load_atom()).Point(cursor.getBufferRow(), cursor.getBufferColumn());
...
var AutocompleteHelpers = (function () {
  ...
  _createDecoratedClass(AutocompleteHelpers, null, [{
    ...
    decorators: [(0, (_nuclideAnalytics || _load_nuclideAnalytics()).trackTiming)('nuclide-clang-atom.autocomplete')],
    value: _asyncToGenerator(function* (request) {
      ...
      var data = yield (0, (_libclang || _load_libclang()).getCompletions)(editor, prefix);
      if (data == null) {
        return [];
      }
      ...
      return data.map(function (completion) {
        ...
        var rightLabel = completion.cursor_kind ? (_nuclideClangRpc || _load_nuclideClangRpc()).ClangCursorToDeclarationTypes[completion.cursor_kind] : null;
