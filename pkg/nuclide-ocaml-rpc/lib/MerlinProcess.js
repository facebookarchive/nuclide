...
var getInstance = _asyncToGenerator(function* (file) {
  ...
  var dotMerlinPath = yield (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.findNearestFile('.merlin', file);

  var options = {
    cwd: dotMerlinPath ? (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.dirname(dotMerlinPath) : '.'
  };

  logger.info('Spawning new ocamlmerlin process');
  var process = yield (0, (_commonsNodeProcess || _load_commonsNodeProcess()).safeSpawn)(merlinPath, flags, options);
...
var isInstalled = _asyncToGenerator(function* (merlinPath) {
  if (isInstalledCache == null) {
    var result = yield (0, (_commonsNodeProcess || _load_commonsNodeProcess()).asyncExecute)('which', [merlinPath]);
...
var MerlinProcessBase = (function () {
  function MerlinProcessBase(proc) {
    ...
    this._proc = proc;
    this._promiseQueue = new (_commonsNodePromiseExecutors || _load_commonsNodePromiseExecutors()).PromiseQueue();
    this._running = true;
...
function _runSingleCommand(process, command) {
  ...
  return new Promise(function (resolve, reject) {
    var reader = (_readline || _load_readline()).default.createInterface({
      input: stdout,
      terminal: false
    });
