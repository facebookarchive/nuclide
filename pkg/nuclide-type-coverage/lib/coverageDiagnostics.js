...
function diagnosticProviderForResultStream(results, isEnabledStream) {
  var toggledResults = (0, (_commonsNodeObservable2 || _commonsNodeObservable()).toggle)(results, isEnabledStream);

  return {
    updates: (0, (_commonsNodeObservable2 || _commonsNodeObservable()).compact)(toggledResults.map(diagnosticsForResult)),
    invalidations: (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.merge(
