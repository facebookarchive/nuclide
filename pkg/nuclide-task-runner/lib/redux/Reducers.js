    ...
    case (_Actions || _load_Actions()).TASK_LIST_UPDATED:
      {
        ...
        const taskRunnerId = _action$payload.taskRunnerId;

        const taskRunner = state.taskRunners.get(taskRunnerId);
        const taskRunnerName = taskRunner && taskRunner.name;
        const annotatedTaskList = taskList.map(taskMeta => _extends({}, taskMeta, { taskRunnerId: taskRunnerId, taskRunnerName: taskRunnerName }));
