...
var Activation = (function () {
  function Activation(rawState) {
    ...
    (0, (_commonsAtomSyncAtomCommands2 || _commonsAtomSyncAtomCommands()).default)(states.debounceTime(500).map(function (state) {
      ...
      var allTasks = (_Array$prototype = Array.prototype).concat.apply(_Array$prototype, _toConsumableArray(Array.from(tasks.values())));
      var tasksByType = new Map();
      ...
      return {
        'atom-workspace': _defineProperty({}, 'nuclide-task-runner:' + task.type, function () {
          _this._actionCreators.runTask(task);
        })
      };
    }, function (task) {
      return task.taskRunnerId + ':' + task.type;
    }));
  }
