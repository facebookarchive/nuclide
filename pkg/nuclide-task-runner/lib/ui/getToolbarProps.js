...
function getToolbarProps(store) {
  ...
  const props = _RxMin.Observable.combineLatest(stickyProps, alwaysUpToDateProps, (a, b) => Object.assign({}, a, b)) // Force events to be omitted in the next microtask rather than synchronously
  // TODO(T34166602) Fix the underlying issue which has been relying on this behavior
  .observeOn(_RxMin.Scheduler.asap).let((0, _observable().throttle)(() => _observable().nextAnimationFrame));
