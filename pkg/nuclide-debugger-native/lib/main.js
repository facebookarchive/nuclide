...
class Activation {
  ...
  _debugPidWithLLDB(pid, buckRoot) {
    ...
    return (0, _asyncToGenerator.default)(function* () {
      ...
      if (yield (0, (_passesGK || _load_passesGK()).default)('nuclide_debugger_native_vsp')) {
        attachInfo = yield (0, (_utils3 || _load_utils3()).getNativeVSPAttachProcessInfo)((_nuclideDebuggerCommon || _load_nuclideDebuggerCommon()).VsAdapterTypes.NATIVE_LLDB, buckRoot, pid, (_nuclideUri || _load_nuclideUri()).default.getPath(buckRoot));
      } else {
        attachInfo = yield _this._getAttachProcessInfoFromPid(pid, buckRoot);
      }
  ...
  _debugBuckTarget(buckService, buckRoot, buildTarget, runArguments) {
    return (0, _asyncToGenerator.default)(function* () {
      ...
      if (yield (0, (_passesGK || _load_passesGK()).default)('nuclide_debugger_native_vsp')) {
        info = yield (0, (_utils3 || _load_utils3()).getNativeVSPLaunchProcessInfo)((_nuclideDebuggerCommon || _load_nuclideDebuggerCommon()).VsAdapterTypes.NATIVE_LLDB, (_nuclideUri || _load_nuclideUri()).default.join(buckRoot, relativeOutputPath), (runArguments.length ? runArguments : targetOutput.args) || [], remoteBuckRoot, env, remoteBuckRoot);
      } else {
        info = new (_LaunchProcessInfo || _load_LaunchProcessInfo()).LaunchProcessInfo(buckRoot, {
