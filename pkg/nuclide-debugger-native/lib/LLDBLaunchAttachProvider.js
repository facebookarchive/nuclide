...
function _load_NativeActionUIProvider() {
  return _NativeActionUIProvider = _interopRequireWildcard(require('./actions/NativeActionUIProvider'));
}
...
class LLDBLaunchAttachProvider extends (_nuclideDebuggerBase || _load_nuclideDebuggerBase()).DebuggerLaunchAttachProvider {
  ...
  constructor(debuggingTypeName, targetUri) {
    ...
    this._loadAction(_NativeActionUIProvider || _load_NativeActionUIProvider());
    try {
      // $FlowFB
  ...
  getCallbacksForAction(action) {
    ...
    return {
      ...
      getComponent: (debuggerTypeName, configIsValidChanged) => {
        ...
        if (provider) {
          return provider.getComponent(this._targetUri, this._store, this._actions, debuggerTypeName, action, configIsValidChanged);
        }
        return null;
      }
    };
