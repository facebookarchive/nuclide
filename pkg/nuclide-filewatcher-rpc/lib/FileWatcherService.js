...
function watchEntity(entityPath, isFile) {
  return (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.fromPromise(getRealPath(entityPath, isFile)).flatMap(function (realPath) {
    var observable = entityObservable.get(realPath);
    if (observable != null) {
      return observable;
    }
    observable = (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.create(function (observer) {
...
function watchDirectoryRecursive(directoryPath) {
  ...
  if (client.hasSubscription(directoryPath)) {
    return (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.of('EXISTING').publish();
  }
  return (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.fromPromise(client.watchDirectoryRecursive(directoryPath)).flatMap(function (watcher) {
...
function onWatcherChange(subscription, entries) {
  ...
  entries.forEach(function (entry) {
    var entryPath = (_commonsNodeNuclideUri2 || _commonsNodeNuclideUri()).default.join(subscription.root, entry.name);
    ...
    if (entry.new || !entry.exists) {
      directoryChanges.add((_commonsNodeNuclideUri2 || _commonsNodeNuclideUri()).default.dirname(entryPath));
    }
  });
