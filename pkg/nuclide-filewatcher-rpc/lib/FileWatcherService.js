...
function watchWithNode(watchedPath, isDirectory) {
  return _rxjsBundlesRxMinJs.Observable.create(observer => {
    const watcher = _fs.default.watch(watchedPath, { persistent: false },
    ...
    (eventType, fileName) => {
      ...
      if (isDirectory) {
        ...
        path = (_nuclideUri || _load_nuclideUri()).default.join(watchedPath, fileName);
      }
      if (eventType === 'rename') {
...
async function getRealPath(entityPath, isFile) {
  ...
  const stat = await (_fsPromise || _load_fsPromise()).default.stat(entityPath);
