...
const entityObservable = new Map();
...
function watchEntity(entityPath, isFile) {
  return _rxjsBundlesRxMinJs.Observable.fromPromise(getRealPath(entityPath, isFile)).flatMap(realPath => {
    let observable = entityObservable.get(realPath);
    if (observable != null) {
      return observable;
    }
    observable = _rxjsBundlesRxMinJs.Observable.create(observer => {
      entityObserver.set(realPath, observer);
      return () => {
        entityObserver.delete(realPath);
        entityObservable.delete(realPath);
      };
    }).map(type => ({ path: realPath, type: type })).share();
