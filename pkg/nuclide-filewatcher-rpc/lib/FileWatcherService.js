...
function watchDirectoryRecursive(directoryPath) {
  ...
  return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.fromPromise(client.watchDirectoryRecursive(directoryPath, 'filewatcher-' + directoryPath,
  // Reloading with file changes should happen
  // during source control operations to reflect the file contents / tree state.
  { defer_vcs: false })).flatMap(function (watcher) {
