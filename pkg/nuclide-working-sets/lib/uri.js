...
export function dedupeNormalizedUris(uris: Array<string>): Array<string> {
  ...
  return dedepped.filter((u, i) => {
    const sep = nuclideUri.pathModuleFor(u).sep;
...
export function splitUri(uri: string): Array<string> {
  const sep = nuclideUri.pathModuleFor(uri).sep;
...
export function isUriBelow(ancestorUri: string, descendantUri: string): boolean {
  const sep = nuclideUri.pathModuleFor(ancestorUri).sep;
...
function normalizePath(path?: string): string {
  invariant(path);
  const normalized = nuclideUri.normalize(path);
  const sep = nuclideUri.pathModuleFor(path).sep;
