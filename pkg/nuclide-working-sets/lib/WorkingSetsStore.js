...
class WorkingSetsStore {
  constructor() {
    ...
    this.getDefinitions = (0, _memoizeUntilChanged().default)(this.getDefinitions, () => [this._savedDefinitions, this._activeProjectDefinition]);
  }
  ...
  updateSavedDefinitions(definitions) {
    if ((0, _collection().arrayEqual)(this._savedDefinitions, definitions)) {
      return;
    }
  ...
  _updateDefinitions(definitions) {
    ...
    const {
      saved,
      activeProject
    } = (0, _groupBy2().default)(definitions, d => d.isActiveProject ? 'activeProject' : 'saved');
