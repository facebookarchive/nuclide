...
function search(directory, regex, subdirs) {
  ...
  if (!subdirs || subdirs.length === 0) {
    ...
    return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.from(subdirs).concatMap(_asyncToGenerator(function* (subdir) {
      try {
        var stat = yield (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.lstat((_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.join(directory, subdir));
        if (stat.isDirectory()) {
...
function searchInSubdir(matchesByFile, directory, subdir, regex) {
  ...
  var cmdDir = (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.join(directory, subdir);
  var linesSource = getLinesFromCommand('hg', ['wgrep'].concat(vcsargs), cmdDir).catch(function () {
    ...
    return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.throw(new Error('Failed to execute a grep search.'));
  });
  ...
  return linesSource.flatMap(function (line) {
    ...
    var filePath = (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.join(subdir, grepMatchResult[1]);
...
function getLinesFromCommand(command, args, localDirectoryPath) {
  return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.create(function (observer) {
    ...
    var proc = (0, (_commonsNodeProcess || _load_commonsNodeProcess()).safeSpawn)(command, args, { cwd: localDirectoryPath });
