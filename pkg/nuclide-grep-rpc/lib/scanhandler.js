...
function search(directory, regex, subdirs) {
  ...
  if (!subdirs || subdirs.length === 0) {
    ...
    return (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.from(subdirs).concatMap(_asyncToGenerator(function* (subdir) {
      try {
...
function searchInSubdir(matchesByFile, directory, subdir, regex) {
  ...
  var linesSource = getLinesFromCommand('hg', ['wgrep'].concat(vcsargs), cmdDir).catch(function () {
    ...
    return (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.throw(new Error('Failed to execute a grep search.'));
  });
...
function getLinesFromCommand(command, args, localDirectoryPath) {
  return (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.create(function (observer) {
    ...
    var proc = (0, (_commonsNodeProcess2 || _commonsNodeProcess()).safeSpawn)(command, args, { cwd: localDirectoryPath });
