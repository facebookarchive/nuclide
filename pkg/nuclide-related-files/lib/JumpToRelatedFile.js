...
class JumpToRelatedFile {
  ...
  constructor() {
    this._subscription = atom.commands.add('atom-workspace', {
      'nuclide-related-files:switch-between-header-source': () => {
        ...
        if (path) {
          (0, (_nuclideAnalytics || _load_nuclideAnalytics()).trackTiming)('nuclide-related-files:switch-between-header-source', async () => this._open((await this.getNextRelatedFile(path))));
        }
      },
      'nuclide-related-files:jump-to-next-related-file': () => {
        ...
        // flowlint-next-line sketchy-null-string:off
        if (path) {
          (0, (_nuclideAnalytics || _load_nuclideAnalytics()).trackTiming)('nuclide-related-files:jump-to-next-related-file', async () => this._open((await this.getNextRelatedFile(path))));
        }
      },
      'nuclide-related-files:jump-to-previous-related-file': () => {
        ...
        // flowlint-next-line sketchy-null-string:off
        if (path) {
          (0, (_nuclideAnalytics || _load_nuclideAnalytics()).trackTiming)('nuclide-related-files:jump-to-previous-related-file', async () => this._open((await this.getPreviousRelatedFile(path))));
        }
      }
    });
  }
  ...
  async getNextRelatedFile(path) {
    const { relatedFiles, index } = await (_RelatedFileFinder || _load_RelatedFileFinder()).default.find(path, this._getFileTypeWhitelist());
    if (index === -1) {
      return path;
    }
