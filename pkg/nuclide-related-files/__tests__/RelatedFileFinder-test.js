...
function mockFiles(files) {
  // $FlowFixMe
  _nuclideRemoteConnection().getFileSystemServiceByNuclideUri.mockReturnValue({
    readdir: async () => {
...
describe('RelatedFileFinder', () => {
  describe('@find', () => {
    it('finds related file with a different extension', async () => {
      ...
      expect((await _RelatedFileFinder().default.find('dir/Test.m'))).toEqual({
    ...
    it('finds related file whose name ends with `-inl`', async () => {
      ...
      expect((await _RelatedFileFinder().default.find('dir/Test.h'))).toEqual({
    ...
    it('finds related file whose name ends with `t` and itself', async () => {
      ...
      expect((await _RelatedFileFinder().default.find('dir/Test.m', new Set(['.t'])))).toEqual({
    ...
    it('finds related file but nothing and then return all', async () => {
      mockFiles(['Test.m', 'Test.v', 'Test.t']);
      expect((await _RelatedFileFinder().default.find('dir/Test.m', new Set(['.o'])))).toEqual({
    ...
    it('finds related file but only itself', async () => {
      mockFiles(['Test.m', 'Test.v', 'Test.t']);
      expect((await _RelatedFileFinder().default.find('dir/Test.m', new Set(['.m'])))).toEqual({
        relatedFiles: ['dir/Test.m'],
        index: 0
      });
    });
    it('finds related file from a provider', async () => {
      _RelatedFileFinder().default.registerRelatedFilesProvider({
