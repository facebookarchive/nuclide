...
var doSearchDirectory = _asyncToGenerator(function* (directoryUri, query) {
  ...
  if (search === undefined) {
    var directory = _nuclideRemoteUri2.default.parse(directoryUri).path;

    var exists = yield _nuclideCommons.fsPromise.exists(directory);
    if (!exists) {
      throw new Error('Could not find directory to search : ' + directory);
    }

    var stat = yield _nuclideCommons.fsPromise.stat(directory);
    if (!stat.isDirectory()) {
      throw new Error('Provided path is not a directory : ' + directory);
    }

    search = yield (0, _nuclidePathSearch.fileSearchForDirectory)(directoryUri);
...
var _nuclidePathSearch = require('../../../nuclide-path-search');
