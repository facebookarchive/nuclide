...
var newFile = _asyncToGenerator(function* (filePath) {
  var isExistingFile = yield (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.exists(filePath);
  if (isExistingFile) {
    return false;
  }
  yield (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.mkdirp((_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.dirname(filePath));
  yield (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.writeFile(filePath, '');
  return true;
}
...
var readdir = _asyncToGenerator(function* (path) {
  var files = yield (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.readdir(path);
  var entries = yield Promise.all(files.map(_asyncToGenerator(function* (file) {
    var fullpath = (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.join(path, file);
    var lstats = yield (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.lstat(fullpath);
    if (!lstats.isSymbolicLink()) {
      return { file: file, stats: lstats, isSymbolicLink: false };
    } else {
      try {
        var _stats = yield (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.stat(fullpath);
  ...
  return (0, (_commonsNodeCollection || _load_commonsNodeCollection()).arrayCompact)(entries).map(function (entry) {
...
var move = _asyncToGenerator(function* (sourcePaths, destDir) {
  yield Promise.all(sourcePaths.map(function (path) {
    var destPath = (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.join(destDir, (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.basename(path));
    return (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.move(path, destPath);
  }));
}
...
var copy = _asyncToGenerator(function* (sourcePath, destinationPath) {
  var isExistingFile = yield (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.exists(destinationPath);
  if (isExistingFile) {
    return false;
  }
  yield (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.copy(sourcePath, destinationPath);
...
var rmdirAll = _asyncToGenerator(function* (paths) {
  yield Promise.all(paths.map(function (p) {
    return (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.rmdir(p);
  }));
}
...
var readFile = _asyncToGenerator(function* (path, options) {
  var stats = yield (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.stat(path);
  if (stats.size > READFILE_SIZE_LIMIT) {
    throw new Error('File is too large (' + stats.size + ' bytes)');
  }
  return (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.readFile(path, options);
}
...
var copyFilePermissions = _asyncToGenerator(function* (sourcePath, destinationPath) {
  ...
  try {
    permissions = (yield (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.stat(sourcePath)).mode;
  } catch (e) {
  ...
  if (permissions != null) {
    yield (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.chmod(destinationPath, permissions);
  }
}
...
var writeFile = _asyncToGenerator(function* (path, data, options) {
  ...
  var complete = false;
  var tempFilePath = yield (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.tempfile('nuclide');
  try {
    yield (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.writeFile(tempFilePath, data, options);
    ...
    if (!complete) {
      yield (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.unlink(tempFilePath);
    }
  }
});
...
function exists(path) {
  return (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.exists(path);
}

function findNearestFile(fileName, pathToDirectory) {
  return (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.findNearestFile(fileName, pathToDirectory);
}
...
function lstat(path) {
  return (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.lstat(path);
}
...
function mkdir(path) {
  return (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.mkdir(path);
}
...
function mkdirp(path) {
  return (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.mkdirp(path);
}
...
function chmod(path, mode) {
  return (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.chmod(path, mode);
}

function realpath(path) {
  return (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.realpath(path);
}
...
function resolveRealPath(path) {
  return (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.realpath((_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.expandHomeDir(path));
}
...
function rename(sourcePath, destinationPath) {
  return (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.move(sourcePath, destinationPath);
}

function rmdir(path) {
  return (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.rmdir(path);
}

function stat(path) {
  return (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.stat(path);
}
...
function unlink(path) {
  return (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.unlink(path).catch(function (error) {
...
function isNfs(path) {
  return (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.isNfs(path);
}

// TODO: Move to nuclide-commons
function mvPromise(sourcePath, destinationPath) {
  return new Promise(function (resolve, reject) {
    (0, (_mv || _load_mv()).default)(sourcePath, destinationPath, { mkdirp: false }, function (error) {
      if (error) {
        reject(error);
      } else {
        resolve();
