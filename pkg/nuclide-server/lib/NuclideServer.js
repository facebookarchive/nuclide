'use strict';Object.defineProperty(exports, "__esModule", { value: true });exports.HEARTBEAT_CHANNEL = undefined;var _asyncToGenerator = _interopRequireDefault(require('async-to-generator'));
...
function _load_log4js() {return _log4js = require('log4js');}var _ws;
function _load_ws() {return _ws = _interopRequireDefault(require('ws'));}var _UniversalDisposable;
function _load_UniversalDisposable() {return _UniversalDisposable = _interopRequireDefault(require('nuclide-commons/UniversalDisposable'));}var _blocked;

function _load_blocked() {return _blocked = _interopRequireDefault(require('./blocked'));}var _QueuedAckTransport;
function _load_QueuedAckTransport() {return _QueuedAckTransport = require('big-dig/src/socket/QueuedAckTransport');}var _utils;
function _load_utils() {return _utils = require('./utils');}var _nuclideAnalytics;
function _load_nuclideAnalytics() {return _nuclideAnalytics = require('../../nuclide-analytics');}var _nuclideVersion;
function _load_nuclideVersion() {return _nuclideVersion = require('../../nuclide-version');}var _nuclideLogging;
function _load_nuclideLogging() {return _nuclideLogging = require('../../nuclide-logging');}var _nuclideRpc;
function _load_nuclideRpc() {return _nuclideRpc = require('../../nuclide-rpc');}var _WebSocketTransport;
function _load_WebSocketTransport() {return _WebSocketTransport = require('big-dig/src/socket/WebSocketTransport');}var _nuclideMarshalersCommon;
function _load_nuclideMarshalersCommon() {return _nuclideMarshalersCommon = require('../../nuclide-marshalers-common');}function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}
 ...
 * 
 * @format
 */const connect = require('connect'); // eslint-disable-next-line rulesdir/no-commonjs
const http = require('http'); // eslint-disable-next-line rulesdir/no-commonjs
const https = require('https');const logger = (0, (_log4js || _load_log4js()).getLogger)('nuclide-server');
...
class NuclideServer {
  ...
  constructor(options, services) {this._disposables = new (_UniversalDisposable || _load_UniversalDisposable()).default();if (!(
    NuclideServer._theServer == null)) {throw new Error('Invariant violation: "NuclideServer._theServer == null"');}
    ...
    if (trackEventLoop) {
      const stallTracker = new (_nuclideAnalytics || _load_nuclideAnalytics()).HistogramTracker(
    ...
    this._rpcServiceRegistry = new (_nuclideRpc || _load_nuclideRpc()).ServiceRegistry((_nuclideMarshalersCommon || _load_nuclideMarshalersCommon()).getServerSideMarshalers,
  ...
  _setupServices() {
    ...
    {
      if (error != null) {
        (0, (_utils || _load_utils()).sendJsonResponse)(
        response,
  ...
  _setupHeartbeatHandler() {var _this = this;
    this._registerService(
    '/' + HEARTBEAT_CHANNEL, (0, _asyncToGenerator.default)(
    function* () {return _this._version;}),
    'post',
    true);
  ...
  {var _this2 = this;
    ...
    serviceName, (() => {var _ref2 = (0, _asyncToGenerator.default)(
      function* (request, response, next) {
        try {
          const result = yield _this2.callService(
          serviceName,
          (0, (_utils || _load_utils()).deserializeArgs)(request.url));
  ...
  _onConnection(socket, req) {
    ...
    if (client == null) {
      client = (_nuclideRpc || _load_nuclideRpc()).RpcConnection.createServer(
      this._rpcServiceRegistry,
      new (_QueuedAckTransport || _load_QueuedAckTransport()).QueuedAckTransport(clientId, transport, (_utils || _load_utils()).protocolLogger),
      {},
      clientId, (_utils || _load_utils()).protocolLogger);


      this._clients.set(clientId, client);
    } else {if (!(
      clientId === client.getTransport().id)) {throw new Error('Invariant violation: "clientId === client.getTransport().id"');}
      client.getTransport().reconnect(transport);
    }
  }

  close() {if (!(
    NuclideServer._theServer === this)) {throw new Error('Invariant violation: "NuclideServer._theServer === this"');}
