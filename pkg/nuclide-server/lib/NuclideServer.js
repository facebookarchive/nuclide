...
var NuclideServer = (function () {
  function NuclideServer(options, services) {
    ...
    this._rpcServiceRegistry = (_nuclideRpc2 || _nuclideRpc()).ServiceRegistry.createRemote(services);
  }
  ...
  _createClass(NuclideServer, [{
    ...
    value: function _onConnection(socket) {
      ...
      socket.once('message', function (clientId) {
        ...
        if (client == null) {
          client = new (_nuclideRpc2 || _nuclideRpc()).RpcConnection('server', _this6._rpcServiceRegistry, new (_QueuedTransport2 || _QueuedTransport()).QueuedTransport(clientId, transport));
