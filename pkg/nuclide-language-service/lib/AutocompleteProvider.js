...
class AutocompleteProvider {
  ...
  static register(name, grammars, config, onDidInsertSuggestion, connectionToLanguageService) {
    return atom.packages.serviceHub.provide('nuclide-autocomplete.provider', '0.0.0', new AutocompleteProvider(name, grammars.map(grammar => '.' + grammar).join(', '), config.inclusionPriority, config.suggestionPriority, config.disableForSelector, config.excludeLowerPriority, config.analytics, onDidInsertSuggestion, config.autocompleteCacherConfig, connectionToLanguageService, config.supportsResolve));
  }
  ...
  _getSuggestionsFromLanguageService(request) {
    ...
    return (0, _asyncToGenerator.default)(function* () {
      ...
      const languageService = _this2._connectionToLanguageService.getForUri(editorPath);
  ...
  getSuggestionDetailsOnSelect(suggestion) {
    ...
    return (0, _asyncToGenerator.default)(function* () {
      if (!_this3._supportsResolve) {
        return null;
      }

      const languageService = _this3._connectionToLanguageService.getForUri(suggestion.remoteUri);
      if (languageService == null) {
        return null;
      }

      const resolved = yield (yield languageService).resolveAutocompleteSuggestion(suggestion);
      if (resolved != null) {
        // A few members of the suggestion aren't RPC-able (such as the provider),
        // so merge the objects together.
        return Object.assign({}, suggestion, resolved);
      }
