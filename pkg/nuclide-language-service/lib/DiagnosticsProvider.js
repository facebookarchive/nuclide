...
let ObservableDiagnosticProvider = exports.ObservableDiagnosticProvider = class ObservableDiagnosticProvider {
  ...
  constructor(analyticsEventName, connectionToLanguageService) {
    this._analyticsEventName = analyticsEventName;
    this._connectionToLanguageService = connectionToLanguageService;
    this.updates = this._connectionToLanguageService.observeValues().switchMap(languageService => {
      return _rxjsBundlesRxMinJs.Observable.fromPromise(languageService);
    }).mergeMap(language => language.observeDiagnostics().refCount()).map((_ref2) => {
