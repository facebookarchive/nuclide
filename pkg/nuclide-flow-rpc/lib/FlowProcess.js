...
var FlowProcess = (function () {
  function FlowProcess(root, execInfoContainer) {
    var _this = this;

    _classCallCheck(this, FlowProcess);

    this._execInfoContainer = execInfoContainer;
    this._serverStatus = new (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).BehaviorSubject((_FlowConstants || _load_FlowConstants()).ServerStatus.UNKNOWN);
    this._root = root;
    ...
    this._serverStatus.filter(function (x) {
      return x === (_FlowConstants || _load_FlowConstants()).ServerStatus.NOT_RUNNING;
    }).subscribe(function () {
    ...
    this._serverStatus.filter(function (status) {
      return status === (_FlowConstants || _load_FlowConstants()).ServerStatus.FAILED;
    }).subscribe(function () {
  ...
  _createClass(FlowProcess, [{
    ...
    value: _asyncToGenerator(function* (args, options) {
      ...
      for (var i = 0;; i++) {
        try {
          ...
          var result = yield this._rawExecFlow(args, options);
          return result;
        } catch (e) {
          var couldRetry = [(_FlowConstants || _load_FlowConstants()).ServerStatus.NOT_RUNNING, (_FlowConstants || _load_FlowConstants()).ServerStatus.INIT, (_FlowConstants || _load_FlowConstants()).ServerStatus.BUSY].indexOf(this._serverStatus.getValue()) !== -1;
          if (i < maxRetries && couldRetry) {
            // eslint-disable-next-line babel/no-await-in-loop
            yield this._serverIsReady();
            // Then try again.
          } else {
    ...
    value: _asyncToGenerator(function* () {
      ...
      var flowExecInfo = yield this._execInfoContainer.getFlowExecInfo(this._root);
      if (flowExecInfo == null) {
        ...
        this._setServerStatus((_FlowConstants || _load_FlowConstants()).ServerStatus.NOT_INSTALLED);
        return;
      }
      ...
      // ChildProcess object.
      // eslint-disable-next-line babel/no-await-in-loop
      var serverProcess = yield (0, (_commonsNodeNice || _load_commonsNodeNice()).niceSafeSpawn)(flowExecInfo.pathToFlow, ['server', '--from', 'nuclide', '--max-workers', this._getMaxWorkers().toString(), this._root], flowExecInfo.execOptions);
    ...
    key: '_rawExecFlow',
    value: _asyncToGenerator(function* (args_) {
      var options = arguments.length <= 1 || arguments[1] === undefined ? {} : arguments[1];

      var args = args_;
      args = [].concat(_toConsumableArray(args), ['--retry-if-init', 'false', '--retries', '0', '--no-auto-start']);
      try {
        var result = yield FlowProcess.execFlowClient(args, this._root, this._execInfoContainer, options);
    ...
    value: _asyncToGenerator(function* () {
      ...
      for (var i = 0; !stateChanged && i < tries; i++) {
        ...
        // Wait 1 second
        // eslint-disable-next-line babel/no-await-in-loop
        yield (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.of(null).delay(1000).toPromise();
      }
    })
    ...
    value: function _serverIsReady() {
      return this._serverStatus.filter(function (x) {
        return x === (_FlowConstants || _load_FlowConstants()).ServerStatus.READY;
      }).map(function () {
        return true;
      }).race((_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.of(false).delay(SERVER_READY_TIMEOUT_MS))
    ...
    value: function _getMaxWorkers() {
      return Math.max((_os || _load_os()).default.cpus().length - 2, 1);
    }
    ...
    key: 'execFlowClient',
    value: _asyncToGenerator(function* (args_, root, execInfoContainer) {
      var options_ = arguments.length <= 3 || arguments[3] === undefined ? {} : arguments[3];

      var args = args_;
      var options = options_;
      args = [].concat(_toConsumableArray(args), ['--from', 'nuclide']);
      var execInfo = yield execInfoContainer.getFlowExecInfo(root);
      if (execInfo == null) {
        return null;
      }
      options = _extends({}, execInfo.execOptions, options);
      var ret = yield (0, (_commonsNodeProcess || _load_commonsNodeProcess()).asyncExecute)(execInfo.pathToFlow, args, options);
      if (ret.exitCode !== 0) {
