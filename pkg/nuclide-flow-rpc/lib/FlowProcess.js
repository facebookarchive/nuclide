...
class FlowProcess {
  ...
  constructor(root, execInfoContainer, fileCache) {
    ...
    this._version = new (_FlowVersion || _load_FlowVersion()).FlowVersion(async () => {
  ...
  async execFlow(args, options, waitForServer = false, suppressErrors = false) {
    ...
    for (let i = 0;; i++) {
      try {
        ...
        const couldRetry = [(_FlowConstants || _load_FlowConstants()).ServerStatus.NOT_RUNNING, (_FlowConstants || _load_FlowConstants()).ServerStatus.INIT, (_FlowConstants || _load_FlowConstants()).ServerStatus.BUSY].indexOf(this._serverStatus.getValue()) !== -1;
  ...
  async _startFlowServer() {
    ...
    const lazy = (0, (_config || _load_config()).getConfig)('lazyMode') === true ? ['--lazy-mode', 'ide'] : [];
    ...
    // ChildProcess object.
    // eslint-disable-next-line no-await-in-loop
    const serverProcess = await (0, (_nice || _load_nice()).niceSafeSpawn)(flowExecInfo.pathToFlow, ['server', ...lazy, '--from', 'nuclide', '--max-workers', this._getMaxWorkers().toString(), this._root], flowExecInfo.execOptions);
    const logIt = data => {
    ...
    serverProcess.on('exit', (code, signal) => {
      ...
      if (code === 2 && signal === null) {
        ...
        this._setServerStatus((_FlowConstants || _load_FlowConstants()).ServerStatus.FAILED);
      }
    });
