...
class FlowSingleProjectLanguageService {
  ...
  async _findRefs(filePath, buffer, position, global_) {
  ...
  async getCoverage(filePath) {
    ...
    const uncoveredRegions = expressions.uncovered_locs.map((_nuclideFlowCommon || _load_nuclideFlowCommon()).flowCoordsToAtomCoords).map(range => ({ range }));
  ...
  static async flowGetAst(root, currentContents, execInfoContainer) {
    ...
    let json;
    try {
      const result = await (_FlowProcess || _load_FlowProcess()).FlowProcess.execFlowClient(args, flowRootPath, execInfoContainer, options);
      if (result == null) {
        return null;
      }
      json = parseJSON(args, result.stdout);
