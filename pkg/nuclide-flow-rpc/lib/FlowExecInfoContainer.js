...
let FlowExecInfoContainer = exports.FlowExecInfoContainer = class FlowExecInfoContainer {
  ...
  findFlowConfigDir(localFile) {
    ...
    return (0, _asyncToGenerator.default)(function* () {
      if (!_this5._flowConfigDirCache.has(localFile)) {
        const flowConfigDir = yield (_fsPromise || _load_fsPromise()).default.findNearestFile('.flowconfig', (_nuclideUri || _load_nuclideUri()).default.dirname(localFile));
        _this5._flowConfigDirCache.set(localFile, flowConfigDir);
      }
      return _this5._flowConfigDirCache.get(localFile);
    })();
  }
