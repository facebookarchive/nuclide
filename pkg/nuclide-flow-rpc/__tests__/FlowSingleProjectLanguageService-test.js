...
function _load_FlowSingleProjectLanguageService() {
  return _FlowSingleProjectLanguageService = require('../lib/FlowSingleProjectLanguageService');
}
...
describe('FlowSingleProjectLanguageService', () => {
  ...
  function newFlowService() {
    return new (_FlowSingleProjectLanguageService || _load_FlowSingleProjectLanguageService()).FlowSingleProjectLanguageService(root, new (_FlowExecInfoContainer || _load_FlowExecInfoContainer()).FlowExecInfoContainer(), null);
  }

  beforeEach(() => {
    buffer = new (_simpleTextBuffer || _load_simpleTextBuffer()).default(currentContents);
    line = 2;
    column = 12;
  ...
  describe('getDefinition', () => {
    function runWith(location) {
      ...
      return flowRoot.getDefinition(file, buffer, new (_simpleTextBuffer2 || _load_simpleTextBuffer2()).Point(line, column));
    }
    ...
    it('should return the location', async () => {
      await (async () => {
        ...
        expect(result.definitions[0]).toEqual({
          path: file,
          position: new (_simpleTextBuffer2 || _load_simpleTextBuffer2()).Point(4, 7),
          language: 'Flow'
        });
      })();
    });
  ...
  describe('flowGetType', () => {
    function runWithString(outputString) {
      ...
      return flowRoot.typeHint(file, buffer, new (_simpleTextBuffer2 || _load_simpleTextBuffer2()).Point(line, column));
    }
    ...
    it('should return the type on success', async () => {
      await (async () => {
        expect((await runWith('thisIsAType', 1, 1, 1, 4))).toEqual({
          ...
          range: new (_simpleTextBuffer2 || _load_simpleTextBuffer2()).Range([0, 0], [0, 4])
        });
      })();
    });
    ...
    it('should return null if the flow process fails', async () => {
      await (async () => {
        ...
        expect((await flowRoot.typeHint(file, buffer, new (_simpleTextBuffer2 || _load_simpleTextBuffer2()).Point(line, column)))).toBe(null);
      })();
    });
  });
  ...
  describe('getAutocompleteSuggestions', () => {
    ...
    function run() {
      ...
      return flowRoot.getAutocompleteSuggestions(file, buffer, new (_simpleTextBuffer2 || _load_simpleTextBuffer2()).Point(line, column), activatedManually, prefix);
    }
  ...
  describe('groupParamNames', () => {
    it('should return a group for each argument', () => {
      ...
      expect((0, (_FlowSingleProjectLanguageService || _load_FlowSingleProjectLanguageService()).groupParamNames)(args)).toEqual(args.map(arg => [arg]));
    });

    it('should group optional params', () => {
      const args = ['arg1', 'arg2?'];
      expect((0, (_FlowSingleProjectLanguageService || _load_FlowSingleProjectLanguageService()).groupParamNames)(args)).toEqual([args]);
    });
    ...
    it('should only group optional params at the end', () => {
      ...
      expect((0, (_FlowSingleProjectLanguageService || _load_FlowSingleProjectLanguageService()).groupParamNames)(args)).toEqual(expectedGrouping);
    });
    ...
    it('should return an empty array for no arguments', () => {
      expect((0, (_FlowSingleProjectLanguageService || _load_FlowSingleProjectLanguageService()).groupParamNames)([])).toEqual([]);
    });
  });
});
...
describe('push diagnostics collation', () => {
  ...
  it('should accumulate errors during a recheck', async () => {
    await (async () => {
      ...
      const expected = [[], [{ filePath: '/path1', messages: [['foo', false]] }, { filePath: '/path2', messages: [['bar', false]] }], [{ filePath: '/path1', messages: [['foo', true]] }, { filePath: '/path2', messages: [['bar', true]] }], [{ filePath: '/path1', messages: [['foo', true], ['new', false]] }], [{ filePath: '/path3', messages: [['asdf', false]] }], [{ filePath: '/path1', messages: [['new', false]] }, { filePath: '/path2', messages: [] }], [
      ...
      { filePath: '/path2', messages: [['another new', false]] }, { filePath: '/path3', messages: [['asdf', false]] }, { filePath: '/path1', messages: [] }]];
      const results = await getAbbreviatedResults(messages);
      expect(results).toEqual(expected);
    })();
  });
  ...
  it('should remove all diagnostics when the IDE connection is lost outside of a recheck', async () => {
    await (async () => {
      ...
      const expected = [[], [{ filePath: '/path1', messages: [['foo', false]] }, { filePath: '/path2', messages: [['bar', false]] }], [{ filePath: '/path1', messages: [] }, { filePath: '/path2', messages: [] }]];
      const results = await getAbbreviatedResults(messages);
      expect(results).toEqual(expected);
    })();
  });
  ...
  it('should remove all diagnostics when the IDE connection is lost during a recheck', async () => {
    await (async () => {
      ...
      const expected = [[], [{ filePath: '/path1', messages: [['foo', false]] }, { filePath: '/path2', messages: [['bar', false]] }], [{ filePath: '/path1', messages: [['foo', true]] }, { filePath: '/path2', messages: [['bar', true]] }], [{ filePath: '/path1', messages: [['foo', true], ['baz', false]] }, { filePath: '/path3', messages: [['asdf', false]] }], [{ filePath: '/path1', messages: [] }, { filePath: '/path2', messages: [] }, { filePath: '/path3', messages: [] }]];
      const results = await getAbbreviatedResults(messages);
      expect(results).toEqual(expected);
    })();
  });
...
async function getAbbreviatedResults(messages) {
  const results = [];
  let state = (0, (_FlowSingleProjectLanguageService || _load_FlowSingleProjectLanguageService()).emptyDiagnosticsState)();
  results.push((await (0, (_FlowSingleProjectLanguageService || _load_FlowSingleProjectLanguageService()).getDiagnosticUpdates)(state).toPromise()));
  for (const message of messages) {
    state = (0, (_FlowSingleProjectLanguageService || _load_FlowSingleProjectLanguageService()).updateDiagnostics)(state, message);
    results.push((await (0, (_FlowSingleProjectLanguageService || _load_FlowSingleProjectLanguageService()).getDiagnosticUpdates)(state).toPromise() // eslint-disable-line no-await-in-loop
    ));
  }
