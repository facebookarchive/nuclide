...
function _load_RecentFilesProvider() {
  return _RecentFilesProvider = require('../lib/RecentFilesProvider');
}
...
describe('RecentFilesProvider', () => {
  beforeEach(() => {
    // $FlowIssue
    provider = Object.assign({}, (_RecentFilesProvider || _load_RecentFilesProvider()).RecentFilesProvider);
    (0, (_RecentFilesProvider || _load_RecentFilesProvider()).setRecentFilesService)(FakeRecentFilesService);
  ...
  describe('Result rendering', () => {
    it('should render complete results', () => {
      ...
      const renderedComponent = (_testUtils || _load_testUtils()).default.renderIntoDocument(_react.createElement(
        Wrapper,
        null,
        reactElement
      ));
      expect((_testUtils || _load_testUtils()).default.scryRenderedDOMComponentsWithClass(renderedComponent, 'recent-files-provider-file-name').length).toBe(1);
      expect((_testUtils || _load_testUtils()).default.scryRenderedDOMComponentsWithClass(renderedComponent, 'recent-files-provider-file-path').length).toBe(1);
      const datetimeLabels = (_testUtils || _load_testUtils()).default.scryRenderedDOMComponentsWithClass(renderedComponent, 'recent-files-provider-datetime-label');
