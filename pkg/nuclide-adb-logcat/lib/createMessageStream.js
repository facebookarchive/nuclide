...
function createMessageStream(line$) {
  ...
  var messages = (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.create(function (observer) {
    var buffer = [];
...
function filter(messages) {
  var patterns = (_commonsAtomFeatureConfig || _load_commonsAtomFeatureConfig()).default.observeAsStream('nuclide-adb-logcat.whitelistedTags').map(function (source) {
    try {
