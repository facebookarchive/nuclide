...
function setCustomAdbPathEpic(actions, store) {
  return actions.ofType((_Actions || _load_Actions()).SET_CUSTOM_ADB_PATH).map(action => {
    ...
    (0, (_utils || _load_utils()).getAdbServiceByNuclideUri)(action.payload.host).registerCustomPath(action.payload.path);
  }).ignoreElements();
}
...
function setAdbPortEpic(actions, store) {
  return actions.ofType((_Actions || _load_Actions()).SET_ADB_PORT).map(action => {
    ...
    (0, (_utils || _load_utils()).getAdbServiceByNuclideUri)(action.payload.host).addAdbPort(action.payload.port);
  }).ignoreElements();
}