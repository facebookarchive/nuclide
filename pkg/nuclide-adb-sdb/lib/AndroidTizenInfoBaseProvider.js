...
import type {DeviceInfoProvider, Device} from '../../nuclide-devices/lib/types';
...
export class AndroidTizenInfoBaseProvider implements DeviceInfoProvider {
  ...
  async fetch(host: NuclideUri, device: Device): Promise<Map<string, string>> {
    const infoMap = new Map();
    for (const [key, value] of await this._rpcFactory(host).getDeviceInfo(device.name)) {
