...
class ATDeviceProcessesProvider {
  ...
  observe(host, device) {
    return _rxjsBundlesRxMinJs.Observable.interval(3000).startWith(0).switchMap(() => this._rpcFactory(host).getProcesses(device).refCount().catch(() => _rxjsBundlesRxMinJs.Observable.of([])));
  }
