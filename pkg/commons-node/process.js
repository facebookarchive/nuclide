...
function monitorStreamErrors(proc, command, args, options) {
  const streams = [['stdin', proc.stdin], ['stdout', proc.stdout], ['stderr', proc.stderr]];
  return _rxjsBundlesRxMinJs.Observable.merge(...(0, (_collection || _load_collection()).arrayCompact)(streams.map(([name, stream]) => {
    return stream == null ? null : _rxjsBundlesRxMinJs.Observable.fromEvent(stream, 'error').do(err => {
      // This can happen without the full execution of the command to fail,
      // but we want to learn about it.
      logError(`stream error on stream ${name} with command:`, command, args, options, 'error:', err);
    });
  }))).ignoreElements();
}

function writeToStdin(proc, input) {
