...
let ProcessExitError = exports.ProcessExitError = class ProcessExitError extends Error {
  ...
  constructor(opts) {
    // TODO: Remove `captureStackTrace()` call and `this.message` assignment when we remove our
    // class transform and switch to native classes.
    const message = `"${ opts.command }" failed with ${ exitEventToMessage(opts.exitMessage) }\n\n${ opts.stderr }`;
    super(message);
...
function asyncExecute(command, args) {
  ...
  return new Promise((resolve, reject) => {
    const process = _child_process.default.execFile(command, args, _extends({
