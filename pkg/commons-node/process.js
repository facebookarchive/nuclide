...
exports.psTree = exports.getChildrenOfProcess = exports.getOriginalEnvironment = exports.checkOutput = exports.asyncExecute = exports.killUnixProcessTree = exports.ProcessTimeoutError = exports.MaxBufferExceededError = exports.ProcessExitError = exports.loggedCalls = undefined;

var _asyncToGenerator = _interopRequireDefault(require('async-to-generator'));

let _killProcess = (() => {
  var _ref6 = (0, _asyncToGenerator.default)(function* (childProcess, killTree) {
...
let killUnixProcessTree = exports.killUnixProcessTree = (() => {
  var _ref7 = (0, _asyncToGenerator.default)(function* (childProcess) {
...
let asyncExecute = exports.asyncExecute = (() => {
  var _ref8 = (0, _asyncToGenerator.default)(function* (command, args, options = {}) {
...
let getChildrenOfProcess = exports.getChildrenOfProcess = (() => {
  var _ref11 = (0, _asyncToGenerator.default)(function* (processId) {
...
let getDescendantsOfProcess = (() => {
  var _ref12 = (0, _asyncToGenerator.default)(function* (pid) {
    const processes = yield psTree();
    let rootProcessInfo;
...
class ProcessExitError extends Error {
  ...
  constructor(exitMessage, proc, stderr) {
...
exports.ProcessTimeoutError = ProcessTimeoutError; // Copied from https://github.com/facebook/flow/blob/v0.43.1/lib/node.js#L11-L16
...
function _createProcessStream(createProcess, options = {}) {
  return (0, (_event || _load_event()).observableFromSubscribeFunction)(whenShellEnvironmentLoaded).take(1).switchMap(() => {
    ...
    const isExitError = ((_ref = options) != null ? _ref.isExitError : _ref) || isExitErrorDefault;
    const exitErrorBufferSize = ((_ref2 = options) != null ? _ref2.exitErrorBufferSize : _ref2) || 2000;
    ...
    ? x => timeoutWith(x, timeout, _rxjsBundlesRxMinJs.Observable.throw(new ProcessTimeoutError(timeout, process))) : x => x;
    let finished = false;
    ...
    const errors = _rxjsBundlesRxMinJs.Observable.fromEvent(process, 'error').flatMap(_rxjsBundlesRxMinJs.Observable.throw);
    ...
    const accumulatedStderr = (0, (_observable || _load_observable()).takeWhileInclusive)((0, (_stream || _load_stream()).observeStream)(process.stderr).scan((acc, stderrData) => (acc + stderrData).slice(0, exitErrorBufferSize), '').startWith(''), acc => acc.length < exitErrorBufferSize);
    ...
    return enforceTimeout(_rxjsBundlesRxMinJs.Observable.of(process)
    // Don't complete until we say so!
    .merge(_rxjsBundlesRxMinJs.Observable.never()).takeUntil(errors).takeUntil(exitEvents).do({
      error: () => {
        finished = true;
      },
      complete: () => {
...
function getOutputStream(process, options, rest) {
  ...
  const chunk = ((_ref3 = options) != null ? _ref3.splitByLines : _ref3) === false ? x => x : (_observable || _load_observable()).splitStream;
  const maxBuffer = (_ref4 = options) != null ? _ref4.maxBuffer : _ref4;
  return _rxjsBundlesRxMinJs.Observable.defer(() => {
    const stdoutEvents = chunk(limitBufferSize((0, (_stream || _load_stream()).observeStream)(process.stdout), maxBuffer, 'stdout')).map(data => ({ kind: 'stdout', data }));
    const stderrEvents = chunk(limitBufferSize((0, (_stream || _load_stream()).observeStream)(process.stderr), maxBuffer, 'stderr')).map(data => ({ kind: 'stderr', data }));
    ...
    return (0, (_observable || _load_observable()).takeWhileInclusive)(_rxjsBundlesRxMinJs.Observable.merge(stdoutEvents, stderrEvents).takeUntil(close).concat(exitEvents), event => event.kind !== 'error' && event.kind !== 'exit').finally(() => {
      exitSub.unsubscribe();
    });
  });
}
...
function observeProcess(command, args, options) {
  return _createProcessStream(() => _makeChildProcess('spawn', command, args, options), options).flatMap(process => getOutputStream(process, options));
}
...
function observeProcessRaw(command, args, options) {
  return _createProcessStream(() => _makeChildProcess('spawn', command, args, options), options).flatMap(process => getOutputStream(process, Object.assign({}, options, { splitByLines: false })));
}
...
function writeToStdin(childProcess, options) {
  ...
  const maxBuffer = ((_ref5 = options) != null ? _ref5.maxBuffer : _ref5) || DEFAULT_MAX_BUFFER;
  return observeProcess(command, args, Object.assign({}, options, { maxBuffer })).filter(event => event.kind === 'stdout').reduce((acc, event) => {
