...
function cacheWhileSubscribed(input) {
  return input.multicast(function () {
    return new (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).ReplaySubject(1);
  }).refCount();
}
...
function diffSets(sets, hash) {
  return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.concat((_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.of(new Set()), // Always start with no items with an empty set
  sets).pairwise().map(function (_ref) {
...
function reconcileSetDiffs(diffs, addAction, hash_) {
  ...
  var disposeItem = function disposeItem(item) {
    ...
    (0, (_assert || _load_assert()).default)(disposable != null);
    disposable.dispose();
  ...
  return new (_eventKit || _load_eventKit()).CompositeDisposable(new (_UniversalDisposable || _load_UniversalDisposable()).default(diffs.subscribe(function (diff) {
  ...
  })), new (_eventKit || _load_eventKit()).Disposable(disposeAll));
}
...
function toggle(source, toggler) {
  return toggler.distinctUntilChanged().switchMap(function (enabled) {
    return enabled ? source : (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.empty();
  });
}
...
function concatLatest() {
  ...
  return (_Observable = (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable).merge.apply(_Observable, _toConsumableArray(tagged)).scan(function (accumulator, _ref3) {
