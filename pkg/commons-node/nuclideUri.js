...
function createRemoteUri(hostname, remotePath) {
  (0, (_assert2 || _assert()).default)(remotePath != null && remotePath !== '', 'NuclideUri must include a path.');
...
function validate(uri, mustBeRemote) {
  ...
  (0, (_assert2 || _assert()).default)(uri != null, 'Unexpected null NuclideUri');
  (0, (_assert2 || _assert()).default)(typeof uri === 'string', 'Unexpected NuclideUri type: ' + String(uri));

  if (isRemote(uri)) {
    parse(uri);
    (0, (_assert2 || _assert()).default)(mustBeRemote !== false, 'Expected remote NuclideUri');
  } else {
    (0, (_assert2 || _assert()).default)(uri !== '', 'NuclideUri must contain a non-empty path');
    (0, (_assert2 || _assert()).default)(mustBeRemote !== true, 'Expected local NuclideUri');
  }
}
