...
function join(uri) {
  ...
  if (isRemote(uri)) {
    ...
    const path = _parseRemoteUri.path;
...
function normalize(uri) {
  ...
  if (isRemote(uri)) {
    ...
    const path = _parseRemoteUri2.path;
...
function dirname(uri) {
  ...
  if (isRemote(uri)) {
    ...
    const path = _parseRemoteUri3.path;
...
function resolve(uri) {
  ...
  if (isRemote(uri)) {
    ...
    const path = _parseRemoteUri4.path;
...
const posixPath = _extends({}, _path.default.posix);
const win32Path = _extends({}, _path.default.win32, { isAbsolute: _win32PathIsAbsolute });
