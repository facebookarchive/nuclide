...
var URI_PREFIX_REGEX = /^[A-Za-z0-9_\-]+:\/\/.*/;
...
function createRemoteUri(hostname, remotePath) {
  (0, (_assert || _load_assert()).default)(remotePath != null && remotePath !== '', 'NuclideUri must include a path.');
 ...
 */
function parse(uri) {
  var parsedUri = (_url || _load_url()).default.parse(_escapeSpecialCharacters(uri));
  ...
  (0, (_assert || _load_assert()).default)(parsedUri.path, 'Nuclide URIs must contain paths, ' + ((0, (_string || _load_string()).maybeToString)(parsedUri.path) + '\' found while parsing \'' + uri + '\''));
  ...
  (0, (_assert || _load_assert()).default)(parsedUri.pathname, 'Nuclide URIs must contain pathnamess, ' + ('\'' + (0, (_string || _load_string()).maybeToString)(parsedUri.pathname) + '\' found while parsing \'' + uri + '\''));
...
function parseRemoteUri(remoteUri) {
  ...
  (0, (_assert || _load_assert()).default)(parsedUri.hostname, 'Remote Nuclide URIs must contain hostnames, \'' + (0, (_string || _load_string()).maybeToString)(parsedUri.hostname) + '\' found ' + ('while parsing \'' + remoteUri + '\''));
...
function join(uri) {
  ...
  if (isRemote(uri)) {
    ...
    var _path2 = _parseRemoteUri.path;

    relativePath.splice(0, 0, _path2);
...
function normalize(uri) {
  ...
  if (isRemote(uri)) {
    ...
    var _path3 = _parseRemoteUri2.path;

    return createRemoteUri(_hostname2, uriPathModule.normalize(_path3));
  } else {
...
function dirname(uri) {
  ...
  if (isRemote(uri)) {
    ...
    var _path4 = _parseRemoteUri3.path;

    return createRemoteUri(_hostname3, uriPathModule.dirname(_path4));
  } else {
...
function uriToNuclideUri(uri) {
  var urlParts = (_url || _load_url()).default.parse(_escapeSpecialCharacters(uri), false);
...
function resolve(uri) {
  ...
  if (isRemote(uri)) {
    ...
    var _path5 = _parseRemoteUri4.path;

    paths.splice(0, 0, _path5);
...
function expandHomeDir(uri) {
  ...
  (0, (_assert || _load_assert()).default)(HOME != null);
...
function splitPathList(paths) {
  (0, (_assert || _load_assert()).default)(paths.indexOf(REMOTE_PATH_URI_PREFIX) < 0, 'Splitting remote URIs is not supported');
...
function joinPathList(paths) {
  ...
  (0, (_assert || _load_assert()).default)(paths.every(function (path) {
...
function ensureLocalPrefix(uri) {
  ...
  (0, (_assert || _load_assert()).default)(!isRemote(uri), 'Local prefix can not be added to a remote path');
  (0, (_assert || _load_assert()).default)(!isAbsolute(uri), 'Local prefix can not be added to an absolute path');
...
var posixPath = _extends({}, (_path || _load_path()).default.posix);
var win32Path = _extends({}, (_path || _load_path()).default.win32, { isAbsolute: _win32PathIsAbsolute });
...
var NUCLIDE_URI_TYPE_NAME = 'NuclideUri';
...
function validate(uri, mustBeRemote) {
  ...
  (0, (_assert || _load_assert()).default)(uri != null, 'Unexpected null NuclideUri');
  (0, (_assert || _load_assert()).default)(typeof uri === 'string', 'Unexpected NuclideUri type: ' + String(uri));

  if (isRemote(uri)) {
    parse(uri);
    (0, (_assert || _load_assert()).default)(mustBeRemote !== false, 'Expected remote NuclideUri');
  } else {
    (0, (_assert || _load_assert()).default)(uri !== '', 'NuclideUri must contain a non-empty path');
    (0, (_assert || _load_assert()).default)(mustBeRemote !== true, 'Expected local NuclideUri');
  }
}
