'use strict';Object.defineProperty(exports, "__esModule", { value: true });exports.__TEST__ = undefined;var _asyncToGenerator = _interopRequireDefault(require('async-to-generator'));var _nuclideUri;
...
function _load_process() {return _process = require('../../modules/nuclide-commons/process');}var _keytar;
...
`;function getApmNodePath() {const apmDir = (_nuclideUri || _load_nuclideUri()).default.dirname(atom.packages.getApmPath());return (_nuclideUri || _load_nuclideUri()).default.normalize((_nuclideUri || _load_nuclideUri()).default.join(apmDir, 'node'));}function getApmNodeModulesPath() {const apmDir = (_nuclideUri || _load_nuclideUri()).default.dirname(atom.packages.getApmPath());return (_nuclideUri || _load_nuclideUri()).default.normalize((_nuclideUri || _load_nuclideUri()).default.join(apmDir, '..', 'node_modules'));}
...
password)
{
  ...
  return (0, (_process || _load_process()).runCommand)(getApmNodePath(), args, options).
  toPromise().
  catch(err => {
...

{
   ...
   */
  getPassword(service, account) {return (0, _asyncToGenerator.default)(function* () {
      if (typeof atom === 'object') {
        return JSON.parse((
        yield runScriptInApmNode(getPasswordScriptAsync, service, account)));
  ...
  /**
      ...
      * Rejects on keychain access failure.
      */
  deletePassword(service, account) {return (0, _asyncToGenerator.default)(function* () {
      if (typeof atom === 'object') {
        return JSON.parse((
        yield runScriptInApmNode(deletePasswordScriptAsync, service, account)));
