'use strict';Object.defineProperty(exports, "__esModule", { value: true });exports.ImmutableSnapshotReader = exports.Map = exports.ImmutableSnapshotter = undefined;exports.
...
Record = Record;var _immutable;function _load_immutable() {return _immutable = _interopRequireDefault(require('immutable'));}function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };} // A snapshot of the Immutable.JS object represented by the ID "root".
 ...
 */ /**
     ...
     */class ImmutableSnapshotter {createDeltaSnapshot(object) {const snapshotObjects = {};const result = _snapshotImpl(object, snapshotObjects, this._previousSnapshotObjects || {});if (!(result != null && typeof result.id === 'number')) {throw new Error('Expected an immutable object as input');}this._previousSnapshotObjects = snapshotObjects;return { root: result.id, snapshotObjects };} // Track the contents of the previous snapshot so that we know what we can reuse.
...
}exports.ImmutableSnapshotter = ImmutableSnapshotter;function List(value) {// Use a custom prototype to override methods for tracking purposes.
  return _wrap((_immutable || _load_immutable()).default.List(value), List);}List.prototype = Object.create((_immutable || _load_immutable()).default.List.prototype);List.of = (...args) => List(args);_overrideMethods(List, (_immutable || _load_immutable()).default.List);function SnapshotMap(value) {return _wrap((_immutable || _load_immutable()).default.Map(value), SnapshotMap);}SnapshotMap.prototype = Object.create((_immutable || _load_immutable()).default.Map.prototype); // $FlowIssue: Map.of is missing
SnapshotMap.of = (...args) => SnapshotMap((_immutable || _load_immutable()).default.Map.of(...args));_overrideMethods(SnapshotMap, (_immutable || _load_immutable()).default.Map);exports.Map = SnapshotMap;function OrderedMap(value) {return _wrap((_immutable || _load_immutable()).default.OrderedMap(value), OrderedMap);}OrderedMap.prototype = Object.create((_immutable || _load_immutable()).default.OrderedMap.prototype);_overrideMethods(OrderedMap, (_immutable || _load_immutable()).default.OrderedMap); // Immutable.Record actually returns a RecordFactory which then produces RecordInstances.
function Record(value) {const OriginalRecord = (_immutable || _load_immutable()).default.Record(value);class DiffableRecord extends OriginalRecord {}_overrideMethods(DiffableRecord, OriginalRecord);return DiffableRecord;} // Immutable.JS does not correctly support inheritance :(
// We often need to manually re-wrap the objects.
function _wrap(object, constructor) {if (object.size === 0) {// Immutable.JS shares empty objects, so we can't mutate these.
    // TODO: implement our own shared empty objects.
    return Object.assign(Object.create(constructor.prototype), object);} // Though setPrototypeOf has performance concerns, benchmarking shows that
  // this is still much faster than the object creation & assignment above.
  return Object.setPrototypeOf(object, constructor.prototype);} // Private symbol to store metadata on objects.
// (This is much faster than using a WeakMap.)
const MetadataKey = Symbol('Metadata'); // Generates a unique auto-incrementing ID for any object.
let _objectCount = 0;function _getMetadata(object) {let metadata = object[MetadataKey];if (metadata == null) {metadata = { id: ++_objectCount };object[MetadataKey] = metadata;}return metadata;}function _snapshotImpl(object, snapshotObjects, previousSnapshotObjects) {// Primitives
  if (typeof object !== 'object') {return object;} // Regular JS objects
  if (!(_immutable || _load_immutable()).default.isImmutable(object)) {return { _: object };}const metadata = _getMetadata(object);const { id, parent, mutations } = metadata;
  ...
  if (parent != null) {if (!(
    mutations != null)) {throw new Error('Invariant violation: "mutations != null"');}
    const parentSnapshot = _snapshotImpl(
    parent,
    snapshotObjects,
    previousSnapshotObjects);if (!(


    parentSnapshot != null && typeof parentSnapshot.id === 'number')) {throw new Error(
  ...
  for (const method in originalPrototype) {
    ...
    wrapped.prototype[method] = function (...args) {
      ...
      if (newObject !== this) {
        ...
        {
          const metadata = _getMetadata(newObject);if (!(

          metadata.parent == null)) {throw new Error(
  ...
  wrapped.prototype.__ensureOwner = function (ownerID) {
    ...
    if (newObject !== this) {
      ...
      if (_shouldRecordMutations(this)) {
        const metadata = _getMetadata(newObject);if (!(
        metadata.parent == null)) {throw new Error('An object cannot have two parents');}
        metadata.parent = this;
        metadata.mutations = [];
      }
    }
...
/**
   ...
   */
class ImmutableSnapshotReader {constructor() {this.


    _previousDeserializedObjects =


    new Map();} // Since ImmutableSnapshotter uses the previous snapshot to create deltas,
  ...
  deserializedObjects)
  {
    const object = snapshotObjects[id];if (!(
    object != null)) {throw new Error(`Missing record for id ${id}`);}
    switch (object.type) {
      case 'ref':
        const stored =
        this._previousDeserializedObjects.get(object.id) ||
        deserializedObjects.get(object.id);if (!(

        stored != null)) {throw new Error(
      ...
      case 'delta':
        ...
        object.parentID,
        snapshotObjects,
        deserializedObjects);if (!


        (_immutable || _load_immutable()).default.isImmutable(parent)) {throw new Error(
        ...
        return value;
      case 'list':
        const list = (_immutable || _load_immutable()).default.List(
        ...
        return list;
      case 'map':
        const map = (_immutable || _load_immutable()).default.Map(
        ...
        return map;
      case 'orderedmap':
        const orderedMap = (_immutable || _load_immutable()).default.OrderedMap(
        object.value.map(([k, v]) => [
        this._deserializeObject(k, snapshotObjects, deserializedObjects),
        this._deserializeObject(v, snapshotObjects, deserializedObjects)]));
