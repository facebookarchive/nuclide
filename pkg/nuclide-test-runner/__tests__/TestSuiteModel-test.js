...
function _load_TestSuiteModel() {
  return _TestSuiteModel = _interopRequireDefault(require('../lib/TestSuiteModel'));
}

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

describe('TestSuiteModel', () => {
  const testClassSummaries = [{ id: 1, fileName: 'foo', className: 'foo', name: 'foo' }, { id: 2, fileName: 'bar', className: 'bar', name: 'bar' }];
  ...
  it('maps test class IDs from a Array<TestClassSummary>', () => {
    const model = new (_TestSuiteModel || _load_TestSuiteModel()).default(testClassSummaries);
