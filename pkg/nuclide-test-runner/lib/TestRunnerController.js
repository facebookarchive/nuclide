...
var _uiTestRunnerPanel;

function _load_uiTestRunnerPanel() {
  return _uiTestRunnerPanel = _interopRequireDefault(require('./ui/TestRunnerPanel'));
}
...
var TestRunnerController = (function () {
  function TestRunnerController(state_, testRunners) {
    ...
    this._buffer = new (_atom || _load_atom()).TextBuffer();
    // Make `delete` a no-op to effectively create a read-only buffer.
    this._buffer.delete = function () {};

    this._executionState = (_uiTestRunnerPanel || _load_uiTestRunnerPanel()).default.ExecutionState.STOPPED;
  ...
  _createClass(TestRunnerController, [{
    ...
    value: function destroy() {
      this._stopListening();
      if (this._root) {
        (_reactForAtom || _load_reactForAtom()).ReactDOM.unmountComponentAtNode(this._root);
        this._root = null;
      }
      if (this._panel) {
        this._panel.destroy();
    ...
    value: _asyncToGenerator(function* (path) {
      ...
      this._setExecutionState((_uiTestRunnerPanel || _load_uiTestRunnerPanel()).default.ExecutionState.RUNNING);
      this._path = testPath;
      this._renderPanel();
    })
  }, {
    ...
    value: function stopTests() {
      ...
      this._setExecutionState((_uiTestRunnerPanel || _load_uiTestRunnerPanel()).default.ExecutionState.STOPPED);
    }
  }, {
    key: 'serialize',
    ...
    value: function _appendToBuffer(text) {
      ...
      this._buffer.append('' + text + (_os || _load_os()).default.EOL, { undo: 'skip' });
    }
  }, {
    ...
    value: function _runTestRunnerServiceForPath(testRun, path, label) {
      ...
      var subscription = testRun.do(function (message) {
        switch (message.kind) {
          case 'summary':
            _this2._testSuiteModel = new (_TestSuiteModel || _load_TestSuiteModel()).default(message.summaryInfo);
            _this2._renderPanel();
            break;
          case 'run-test':
            ...
            _this2._appendToBuffer((_TestRunModel || _load_TestRunModel()).default.formatStatusMessage(testInfo.name, testInfo.durationSecs, testInfo.status));
            _this2._renderPanel();
            break;
          case 'start':
            if (_this2._run) {
          ...
          case 'error':
            ...
            if (error.code === 'ENOENT') {
              _this2._appendToBuffer((_Ansi || _load_Ansi()).default.YELLOW + 'Command \'' + error.path + '\' does not exist' + (_Ansi || _load_Ansi()).default.RESET);
              _this2._appendToBuffer((_Ansi || _load_Ansi()).default.YELLOW + 'Are you trying to run remotely?' + (_Ansi || _load_Ansi()).default.RESET);
              _this2._appendToBuffer((_Ansi || _load_Ansi()).default.YELLOW + 'Path: ' + path + (_Ansi || _load_Ansi()).default.RESET);
            }
            _this2._appendToBuffer((_Ansi || _load_Ansi()).default.RED + 'Original Error: ' + error.message + (_Ansi || _load_Ansi()).default.RESET);
            _this2._setExecutionState((_uiTestRunnerPanel || _load_uiTestRunnerPanel()).default.ExecutionState.STOPPED);
            logger.error('Error running tests: "' + error.message + '"');
            break;
          case 'stderr':
            // Color stderr output red in the console to distinguish it as error.
            _this2._appendToBuffer('' + (_Ansi || _load_Ansi()).default.RED + message.data + (_Ansi || _load_Ansi()).default.RESET);
            break;
        }
      }).finally(function () {
    ...
    value: function _renderPanel(didRender) {
      ...
      if (this._testSuiteModel && this._executionState === (_uiTestRunnerPanel || _load_uiTestRunnerPanel()).default.ExecutionState.RUNNING) {
      ...
      var component = (_reactForAtom || _load_reactForAtom()).ReactDOM.render((_reactForAtom || _load_reactForAtom()).React.createElement((_uiTestRunnerPanel || _load_uiTestRunnerPanel()).default, {
      ...
      (0, (_assert || _load_assert()).default)(component instanceof (_uiTestRunnerPanel || _load_uiTestRunnerPanel()).default);
    ...
    value: function _stopListening() {
      ...
      if (this._run && this._run.dispose != null) {
        try {
          ...
          this._run.stop();
          (0, (_assert || _load_assert()).default)(this._run); // Calling `stop()` should never null the `_run` property.
          (0, (_nuclideAnalytics || _load_nuclideAnalytics()).track)('testrunner-stop-tests', {
            testRunner: this._run.label
          });
          dispose();
        } catch (e) {
          (0, (_assert || _load_assert()).default)(this._run); // Nothing in the try block should ever null the `_run` property.
