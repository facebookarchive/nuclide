  ...
  _createClass(NuclideSettingsPaneItem, [{
    ...
    value: function _getConfigData() {
      ...
      if (!this._disposables) {
        this._disposables = disposables = new (_atom || _load_atom()).CompositeDisposable();
      }
        ...
        if (nuclide.config && nuclide.configMetadata) {
          (function () {
            ...
            Object.keys(nuclide.config).forEach(function (settingName) {
              ...
              var schema = (_commonsAtomFeatureConfig || _load_commonsAtomFeatureConfig()).default.getSchema(keyPath);
              var title = getTitle(schema, settingName);
              var description = getDescription(schema);
              if (_this.state == null || (0, (_settingsUtils || _load_settingsUtils()).matchesFilter)(_this.state.filter, title) || (0, (_settingsUtils || _load_settingsUtils()).matchesFilter)(_this.state.filter, description)) {
                settings[settingName] = {
                  ...
                  schema: schema,
                  title: title,
                  value: (_commonsAtomFeatureConfig || _load_commonsAtomFeatureConfig()).default.get(keyPath)
                };
              }

              if (disposables) {
                var disposable = (_commonsAtomFeatureConfig || _load_commonsAtomFeatureConfig()).default.onDidChange(keyPath, _this._handleConfigChange);
    ...
    value: function _handleComponentChange(keyPath, value) {
      (_commonsAtomFeatureConfig || _load_commonsAtomFeatureConfig()).default.set(keyPath, value);
    }
  }, {
    key: 'render',
    value: function render() {
      ...
      Object.keys(configData).sort().forEach(function (categoryName) {
        ...
        if (Object.keys(packages).length > 0) {
          elements.push((_reactForAtom || _load_reactForAtom()).React.createElement((_SettingsCategory || _load_SettingsCategory()).default, {
            key: categoryName,
            name: categoryName,
            packages: packages
          }));
