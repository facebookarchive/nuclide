...
var PackagerActivation = (function () {
  function PackagerActivation() {
    ...
    var packagerEvents = (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.defer(function () {
      return getPackagerObservable(_this._projectRootPath);
    }).share();
    var messages = packagerEvents.filter(function (event) {
      return event.kind === 'message';
    }).map(function (event) {
      (0, (_assert || _load_assert()).default)(event.kind === 'message');
...
var PackagerError = (function (_Error2) {
  ...
  function PackagerError(exitMessage, stderr) {
...
function getPackagerObservable(projectRootPath) {
  var stdout = (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.fromPromise((0, (_nuclideReactNativeBase || _load_nuclideReactNativeBase()).getCommandInfo)(projectRootPath)).switchMap(function (commandInfo) {
    return commandInfo == null ? (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.throw(new NoReactNativeProjectError()) : (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.of(commandInfo);
    ...
    return (0, (_commonsNodeProcess || _load_commonsNodeProcess()).observeProcess)(function () {
      return (0, (_commonsNodeProcess || _load_commonsNodeProcess()).safeSpawn)(command, args, { cwd: cwd });
    });
  })
  ...
  .scan(function (acc, event) {
    ...
    switch (event.kind) {
      case 'error':
        return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.throw(event.error);
      case 'stdout':
        return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.of(event.data);
      case 'exit':
        if (event.exitCode !== 0) {
          return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.throw(new PackagerError((0, (_commonsNodeProcess || _load_commonsNodeProcess()).exitEventToMessage)(event), stderr));
        }
        return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.empty();
      case 'stderr':
      default:
    ...
    switch (err.name) {
      case 'NoReactNativeProjectError':
        ...
        return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.empty();
      case 'PackagerError':
        atom.notifications.addError('Packager exited with non-zero ' + err.exitMessage, {
          dismissable: true,
