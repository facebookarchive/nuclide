...
var DebuggerProxyClient = (function () {
  function DebuggerProxyClient() {
    ...
    var executorResults = new (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Subject();

    this._rnMessages = (0, (_runApp || _load_runApp()).runApp)(executorResults).catch(function (err) {
    ...
    this._executorResponses = (0, (_executeRequests || _load_executeRequests()).executeRequests)(executorRequests).catch(function (err) {
      if (err.code === 'ENOENT') {
    ...
    this._pids = this._executorResponses.filter(function (response) {
      ...
      (0, (_assert || _load_assert()).default)(response.kind === 'pid');
      return response.pid;
    });
    ...
    this._executorResponses.filter(function (response) {
      ...
      (0, (_assert || _load_assert()).default)(response.kind === 'error');
  ...
  _createClass(DebuggerProxyClient, [{
    ...
    value: function onDidEvalApplicationScript(callback) {
      return new (_commonsNodeUniversalDisposable || _load_commonsNodeUniversalDisposable()).default(this._pids.subscribe(callback));
    }
  }]);
