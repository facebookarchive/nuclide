...
var DebuggerProxyClient = (function () {
  function DebuggerProxyClient() {
    ...
    var executorResults = new (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Subject();
    ...
    this._pids = this._executorResponses.filter(function (response) {
      ...
      (0, (_assert2 || _assert()).default)(response.kind === 'pid');
      return response.pid;
    });
    ...
    this._executorResponses.filter(function (response) {
      ...
      (0, (_assert2 || _assert()).default)(response.kind === 'error');
  ...
  _createClass(DebuggerProxyClient, [{
    ...
    value: function onDidEvalApplicationScript(callback) {
      return new (_commonsNodeUniversalDisposable2 || _commonsNodeUniversalDisposable()).default(this._pids.subscribe(callback));
    }
  }]);
