...
var _nuclideDebuggerNodeRpcLibSession;

function _load_nuclideDebuggerNodeRpcLibSession() {
  return _nuclideDebuggerNodeRpcLibSession = require('../../../nuclide-debugger-node-rpc/lib/Session');
}
...
var pid$ = (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.using(function () {
  var client = new (_DebuggerProxyClient || _load_DebuggerProxyClient()).DebuggerProxyClient();
  client.connect();
  return {
    client: client,
  ...
  return (0, (_commonsNodeEvent || _load_commonsNodeEvent()).observableFromSubscribeFunction)(client.onDidEvalApplicationScript.bind(client));
}).publish();
 ...
 */
// $FlowFixMe(matthewwithanm): Type this.
var uiConnection$ = (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.using(function () {
  // TODO(natthu): Assign random port instead.
  ...
  return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.merge((_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.fromEvent(server, 'error').flatMap((_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.throw), (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.fromEvent(server, 'connection')).takeUntil((_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.fromEvent(server, 'close'));
}).publish();
...
function createSessionStream(ws, debugPort) {
  ...
  return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.create(function (observer) {
    ...
    var session = new (_nuclideDebuggerNodeRpcLibSession || _load_nuclideDebuggerNodeRpcLibSession()).Session(config, debugPort, ws);
