...
function runApp(executorResults) {
  ...
  return websockets.switchMap(function (ws) {
    return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.merge(
    // The messages from the RN app.
    (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.fromEvent(ws, 'message').map(JSON.parse),
...
function connectToRnApp() {
  ...
  var websockets = (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.using(function () {
    var ws = new (_ws || _load_ws()).default(WS_URL);
    return { ws: ws, unsubscribe: function unsubscribe() {
        ws.close();
      } };
  }, function (_ref) {
    var ws = _ref.ws;
    return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.of(ws);
  }).switchMap(function (ws) {
    return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.merge((_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.never(),

    // A stream of websockets...
    (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.of(ws),

    // ...that errors if the websocket closes before we unsubscribe.
    (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.fromEvent(ws, 'close').map(function () {
      throw new PrematureCloseError();
    }),

    // ...or when there's a websocket error.
    (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.fromEvent(ws, 'error').switchMap((_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.throw));
  ...
  return (0, (_commonsNodeObservable || _load_commonsNodeObservable()).cacheWhileSubscribed)(websockets);
}
