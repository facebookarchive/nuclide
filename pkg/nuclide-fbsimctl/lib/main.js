...
observeDevices = observeDevices;exports.
 ...
 * 
 * @format
 */function observeDevices(callback) {const subscription = poller.subscribe(devices => callback(devices));return new (_UniversalDisposable || _load_UniversalDisposable()).default(() => subscription.unsubscribe());} // Observable version
function getDevices() {return poller;}function createPoller() {return _rxjsBundlesRxMinJs.Observable.interval(2000).startWith(0).switchMap(() => {return (0, (_process || _load_process()).runCommand)('fbsimctl', ['--json', '--format=%n%u%s%o%a', 'list']).map(parseFbsimctlJsonOutput).catch(error => {const friendlyError = new Error(
