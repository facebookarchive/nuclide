...
function _HistogramTracker() {
  const data = require("../lib/HistogramTracker");
...
describe('HistogramTracker', () => {
  ...
  beforeEach(() => {
    ...
    tracker = new (_HistogramTracker().HistogramTracker)('test', 100, 10, 5);
  });
  afterEach(() => {
    tracker.dispose();
  });
  ...
  it('can save analytics', () => {
    ...
    expect(trackSpy.mock.calls.map(x => x)).toEqual([['performance-histogram', {
      average: 2,
      samples: 1,
      eventName: 'test'
  ...
  it('can save analytics at an interval', () => {
    ...
    expect(trackSpy.mock.calls[0]).toEqual(['performance-histogram', {
      average: 2,
      samples: 1,
      eventName: 'test'
    }]);
    ...
    expect(trackSpy.mock.calls[1]).toEqual(['performance-histogram', {
      average: 42,
      samples: 1,
      eventName: 'test'
    }]);
...
describe('Histogram.dispose', () => {
  it('stops after dispose', () => {
    ...
    const tracker = new (_HistogramTracker().HistogramTracker)('test', 100, 10, 5);
    tracker.track(1);
    tracker.dispose();
    jest.advanceTimersByTime(5 * 1000);
    expect(trackSpy.mock.calls.length).toBe(0);
