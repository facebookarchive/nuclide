...
function trackEvents(events) {
  return new (_commonsNodeUniversalDisposable || _load_commonsNodeUniversalDisposable()).default(events.subscribe(trackEvent));
}
...
function trackTiming() {
  var eventName_ = arguments.length <= 0 || arguments[0] === undefined ? null : arguments[0];

  var eventName = eventName_;
  ...
  return function (target, name, descriptor) {
    ...
    descriptor.value = function () {
      ...
      if (!eventName) {
        ...
        eventName = (0, (_commonsNodeString || _load_commonsNodeString()).maybeToString)(constructorName) + '.' + name;
      }
...
function trackOperationTiming(eventName, operation) {
  ...
  try {
    ...
    if ((0, (_commonsNodePromise || _load_commonsNodePromise()).isPromise)(result)) {
