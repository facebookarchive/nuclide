...
async function hgTestFolder() {
  ...
  await (0, (_process || _load_process()).runCommand)('hg', ['addremove'], { cwd: folder }).toPromise();
  ...
  _fs.default.writeFileSync((_nuclideUri || _load_nuclideUri()).default.join(folder, '.hgignore'), `.hgignore\n${ignoredFile}`);

  return folder;
}

(0, (_a_file_search_should || _load_a_file_search_should()).aFileSearchShould)('Mercurial', hgTestFolder);