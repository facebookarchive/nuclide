...
var PathSet = (function () {
  function PathSet(paths, ignoredNames) {
    ...
    this._ignoredPatterns = ignoredNames.map(function (name) {
      return (0, (_minimatch || _load_minimatch()).makeRe)(name, { matchBase: true, dot: true });
    })
    // makeRe returns false for invalid patterns.
    .filter(function (x) {
      return x;
    });
    this._matcher = new (_nuclideFuzzyNative || _load_nuclideFuzzyNative()).Matcher(paths.filter(function (path) {
  ...
  _createClass(PathSet, [{
    ...
    value: function match(query) {
      return this._matcher.match(query, {
        maxResults: 20,
        numThreads: (_os || _load_os()).default.cpus().length,
