...
function getFilesFromCommand(command, args, localDirectory, transform) {
  return new Promise(function (resolve, reject) {
    ...
    var proc = (_child_process || _load_child_process()).default.spawn(command, args, { cwd: localDirectory });

    proc.on('error', reject);

    var filePaths = [];
    proc.stdout.pipe((0, (_split || _load_split()).default)()).on('data', function (filePath_) {
