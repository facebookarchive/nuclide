...
async function getFilesFromRepo(localDirectory) {
  if (!(await (_fsPromise || _load_fsPromise()).default.exists((_nuclideUri || _load_nuclideUri()).default.join(localDirectory, '.repo')))) {
    throw new Error(`${localDirectory} is not a repo root`);
  }
  const subRoots = (await (0, (_process || _load_process()).runCommand)('repo', ['list', '-p'], {
    cwd: localDirectory
  }).toPromise()).split(/\n/).filter(s => s.length > 0);

  const fileLists = await (0, (_promise || _load_promise()).asyncLimit)(subRoots, 20, subRoot => {
    return getFilesFromGit((_nuclideUri || _load_nuclideUri()).default.join(localDirectory, subRoot)).catch(() => []).then(files => files.map(file => (_nuclideUri || _load_nuclideUri()).default.join(subRoot, file)));
  });
