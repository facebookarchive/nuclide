...
var _commonsAtomContextMenu;

function _load_commonsAtomContextMenu() {
  return _commonsAtomContextMenu = _interopRequireDefault(require('../../commons-atom/ContextMenu'));
}
...
var FileTreeContextMenu = (function () {
  function FileTreeContextMenu() {
    ...
    this._contextMenu = new (_commonsAtomContextMenu || _load_commonsAtomContextMenu()).default({
      type: 'root',
    ...
    var shouldDisplaySetToCurrentWorkingRootOption = function shouldDisplaySetToCurrentWorkingRootOption() {
      ...
      return node != null && node.isContainer && _this._store.hasCwd() && !node.isCwd;
    };
    ...
    this._sourceControlMenu = new (_commonsAtomContextMenu || _load_commonsAtomContextMenu()).default({
      type: 'submenu',
  ...
  _createClass(FileTreeContextMenu, [{
    ...
    value: function _shouldDisplayShowInFileManager(platform) {
      ...
      return node != null && (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.isAbsolute(node.uri) && process.platform === platform;
    }
  }]);
...
function initCommandIfPresent(item) {
  var itemDisposable = new (_atom || _load_atom()).CompositeDisposable();
