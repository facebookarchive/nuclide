'use strict';Object.defineProperty(exports, "__esModule", { value: true });var _asyncToGenerator = _interopRequireDefault(require('async-to-generator'));var _FileTreeDispatcher;
...
function _load_nuclideAnalytics() {return _nuclideAnalytics = require('../../nuclide-analytics');}var _nuclideVcsBase;
function _load_nuclideVcsBase() {return _nuclideVcsBase = require('../../nuclide-vcs-base');}var _nuclideHgRpc;
function _load_nuclideHgRpc() {return _nuclideHgRpc = require('../../nuclide-hg-rpc');}var _log4js;
...
function _load_nuclideUri() {return _nuclideUri = _interopRequireDefault(require('../../../modules/nuclide-commons/nuclideUri'));}var _event;
function _load_event() {return _event = require('../../../modules/nuclide-commons/event');}
var _rxjsBundlesRxMinJs = require('rxjs/bundles/Rx.min.js');var _collection;
...
/**
               ...
               */ /**
                   ...
                   * 
                   * @format
                   */class FileTreeActions {static getInstance() {if (!instance) {instance = new FileTreeActions();}
    return instance;
  }
  ...
  side)
  {
    ...
    (_FileTreeHelpers || _load_FileTreeHelpers()).default.keyToPath(nodeKey),
  ...
  {var _this = this;return (0, _asyncToGenerator.default)(function* () {
      const rootKeys = rootDirectories.map(function (directory) {return (
          (_FileTreeHelpers || _load_FileTreeHelpers()).default.dirPathToKey(directory.getPath()));});

      // $FlowFixMe
      const rootRepos = yield Promise.all(
      rootDirectories.map(function (directory) {return (0, (_nuclideVcsBase || _load_nuclideVcsBase()).repositoryForPath)(directory.getPath());}));
      ...
      // repository.
      const rootKeysForRepository = (_immutable || _load_immutable()).Map(
      omitNullKeys(
      (_immutable || _load_immutable()).List(rootKeys).groupBy(function (rootKey, index) {return rootRepos[index];})).
      map(function (v) {return (_immutable || _load_immutable()).Set(v);}));


      const prevRepos = _this._store.getRepositories();

      // Let the store know we have some new repos!
      const nextRepos = (_immutable || _load_immutable()).Set(
      ...
      removedRepos.forEach(function (repo) {return _this._repositoryRemoved(repo);});

      // Create subscriptions for addedRepos.
      addedRepos.forEach(function (repo) {return (
          _this._repositoryAdded(repo, rootKeysForRepository));});})();
  ...
  {var _this2 = this;return (0, _asyncToGenerator.default)(function* () {
      ...
      if (repo.isDestroyed()) {
        ...
        vcsChanges = _rxjsBundlesRxMinJs.Observable.merge(
        (0, (_event || _load_event()).observableFromSubscribeFunction)(repo.onDidChangeStatus.bind(repo)),
        (0, (_event || _load_event()).observableFromSubscribeFunction)(repo.onDidChangeStatuses.bind(repo))).

        let((0, (_observable || _load_observable()).fastDebounce)(1000)).
        startWith(null).
        map(function () {return (
            _this2._getCachedPathStatusesForGitRepo(
            repo));});
        ...
        const hgChanges = (_FileTreeHelpers || _load_FileTreeHelpers()).default.observeUncommittedChangesKindConfigKey().
        map(function (kind) {
          switch (kind) {
            ...
            default:
              kind;
              const error = _rxjsBundlesRxMinJs.Observable.throw(
        ...
        switchMap(function (c) {return c.statusChanges;}).
        distinctUntilChanged((_collection || _load_collection()).mapEqual);
        vcsCalculating = hgChanges.switchMap(function (c) {return c.isCalculatingChanges;});
      }

      const subscription = vcsChanges.subscribe(function (statusCodeForPath) {
        for (const rootKeyForRepo of (0, (_nullthrows || _load_nullthrows()).default)(
      ...
      function (isCalculatingChanges) {
        _this2.setIsCalculatingChanges(isCalculatingChanges);
      });


      _this2._disposableForRepository = _this2._disposableForRepository.set(
      repo,
      new (_UniversalDisposable || _load_UniversalDisposable()).default(subscription, subscriptionCalculating));})();
  ...
  repo)
  {
    ...
    for (const relativePath in statuses) {
      ...
      if (internalGitRepo.isStatusNew(gitStatusNumber)) {
        ...
        (0, (_log4js || _load_log4js()).getLogger)('nuclide-file-tree').warn(
