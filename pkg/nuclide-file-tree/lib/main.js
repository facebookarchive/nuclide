...
class Activation {
  ...
  constructor(rawState) {
    ...
    atom.workspace.observePaneItems(item => {
      if (item != null && typeof item.getURI === 'function' && item.getURI() === 'atom://tree-view') {
        timerId = setImmediate(() => {
          if (atom.packages.isPackageDisabled('tree-view') && atom.workspace.paneForItem(item) && // Make sure it's still in the workspace.
