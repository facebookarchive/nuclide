...
var _componentsFileTreeSidebarComponent;

function _load_componentsFileTreeSidebarComponent() {
  return _componentsFileTreeSidebarComponent = _interopRequireDefault(require('../components/FileTreeSidebarComponent'));
}
...
var Activation = (function () {
  function Activation(state) {
    ...
    this._subscriptions.add((_commonsAtomFeatureConfig || _load_commonsAtomFeatureConfig()).default.observe(prefixKeyNavSetting, this._setPrefixKeyNavSetting.bind(this)), (_commonsAtomFeatureConfig || _load_commonsAtomFeatureConfig()).default.observe(REVEAL_FILE_ON_SWITCH_SETTING, this._setRevealOnFileSwitch.bind(this)), atom.config.observe(ignoredNamesSetting, this._setIgnoredNames.bind(this)), (_commonsAtomFeatureConfig || _load_commonsAtomFeatureConfig()).default.observe(hideIgnoredNamesSetting, this._setHideIgnoredNames.bind(this)), atom.config.observe(excludeVcsIgnoredPathsSetting, this._setExcludeVcsIgnoredPaths.bind(this)), atom.config.observe(allowPendingPaneItems, this._setUsePreviewTabs.bind(this)));
  }

  _createClass(Activation, [{
    key: 'consumeCwdApi',
    value: function consumeCwdApi(cwdApi) {
      (0, (_assert || _load_assert()).default)(this._fileTreeController);
      ...
      this._cwdApiSubscription = new (_atom || _load_atom()).Disposable(function () {
    ...
    value: function consumeWorkingSetsStore(workingSetsStore) {
      ...
      var rebuildOpenFilesWorkingSet = (0, (_commonsNodeDebounce || _load_commonsNodeDebounce()).default)(function () {
        var openUris = atom.workspace.getTextEditors().filter(function (te) {
          return te.getPath() != null && te.getPath() !== '';
        }).map(function (te) {
          return te.getPath();
        });
        var openFilesWorkingSet = new (_nuclideWorkingSetsCommon || _load_nuclideWorkingSetsCommon()).WorkingSet(openUris);
        updateOpenFilesWorkingSet(openFilesWorkingSet);
      }, OPEN_FILES_UPDATE_DEBOUNCE_INTERVAL_MS);

      rebuildOpenFilesWorkingSet();

      var paneObservingDisposable = new (_atom || _load_atom()).CompositeDisposable();
      ...
      return new (_atom || _load_atom()).Disposable(function () {
        ...
        _this._fileTreeController.updateWorkingSet(new (_nuclideWorkingSetsCommon || _load_nuclideWorkingSetsCommon()).WorkingSet());
        _this._fileTreeController.updateOpenFilesWorkingSet(new (_nuclideWorkingSetsCommon || _load_nuclideWorkingSetsCommon()).WorkingSet());
...
function deactivate() {
  ...
  if ((_commonsAtomFeatureConfig || _load_commonsAtomFeatureConfig()).default.isFeatureDisabled('nuclide-file-tree') && atom.packages.isPackageDisabled('tree-view')) {
...
function consumeNuclideSideBar(sidebar) {
  ...
  sidebar.registerView({
    getComponent: function getComponent() {
      return (_componentsFileTreeSidebarComponent || _load_componentsFileTreeSidebarComponent()).default;
    },
    onDidShow: function onDidShow() {
      ...
      if ((_commonsAtomFeatureConfig || _load_commonsAtomFeatureConfig()).default.get(REVEAL_FILE_ON_SWITCH_SETTING)) {
  ...
  sideBarDisposable = new (_atom || _load_atom()).Disposable(function () {
