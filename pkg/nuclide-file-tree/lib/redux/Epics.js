...
export function revealNodeKeyEpic(
  ...
  return actions
    ...
    .map(action => {
      ...
      return Actions.ensureChildNode(nodeKey);
    })
    .filter(Boolean);
}
...
export function updateRootDirectoriesEpic(
  ...
  return actions.ofType(ActionTypes.UPDATE_ROOT_DIRECTORIES).switchMap(() => {
    ...
    return Observable.from([
      Actions.setRootKeys(rootKeys),
      Actions.updateRepositories(rootDirectories),
    ]);
  });
}
