'use strict';Object.defineProperty(exports, "__esModule", { value: true });var _asyncToGenerator = _interopRequireDefault(require('async-to-generator'));var _FileTreeConstants;
...
function _load_FileTreeConstants() {return _FileTreeConstants = require('./FileTreeConstants');}var _FileSystemActions;
function _load_FileSystemActions() {return _FileSystemActions = _interopRequireDefault(require('./FileSystemActions'));}var _FileTreeActions;
function _load_FileTreeActions() {return _FileTreeActions = _interopRequireDefault(require('./FileTreeActions'));}var _FileTreeContextMenu;
function _load_FileTreeContextMenu() {return _FileTreeContextMenu = _interopRequireDefault(require('./FileTreeContextMenu'));}var _FileTreeHelpers;
function _load_FileTreeHelpers() {return _FileTreeHelpers = _interopRequireDefault(require('./FileTreeHelpers'));}var _FileTreeStore;
function _load_FileTreeStore() {return _FileTreeStore = require('./FileTreeStore');}var _immutable;
function _load_immutable() {return _immutable = _interopRequireWildcard(require('immutable'));}var _nuclideAnalytics;
function _load_nuclideAnalytics() {return _nuclideAnalytics = require('../../nuclide-analytics');}var _nullthrows;
function _load_nullthrows() {return _nullthrows = _interopRequireDefault(require('nullthrows'));}var _nuclideUri;
function _load_nuclideUri() {return _nuclideUri = _interopRequireDefault(require('../../../modules/nuclide-commons/nuclideUri'));}var _goToLocation;
function _load_goToLocation() {return _goToLocation = require('../../../modules/nuclide-commons-atom/go-to-location');}var _getElementFilePath;
function _load_getElementFilePath() {return _getElementFilePath = _interopRequireDefault(require('../../../modules/nuclide-commons-atom/getElementFilePath'));}var _removeProjectPath;
function _load_removeProjectPath() {return _removeProjectPath = _interopRequireDefault(require('../../commons-atom/removeProjectPath'));}var _systemInfo;
function _load_systemInfo() {return _systemInfo = require('../../commons-node/system-info');}var _UniversalDisposable;
function _load_UniversalDisposable() {return _UniversalDisposable = _interopRequireDefault(require('../../../modules/nuclide-commons/UniversalDisposable'));}var _Constants;
function _load_Constants() {return _Constants = require('./Constants');}var _log4js;
function _load_log4js() {return _log4js = require('log4js');}



var _os = _interopRequireDefault(require('os'));
var _electron = require('electron');function _interopRequireWildcard(obj) {if (obj && obj.__esModule) {return obj;} else {var newObj = {};if (obj != null) {for (var key in obj) {if (Object.prototype.hasOwnProperty.call(obj, key)) newObj[key] = obj[key];}}newObj.default = obj;return newObj;}}function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };} // $FlowFixMe(>=0.53.0) Flow suppress
...
class FileTreeController {
  ...
  constructor(state) {
    ...
    atom.commands.add('atom-workspace', {
      ...
      'tree-view:add-file-relative': () => {
        (_FileSystemActions || _load_FileSystemActions()).default.openAddFileDialogRelative(
    ...
    this._disposables.add(
    atom.commands.add((_FileTreeConstants || _load_FileTreeConstants()).COMMANDS_SELECTOR, Object.assign({
      ...
      'tree-view:add-file': () => {
        (_FileSystemActions || _load_FileSystemActions()).default.openAddFileDialog(
        this._openAndRevealFilePath.bind(this));

      },
      'tree-view:add-folder': () => {
        (_FileSystemActions || _load_FileSystemActions()).default.openAddFolderDialog(
      ...
      (_FileSystemActions || _load_FileSystemActions()).default.openRenameDialog(),
      'tree-view:duplicate-selection': () => {
        (_FileSystemActions || _load_FileSystemActions()).default.openDuplicateDialog(
  ...
  _updateRootDirectories() {
    ...
    getDirectories().
    filter(directory => (_FileTreeHelpers || _load_FileTreeHelpers()).default.isValidDirectory(directory));
    const rootKeys = rootDirectories.map(directory =>
    (_FileTreeHelpers || _load_FileTreeHelpers()).default.dirPathToKey(directory.getPath()));
  ...
  setCwdApi(cwdApi) {
    if (cwdApi == null) {
      ...
      this._cwdApiSubscription == null)) {throw new Error('Invariant violation: "this._cwdApiSubscription == null"');}
  ...
  _deleteSelection() {
    ...
    if (rootPaths.size === 0) {
      const selectedPaths = nodes.map(node => {
        ...
        return (0, (_systemInfo || _load_systemInfo()).isRunningInWindows)() ?
        (_nuclideUri || _load_nuclideUri()).default.relative(parentOfRoot, nodePath).replace(/\//g, '\\') :
        (_nuclideUri || _load_nuclideUri()).default.relative(parentOfRoot, nodePath);
      });
      const message =
      ...
      atom.confirm({
        ...
        detailedMessage: `You are deleting:${_os.default.EOL}${selectedPaths.join(
        _os.default.EOL)
        }`,
        message });
      ...
      if (rootPaths.size === 1) {
        ...
        (0, (_nullthrows || _load_nullthrows()).default)(rootPaths.first()).name
        }' can't be removed.`;
      } else {
  ...
  _removeRootFolderSelection() {var _this = this;return (0, _asyncToGenerator.default)(function* () {
  ...
  _searchInDirectory(event) {
    ...
    if (targetElement.classList.contains('nuclide-file-tree')) {
      ...
      let path = node.uri;
      if (!node.isContainer) {if (!
        node.parent) {throw new Error('Invariant violation: "node.parent"');}
