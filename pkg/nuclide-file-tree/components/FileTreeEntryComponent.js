  ...
  _createClass(FileTreeEntryComponent, [{
    ...
    value: function render() {
      ...
      if (!node.conf.isEditingWorkingSet) {
        ...
        if (vcsStatusCode === (_nuclideHgRpcLibHgConstants2 || _nuclideHgRpcLibHgConstants()).StatusCodeNumber.MODIFIED) {
          statusClass = 'status-modified';
        } else if (vcsStatusCode === (_nuclideHgRpcLibHgConstants2 || _nuclideHgRpcLibHgConstants()).StatusCodeNumber.ADDED) {
    ...
    value: function _renderCheckbox() {
      ...
      return (_reactForAtom2 || _reactForAtom()).React.createElement((_nuclideUiCheckbox2 || _nuclideUiCheckbox()).Checkbox, {
    ...
    value: function _isToggleNodeExpand(event) {
      var node = this.props.node;
      return node.isContainer && (_reactForAtom2 || _reactForAtom()).ReactDOM.findDOMNode(this.refs.arrowContainer).contains(event.target) && event.clientX < (_reactForAtom2 || _reactForAtom()).ReactDOM.findDOMNode(this.refs.pathContainer).getBoundingClientRect().left;
    }
  }, {
    key: '_onMouseDown',
...
function getSelectionMode(event) {
  ...
  if ((_os2 || _os()).default.platform() === 'darwin' && event.metaKey && event.button === 0 || (_os2 || _os()).default.platform() !== 'darwin' && event.ctrlKey && event.button === 0) {
    return 'multi-select';
  }
