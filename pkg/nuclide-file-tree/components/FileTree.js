  ...
  _renderChildren() {
    ...
    if (reorderPreviewStatus != null) {
      ...
      if (sourceNode != null && target != null && targetIdx != null && targetIdx !== sourceIdx) {
        reorderPreview = {
          entry: _react.default.createElement((_FileTreeEntryComponent || _load_FileTreeEntryComponent()).FileTreeEntryComponent, { node: sourceNode, isPreview: true }),
    ...
    while (node != null && visibleChildren.length < amountToRender) {
      if (!node.isRoot && !chosenMeasured) {
        const entry = _react.default.createElement((_FileTreeEntryComponent || _load_FileTreeEntryComponent()).FileTreeEntryComponent, { key: key, node: node, ref: 'measured' });
        ...
        const entry = _react.default.createElement((_FileTreeEntryComponent || _load_FileTreeEntryComponent()).FileTreeEntryComponent, { key: key, node: node });
        if (reorderPreview != null && reorderPreview.target === node.uri) {
          if (reorderPreview.above) {
            visibleChildren.push(reorderPreview.entry, entry);
          } else {
