...
class NativeDebuggerService extends (_nuclideDebuggerCommon || _load_nuclideDebuggerCommon()).DebuggerRpcWebSocketService {
  ...
  launch(launchInfo) {
    ...
    const resolvedPath = exePath.startsWith('./') ? (_nuclideUri || _load_nuclideUri()).default.resolve((_nuclideUri || _load_nuclideUri()).default.expandHomeDir(launchInfo.workingDirectory), exePath.substring(2)) : (_nuclideUri || _load_nuclideUri()).default.expandHomeDir(exePath);
