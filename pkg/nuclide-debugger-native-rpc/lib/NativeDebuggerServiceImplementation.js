...
exports.NativeDebuggerService = exports.getProcessTree = exports.getAttachTargetInfoList = undefined;
...
function getLocalPathIfRemote(path) {
  const maybeRemotePath = path;
  if ((_nuclideUri || _load_nuclideUri()).default.isRemote(maybeRemotePath)) {
    const { path: localPath } = (_nuclideUri || _load_nuclideUri()).default.parseRemoteUri(maybeRemotePath);
    return localPath;
  } else {
  ...
  _startDebugging(inferiorArguments) {
    ...
    return (0, _asyncToGenerator.default)(function* () {
      ...
      if (inferiorArguments.basepath) {
        inferiorArguments.basepath = getLocalPathIfRemote(inferiorArguments.basepath);
      }
