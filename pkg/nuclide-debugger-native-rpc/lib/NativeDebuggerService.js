...
var NativeDebuggerService = (function () {
  function NativeDebuggerService(config) {
    ...
    this._subscriptions = new (_eventKit2 || _eventKit()).CompositeDisposable(this._clientCallback);
  }
  ...
  _createClass(NativeDebuggerService, [{
    ...
    value: function _registerIpcChannel(lldbProcess) {
      ...
      this._subscriptions.add(new (_commonsNodeUniversalDisposable2 || _commonsNodeUniversalDisposable()).default((0, (_commonsNodeObservable2 || _commonsNodeObservable()).splitStream)((0, (_commonsNodeStream2 || _commonsNodeStream()).observeStream)(ipcStream)).subscribe(this._handleIpcMessage.bind(this, ipcStream), function (error) {
    ...
    value: function _spawnPythonBackend() {
      var lldbPythonScriptPath = (_commonsNodeNuclideUri2 || _commonsNodeNuclideUri()).default.join(__dirname, '../scripts/main.py');
      var python_args = [lldbPythonScriptPath, '--arguments_in_json'];
      var options = {
        cwd: (_commonsNodeNuclideUri2 || _commonsNodeNuclideUri()).default.dirname(lldbPythonScriptPath),
    ...
    value: function _sendArgumentsToPythonBackend(child, args) {
      ...
      this._subscriptions.add(new (_commonsNodeUniversalDisposable2 || _commonsNodeUniversalDisposable()).default((0, (_commonsNodeStream2 || _commonsNodeStream()).observeStream)(argumentsStream).first().subscribe(function (text) {
