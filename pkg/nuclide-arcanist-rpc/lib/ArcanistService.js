...
var readArcConfig = _asyncToGenerator(function* (fileName) {
  ...
  if (!arcProjectMap.has(arcConfigDirectory)) {
    var arcconfigFile = (_commonsNodeNuclideUri2 || _commonsNodeNuclideUri()).default.join(arcConfigDirectory, ARC_CONFIG_FILE_NAME);
...
var getProjectRelativePath = _asyncToGenerator(function* (fileName) {
  ...
  return arcPath && fileName ? (_commonsNodeNuclideUri2 || _commonsNodeNuclideUri()).default.relative(arcPath, fileName) : null;
});
...
var getMercurialHeadCommitChanges = _asyncToGenerator(function* (filePath) {
  ...
  var filesChanged = yield (0, (_nuclideHgRpcLibHgRevisionStateHelpers2 || _nuclideHgRpcLibHgRevisionStateHelpers()).fetchFilesChangedAtRevision)((0, (_nuclideHgRpcLibHgRevisionExpressionHelpers2 || _nuclideHgRpcLibHgRevisionExpressionHelpers()).expressionForRevisionsBeforeHead)(0), hgRepoDetails.workingDirectoryPath).refCount().toPromise();
...
var execArcLint = _asyncToGenerator(function* (cwd, filePaths, skip) {
  ...
  for (var _line of result.stdout.trim().split('\n')) {
    ...
    try {
      ...
      (0, (_nuclideLogging2 || _nuclideLogging()).getLogger)().warn('Error parsing `arc lint` JSON output', _line);
      continue;
    }
  ...
  for (var file of filePaths) {
    ...
    var key = (_commonsNodeNuclideUri2 || _commonsNodeNuclideUri()).default.relative(cwd, file);
...
var _nuclideHgRpcLibHgRevisionStateHelpers2;

function _nuclideHgRpcLibHgRevisionStateHelpers() {
  return _nuclideHgRpcLibHgRevisionStateHelpers2 = require('../../nuclide-hg-rpc/lib/hg-revision-state-helpers');
}

var _nuclideHgRpcLibHgRevisionExpressionHelpers2;

function _nuclideHgRpcLibHgRevisionExpressionHelpers() {
  return _nuclideHgRpcLibHgRevisionExpressionHelpers2 = require('../../nuclide-hg-rpc/lib/hg-revision-expression-helpers');
}
...
function _callArcDiff(filePath, extraArcDiffArgs) {
  ...
  return (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.fromPromise(getCommitBasedArcConfigDirectory(filePath)).flatMap(function (arcConfigDir) {
...
function createPhabricatorRevision(filePath, lintExcuse) {
  var args = ['--verbatim'].concat(_toConsumableArray(getArcDiffParams(lintExcuse)));
