...
function findDiagnostics(path, skip) {
  if ((_nuclideUri || _load_nuclideUri()).default.isInArchive(path)) {
    return _rxjsBundlesRxMinJs.Observable.of().publish();
  }
...
function updatePhabricatorRevision(filePath, message, allowUntracked, lintExcuse, updatePhabricatorFields) {
  const baseArgs = ['-m', message, ...getArcDiffParams(lintExcuse)];
  const args = [...(updatePhabricatorFields ? ['--verbatim'] : []), ...baseArgs];
