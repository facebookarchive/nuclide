...
const arcConfigDirectoryMap = (0, (_lruCache || _load_lruCache()).default)({ maxAge: CACHE_TIME });
...
function _callArcDiff(filePath, extraArcDiffArgs) {
  ...
  return _rxjsBundlesRxMinJs.Observable.fromPromise(getCommitBasedArcConfigDirectory(filePath)).flatMap(arcConfigDir => {
    ...
    return _rxjsBundlesRxMinJs.Observable.fromPromise(getArcExecOptions(arcConfigDir)).switchMap(opts => {
      ...
      return (0, (_observable || _load_observable()).compact)((0, (_process || _load_process()).observeProcess)('script', scriptArgs, opts).catch(error => _rxjsBundlesRxMinJs.Observable.of({ kind: 'error', error })) // TODO(T17463635)
      .map(event => {
        switch (event.kind) {
          case 'stdout':
...
function execArcPull(cwd, fetchLatest, allowDirtyChanges) {
  ...
  return _rxjsBundlesRxMinJs.Observable.fromPromise(getArcExecOptions(cwd)).switchMap(opts => (0, (_process || _load_process()).observeProcess)('arc', args, Object.assign({}, opts, { /* TODO(T17353599) */isExitError: () => false })).catch(error => _rxjsBundlesRxMinJs.Observable.of({ kind: 'error', error }))).publish();
}
...
function execArcLint(cwd, filePaths, skip) {
  ...
  return _rxjsBundlesRxMinJs.Observable.fromPromise(getArcExecOptions(cwd)).switchMap(opts => (0, (_nice || _load_nice()).niceObserveProcess)('arc', args, Object.assign({}, opts, { killTreeOnComplete: true })).catch(error => _rxjsBundlesRxMinJs.Observable.of({ kind: 'error', error }))).mergeMap(event => {
    if (event.kind === 'error') {
