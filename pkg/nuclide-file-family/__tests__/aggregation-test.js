...
function _load_FileFamilyAggregator() {
  return _FileFamilyAggregator = _interopRequireDefault(require('../lib/FileFamilyAggregator'));
}
...
describe('aggregation', () => {
  it('combines nothing', async () => {
    const aggregator = new (_FileFamilyAggregator || _load_FileFamilyAggregator()).default(_rxjsBundlesRxMinJs.Observable.of(new Set()));
  ...
  it('combines unique graphs', async () => {
    const aggregator = new (_FileFamilyAggregator || _load_FileFamilyAggregator()).default(_rxjsBundlesRxMinJs.Observable.of(new Set([directedProvider, undirectedProvider])));
    const graph = await aggregator.getRelatedFiles('test');
    ...
    expect(Array.from(graph.files)).toEqual(Array.from((0, (_collection || _load_collection()).mapUnion)(expectedGraph.files, expectedGraph2.files)));
  ...
  it('deduplicates files and combines labels', async () => {
    const aggregator = new (_FileFamilyAggregator || _load_FileFamilyAggregator()).default(_rxjsBundlesRxMinJs.Observable.of(new Set([directedProvider, directedProvider2])));
    const graph = await aggregator.getRelatedFiles('test');
    const expectedGraph = await directedProvider.getRelatedFiles('test');
  ...
  it('deduplicates and combines labels for directed relations', async () => {
    ...
    expect(graph.relations).toEqual([{
      ...
      labels: (0, (_collection || _load_collection()).setUnion)(expectedGraph.relations[0].labels, expectedGraph2.relations[0].labels),
  ...
  it('deduplicates and combines labels for undirected relations', async () => {
    ...
    const aggregator = new (_FileFamilyAggregator || _load_FileFamilyAggregator()).default(_rxjsBundlesRxMinJs.Observable.of(new Set([undirectedProvider, undirectedProvider, reverseUndirectedProvider])));
    const graph = await aggregator.getRelatedFiles('test');
