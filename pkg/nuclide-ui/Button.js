...
const Button = exports.Button = props => {
  ...
  const remainingProps = _objectWithoutProperties(props, ['icon', 'iconset', 'buttonType', 'selected', 'size', 'children', 'className', 'wrapperElement', 'tooltip']);
  ...
  const newClassName = (0, (_classnames || _load_classnames()).default)(className, 'btn', {
    [`icon ${ iconset || 'icon' }-${ (0, (_string || _load_string()).maybeToString)(icon) }`]: icon != null,
