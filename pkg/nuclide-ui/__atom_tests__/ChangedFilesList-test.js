...
function _load_ChangedFilesList() {
  return _ChangedFilesList = require('../ChangedFilesList');
}
...
describe('computeDisplayPaths', () => {
  it('should compute depth 1 paths correctly', () => {
    ...
    const actual = (0, (_ChangedFilesList || _load_ChangedFilesList()).computeDisplayPaths)(input);
  ...
  it('should compute a mix of depths 1 to 5 paths correctly', () => {
    const input = ['/a/b/c/d/e.js', '/z/b/c/d/e.js', 'a/y/z/e.js', 'a/y/e.js', 'a/z/e.js', 'a/f.js'];
  ...
  it('should honor the max depth argument', () => {
    ...
    const actual = (0, (_ChangedFilesList || _load_ChangedFilesList()).computeDisplayPaths)(input, maxDepth);
    expect(actual).toEqual(expected);
  });
  ...
  it('should handle a realistic scenario path separators', () => {
    const input = ['nuclide://data/users/x/fbsource/fbcode/foo/bar.js', 'nuclide://data/users/x/fbsource/fbcode/bar/bar.js', '/Users/y/fbsource/fbcode/foo/bar.js', '/Users/y/fbsource/fbcode/bar/bar.js'];
    const expected = ['x/fbsource/fbcode/foo/bar.js', 'x/fbsource/fbcode/bar/bar.js', 'y/fbsource/fbcode/foo/bar.js', 'y/fbsource/fbcode/bar/bar.js'];
    const actual = (0, (_ChangedFilesList || _load_ChangedFilesList()).computeDisplayPaths)(input);
    expect(actual).toEqual(expected);
  });
}); /**
