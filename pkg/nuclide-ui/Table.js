...
var Table = (function (_React$Component) {
  ...
  _createClass(Table, [{
    ...
    value: function _handleResizerMouseDown(key, event) {
      ...
      this._tableWidth = (_reactForAtom || _load_reactForAtom()).ReactDOM.findDOMNode(this.refs.table).getBoundingClientRect().width;
      this._columnBeingResized = key;
      this._globalEventsDisposable = new (_atom || _load_atom()).Disposable(function () {
    ...
    value: function render() {
      ...
      var header = columns.map(function (column, i) {
        ...
        var resizeHandle = i === columns.length - 1 ? null : (_reactForAtom || _load_reactForAtom()).React.createElement('div', {
        ...
        if (sortable) {
          ...
          if (sortedColumn === key) {
            sortIndicator = (_reactForAtom || _load_reactForAtom()).React.createElement(
              'span',
              null,
              ' ',
              (_reactForAtom || _load_reactForAtom()).React.createElement((_Icon || _load_Icon()).Icon, { icon: sortDescending ? 'triangle-down' : 'triangle-up' })
            );
          }
        }
        return (_reactForAtom || _load_reactForAtom()).React.createElement(
      ...
      var body = rows.map(function (row, i) {
        ...
        var renderedRow = columns.map(function (column, j) {
          ...
          var datum = data[key];
          if (Component != null) {
            datum = (_reactForAtom || _load_reactForAtom()).React.createElement(Component, { data: datum });
          } else if (datum == null) {
      ...
      if (rows.length === 0) {
        ...
        body = (_reactForAtom || _load_reactForAtom()).React.createElement(
          'tr',
          null,
          (_reactForAtom || _load_reactForAtom()).React.createElement(
            'td',
            null,
            (_reactForAtom || _load_reactForAtom()).React.createElement(EmptyComponent, null)
          )
        );
      }
