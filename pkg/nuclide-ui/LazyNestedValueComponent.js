...
function TreeItemWithLoadingSpinner() {
  return (_reactForAtom || _load_reactForAtom()).React.createElement(
    ...
    null,
    (_reactForAtom || _load_reactForAtom()).React.createElement((_LoadingSpinner || _load_LoadingSpinner()).LoadingSpinner, { size: 'EXTRA_SMALL', delay: SPINNER_DELAY })
  );
}
  ...
  _createClass(ValueComponent, [{
    ...
    value: function componentDidMount() {
      ...
      if (!this.state.isExpanded && nodeData != null && nodeData.isExpanded && this._shouldFetch() && evaluationResult != null && evaluationResult.objectId != null && fetchChildren != null) {
        (0, (_assert || _load_assert()).default)(evaluationResult.objectId != null);
        this.setState({
    ...
    value: function componentWillReceiveProps(nextProps) {
      if (this._shouldFetch() && this.state.isExpanded && nextProps.evaluationResult != null && nextProps.fetchChildren != null) {
        var _objectId = nextProps.evaluationResult.objectId;
    ...
    value: function render() {
      ...
      if (!isObjectValue(evaluationResult)) {
        var simpleValueElement = (_reactForAtom || _load_reactForAtom()).React.createElement(SimpleValueComponent, {
          expression: expression,
          evaluationResult: evaluationResult,
          simpleValueComponent: SimpleValueComponent
        });
        return isRoot ? simpleValueElement : (_reactForAtom || _load_reactForAtom()).React.createElement(
          (_Tree || _load_Tree()).TreeItem,
          null,
          simpleValueElement
        );
      }
      var description = evaluationResult.description || '<no description provided>';
      ...
      if (isExpanded) {
        ...
        if (shouldCacheChildren && _cachedChildren != null) {
          childListElement = (_reactForAtom || _load_reactForAtom()).React.createElement(LoadableValueComponent, {
          ...
          childListElement = (_reactForAtom || _load_reactForAtom()).React.createElement(TreeItemWithLoadingSpinner, null);
        } else {
          var ChildrenComponent = (0, (_bindObservableAsProps || _load_bindObservableAsProps()).bindObservableAsProps)(children.map(function (childrenValue) {
            return { children: childrenValue };
          }).startWith({ children: null }), LoadableValueComponent);
          childListElement = (_reactForAtom || _load_reactForAtom()).React.createElement(ChildrenComponent, {
            fetchChildren: fetchChildren,
            path: path,
            expandedValuePaths: expandedValuePaths,
            onExpandedStateChange: onExpandedStateChange,
      ...
      var title = renderValueLine(expression, description);
      return (_reactForAtom || _load_reactForAtom()).React.createElement(
...
var TopLevelLazyNestedValueComponent = (function (_React$Component2) {
  ...
  _createClass(TopLevelLazyNestedValueComponent, [{
    ...
    key: 'render',
    value: function render() {
      var className = this.props.className != null ? this.props.className : 'nuclide-ui-lazy-nested-value';
      return (_reactForAtom || _load_reactForAtom()).React.createElement(
        'span',
        { className: className },
        (_reactForAtom || _load_reactForAtom()).React.createElement(ValueComponent, _extends({}, this.props, {
          isRoot: true,
...
var LazyNestedValueComponent = (0, (_highlightOnUpdate || _load_highlightOnUpdate()).highlightOnUpdate)(TopLevelLazyNestedValueComponent, arePropsEqual, undefined, /* custom classname */
undefined);
