...
let MultiRootChangedFilesView = exports.MultiRootChangedFilesView = class MultiRootChangedFilesView extends _reactForAtom.React.Component {
  ...
  componentDidMount() {
    ...
    const commandPrefix = _props.commandPrefix,
          getRevertTargetRevision = _props.getRevertTargetRevision;
    ...
    this._subscriptions.add(atom.contextMenu.add({
      [`.${ commandPrefix }-file-entry`]: [{ type: 'separator' }, {
        ...
        shouldDisplay: event => {
          const filePath = event.target.getAttribute('data-path');
          const fsService = (0, (_nuclideRemoteConnection || _load_nuclideRemoteConnection()).getFileSystemServiceByNuclideUri)(filePath);
    ...
    this._subscriptions.add(atom.commands.add(`.${ commandPrefix }-file-entry`, `${ commandPrefix }:delete-file`, event => {
      ...
      repository.remove([filePath], true);
    }));
