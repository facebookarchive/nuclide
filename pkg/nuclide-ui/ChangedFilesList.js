...
computeDisplayPaths = computeDisplayPaths;var _nuclideVcsBase;function _load_nuclideVcsBase() {return _nuclideVcsBase = require('../nuclide-vcs-base');}var _addTooltip;function _load_addTooltip() {return _addTooltip = _interopRequireDefault(require('nuclide-commons-ui/addTooltip'));}var _classnames;function _load_classnames() {return _classnames = _interopRequireDefault(require('classnames'));}var _nuclideUri;function _load_nuclideUri() {return _nuclideUri = _interopRequireDefault(require('nuclide-commons/nuclideUri'));}var _nullthrows;function _load_nullthrows() {return _nullthrows = _interopRequireDefault(require('nullthrows'));}var _react = _interopRequireWildcard(require('react'));var _ChangedFile;function _load_ChangedFile() {return _ChangedFile = _interopRequireDefault(require('./ChangedFile'));}function _interopRequireWildcard(obj) {if (obj && obj.__esModule) {return obj;} else {var newObj = {};if (obj != null) {for (var key in obj) {if (Object.prototype.hasOwnProperty.call(obj, key)) newObj[key] = obj[key];}}newObj.default = obj;return newObj;}}function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}function isHgPath(path) {const repo = (0, (_nuclideVcsBase || _load_nuclideVcsBase()).repositoryForPath)(path);return repo != null && repo.getType() === 'hg';} // Computes the minimally differentiable display path for each file.
 ...
 * 
 * @format
 */function computeDisplayPaths(filePaths, maxDepth = 5) {const displayPaths = filePaths.map(path => {const separator = (_nuclideUri || _load_nuclideUri()).default.pathSeparatorFor(path);return { separator, pathParts: path.split(separator).reverse(), depth: 1, done: false };});let seenCount = {};let currentDepth = 1;let toProcess = displayPaths;while (currentDepth < maxDepth && toProcess.length > 0) {
...
class ChangedFilesList extends _react.Component {
  ...
  render() {
    ...
    return (
      _react.createElement('ul', { className: 'list-tree has-collapsable-children nuclide-changed-files-list' },
        _react.createElement('li', { className: rootClassName },
          ...
          null,
          _react.createElement('ul', { className: 'list-tree has-flat-children' },
            sizeLimitedFileChanges.map(
            ({ displayPath, filePath, fileStatus }) => {
              return (
                _react.createElement((_ChangedFile || _load_ChangedFile()).default, {
            ...
            _react.createElement('li', null, showMoreFilesElement)))));
