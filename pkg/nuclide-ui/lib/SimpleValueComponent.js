...
function _TextRenderer() {
  return _TextRenderer2 = require('./TextRenderer');
}

function renderNullish(evaluationResult) {
  var type = evaluationResult.type;

  return type === 'undefined' || type === 'null' ? (_reactForAtom2 || _reactForAtom()).React.createElement(
    'span',
...
function renderString(evaluationResult) {
  ...
  return type === 'string' ? (_reactForAtom2 || _reactForAtom()).React.createElement(
    'span',
...
function renderNumber(evaluationResult) {
  ...
  return type === 'number' ? (_reactForAtom2 || _reactForAtom()).React.createElement(
    'span',
...
function renderBoolean(evaluationResult) {
  ...
  return type === 'boolean' ? (_reactForAtom2 || _reactForAtom()).React.createElement(
    'span',
    { className: (_ValueComponentClassNames2 || _ValueComponentClassNames()).ValueComponentClassNames.boolean },
    String(value)
  ) : null;
}
...
var valueRenderers = [(_TextRenderer2 || _TextRenderer()).TextRenderer, renderString, renderNumber, renderNullish, renderBoolean, renderDefault];
