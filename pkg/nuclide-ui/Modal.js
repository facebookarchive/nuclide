...
var Modal = (function (_React$Component) {
  ...
  _createClass(Modal, [{
    ...
    value: function _handleContainerInnerElement(el) {
      ...
      el.focus();
      this._cancelDisposable = new (_commonsNodeUniversalDisposable || _load_commonsNodeUniversalDisposable()).default(atom.commands.add(window, 'core:cancel', function () {
        _this.props.onDismiss();
      }), (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.fromEvent(window, 'click')
      // Ignore clicks in the current tick. We don't want to capture the click that showed this
      // modal.
      .skipUntil((_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.interval(0).first()).subscribe(this._handleWindowClick));
    }
  }, {
    key: 'render',
