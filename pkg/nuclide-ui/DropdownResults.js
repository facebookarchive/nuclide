'use strict';Object.defineProperty(exports, "__esModule", { value: true });var _AtomInput;
...
function _load_AtomInput() {return _AtomInput = require('../../modules/nuclide-commons-ui/AtomInput');}var _classnames;

function _load_classnames() {return _classnames = _interopRequireDefault(require('classnames'));}var _event;
function _load_event() {return _event = require('../../modules/nuclide-commons/event');}var _observable;
...
class DropdownResults extends _react.Component {
  ...
  constructor(props) {
    super(props);this.state = { optionsVisible: false, optionsRect: null, selectedIndex: -1 };this.
    ...
    _onSelect = (option, fromConfirm = false) => {
    ...
    };const workspaceElement = atom.views.getView(atom.workspace);if (!(workspaceElement != null)) {throw new Error('Invariant violation: "workspaceElement != null"');}this._optionsElement = document.createElement('div');workspaceElement.appendChild(this._optionsElement);this._shouldShowOnChange = true;this._shouldShowOnFocus = true;this._keepFocus = false;}componentDidMount() {const editorElement = this.props.atomInput.getTextEditorElement();const editor = this.props.atomInput.getTextEditor();this._disposables = new (_UniversalDisposable || _load_UniversalDisposable()).default(() => {this._optionsElement.remove();}, _rxjsBundlesRxMinJs.Observable.fromEvent(window, 'resize').let((0, (_observable || _load_observable()).fastDebounce)(200)).subscribe(this._recalculateRect), _rxjsBundlesRxMinJs.Observable.fromEvent(editorElement, 'focus').subscribe(this._handleInputFocus), _rxjsBundlesRxMinJs.Observable.fromEvent(editorElement, 'blur').subscribe(this._handleInputBlur), atom.commands.add(editorElement, 'core:move-up', () => this._handleMove(UP)), atom.commands.add(editorElement, 'core:move-down', () => this._handleMove(DOWN)), _rxjsBundlesRxMinJs.Observable.fromEvent(editorElement, 'keydown').subscribe(this._handleConfirm), (0, (_event || _load_event()).observableFromSubscribeFunction)(editor.onDidChange.bind(editor)).subscribe(this._handleInputChange));this._recalculateRect();}componentWillUnmount() {this._disposables.dispose();}render() {if (!this.state.optionsVisible) {return null;}let options = [];if (this.props.errorMessage != null && this.props.errorMessage.length > 0) {options.push(_react.createElement('li', { key: 'error-text', className: 'error' }, _react.createElement('span', { className: 'error-message' }, this.props.errorMessage)));} else if (this.props.isLoading) {options.push(_react.createElement('li', { key: 'loading-text', className: 'loading' }, _react.createElement('span', { className: 'loading-message' }, this.props.loadingMessage)));} else {options = this.props.options.map((option, i) => _react.createElement('li', { key: 'option-' + option, className: (0, (_classnames || _load_classnames()).default)({ selected: i === this.state.selectedIndex, 'nuclide-dropdown-result': true }), onMouseDown: () => this._onSelect(option), onMouseOver: () => this._setSelectedIndex(i) }, option));}return _reactDom.default.createPortal(_react.createElement('div', { className: 'nuclide-combobox-options nuclide-dropdown-results', style: this.state.optionsRect }, _react.createElement('div', { className: 'select-list' }, _react.createElement('ol', { className: 'nuclide-combobox-list-group list-group' }, options))), this._optionsElement);}}exports.default = DropdownResults;DropdownResults.defaultProps = { displayOnFocus: false, errorMessage: null, isLoading: false, loadingMessage: 'Loading Results' };