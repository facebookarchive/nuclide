...
class Activation {
  ...
  _registerPythonDebugProvider() {
    this._registerDebugProvider({
      ...
      getLaunchAttachProvider: connection => {
        return new (_AutoGenLaunchAttachProvider || _load_AutoGenLaunchAttachProvider()).default('Python', connection, (0, (_utils || _load_utils()).getPythonAutoGenConfig)());
      }
    });
  }
  ...
  _registerNodeDebugProvider() {
    this._registerDebugProvider({
      name: 'Node',
      getLaunchAttachProvider: connection => {
        return new (_AutoGenLaunchAttachProvider || _load_AutoGenLaunchAttachProvider()).default('Node', connection, (0, (_utils || _load_utils()).getNodeAutoGenConfig)());
      }
    });
  }
    ...
    return (0, _asyncToGenerator.default)(function* () {
      if (yield (0, (_passesGK || _load_passesGK()).default)('nuclide_debugger_ocaml')) {
        _this3._registerDebugProvider({
          name: 'OCaml',
          getLaunchAttachProvider: function (connection) {
            return new (_AutoGenLaunchAttachProvider || _load_AutoGenLaunchAttachProvider()).default('OCaml', connection, (0, (_utils || _load_utils()).getOCamlAutoGenConfig)());
          }
        });
      }
    })();
