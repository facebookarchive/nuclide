...
class LspLanguageService {
  ...
  async getDefinition(fileVersion, position) {
    ...
    return {
      ...
      definitions: responseArray.map(d => (_convert || _load_convert()).lspLocationWithTitle_atomDefinition(d, this._projectRoot))
    };
  }
  ...
  async getOutline(fileVersion) {
    ...
    const list = response.map(symbol => [symbol, {
      ...
      endPosition: (_convert || _load_convert()).lspPosition_atomPoint(symbol.location.range.end),
      children: []
    }]);
    ...
    if (this._lspPreferences.reconstructOutlineStrategy === 'containerName') {
      ...
      for (const [, node] of list) {
        while (spine.length > 1) {
          ...
          const nodeRange = new (_simpleTextBuffer || _load_simpleTextBuffer()).Range(node.startPosition, node.endPosition);

          if (!(candidate.endPosition != null)) {
            throw new Error('Invariant violation: "candidate.endPosition != null"');
          }

          const candidateRange = new (_simpleTextBuffer || _load_simpleTextBuffer()).Range(candidate.startPosition, candidate.endPosition);
