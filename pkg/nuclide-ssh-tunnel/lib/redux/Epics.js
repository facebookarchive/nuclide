...
function subscribeToTunnelEpic(actions, store) {
  return actions.ofType((_Actions || _load_Actions()).SUBSCRIBE_TO_TUNNEL).mergeMap((() => {
...
function unsubscribeFromTunnelEpic(actions, store) {
  return actions.ofType((_Actions || _load_Actions()).UNSUBSCRIBE_FROM_TUNNEL).mergeMap((() => {
...
function requestTunnelEpic(actions, store) {
  return actions.ofType((_Actions || _load_Actions()).REQUEST_TUNNEL).mergeMap((() => {
    var _ref3 = (0, _asyncToGenerator.default)(function* (action) {
        ...
        subscription = events.refCount().subscribe({
          next: function (event) {
            if (event.type === 'server_started') {
              ...
              const friendlyString = `${(0, (_Tunnel || _load_Tunnel()).tunnelDescription)(tunnel)} (${activeTunnel.subscriptions.map(function (s) {
