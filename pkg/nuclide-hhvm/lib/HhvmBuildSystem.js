...
class HhvmBuildSystem {
  ...
  setProjectRoot(projectRoot, callback) {
    ...
    const getTask = disabled => [{
      type: 'debug',
      label: 'Debug',
      description: disabled ? 'The HHVM debugger is already attached to this server' : this._projectStore.getDebugMode() === 'webserver' ? 'Attach HHVM debugger to webserver' : 'Debug Hack/PHP Script',
    ...
    const tasksObservable = _RxMin.Observable.concat(_RxMin.Observable.of(null), _RxMin.Observable.fromPromise((0, _debugger().getDebuggerService)())).switchMap(debugService => {
      if (debugService == null) {
        return _RxMin.Observable.of(getTask(false));
      }

      return _RxMin.Observable.concat(_RxMin.Observable.of(getTask(false)), _RxMin.Observable.merge((0, _event().observableFromSubscribeFunction)(debugService.onDidChangeDebuggerSessions.bind(debugService)), (0, _event().observableFromSubscribeFunction)(this._projectStore.onChange.bind(this._projectStore))).switchMap(() => {
        const disabled = this._projectStore.getDebugMode() === 'webserver' && debugService.getDebugSessions().some(c => c.adapterType === _nuclideDebuggerCommon().VsAdapterTypes.HHVM && c.targetUri === projectRoot);
        return _RxMin.Observable.of(getTask(disabled));
      }));
    });
