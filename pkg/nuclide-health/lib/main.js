...
let Activation = class Activation {
  ...
  constructor(state) {
    ...
    const packageStates = statsStream.withLatestFrom(toolbarJewels).map((_ref) => {
      var _ref2 = _slicedToArray(_ref, 2);

      let stats = _ref2[0];
      let toolbarJewel = _ref2[1];
    ...
    this._paneItemStates = _rxjsBundlesRxMinJs.Observable.combineLatest(packageStates, _rxjsBundlesRxMinJs.Observable.of(null).concat(childProcessesTreeStream), (packageState, childProcessesTree) => _extends({}, packageState, {
...
function formatToolbarJewelLabel(opts) {
  ...
  const toolbarJewel = opts.toolbarJewel;
