...
class Activation {
  ...
  constructor(state) {
    ...
    this._subscriptions = new (_UniversalDisposable || _load_UniversalDisposable()).default(_rxjsBundlesRxMinJs.Observable.zip(statsStream.buffer(analyticsInterval), analyticsInterval.switchMap((_getDOMCounters || _load_getDOMCounters()).default), analyticsInterval.switchMap(() => (0, (_getChildProcesses || _load_getChildProcesses()).queryPs)('comm').map((_getChildProcesses || _load_getChildProcesses()).childProcessSummary))).subscribe(([buffer, domCounters, processes]) => {
