...
trackStalls;var _nullthrows;function _load_nullthrows() {return _nullthrows = _interopRequireDefault(require('nullthrows'));}var _electron = require('electron');var _rxjsBundlesRxMinJs = require('rxjs/bundles/Rx.min.js');var _observableDom;function _load_observableDom() {return _observableDom = require('../../../modules/nuclide-commons-ui/observable-dom');}var _UniversalDisposable;function _load_UniversalDisposable() {return _UniversalDisposable = _interopRequireDefault(require('../../../modules/nuclide-commons/UniversalDisposable'));}var _nuclideAnalytics;function _load_nuclideAnalytics() {return _nuclideAnalytics = require('../../nuclide-analytics');}function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}if (!(_electron.remote != null)) {throw new Error('Invariant violation: "remote != null"');} /**
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                * 
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                * @format
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                */ /* eslint-env browser */const CHROME_VERSION = Number((0, (_nullthrows || _load_nullthrows()).default)(process.versions.chrome).split('.')[0]); // The startup period naturally causes many event loop blockages.
// Don't start checking blockages until some time has passed.
const BLOCKED_GRACE_PERIOD = 30000; // Report all blockages over this threshold.
...
const BLOCKED_RANGE_PADDING = 15;function trackStalls() {const trackStart = performance.now();const browserWindow = _electron.remote.getCurrentWindow();const histogram = new (_nuclideAnalytics || _load_nuclideAnalytics()).HistogramTracker('event-loop-blocked', /* max */1000, /* buckets */10);let intentionalBlockTime = 0;const onIntentionalBlock = () => {intentionalBlockTime = performance.now();};const blockedEvents = new (_observableDom || _load_observableDom()).PerformanceObservable({ entryTypes: ['longtask'] }).flattenEntries() // only count if the window is focused when the task ran long
  ...
  .takeUntil(_rxjsBundlesRxMinJs.Observable.fromEvent(browserWindow, 'close')).
  subscribe(onIntentionalBlock),
  _rxjsBundlesRxMinJs.Observable.merge(
  // kick off subscription with a one-time query on start
  _rxjsBundlesRxMinJs.Observable.of(document.hasFocus()),
  _rxjsBundlesRxMinJs.Observable.fromEvent(browserWindow, 'focus').mapTo(true),
  _rxjsBundlesRxMinJs.Observable.fromEvent(browserWindow, 'blur').mapTo(false)).

  distinctUntilChanged().
  switchMap(isFocused => isFocused ? blockedEvents : _rxjsBundlesRxMinJs.Observable.empty()).
  subscribe(entry => {
