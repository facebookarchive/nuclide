...
function trackStalls() {
  ...
  .takeUntil(_rxjsBundlesRxMinJs.Observable.fromEvent(browserWindow, 'close')).subscribe(onIntentionalBlock), _rxjsBundlesRxMinJs.Observable.merge(
  // kick off subscription with a one-time query on start
  _rxjsBundlesRxMinJs.Observable.of(document.hasFocus()), _rxjsBundlesRxMinJs.Observable.fromEvent(browserWindow, 'focus').mapTo(true), _rxjsBundlesRxMinJs.Observable.fromEvent(browserWindow, 'blur').mapTo(false)).distinctUntilChanged().switchMap(isFocused => isFocused ? blockedEvents : _rxjsBundlesRxMinJs.Observable.empty()).subscribe(entry => {
