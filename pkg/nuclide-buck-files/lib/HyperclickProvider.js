...
var findTargetLocation = _asyncToGenerator(function* (target) {
  var data = undefined;
  try {
    data = yield _nuclideCommons.fsPromise.readFile(target.path, 'utf-8');
  } catch (e) {
    return null;
  }
...
var findBuildTarget = _asyncToGenerator(function* (textEditor, position, absolutePath, buckProject) {
  var wordMatchAndRange = (0, _nuclideAtomHelpers.extractWordAtPosition)(textEditor, position, TARGET_REGEX);
...
var findRelativeFilePath = _asyncToGenerator(function* (textEditor, position, directory) {
  var wordMatchAndRange = (0, _nuclideAtomHelpers.extractWordAtPosition)(textEditor, position, RELATIVE_FILE_PATH_REGEX);
  if (!wordMatchAndRange) {
    return null;
  }
  var wordMatch = wordMatchAndRange.wordMatch;
  ...
  var potentialPath = _path2.default.join(directory, wordMatch[2]);
  var stat = undefined;
  try {
    stat = yield _nuclideCommons.fsPromise.stat(potentialPath);
  } catch (e) {
    return null;
  }
...
module.exports = {
  ...
  getSuggestion: _asyncToGenerator(function* (textEditor, position) {
    ...
    var buckProject = yield (0, _nuclideBuckCommons.buckProjectRootForPath)(absolutePath);
    if (!buckProject) {
      return null;
    }

    var results = yield Promise.all([findBuildTarget(textEditor, position, absolutePath, buckProject), findRelativeFilePath(textEditor, position, _path2.default.dirname(absolutePath))]);
        ...
        return {
          v: {
            ...
            callback: function callback() {
              (0, _nuclideAtomHelpers.goToLocation)(match.path, match.line, match.column);
            }
          }
        };
      })();
