...
var _nuclideOpenFilesCommon2;

function _nuclideOpenFilesCommon() {
  return _nuclideOpenFilesCommon2 = require('../../nuclide-open-files-common');
}
...
var BufferSubscription = (function () {
  function BufferSubscription(notifiers, buffer) {
    ...
    subscriptions.add(buffer.onDidChange(function (event) {
      if (_this._notifier != null) {
        _this.sendEvent({
          kind: 'edit',
          fileVersion: (0, (_nuclideOpenFilesCommon2 || _nuclideOpenFilesCommon()).getFileVersionOfBuffer)(buffer),
          oldRange: (0, (_nuclideOpenFilesCommon2 || _nuclideOpenFilesCommon()).convertRange)(event.oldRange),
          newRange: (0, (_nuclideOpenFilesCommon2 || _nuclideOpenFilesCommon()).convertRange)(event.newRange),
  ...
  _createClass(BufferSubscription, [{
    ...
    value: _asyncToGenerator(function* (event) {
      ...
      if (this._notifier != null) {
        try {
          yield (yield this._notifier).onEvent(event);
      ...
      if (resyncVersion > this._lastAttemptedSync) {
        (function () {
          ...
          var sendResync = _asyncToGenerator(function* () {
            ...
            } else if (_this2._notifier == null) {
              logger.error('Resync preempted by remote connection closed, or file rename');
            } else if (resyncVersion !== _this2._lastAttemptedSync || resyncVersion !== _this2._buffer.changeCount) {
              logger.error('Resync preempted by later edit or resync');
            } else {
              var syncEvent = {
                kind: 'sync',
                fileVersion: (0, (_nuclideOpenFilesCommon2 || _nuclideOpenFilesCommon()).getFileVersionOfBuffer)(_this2._buffer),
                contents: _this2._buffer.getText()
              };
              try {
                (0, (_assert2 || _assert()).default)(_this2._notifier != null);
    ...
    key: 'sendOpen',
    value: function sendOpen() {
      if (this._notifier != null) {
        this.sendEvent({
          kind: 'open',
          fileVersion: (0, (_nuclideOpenFilesCommon2 || _nuclideOpenFilesCommon()).getFileVersionOfBuffer)(this._buffer),
