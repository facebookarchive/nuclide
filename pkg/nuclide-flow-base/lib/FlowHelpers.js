...
var canFindFlow = _asyncToGenerator(function* (flowPath) {
  try {
    yield (0, _nuclideCommons.asyncExecute)('which', [flowPath]);
    return true;
  } catch (e) {
    return false;
  }
...
function findFlowConfigDir(localFile) {
  if (!flowConfigDirCache.has(localFile)) {
    var flowConfigDir = _nuclideCommons.fsPromise.findNearestFile('.flowconfig', _path2.default.dirname(localFile));
