...
var getDevices = (0, (_lodashMemoize || _load_lodashMemoize()).default)(function () {
  return (0, (_commonsNodeProcess || _load_commonsNodeProcess()).observeProcess)(function () {
    return (0, (_commonsNodeProcess || _load_commonsNodeProcess()).safeSpawn)('xcrun', ['simctl', 'list', 'devices']);
  }).map(function (event) {
    // Throw errors.
    if (event.kind === 'error') {
      var error = new Error();
    ...
    return(
      ...
      error.name === 'XcrunError' ? (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.of([]) : (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.throw(error)
    );
  }).share();
});
