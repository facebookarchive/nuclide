...
var ActiveEditorRegistry = (function () {
  function ActiveEditorRegistry(resultFunction) {
    ...
    this._newProviderEvents = new (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Subject();
  ...
  _createClass(ActiveEditorRegistry, [{
    ...
    value: function _createResultsStream(eventSources) {
      ...
      var repeatedEditors = eventSources.activeEditors.switchMap(function (editor) {
        if (editor == null) {
          return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.of(editor);
        }
        return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.concat((_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.of(editor), _this2._newProviderEvents.mapTo(editor));
      });
      var results = repeatedEditors.switchMap(function (editorArg) {
        ...
        (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.of({
          kind: 'pane-change',
          editor: editor
        }), (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.fromPromise(_this2._getResultForEditor(_this2._getProviderForEditor(editor), editor)), _this2._resultsForEditor(editor, eventSources));
      });
      return (0, (_commonsNodeObservable || _load_commonsNodeObservable()).cacheWhileSubscribed)(results);
    }
  }, {
    ...
    value: function _resultsForEditor(editor, eventSources) {
      ...
      return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.merge(eventSources.changesForEditor(editor).map(function () {
        return 'edit';
        ...
        (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.of({ kind: event, editor: editor }), (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.fromPromise(_this3._getResultForEditor(provider, editor)));
      });
    }
  }, {
