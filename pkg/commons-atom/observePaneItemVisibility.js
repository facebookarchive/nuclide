...
export default function observePaneItemVisibility(item: Object): Observable<boolean> {
...
const observeActiveItems = memoize(() => {
  ...
  const itemActivations = Observable.merge(
    ...
    ...atom.workspace.getPaneContainers().map(paneContainer => {
      ...
      return observableFromSubscribeFunction(observePanes).flatMap(pane => {
        const paneDestroyed = observableFromSubscribeFunction(pane.onDidDestroy.bind(pane));
        const activeItems = observableFromSubscribeFunction(pane.observeActiveItem.bind(pane))
  ...
  const panesToActiveItem = itemActivations.scan(
    (acc, {pane, item}) => {
      if (item == null) {
        acc.delete(pane);
      } else {
