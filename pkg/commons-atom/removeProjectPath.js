...
function _load_nuclideUri() {return _nuclideUri = _interopRequireDefault(require('../../modules/nuclide-commons/nuclideUri'));}var _log4js;
...
const logger = (0, (_log4js || _load_log4js()).getLogger)('commons-atom'); /**
                                                                            ...
                                                                            * 
                                                                            * @format
                                                                            */exports.default = (() => {var _ref = (0, _asyncToGenerator.default)(function* (projectPath) {logger.info(`Removing project path ${projectPath}`); // close all the files associated with the project before closing
    const projectEditors = atom.workspace.getTextEditors();for (const editor of projectEditors) {const path = editor.getPath();
      ...
      if (
      path != null &&
      path.startsWith((_nuclideUri || _load_nuclideUri()).default.ensureTrailingSeparator(projectPath)) &&
      atom.project.
      getPaths().
      filter(function (root) {return (
          path.startsWith((_nuclideUri || _load_nuclideUri()).default.ensureTrailingSeparator(root)));}).
      length === 1)
      {
        // eslint-disable-next-line no-await-in-loop
        const didDestroy = yield atom.workspace.
        paneForURI(path).
        destroyItem(editor);
  ...
  });function removeProjectPath(_x) {return _ref.apply(this, arguments);}return removeProjectPath;})();