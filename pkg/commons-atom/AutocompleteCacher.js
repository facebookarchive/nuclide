...
function track(request, responsePromise) {
  return responsePromise.then(response => response == null ? null : { request, response });
}
...
class AutocompleteCacher {
  ...
  async _setEnabled() {
    ...
    if (gk == null) {
      ...
      this._enabled = false;
      this._enabled = await (0, (_passesGK || _load_passesGK()).default)(gk);
    }
  }
  ...
  getSuggestions(request) {
    ...
    if (session != null && this._canMaybeFilterResults(session, request)) {
      ...
      this._session = {
        firstResultPromise: new (_promise || _load_promise()).PromiseWithState(getNewFirstResult(session.firstResultPromise.getPromise(), track(request, resultFromLanguageService))),
        lastRequest: request
      };
      return result;
    } else {
      ...
      this._session = {
        firstResultPromise: new (_promise || _load_promise()).PromiseWithState(track(request, result)),
