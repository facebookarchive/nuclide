...
var rangeMatchers = {
  ...
  toEqualAtomRanges: function toEqualAtomRanges(expected) {
    ...
    this.actual.some(function (range, index) {
      (0, (_assert || _load_assert()).default)(expected); // Tell Flow this is definitely non-null now.
...
function waitsForFile(filename) {
  ...
  waitsFor(filename + ' to become active', timeoutMs, function () {
    ...
    return (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.basename(editorPath) === filename;
  });
}
...
function waitsForFilePosition(filename, row, column) {
  ...
  waitsFor(filename + ' to become active at ' + row + ':' + column, timeoutMs, function () {
    ...
    return (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.basename(editorPath) === filename && pos.row === row && pos.column === column;
  });
}
...
function getMountedReactRootNames() {
  ...
  (0, (_assert || _load_assert()).default)(ReactComponentTreeHookPath != null, 'ReactComponentTreeHook could not be found in the module cache.');
