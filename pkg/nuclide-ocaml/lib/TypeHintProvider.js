...
var TypeHintProvider = (function () {
  ...
  _createDecoratedClass(TypeHintProvider, [{
    key: 'typeHint',
    decorators: [(0, (_nuclideAnalytics || _load_nuclideAnalytics()).trackTiming)('nuclide-ocaml.typeHint')],
    value: _asyncToGenerator(function* (editor, position) {
      ...
      return {
        hint: hint,
        range: new (_atom || _load_atom()).Range(new (_atom || _load_atom()).Point(type.start.line - 1, type.start.col), new (_atom || _load_atom()).Point(type.end.line - 1, type.end.col))
      };
    })
  }]);
