...
let getCodeLens = (() => {
  var _ref = (0, _asyncToGenerator.default)(function* (languageService, fileVersion) {
    ...
    for (let i = 0; i < RETRIES; i++) {
      ...
      const codeLens = yield languageService.getCodeLens(fileVersion);
      if (codeLens != null) {
        return codeLens;
      }
  ...
  return function getCodeLens(_x, _x2) {
...
function resolveVisible() {
  for (const [editor, resolveInfo] of allEditors.entries()) {
    ...
    // comparisons.
    //  -wipi
    const resolvableLenses = resolveInfo.lenses.filter(lensInfo => !lensInfo.resolved && lensInfo.lens.range.start.row >= firstLine && lensInfo.lens.range.start.row <= lastLine);
    resolvableLenses.forEach((() => {
      var _ref2 = (0, _asyncToGenerator.default)(function* (lensInfo) {
        // Set this *before* we get the data so we don't send duplicate
        // requests.
        lensInfo.resolved = true;
        const lens = yield resolveInfo.languageService.resolveCodeLens(resolveInfo.fileVersion.filePath, lensInfo.lens);

        const currentInfo = allEditors.get(editor);
        if (currentInfo == null || currentInfo.fileVersion.version !== resolveInfo.fileVersion.version) {
...
function observeForCodeLens(atomLanguageService, logger) {
  ...
  disposable.add((0, (_textEditor || _load_textEditor()).observeTextEditors)((() => {
    var _ref3 = (0, _asyncToGenerator.default)(function* (editor) {
      const editorDisposable = new (_UniversalDisposable || _load_UniversalDisposable()).default();
      editorDisposable.add(editor.onDidDestroy(function () {
      ...
      const updateCodeLens = (() => {
        var _ref4 = (0, _asyncToGenerator.default)(function* () {
          ...
          const languageService = yield atomLanguageService.getLanguageServiceForUri(uri);
          if (languageService == null) {
            logger.warn(`Could not find language service for ${JSON.stringify(uri)}.`);
            return null;
          }

          const fileVersion = yield (0, (_nuclideOpenFiles || _load_nuclideOpenFiles()).getFileVersionOfEditor)(editor);
          if (fileVersion == null) {
            logger.warn(`Could not find file version for ${JSON.stringify(uri)}.`);
            return null;
          }
          ...
          if (codeLens != null) {
            ...
            const lenses = codeLens.map(function (lens) {
              const marker = markerLayer.markBufferPosition([lens.range.start.row, lens.range.start.column]);
              ...
              elementsDisposable.add(atom.tooltips.add(element, {
        ...
        return function updateCodeLens() {
