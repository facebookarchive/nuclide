...
var LinkTreeManager = (function () {
  ...
  _createClass(LinkTreeManager, [{
    ...
    value: _asyncToGenerator(function* (src, basePath, kind) {
      ...
      var realBasePath = yield (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.realpath(basePath);
      var realSrcPath = yield (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.realpath(src);

      var currPath = (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.dirname(realSrcPath);

      while ((_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.contains(realBasePath, currPath)) {
        var relativePath = (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.relative(realBasePath, currPath);
        ...
        try {
          ...
          var results = yield (_nuclideBuckRpc || _load_nuclideBuckRpc()).query(basePath, 'kind(' + kind + ', rdeps(' + searchRoot + ', owner(' + src + ')))');
          if (results.length > 0) {
            return results;
          }
        } catch (e) {
        ...
        currPath = (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.dirname(currPath);
      }
        ...
        var _ret = yield* (function* () {
          ...
          return {
            v: bins.map(function (bin) {
              ...
              return (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.join(buckRoot, BUCK_GEN_PATH, binPath + linkTreeSuffix);
            })
          };
        })();
