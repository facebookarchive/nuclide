...
var HyperclickProviderHelpers = (function () {
  ...
  _createClass(HyperclickProviderHelpers, null, [{
    ...
    value: _asyncToGenerator(function* (textEditor, text, range) {
      ...
      var match = (_urlregexp || _load_urlregexp()).default.exec(text);
      if (match == null) {
        return null;
      }
      ...
      var urlRange = new (_atom || _load_atom()).Range([range.start.row, range.start.column + index], [range.end.row, range.start.column + index + matchLength]);
      ...
      return {
        ...
        callback: function callback() {
          ...
          (_electron || _load_electron()).shell.openExternal(validUrl);
        }
      };
    })
  }]);
