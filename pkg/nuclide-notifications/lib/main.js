...
function _sanitize() {
  const data = _interopRequireDefault(require("./sanitize"));
...
function proxyToNativeNotification(notification) {
  ...
  const sanitizedMessage = (0, _sanitize().default)(notification.getMessage()); // If the message is multiline, take the first line for the title. Titles can only be a single
  // line and anything after the first line break will be ignored, at least on OSX.

  const [title, ...body] = sanitizedMessage.split(/\n/g);
  const sanitizedDescription = options.description == null ? '' : (0, _sanitize().default)(options.description);
