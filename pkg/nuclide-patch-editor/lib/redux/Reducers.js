    ...
    case (_ActionTypes || _load_ActionTypes()).CLICK_CHECKBOX_ACTION:
      {
        ...
        if (!(hunkOldStartLine != null || line == null)) {
          throw new Error('Invariant violation: "hunkOldStartLine != null || line == null"');
        }
...
function updatePatchData(patchData, fileName, hunkOldStartLine, line) {
  const files = new Map(patchData.files);
  const oldFile = (0, (_nullthrows || _load_nullthrows()).default)(files.get(fileName));
...
function updateFileData(fileData, hunkOldStartLine, line) {
  ...
  if (hunkOldStartLine != null) {
    ...
    const oldHunk = (0, (_nullthrows || _load_nullthrows()).default)(chunks.get(hunkOldStartLine));
    ...
    if (fileData.chunks != null) {
      ...
      chunks = new Map();
      fileData.chunks.forEach((hunkData, oldStartLine) => chunks.set(oldStartLine, selectWholeHunk(hunkData, isEnabling)));
