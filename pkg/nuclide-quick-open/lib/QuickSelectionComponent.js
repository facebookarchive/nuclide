...
class QuickSelectionComponent extends _reactForAtom.React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      activeTab: this._searchResultManager.getProviderByName(this._searchResultManager.getActiveProviderName()),
  ...
  componentDidMount() {
    ...
    this._subscriptions.add(this._searchResultManager.on(this._searchResultManager.PROVIDERS_CHANGED, this.handleProvidersChange), this._searchResultManager.on(this._searchResultManager.RESULTS_CHANGED, (0, (_debounce || _load_debounce()).default)(this.handleResultsChange, RESULTS_CHANGED_DEBOUNCE_DELAY, false)));
  ...
  _updateResults(activeProviderName) {
    const updatedResults = this._searchResultManager.getResults(this.refs.queryInput.getText(), activeProviderName);
  ...
  componentForItem(item, serviceName, dirName) {
    return this._searchResultManager.getRendererForProvider(serviceName)(item, serviceName, dirName);
  }
