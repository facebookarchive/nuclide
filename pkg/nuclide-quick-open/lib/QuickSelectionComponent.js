...
var _rxjsBundlesRxMinJs = require('rxjs/bundles/Rx.min.js');var _AtomInput;
function _load_AtomInput() {return _AtomInput = require('../../../modules/nuclide-commons-ui/AtomInput');}var _Button;
function _load_Button() {return _Button = require('../../../modules/nuclide-commons-ui/Button');}var _Icon;
function _load_Icon() {return _Icon = require('../../../modules/nuclide-commons-ui/Icon');}var _scrollIntoView;
function _load_scrollIntoView() {return _scrollIntoView = require('../../../modules/nuclide-commons-ui/scrollIntoView');}var _Tabs;
function _load_Tabs() {return _Tabs = _interopRequireDefault(require('../../../modules/nuclide-commons-ui/Tabs'));}var _Badge;
function _load_Badge() {return _Badge = require('../../nuclide-ui/Badge');}var _UniversalDisposable;
...
function _load_event() {return _event = require('../../../modules/nuclide-commons/event');}var _humanizeKeystroke;
function _load_humanizeKeystroke() {return _humanizeKeystroke = _interopRequireDefault(require('../../../modules/nuclide-commons/humanizeKeystroke'));}var _observable;
...
function _load_classnames() {return _classnames = _interopRequireDefault(require('classnames'));}var _nuclideUri;
function _load_nuclideUri() {return _nuclideUri = _interopRequireDefault(require('../../../modules/nuclide-commons/nuclideUri'));}var _searchResultHelpers;
function _load_searchResultHelpers() {return _searchResultHelpers = require('./searchResultHelpers');}function _interopRequireWildcard(obj) {if (obj && obj.__esModule) {return obj;} else {var newObj = {};if (obj != null) {for (var key in obj) {if (Object.prototype.hasOwnProperty.call(obj, key)) newObj[key] = obj[key];}}newObj.default = obj;return newObj;}}function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}
...

{
  ...
  constructor(props) {
    ...
    _handleDocumentMouseDown = event => {
      ...
      !(0, (_nullthrows || _load_nullthrows()).default)(this._modal).contains(event.target))
      {
    ...
    modalNode, 'pane:show-next-item', this._handleMoveNextTab), atom.commands.add('body', 'core:cancel', () => {this.props.onCancellation();}), _rxjsBundlesRxMinJs.Observable.fromEvent(document, 'mousedown').subscribe(this._handleDocumentMouseDown), // The text editor often changes during dispatches, so wait until the next tick.
    (0, (_event || _load_event()).observableFromSubscribeFunction)(cb => (0, (_nullthrows || _load_nullthrows()).default)(this._queryInput).onDidChange(cb)).startWith(null).let((0, (_observable || _load_observable()).throttle)((_observable || _load_observable()).microtask, { leading: false })).subscribe(this._handleTextInputChange), (0, (_event || _load_event()).observableFromSubscribeFunction)(cb => this.props.searchResultManager.onProvidersChanged(cb)).debounceTime(0, _rxjsBundlesRxMinJs.Scheduler.animationFrame).subscribe(this._handleProvidersChange), (0, (_event || _load_event()).observableFromSubscribeFunction)(cb => this.props.searchResultManager.onResultsChanged(cb)).let((0, (_observable || _load_observable()).fastDebounce)(50)) // debounceTime seems to have issues canceling scheduled work. So
    // schedule it after we've debounced the events. See
    // https://github.com/ReactiveX/rxjs/pull/2135
    .debounceTime(0, _rxjsBundlesRxMinJs.Scheduler.animationFrame).subscribe(this._handleResultsChange));this._getTextEditor().selectAll();}componentWillUnmount() {this._subscriptions.dispose();}_updateResults() {const activeProviderName = this.props.searchResultManager.getActiveProviderName();const updatedResults = this.props.searchResultManager.getResults((0, (_nullthrows || _load_nullthrows()).default)(this._queryInput).getText(), activeProviderName);const [topProviderName] = Object.keys(updatedResults);const renderableProviders = this.props.searchResultManager.getRenderableProviders();this.setState({ renderableProviders, resultsByService: updatedResults }, () => {if (!this.state.hasUserSelection && topProviderName != null && this.state.resultsByService[topProviderName] != null) {const topProviderResults = this.state.resultsByService[topProviderName].results;if (!Object.keys(topProviderResults).some(dirName => topProviderResults[dirName].loading)) {this._moveSelectionToTop( /* userInitiated */false);}}});}_getCurrentResultContext() {const nonEmptyResults = (0, (_searchResultHelpers || _load_searchResultHelpers()).filterEmptyResults)(this.state.resultsByService);const currentService = nonEmptyResults[this.state.selectedService];if (!currentService) {return null;}const serviceNames = Object.keys(nonEmptyResults);const currentServiceIndex = serviceNames.indexOf(this.state.selectedService);const directoryNames = Object.keys(currentService.results);const currentDirectoryIndex = directoryNames.indexOf(this.state.selectedDirectory);const currentDirectory = currentService.results[this.state.selectedDirectory];if (!currentDirectory || !currentDirectory.results) {return null;}return { nonEmptyResults, serviceNames, currentServiceIndex, currentService, directoryNames, currentDirectoryIndex, currentDirectory };}_moveSelectionDown(userInitiated) {const context = this._getCurrentResultContext();if (!context) {this._moveSelectionToTop(userInitiated);return;}if (this.state.selectedItemIndex < context.currentDirectory.results.length - 1) {// only bump the index if remaining in current directory
      this._setSelectedIndex(this.state.selectedService, this.state.selectedDirectory, this.state.selectedItemIndex + 1, userInitiated);} else {// otherwise go to next directory...
      if (context.currentDirectoryIndex < context.directoryNames.length - 1) {this._setSelectedIndex(this.state.selectedService, context.directoryNames[context.currentDirectoryIndex + 1], 0, userInitiated);} else {// ...or the next service...
        if (context.currentServiceIndex < context.serviceNames.length - 1) {const newServiceName = context.serviceNames[context.currentServiceIndex + 1];const newDirectoryName = Object.keys(context.nonEmptyResults[newServiceName].results).shift();this._setSelectedIndex(newServiceName, newDirectoryName, 0, userInitiated);} else {// ...or wrap around to the very top
          this._moveSelectionToTop(userInitiated);}}}}_moveSelectionUp(userInitiated) {const context = this._getCurrentResultContext();if (!context) {this._moveSelectionToBottom(userInitiated);return;}if (this.state.selectedItemIndex > 0) {// only decrease the index if remaining in current directory
      this._setSelectedIndex(this.state.selectedService, this.state.selectedDirectory, this.state.selectedItemIndex - 1, userInitiated);} else {// otherwise, go to the previous directory...
      if (context.currentDirectoryIndex > 0) {this._setSelectedIndex(this.state.selectedService, context.directoryNames[context.currentDirectoryIndex - 1], context.currentService.results[context.directoryNames[context.currentDirectoryIndex - 1]].results.length - 1, userInitiated);} else {// ...or the previous service...
        if (context.currentServiceIndex > 0) {const newServiceName = context.serviceNames[context.currentServiceIndex - 1];const newDirectoryName = Object.keys(context.nonEmptyResults[newServiceName].results).pop();if (newDirectoryName == null) {return;}const resultsForDirectory = context.nonEmptyResults[newServiceName].results[newDirectoryName];if (resultsForDirectory == null || resultsForDirectory.results == null) {return;}this._setSelectedIndex(newServiceName, newDirectoryName, resultsForDirectory.results.length - 1, userInitiated);} else {// ...or wrap around to the very bottom
          this._moveSelectionToBottom(userInitiated);}}}} // Update the scroll position of the list view to ensure the selected item is visible.
  _updateScrollPosition() {if (this._selectionList == null) {return;}const listNode = (0, (_nullthrows || _load_nullthrows()).default)(this._selectionList);const selectedNode = listNode.getElementsByClassName('selected')[0]; // false is passed for @centerIfNeeded parameter, which defaults to true.
    // Passing false causes the minimum necessary scroll to occur, so the selection sticks to the
    // top/bottom.
    if (selectedNode) {(0, (_scrollIntoView || _load_scrollIntoView()).scrollIntoViewIfNeeded)(selectedNode, false);}}_moveSelectionToBottom(userInitiated) {const bottom = (0, (_searchResultHelpers || _load_searchResultHelpers()).getOuterResults)('bottom', this.state.resultsByService);if (!bottom) {return;}this._setSelectedIndex(bottom.serviceName, bottom.directoryName, bottom.results.length - 1, userInitiated);}_moveSelectionToTop(userInitiated) {const top = (0, (_searchResultHelpers || _load_searchResultHelpers()).getOuterResults)('top', this.state.resultsByService);if (!top) {return;}this._setSelectedIndex(top.serviceName, top.directoryName, 0, userInitiated);}_getItemAtIndex(serviceName, directory, itemIndex) {if (itemIndex === -1 || !this.state.resultsByService[serviceName] || !this.state.resultsByService[serviceName].results[directory] || !this.state.resultsByService[serviceName].results[directory].results[itemIndex]) {return null;}return this.state.resultsByService[serviceName].results[directory].results[itemIndex];}_componentForItem(item, serviceName, dirName) {if (item.resultType === 'FILE') {item;return this.props.searchResultManager.getRendererForProvider(serviceName, item)(item, serviceName, dirName);}return this.props.searchResultManager.getRendererForProvider(serviceName, item)(item, serviceName, dirName);}_getSelectedIndex() {return { selectedDirectory: this.state.selectedDirectory, selectedService: this.state.selectedService, selectedItemIndex: this.state.selectedItemIndex };}_setSelectedIndex(service, directory, itemIndex, userInitiated) {const newState = { selectedService: service, selectedDirectory: directory, selectedItemIndex: itemIndex, hasUserSelection: userInitiated };this.setState(newState, () => {const selectedIndex = this._getSelectedIndex();const providerName = this.props.searchResultManager.getActiveProviderName();const query = this._getTextEditor().getText();if (this.props.onSelectionChanged != null) {this.props.onSelectionChanged(selectedIndex, providerName, query);}});}_getInputTextEditor() {if (this._queryInput != null) {return this._queryInput.getTextEditor().getElement();}return null;}_getTextEditor() {return (0, (_nullthrows || _load_nullthrows()).default)(this._queryInput).getTextEditor();} /**
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            */_renderTabs() {const workspace = atom.views.getView(atom.workspace);const tabs = this.state.renderableProviders.map(tab => {let keyBinding = null; // TODO
      const humanizedKeybinding = tab.action ? _findKeybindingForAction(tab.action, workspace) : '';if (humanizedKeybinding !== '') {keyBinding = _react.createElement('kbd', { className: 'key-binding' }, humanizedKeybinding);}return { name: tab.name, tabContent: _react.createElement('span', null, tab.title, keyBinding) };});return _react.createElement('div', { className: 'omnisearch-tabs' }, _react.createElement((_Tabs || _load_Tabs()).default, { tabs: tabs, activeTabName: this.state.activeTab.name, onActiveTabChange: this._handleTabChange }));}_openAll() {const selections = (0, (_searchResultHelpers || _load_searchResultHelpers()).flattenResults)(this.state.resultsByService);
    const providerName = this.props.searchResultManager.getActiveProviderName();
    const query = this._getTextEditor().getText();
  ...
  render() {
    ...
    serviceName => {
      ...
      const directoriesForService = directoryNames.map(dirName => {
        ...
        if (resultsForDirectory.loading) {
          ...
          if (!isOmniSearchActive) {
            ...
            _react.createElement('span', null,
              _react.createElement('span', { className: 'loading loading-spinner-tiny inline-block' }), 'Loading...');
          ...
          message =
          _react.createElement('span', null,
            _react.createElement('span', { className: 'icon icon-circle-slash' }), 'Error: ',
            _react.createElement('pre', null, resultsForDirectory.error));
        ...
        !isOmniSearchActive)
        {
          message =
          _react.createElement('span', null,
            _react.createElement('span', { className: 'icon icon-x' }), 'No results');
        ...
        if (showDirectories) {
          directoryLabel =
          _react.createElement('div', { className: 'list-item' },
            _react.createElement('span', { className: 'icon icon-file-directory' },
        ...
        return (
          _react.createElement('li', {
            ...
            directoryLabel,
            message,
            _react.createElement('ul', { className: 'list-tree' }, itemComponents)));
      ...
      if (isOmniSearchActive && numResultsForService > 0) {
        ...
        return (
          _react.createElement('li', { className: 'list-nested-item', key: serviceName },
            serviceLabel,
            _react.createElement('ul', { className: 'list-tree' }, directoriesForService)));
    ...
    return (
      _react.createElement('div', {
        ...
        _react.createElement('div', { className: 'omnisearch-search-bar' },
          ...
          _react.createElement((_Button || _load_Button()).Button, {
              ...
              disabled: disableOpenAll }, 'Open All')),



        this._renderTabs(),
        _react.createElement('div', { className: 'omnisearch-results' },
          _react.createElement('div', { className: 'omnisearch-pane' },
