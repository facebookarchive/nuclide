...
class DebuggerThreadsComponent extends _react.Component {
  ...
  constructor(props) {
    ...
    this._handleSelectThread = (() => {
      var _ref = (0, _asyncToGenerator.default)(function* (data) {
        const { service } = _this.props;
        const matchedThread = _this.state.threadList.filter(function (t) {
          return t.threadId === data.id;
        });
    ...
    this.state = Object.assign({
      ...
      threadList: [],
      selectedThreadId: -1,
      threadsLoading: false }, this._getState());
  }
  ...
  render() {
    ...
    if (focusedProcess != null && focusedProcess.session != null && Boolean(focusedProcess.session.capabilities.supportsTerminateThread)) {
    ...
    if (supportsTerminateThread) {
      columns.push({
        ...
        key: 'terminateThread',
        width: 0.1,
        component: () => _react.createElement((_Icon || _load_Icon()).Icon, {
          icon: 'x',
          title: 'Terminate Thread',
          onClick: event => {
            atom.commands.dispatch(event.target.parentElement, 'nuclide-debugger:terminate-thread');
    ...
    const rows = threadList == null ? [] : threadList.map(thread => {
      ...
      if (thread.threadId === selectedThreadId) {
        cellData.className = 'nuclide-debugger-thread-list-item nuclide-debugger-thread-list-item-selected';
      } else {
