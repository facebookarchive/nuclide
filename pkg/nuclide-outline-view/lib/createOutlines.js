...
function uiOutlinesForResult(result) {
  switch (result.kind) {
    ...
    case 'pane-change':
      ...
      return (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.concat((_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.of({ kind: 'empty' }), (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.of({ kind: 'loading' }).delay(LOADING_DELAY_MS));
    case 'result':
      var outline = result.result;
      if (outline == null) {
        return (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.of({ kind: 'provider-no-outline' });
      }
...
function highlightedOutlines(outline, editor) {
  var nameOnly = (_commonsAtomFeatureConfig2 || _commonsAtomFeatureConfig()).default.get('nuclide-outline-view.nameOnly');
  var outlineForUi = {
    kind: 'outline',
