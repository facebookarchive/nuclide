...
class AutocompleteProvider {
  constructor(libClangProcess: LibClangProcess) {
    this.libClangProcess = libClangProcess;
  }
  ...
  async getAutocompleteSuggestions(
    ...
    var data = await this.libClangProcess.getCompletions(editor);
    return data.completions.map((completion) => {
      var snippet = this.getCompletionBody(completion, column, indentation);
  ...
  getCompletionBody(completion: Completion, columnOffset: number, indentation: number): string {
    var inlineBody = this.getCompletionBodyInline(completion);
    var multiLineBody = this.getCompletionBodyMultiLine(completion, columnOffset, indentation);
  ...
  getCompletionBodyMultiLine(completion: Completion, columnOffset: number, indentation: number): ?string {
    ...
    return this._convertArgsToMultiLineSnippet(args);
  }

  _convertArgsToMultiLineSnippet(args: Array<{text: string; placeholder: string; offset: number}>): string {
  ...
  getCompletionBodyInline(completion: Completion): string {
    var body = '';
