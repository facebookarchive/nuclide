...
const ClangService = require('./ClangService');
...
class LocalClangService extends ClangService {
    ...
    if (asyncConnection == null) {
      asyncConnection = createAsyncConnection(this._pathToLibClangServer).then(
        (connection: Connection) => {
          const {readableStream, writableStream} = connection;
  ...
  async _makeRequest(request: Object): Promise {
    ...
    const id = request['reqid'] = this._getNextRequestId();
    ...
    const writableStream = this._writableStream;
