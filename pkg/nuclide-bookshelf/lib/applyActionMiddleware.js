...
applyActionMiddleware = applyActionMiddleware;var _constants;function _load_constants() {return _constants = require('./constants');}var _utils;function _load_utils() {return _utils = require('./utils');}var _event;function _load_event() {return _event = require('nuclide-commons/event');}var _log4js;function _load_log4js() {return _log4js = require('log4js');}var _goToLocation;function _load_goToLocation() {return _goToLocation = require('nuclide-commons-atom/go-to-location');}var _rxjsBundlesRxMinJs = require('rxjs/bundles/Rx.min.js'); /**
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                * 
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                * @format
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                */const HANDLED_ACTION_TYPES = [(_constants || _load_constants()).ActionType.ADD_PROJECT_REPOSITORY, (_constants || _load_constants()).ActionType.RESTORE_PANE_ITEM_STATE];function getActionsOfType(actions, type) {return actions.filter(action => action.type === type);}function applyActionMiddleware(actions, getState) {const output = _rxjsBundlesRxMinJs.Observable.merge( // Let the unhandled ActionTypes pass through.
  actions.filter(action => HANDLED_ACTION_TYPES.indexOf(action.type) === -1), getActionsOfType(actions, (_constants || _load_constants()).ActionType.ADD_PROJECT_REPOSITORY).flatMap(action => {if (!(action.type === (_constants || _load_constants()).ActionType.ADD_PROJECT_REPOSITORY)) {throw new Error('Invariant violation: "action.type === ActionType.ADD_PROJECT_REPOSITORY"');}return watchProjectRepository(action, getState);
  }),

  getActionsOfType(actions, (_constants || _load_constants()).ActionType.RESTORE_PANE_ITEM_STATE).switchMap(
  action => {if (!(
    action.type === (_constants || _load_constants()).ActionType.RESTORE_PANE_ITEM_STATE)) {throw new Error('Invariant violation: "action.type === ActionType.RESTORE_PANE_ITEM_STATE"');}
...
getState)
{
  ...
  map(bookmarks => {
    ...
    bookmarks.map(bookmark => bookmark.bookmark).concat([(_constants || _load_constants()).EMPTY_SHORTHEAD]));


    const activeBookmark = bookmarks.filter(bookmark => bookmark.active)[0];
    const activeShortHead =
    activeBookmark == null ? (_constants || _load_constants()).EMPTY_SHORTHEAD : activeBookmark.bookmark;
  ...
  takeUntil(
  (0, (_event || _load_event()).observableFromSubscribeFunction)(repository.onDidDestroy.bind(repository))).
...
getState)
{
  ...
  (0, (_utils || _load_utils()).getRepoPathToEditors)().get(repository.getWorkingDirectory()) || [];
  ...
  _rxjsBundlesRxMinJs.Observable.from(editorsToReload).
  flatMap(textEditor => {
  ...
  ignoreElements(),
  _rxjsBundlesRxMinJs.Observable.from(urisToOpen).
  flatMap(fileUri => {
