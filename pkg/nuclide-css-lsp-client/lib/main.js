...
async function connectToService(connection) {
  const cssLSPService = (0, (_nuclideRemoteConnection || _load_nuclideRemoteConnection()).getServiceByConnection)(SERVICE_NAME, connection);
  ...
  const lspService = await cssLSPService.initializeLsp(['.arcconfig', '.flowconfig', '.hg', '.git'], ['.css', '.less', '.scss'], (_featureConfig || _load_featureConfig()).default.get('nuclide-css-lsp-client.logLevel'), fileNotifier, host, {} // initializationOptions
  );
