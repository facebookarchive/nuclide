...
class VsDebugSessionTranslator {
  ...
  constructor(adapterType, adapter, debuggerArgs, clientCallback, logger) {
  ...
  _handleCommands() {
    ...
    this._commandsOfType('Debugger.enable').flatMap(command => _rxjsBundlesRxMinJs.Observable.of(getEmptyResponse(command.id), getFakeLoaderPauseEvent())), this._commandsOfType('Debugger.pause').flatMap(catchCommandError((() => {
      var _ref2 = (0, _asyncToGenerator.default)(function* (command) {
        const mainThreadId = _this._mainThreadId || Array.from(_this._threadsById.keys())[0] || -1;
  ...
  _handleSetBreakpointsCommands() {
    ...
    setBreakpointsCommands.flatMap(command => this._setBulkBreakpoints([command]))).flatMap(responses => _rxjsBundlesRxMinJs.Observable.from(responses));
  }
  ...
  _setBulkBreakpoints(setBreakpointsCommands) {
    ...
    return (0, _asyncToGenerator.default)(function* () {
      ...
      const responseGroups = yield Promise.all(Array.from(breakpointCommandsByUrl).map((() => {
        var _ref13 = (0, _asyncToGenerator.default)(function* ([url, breakpointCommands]) {
          yield _this3._files.registerFile(url);
          const path = (0, (_helpers || _load_helpers()).uriToPath)(url);

          const breakpointDescriptors = breakpointCommands.map(function (c) {
            return {
              lineNumber: c.params.lineNumber + 1,
              condition: c.params.condition || ''
            };
          }).concat(_this3._getBreakpointsForFilePath(path).map(function (bp) {
            return Object.assign({}, bp);
          }));

          const translatorBreakpoins = yield _this3._setBreakpointsForFilePath(path, breakpointDescriptors);
  ...
  _syncBreakpoints() {
    ...
    for (const filePath of filePaths) {
      setBreakpointPromises.push(this._setBreakpointsForFilePath(filePath, this._getBreakpointsForFilePath(filePath).map(bp => Object.assign({}, bp))));
    }
  ...
  _configDone() {
    ...
    return (0, _asyncToGenerator.default)(function* () {
      yield _this4._session.setExceptionBreakpoints({
        filters: _this4._exceptionFilters
      });
      if (_this4._session.getCapabilities().supportsConfigurationDoneRequest) {
        yield _this4._session.configurationDone();
      }
    })();
  }

  _setBreakpointsForFilePath(path, breakpoints) {
    var _this5 = this;
    ...
    return (0, _asyncToGenerator.default)(function* () {
      ...
      } = yield _this5._session.setBreakpoints({
        source,
      ...
      return vsBreakpoints.map(function (vsBreakpoint, i) {
        const bpDescriptior = breakpoints[i];
        const breakpointId = bpDescriptior.breakpointId || String(vsBreakpoint.id) || _this5._nextBreakpointId();
        ...
        _this5._breakpointsById.set(breakpointId, translatorBreakpoint);
  ...
  _removeBreakpoint(breakpointId) {
    ...
    return (0, _asyncToGenerator.default)(function* () {
      const foundBreakpoint = _this6._breakpointsById.get(breakpointId);
      if (foundBreakpoint == null) {
        _this6._logger.info(`No breakpoint with id: ${breakpointId} to remove!`);
        return;
      }
      const remainingBreakpoints = _this6._getBreakpointsForFilePath(foundBreakpoint.path).filter(function (breakpoint) {
        return breakpoint.breakpointId !== breakpointId;
      });
      _this6._breakpointsById.delete(breakpointId);

      yield _this6._setBreakpointsForFilePath(foundBreakpoint.path, remainingBreakpoints.map(function (bp) {
        return Object.assign({}, bp);
      }));
    })();
  }
  ...
  _evaluateOnCallFrame(expression, frameId) {
    ...
    return (0, _asyncToGenerator.default)(function* () {
      const { body } = yield _this7._session.evaluate({
        expression,
        frameId
      });
      return {
  ...
  _listenToSessionEvents() {
    ...
    return new (_UniversalDisposable || _load_UniversalDisposable()).default(this._session.observeAllEvents().subscribe(event => {
    ...
    }), this._session.observeBreakpointEvents().flatMap((() => {
      var _ref14 = (0, _asyncToGenerator.default)(function* ({ body }) {
      ...
      var _ref15 = (0, _asyncToGenerator.default)(function* ({ body }) {
        const { threadId, reason } = body;
        let callFrames = [];
        const translatedStopReason = translateStopReason(reason);
        if (threadId != null) {
          callFrames = yield _this8._getTranslatedCallFramesForThread(threadId);
          _this8._threadsById.set(threadId, {
            state: 'paused',
            callFrames,
        ...
        const threadsUpdatedEvent = _this8._getThreadsUpdatedEvent();
      ...
      const existingBreakpoint = this._breakpointsById.get(String(breakpoint.id || -1));
      if (existingBreakpoint != null && !existingBreakpoint.resolved && breakpoint.verified) {
        this._sendMessageToClient({
    ...
    .switchMap((0, _asyncToGenerator.default)(function* () {
      yield _this8._syncBreakpoints();
      yield _this8._configDone();
  ...
  _getTranslatedCallFramesForThread(threadId) {
    ...
    return (0, _asyncToGenerator.default)(function* () {
      const { body: { stackFrames } } = yield _this9._session.stackTrace({
        threadId
      });
      return Promise.all(stackFrames.map((() => {
        var _ref17 = (0, _asyncToGenerator.default)(function* (frame) {
          let scriptId;
          if (frame.source != null && frame.source.path != null) {
            scriptId = frame.source.path;
          } else {
            _this9._logger.error('Cannot find source/script of frame: ', frame);
            scriptId = 'N/A';
          }
          yield _this9._files.registerFile((0, (_helpers || _load_helpers()).pathToUri)(scriptId));
          return {
            ...
            scopeChain: yield _this9._getScopesForFrame(frame.id),
            this: undefined
          };
        });
  ...
  _getScopesForFrame(frameId) {
    ...
    return (0, _asyncToGenerator.default)(function* () {
      const { body: { scopes } } = yield _this10._session.scopes({ frameId });
  ...
  _getProperties(id, params) {
    ...
    return (0, _asyncToGenerator.default)(function* () {
      ...
      const { body: { variables } } = yield _this11._session.variables({
        variablesReference
      });
