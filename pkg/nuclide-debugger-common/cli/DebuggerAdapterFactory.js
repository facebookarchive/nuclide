...
class DebuggerAdapterFactory {
  constructor() {
    this._vspServersByTargetType = new Map([[(_constants || _load_constants()).VsAdapterTypes.PYTHON, {
      interpreter: 'node',
      serverScript: (_nuclideUri || _load_nuclideUri()).default.join(__dirname, '../../nuclide-debugger-vsp/VendorLib/vs-py-debugger/out/client/debugger/Main.js'),
      packagePath: (_nuclideUri || _load_nuclideUri()).default.join(__dirname, '../../nuclide-debugger-vsp/VendorLib/vs-py-debugger'),
      type: 'python',
      customArguments: new Map()
    }], [(_constants || _load_constants()).VsAdapterTypes.NODE, {
      interpreter: 'node',
      serverScript: (_nuclideUri || _load_nuclideUri()).default.join(__dirname, '../../nuclide-debugger-vsp/VendorLib/vscode-node-debug2/out/src/nodeDebug.js'),
      packagePath: (_nuclideUri || _load_nuclideUri()).default.join(__dirname, '../../nuclide-debugger-vsp/VendorLib/vscode-node-debug2'),
      type: 'node2',
  ...
  showContextSensitiveHelp(args) {
    ...
    const optionsParser = new (_VSPOptionsParser || _load_VSPOptionsParser()).default(adapter.packagePath);
  ...
  _parseAttachArguments(args) {
    ...
    const parser = new (_VSPOptionsParser || _load_VSPOptionsParser()).default(adapter.packagePath);
