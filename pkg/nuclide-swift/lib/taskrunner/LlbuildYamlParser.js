...
var readCompileCommands = _asyncToGenerator(function* (path) {
  ...
  var data = undefined;
  try {
    data = yield (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.readFile(path, 'utf8');
  } catch (e) {
    return new Map();
  }

  // Attempt to parse the YAML, or bail if a parsing error occurs.
  var llbuildYaml = (_jsYaml || _load_jsYaml()).default.safeLoad(data);
...
function llbuildYamlPath(chdir, configuration, buildPath) {
  ...
  if (buildPath.length > 0) {
    return (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.join(buildPath, yamlFileName);
  } else {
    return (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.join(chdir, '.build', yamlFileName);
  }
}