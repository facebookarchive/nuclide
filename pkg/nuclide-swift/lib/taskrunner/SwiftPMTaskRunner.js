...
class SwiftPMTaskRunner {
  ...
  runTask(taskName) {
    ...
    const observable = (0, (_process || _load_process()).observeProcess)(command.command, command.args, { /* TODO(T17353599) */isExitError: () => false }).catch(error => _rxjsBundlesRxMinJs.Observable.of({ kind: 'error', error })) // TODO(T17463635)
    .do(message => {
      switch (message.kind) {
        case 'stderr':
        case 'stdout':
