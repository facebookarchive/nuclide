...
var asyncExecuteSourceKitten = _asyncToGenerator(function* (command, args) {
  ...
  var result = yield (0, (_commonsNodeProcess || _load_commonsNodeProcess()).asyncExecute)(sourceKittenPath, [command].concat(args));
...
function getSourceKittenPath() {
  return (_commonsAtomFeatureConfig || _load_commonsAtomFeatureConfig()).default.get('nuclide-swift.sourceKittenPath');
}