...
var ConsoleContainer = (function (_React$Component) {
  ...
  function ConsoleContainer(props) {
    ...
    this._stateChanges = new (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Subject();
  ...
  _createClass(ConsoleContainer, [{
    ...
    value: function onDidChangeTitle(callback) {
      return new (_commonsNodeUniversalDisposable || _load_commonsNodeUniversalDisposable()).default(this._titleChanges.subscribe(callback));
    }
  }, {
    ...
    value: function componentDidMount() {
      ...
      var raf = (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.create(function (observer) {
        window.requestAnimationFrame(observer.complete.bind(observer));
      });
      // $FlowFixMe: How do we tell flow about Symbol.observable?
      this._statesSubscription = (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.from(this.props.store).audit(function () {
        return raf;
      }).subscribe(function (state) {
        var currentExecutorId = (0, (_getCurrentExecutorId || _load_getCurrentExecutorId()).default)(state);
    ...
    key: 'copy',
    value: function copy() {
      return (0, (_commonsAtomViewableFromReactElement || _load_commonsAtomViewableFromReactElement()).viewableFromReactElement)((_reactForAtom || _load_reactForAtom()).React.createElement(ConsoleContainer, {
        ...
        (function () {
          ...
          _this4._actionCreators = {
            execute: function execute(code) {
              store.dispatch((_reduxActions || _load_reduxActions()).execute(code));
            },
            selectExecutor: function selectExecutor(executorId) {
              store.dispatch((_reduxActions || _load_reduxActions()).selectExecutor(executorId));
            },
            clearRecords: function clearRecords() {
              store.dispatch((_reduxActions || _load_reduxActions()).clearRecords());
            }
          };
        })();
      }
    ...
    value: function render() {
      ...
      if (!this.state.ready) {
        return (_reactForAtom || _load_reactForAtom()).React.createElement('span', null);
      }
      ...
      return (_reactForAtom || _load_reactForAtom()).React.createElement((_Console || _load_Console()).default, {
    ...
    value: function _getFilterPattern(filterText, isRegExp) {
      ...
      var source = isRegExp ? filterText : (0, (_escapeStringRegexp || _load_escapeStringRegexp()).default)(filterText);
      try {
        return {
