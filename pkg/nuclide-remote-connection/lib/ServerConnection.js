...
class ServerConnection {
  ...
  async _makeThriftRfsCall(fsOperation, args) {
    return (0, _nuclideAnalytics().trackTimingSampled)(`file-system-service:${fsOperation}`, async () => {
      try {
        const thriftRfsClient = await (0, _createRfsClientAdapter().getOrCreateRfsClientAdapter)(this.getBigDigClient()); // $FlowFixMe: suppress 'indexer property is missing warning'
