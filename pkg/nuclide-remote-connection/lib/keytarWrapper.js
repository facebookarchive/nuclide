...
function getApmNodePath() {
  var apmDir = (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.dirname(atom.packages.getApmPath());
  return (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.normalize((_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.join(apmDir, 'node'));
}

function getApmNodeModulesPath() {
  var apmDir = (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.dirname(atom.packages.getApmPath());
  return (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.normalize((_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.join(apmDir, '..', 'node_modules'));
}

function runScriptInApmNode(script) {
  var args = ['-e', script];
  var options = { env: { NODE_PATH: getApmNodeModulesPath() } };
  var output = (_child_process || _load_child_process()).default.spawnSync(getApmNodePath(), args, options);
