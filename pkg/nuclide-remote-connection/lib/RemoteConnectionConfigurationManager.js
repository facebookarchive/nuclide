...
function encryptConfig(remoteProjectConfig) {
  ...
  const clientCertificate = remoteProjectConfig.clientCertificate;
  const clientKey = remoteProjectConfig.clientKey;
  ...
  const password = _encryptString.password;
  const encryptedString = _encryptString.encryptedString;
...
function decryptConfig(remoteProjectConfig) {
  ...
  const salt = _clientKey$split2[1];
