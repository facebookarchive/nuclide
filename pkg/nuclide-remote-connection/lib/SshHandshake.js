...
var _lookupPreferIpV6;

function _load_lookupPreferIpV6() {
  return _lookupPreferIpV6 = _interopRequireDefault(require('./lookup-prefer-ip-v6'));
}
...
var SshHandshake = (function () {
  ...
  function SshHandshake(delegate, connection) {
    ...
    this._connection = connection ? connection : new (_ssh2 || _load_ssh2()).Client();
  ...
  _createClass(SshHandshake, [{
    ...
    value: function _onSshConnectionError(error) {
      ...
      if (errorLevel === 'client-authentication' && this._passwordRetryCount < PASSWORD_RETRIES) {
        var _ret = (function () {
          ...
          var retryText = _this._passwordRetryCount ? ' again' : '';
    ...
    value: _asyncToGenerator(function* (config) {
      ...
      var existingConnection = (_RemoteConnection || _load_RemoteConnection()).RemoteConnection.getByHostnameAndPath(this._config.host, this._config.cwd);
      ...
      var connection = yield (_RemoteConnection || _load_RemoteConnection()).RemoteConnection.createConnectionBySavedConfig(this._config.host, this._config.cwd, this._config.displayTitle);
      ...
      var address = null;
      try {
        address = yield (0, (_lookupPreferIpV6 || _load_lookupPreferIpV6()).default)(config.host);
      } catch (e) {
      ...
      if (config.authMethod === SupportedMethods.SSL_AGENT) {
        ...
        var expandedPath = (_fsPlus || _load_fsPlus()).default.normalize(config.pathToPrivateKey);
        try {
          var privateKey = yield (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.readFile(expandedPath);
          this._connection.connect({
            host: address,
            port: config.sshPort,
            username: config.username,
    ...
    value: function _updateServerInfo(serverInfo) {
      (0, (_assert || _load_assert()).default)(typeof serverInfo.port === 'number');
      ...
      (0, (_assert || _load_assert()).default)(typeof serverInfo.workspace === 'string');
    ...
    value: function _startRemoteServer() {
      ...
      return new Promise(function (resolve, reject) {
        ...
        var cmd = _this2._config.remoteServerCommand + ' --workspace=' + _this2._config.cwd + (' --common-name=' + _this2._config.host + ' --json-output-file=' + remoteTempFile + ' -t 60');

        _this2._connection.exec(cmd, { pty: { term: 'nuclide' } }, function (err, stream) {
          if (err) {
            _this2._onSshConnectionError(err);
          ...
          stream.on('close', _asyncToGenerator(function* (code, signal) {
            ...
            if (code === 0) {
              ...
              yield (0, (_commonsNodePromise || _load_commonsNodePromise()).sleep)(100);
              ...
              _this2._connection.sftp(_asyncToGenerator(function* (error, sftp) {
                if (sftpTimer != null) {
                ...
                var localTempFile = yield (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.tempfile();
                sftp.fastGet(remoteTempFile, localTempFile, _asyncToGenerator(function* (sftpError) {
                  ...
                  var serverInfo = null;
                  var serverInfoJson = yield (_commonsNodeFsPromise || _load_commonsNodeFsPromise()).default.readFile(localTempFile, 'utf8');
                  try {
    ...
    value: _asyncToGenerator(function* () {
      ...
      if (this._isSecure()) {
        (0, (_assert || _load_assert()).default)(this._remoteHost);
        (0, (_assert || _load_assert()).default)(this._remotePort);
        connect({
        ...
        /* $FlowIssue t9212378 */
        this._forwardingServer = (_net || _load_net()).default.createServer(function (sock) {
          _this3._forwardSocket(sock);
        }).listen(0, 'localhost', function () {
          var localPort = _this3._getLocalPort();
          (0, (_assert || _load_assert()).default)(localPort);
          connect({
            host: 'localhost',
            port: localPort,
...
function decorateSshConnectionDelegateWithTracking(delegate) {
  ...
  return {
    onKeyboardInteractive: function onKeyboardInteractive(name, instructions, instructionsLang, prompts, finish) {
      (0, (_assert || _load_assert()).default)(connectionTracker);
