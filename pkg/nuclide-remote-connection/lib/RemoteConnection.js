...
class RemoteConnection {
  ...
  static findOrCreateFromConnection(serverConnection, cwd, displayTitle, promptReconnectOnFailure = true) {
    ...
    return (0, _asyncToGenerator.default)(function* () {
      ...
      const contents = hasAtomProjectFormat(realPath) ? yield fsService.readFile(realPath).catch(function () {
        return null;
      }) : null;
      ...
      if (contents == null) {
        ...
        if (realPath !== cwd && (_nuclideUri || _load_nuclideUri()).default.isRemote(cwd)) {
          const existingConnection = _this.getByHostnameAndPath((_nuclideUri || _load_nuclideUri()).default.getHostname(cwd), realPath);
        ...
        const projectContents = (_season || _load_season()).default.parse(contents.toString());
        ...
        if (projectPaths != null && Array.isArray(projectPaths)) {
          directories.push(...projectPaths.map(function (path) {
            return (_nuclideUri || _load_nuclideUri()).default.resolve(dirname, path);
          }));
        } else {
      ...
      const connections = yield Promise.all(directories.map(function (dir, i) {
        const connection = new RemoteConnection(serverConnection, dir, i === 0 ? displayTitle : '', promptReconnectOnFailure);
  ...
  _setHgRepoInfo() {
    ...
    return (0, _asyncToGenerator.default)(function* () {
      ...
      const { getHgRepository } = _this2.getService('SourceControlService');
  ...
  _watchRootProjectDirectory() {
    ...
    const subscription = watchStream.subscribe(watchUpdate => {
      ...
      var _ref = (0, _asyncToGenerator.default)(function* (error) {
        ...
        let detail;
        const fileSystemService = _this4.getService(FILE_SYSTEM_SERVICE);
