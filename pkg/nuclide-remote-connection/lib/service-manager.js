...
function createLocalRpcClient() {
  ...
  var serviceRegistry = (_nuclideRpc2 || _nuclideRpc()).ServiceRegistry.createRemote(newServices);
  var localClientConnection = new (_nuclideRpc2 || _nuclideRpc()).RpcConnection('server', serviceRegistry, localTransports.serverTransport);
  (0, (_assert2 || _assert()).default)(localClientConnection != null); // silence lint...
  return (_nuclideRpc2 || _nuclideRpc()).RpcConnection.createLocal(localTransports.clientTransport, newServices);
}
