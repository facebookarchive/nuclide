...
function getServiceByNuclideUri(serviceName) {
  ...
  var hostname = nuclideUri && (0, _nuclideRemoteUri.isRemote)(nuclideUri) ? (0, _nuclideRemoteUri.getHostname)(nuclideUri) : null;
...
function getService(serviceName, hostname) {
  if (hostname) {
    var serverConnection = _ServerConnection.ServerConnection.getByHostname(hostname);
