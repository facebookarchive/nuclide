...
function createProxyGenerator(t, generate) {
  ...
  function generateProxy(serviceName, preserveFunctionNames, defs) {
    ...
    t.expressionStatement(t.stringLiteral('use strict')), t.expressionStatement(assignment), t.expressionStatement(objectDefinePropertyCall('defs', objectToLiteral(defs)))]);
  ...
  function generateRemoteConstructor() {
    // throw Error(...)
    const throwStatement = t.throwStatement(t.callExpression(t.identifier('Error'), [t.stringLiteral('constructors are not supported for remote objects')]));
  ...
  function generateRemoteDispatch(methodName, thisType, funcType) {
    ...
    const callRemoteMethod = t.callExpression(callRemoteMethodExpression, [generateTransformStatement(t.thisExpression(), thisType, true), t.stringLiteral(methodName), t.stringLiteral(funcType.returnType.kind), marshalArgsCall(funcType.argumentTypes)]);

    // ... .then(value => _client.unmarshal(...))
    const result = generateUnmarshalResult(funcType.returnType, callRemoteMethod);
  ...
  function generateUnmarshalResult(returnType, valueExpression) {
    switch (returnType.kind) {
      ...
      case 'observable':
        ...
        const unmarshalledObservable = t.callExpression(t.memberExpression(valueExpression, t.identifier('map')), [observableTransformer]);
