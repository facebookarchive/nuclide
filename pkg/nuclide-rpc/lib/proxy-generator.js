...
function generateProxy(serviceName, preserveFunctionNames, defs) {
  ...
  var deps = dependenciesNodes(['Observable']);
...
function generateRemoteDispatch(methodName, thisType, funcType) {
  ...
  var funcExpression = (_babelCoreLibTypes || _load_babelCoreLibTypes()).functionExpression(null, funcTypeArgs, (_babelCoreLibTypes || _load_babelCoreLibTypes()).blockStatement([(_babelCoreLibTypes || _load_babelCoreLibTypes()).returnStatement(result)]));
