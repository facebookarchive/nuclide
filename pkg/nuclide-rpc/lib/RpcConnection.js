...
class RpcConnection {
  ...
  _returnObservable(id, returnVal, elementType) {
    ...
    .subscribe(data => {
      this._transport.send(JSON.stringify((0, (_messages || _load_messages()).createNextMessage)(this._getProtocol(), id, this._generateResponseId(), this.marshal(data, elementType))));
    }, error => {
