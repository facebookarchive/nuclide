'use strict';Object.defineProperty(exports, "__esModule", { value: true });exports.RpcConnection = exports.RpcTimeoutError = undefined;var _asyncToGenerator = _interopRequireDefault(require('async-to-generator'));
...
var _rxjsBundlesRxMinJs = require('rxjs/bundles/Rx.min.js');var _ServiceRegistry;
function _load_ServiceRegistry() {return _ServiceRegistry = require('./ServiceRegistry');}var _ObjectRegistry;
function _load_ObjectRegistry() {return _ObjectRegistry = require('./ObjectRegistry');}var _messages;
function _load_messages() {return _messages = require('./messages');}var _builtinTypes;
...
function _load_builtinTypes() {return _builtinTypes = require('./builtin-types');}var _nuclideAnalytics;
function _load_nuclideAnalytics() {return _nuclideAnalytics = require('../../nuclide-analytics');}var _config;
function _load_config() {return _config = require('./config');}var _string;
function _load_string() {return _string = require('../../../modules/nuclide-commons/string');}var _log4js;
...
const logger = (0, (_log4js || _load_log4js()).getLogger)('nuclide-rpc'); /**
                                                                           ...
                                                                           * 
                                                                           * @format
                                                                           */const SERVICE_FRAMEWORK_RPC_TIMEOUT_MS = 60 * 1000;const LARGE_RESPONSE_SIZE = 100000;class Subscription {
...
class RpcTimeoutError extends Error {constructor(...args) {var _temp;return _temp = super(...args), this.
    name = 'RpcTimeoutError', _temp;}}exports.RpcTimeoutError = RpcTimeoutError;
...
class Call {
  ...
  _timeout() {
    const timeoutMessage = this._timeoutMessage;if (!(
    timeoutMessage != null)) {throw new Error('Invariant violation: "timeoutMessage != null"');}
    if (!this._complete) {
      this.cleanup();
      this._reject(
      new RpcTimeoutError(
...
class RpcConnection {
  ...
  protocolLogger = null)
  {
    ...
    this._objectRegistry = new (_ObjectRegistry || _load_ObjectRegistry()).ObjectRegistry(
    kind,
  ...
  protocolLogger = null)
  {
    return new RpcConnection(
    'client',
    new (_ServiceRegistry || _load_ServiceRegistry()).ServiceRegistry(predefinedTypes, services, protocol, { lazy: true }),
    transport,
    options,
    connectionId,
    protocolLogger);
  ...
  getService(serviceName) {
    const service = this._objectRegistry.getService(serviceName);if (!(
    service != null)) {throw new Error(`No config found for service ${serviceName}`);}
    return service;
  }
  ...
  args)
  {
    ...
    (0, (_messages || _load_messages()).createCallMessage)(
    this._getProtocol(),
    functionName,
  ...
  {
    return this._sendMessageAndListenForResult(
    (0, (_messages || _load_messages()).createCallObjectMessage)(
    this._getProtocol(),
    methodName,
    objectId,
    this._generateRequestId(),
  ...
  {var _this = this;
    const idPromise = (0, _asyncToGenerator.default)(function* () {
      const marshalledArgs = yield _this._getTypeRegistry().marshalArguments(
      _this._objectRegistry,
      unmarshalledArgs,
      argTypes);

      return _this._sendMessageAndListenForResult(
      (0, (_messages || _load_messages()).createNewObjectMessage)(
      _this._getProtocol(),
      interfaceName,
      _this._generateRequestId(),
      marshalledArgs),
  ...
  /**
     ...
     */
  disposeRemoteObject(object) {var _this2 = this;return (0, _asyncToGenerator.default)(function* () {
      ...
      if (objectId == null) {
        ...
        return _this2._sendMessageAndListenForResult(
        (0, (_messages || _load_messages()).createDisposeMessage)(
        _this2._getProtocol(),
        _this2._generateRequestId(),
        objectId),
  ...
  timeoutMessage)
  {
    switch (returnType) {
      ...
      case 'promise':
        ...
        return promise;
      case 'observable':{
          const id = message.id;if (!
          !this._subscriptions.has(id)) {throw new Error('Invariant violation: "!this._subscriptions.has(id)"');}
          ...
          const sendUnsubscribe = () => {
            if (!this._transport.isClosed()) {
              ...
              JSON.stringify((0, (_messages || _load_messages()).createUnsubscribeMessage)(this._getProtocol(), id)));
  ...
  _returnPromise(id, candidate, type) {
    ...
    if (!(returnVal != null)) {throw new Error('Invariant violation: "returnVal != null"');}
    ...
    result => {
      this._transport.send(
      JSON.stringify(
      (0, (_messages || _load_messages()).createPromiseMessage)(
      this._getProtocol(),
      id,
    ...
    error => {
      this._transport.send(
      JSON.stringify(
      (0, (_messages || _load_messages()).createErrorResponseMessage)(
      this._getProtocol(),
      id,
      this._generateResponseId(),
      error)));
  ...
  _returnObservable(id, returnVal, elementType) {
    ...
    if (!isConnectableObservable(returnVal)) {
      result = _rxjsBundlesRxMinJs.Observable.throw(
      new Error(
    ...
    data => {
      this._transport.send(
      JSON.stringify(
      (0, (_messages || _load_messages()).createNextMessage)(
      this._getProtocol(),
      id,
      this._generateResponseId(),
      data)));
    ...
    error => {
      this._transport.send(
      JSON.stringify(
      (0, (_messages || _load_messages()).createObserveErrorMessage)(
      this._getProtocol(),
      id,
      this._generateResponseId(),
      error)));
    ...
    completed => {
      this._transport.send(
      JSON.stringify(
      (0, (_messages || _load_messages()).createCompleteMessage)(
      this._getProtocol(),
      id,
      this._generateResponseId())));
  ...
  _callFunction(id, call) {var _this3 = this;return (0, _asyncToGenerator.default)(function* () {
      const { getLocalImplementation, type } = _this3._getFunctionImplemention(
      call.method);

      const marshalledArgs = yield _this3._getTypeRegistry().unmarshalArguments(
      _this3._objectRegistry,
      call.args,
      type.argumentTypes);
      ...
      _this3._returnValue(
      id,
      localImplementation.apply(_this3, marshalledArgs),
      type.returnType);})();

  }

  _callMethod(id, call) {var _this4 = this;return (0, _asyncToGenerator.default)(function* () {
      const object = _this4._objectRegistry.unmarshal(call.objectId);if (!(
      object != null)) {throw new Error('Invariant violation: "object != null"');}
      ...
      const marshalledArgs = yield _this4._getTypeRegistry().unmarshalArguments(
      _this4._objectRegistry,
      call.args,
      type.argumentTypes);
  ...
  {var _this5 = this;return (0, _asyncToGenerator.default)(function* () {
      const { getLocalImplementation, definition } = _this5._getClassDefinition(
      constructorMessage.interface);

      const { constructorArgs } = definition;if (!(
      constructorArgs != null)) {throw new Error('Invariant violation: "constructorArgs != null"');}
      const marshalledArgs = yield _this5._getTypeRegistry().unmarshalArguments(
      _this5._objectRegistry,
  ...
  _handleRequestMessage(message) {var _this6 = this;return (0, _asyncToGenerator.default)(function* () {
      const id = message.id;

      if (
      id !== _this6._lastRequestId + 1 &&
      _this6._lastRequestId !== -1 &&
      ...
      message.type !== 'unsubscribe')
      {
        ...
        `_lastRequestId === ${_this6._lastRequestId}`);
      ...
      try {
        ...
        _this6._transport.send(
        JSON.stringify(
        (0, (_messages || _load_messages()).createErrorResponseMessage)(
        _this6._getProtocol(),
        id,
        _this6._generateResponseId(),
        e)));
  ...
  _trackLargeResponse(message, size) {
    ...
    const args =
    message.args != null ?
    (0, (_string || _load_string()).shorten)(JSON.stringify(message.args), 100, '...') :
    '';
