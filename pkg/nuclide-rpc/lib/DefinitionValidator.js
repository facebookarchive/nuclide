...
function validateDefinitions(definitions) {
  ...
  function findObjectUnionDiscriminant(type, alternates) {
    ...
    var possibleFields = alternates.reduce(function (possibilities, alternate) {
      ...
      if (possibilities == null) {
        ...
        return (_nuclideCommons2 || _nuclideCommons()).set.intersect(alternatePossibilities, possibilities);
      }
    }, null);
  ...
  function errorLocations(locations, message) {
    ...
    var fullMessage = (0, (_builtinTypes2 || _builtinTypes()).locationToString)(locations[0]) + ':' + message;
    fullMessage = (_fullMessage = fullMessage).concat.apply(_fullMessage, _toConsumableArray(locations.slice(1).map(function (location) {
      return '\n' + (0, (_builtinTypes2 || _builtinTypes()).locationToString)(location) + ': Related location';
    })));
  ...
  function errorDefinitions(defs, message) {
    ...
    var fullMessage = (0, (_builtinTypes2 || _builtinTypes()).locationToString)(defs[0].location) + ':' + message;
    fullMessage = (_fullMessage2 = fullMessage).concat.apply(_fullMessage2, _toConsumableArray(defs.slice(1).map(function (definition) {
      return '\n' + (0, (_builtinTypes2 || _builtinTypes()).locationToString)(definition.location) + ': Related definition ' + definition.name;
    })));
    return new Error(fullMessage);
  }
}