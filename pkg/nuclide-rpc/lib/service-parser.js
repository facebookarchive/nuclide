...
function _load_babelParse() {
  return _babelParse = _interopRequireDefault(require('./babel-parse'));
}
...
var ServiceParser = (function () {
  function ServiceParser(predefinedTypes) {
    var _this = this;
    ...
    var defineBuiltinType = function defineBuiltinType(name) {
      (0, (_assert || _load_assert()).default)(!_this._defs.has(name), 'Duplicate builtin type');
  ...
  _createClass(ServiceParser, [{
    ...
    value: function parseService(fileName, source) {
      ...
      while (this._filesTodo.length > 0) {
        ...
        var contents = (_fs || _load_fs()).default.readFileSync(_file, 'utf8');
    ...
    value: function _parseFile(fileName, fileType, source) {
      ...
      for (var imp of imports) {
        var resolvedFrom = (0, (_resolveFrom || _load_resolveFrom()).default)((_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.dirname(fileName), imp);
...
var FileParser = (function () {
  ...
  _createClass(FileParser, [{
    ...
    value: function _errorLocations(locations, message) {
      ...
      var fullMessage = (0, (_location || _load_location()).locationToString)(locations[0]) + ':' + message;
    ...
    value: function parse(source) {
      ...
      var program = (0, (_babelParse || _load_babelParse()).default)(source);
      (0, (_assert || _load_assert()).default)(program && program.type === 'Program', 'The result of parsing is a Program node.');
    ...
    value: function _parseExport(node) {
      (0, (_assert || _load_assert()).default)(node.type === 'ExportNamedDeclaration');
    ...
    value: function _parseImport(node) {
      ...
      (0, (_assert || _load_assert()).default)(typeof from === 'string');
    ...
    value: function _add(definition) {
      if (this._defs.has(definition.name)) {
        ...
        (0, (_assert || _load_assert()).default)(existingDef != null);
    ...
    value: function _parseInterfaceDeclaration(declaration) {
      ...
      (0, (_assert || _load_assert()).default)(declaration.body.type === 'ObjectTypeAnnotation');
      var properties = declaration.body.properties;
      for (var property of properties) {
        (0, (_assert || _load_assert()).default)(property.type === 'ObjectTypeProperty');
        ...
        if (!isPrivateMemberName(property.key.name)) {
          ...
          (0, (_assert || _load_assert()).default)(!property.static, 'static interface members are a parse error');
    ...
    value: function _parseInterfaceMethodDefinition(definition) {
      ...
      (0, (_assert || _load_assert()).default)(typeof definition.key.name === 'string');
      return {
        location: this._locationOfNode(definition.key),
        name: definition.key.name,
        type: {
    ...
    value: function _parseInterfaceParameter(param) {
      if (!param.typeAnnotation) {
        ...
        var _name3 = param.name.name;
        (0, (_assert || _load_assert()).default)(typeof _name3 === 'string');
        ...
        case 'ObjectTypeAnnotation':
          return {
            ...
            fields: typeAnnotation.properties.map(function (prop) {
              (0, (_assert || _load_assert()).default)(prop.type === 'ObjectTypeProperty');
              return {
    ...
    value: function _parseGenericTypeAnnotation(typeAnnotation) {
      (0, (_assert || _load_assert()).default)(typeAnnotation.type === 'GenericTypeAnnotation');
    ...
    value: function _parseTypeName(type) {
      switch (type.type) {
        ...
        case 'QualifiedTypeIdentifier':
          (0, (_assert || _load_assert()).default)(type.id.type === 'Identifier');
