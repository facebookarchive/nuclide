...
class TypeRegistry {
  ...
  constructor(predefinedTypes) {
    ...
    this._registerKind('void', (value, type, context) => undefined, (value, type, context) => undefined);
  ...
  marshalArguments(context, args, argTypes) {
    const marshalledargs = argTypes.map((param, i) => this.marshal(context, args[i], param.type));
    const result = {};
  ...
  _registerContainers() {
    ...
    this._registerKind('set', (value, type, context) => {
      ...
      return Array.from(value).map(elem => this._marshal(context, elem, type.type));
    }, (value, type, context) => {
      (0, _assert.default)(value instanceof Array, 'Expected an object of type Array.');
