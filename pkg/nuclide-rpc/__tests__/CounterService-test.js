...
describe('CounterService', () => {
  ...
  beforeEach(async () => {
    ...
    await testHelper.start([{
      name: 'CounterService',
      definition: (_nuclideUri || _load_nuclideUri()).default.join(__dirname, '../__mocks__/CounterService.js'),
      implementation: (_nuclideUri || _load_nuclideUri()).default.join(__dirname, '../__mocks__/CounterService.js')
    }], 'counter_protocol');
  ...
  it('Can create two remotable counters in parallel.', async () => {
    ...
    (0, (_waits_for || _load_waits_for()).default)(() => completed1, 'The counter1 Observable to complete.');

    // Wait for our watch to have seen all of the Counters.
    (0, (_waits_for || _load_waits_for()).default)(() => watchedCounters === 2, 'We have watched two counters get created.');
  });
