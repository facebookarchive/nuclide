...
var getRootForPath = _asyncToGenerator(function* (file) {
...
var _build = _asyncToGenerator(function* (rootPath, buildTargets, options) {
  ...
  try {
    yield _runBuckCommandFromProjectRoot(rootPath, args, options.commandOptions, true);
...
function getPool(path, readOnly) {
  ...
  pool = new (_commonsNodePromiseExecutors2 || _commonsNodePromiseExecutors()).PromisePool(readOnly ? MAX_CONCURRENT_READ_ONLY : 1);
  pools.set(key, pool);
  return pool;
}function _runBuckCommandFromProjectRoot(rootPath, args, commandOptions) {
  var readOnly = arguments.length <= 3 || arguments[3] === undefined ? true : arguments[3];
