...
async function getDefaultPlatform(rootPath, target) {
  const result = await query(rootPath, target, ['--output-attributes', 'defaults']).toPromise();
...
function query(rootPath, queryString, extraArguments) {
  return _RxMin.Observable.fromPromise(_getFbRepoSpecificArgs(rootPath)).switchMap(fbRepoSpecificArgs => {
...
async function getBuildFile(rootPath, targetName) {
  try {
    const result = await query(rootPath, `buildfile(${targetName})`, []).toPromise();
