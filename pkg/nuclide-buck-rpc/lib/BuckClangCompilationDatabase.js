...
function _load_RelatedFileFinder() {
  return _RelatedFileFinder = require('../../nuclide-clang-rpc/lib/RelatedFileFinder');
}
...
class BuckClangCompilationDatabaseHandler {
  ...
  getCompilationDatabase(file) {
    ...
    return this._sourceCache.getOrCreate(file, (0, _asyncToGenerator.default)(function* () {
      ...
      let compDb = null;
      try {
        compDb = yield _this._getCompilationDatabase(file);
      } catch (e) {
        caughtError = e;
      }
      if ((compDb == null || compDb.file == null) && (0, (_utils || _load_utils()).isHeaderFile)(file)) {
        logger.error(`Couldn't find flags for header ${file}.`);
        const source = yield new (_RelatedFileFinder || _load_RelatedFileFinder()).RelatedFileFinder().getRelatedSourceForHeader(file);
        if (source != null) {
