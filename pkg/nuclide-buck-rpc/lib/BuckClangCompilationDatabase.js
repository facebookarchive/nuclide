...
class BuckClangCompilationDatabaseHandler {
  ...
  getCompilationDatabase(src) {
    return this._sourceCache.getOrCreate(src, () => this._loadCompilationDatabaseFromBuck(src).catch(err => {
  ...
  _loadCompilationDatabaseFromBuck(src) {
    var _this = this;
    ...
    return (0, _asyncToGenerator.default)(function* () {
      ...
      _this._sourceToTargetKey.set(src, _this._targetCache.keyForArgs([buckRoot, target]));

      return _this._targetCache.getOrCreate([buckRoot, target], function () {
        return _this._loadCompilationDatabaseForBuckTarget(buckRoot, target);
      });
    })();
  }
  ...
  _loadCompilationDatabaseForBuckTarget(buckProjectRoot, target) {
    ...
    return (0, _asyncToGenerator.default)(function* () {
      ...
      const allFlavors = ['compilation-database', ..._this2._params.flavorsForTarget];
      const allArgs = _this2._params.args.length === 0 ? yield _this2._getFallbackArgs(buckProjectRoot) : _this2._params.args;
      ...
      return _this2._processCompilationDb(compilationDB);
    })();
  }

  _processCompilationDb(db) {
    return (0, _asyncToGenerator.default)(function* () {
      try {
        // $FlowFB
        const { createOmCompilationDb } = require('./fb/omCompilationDb');
        return yield createOmCompilationDb(db);
      } catch (e) {}
      return db;
    })();
  }

  _cacheAllTheFilesInTheSameDB(db) {
    var _this3 = this;

    return (0, _asyncToGenerator.default)(function* () {
      const pathToFlags = yield (_nuclideClangRpc || _load_nuclideClangRpc()).loadFlagsFromCompilationDatabaseAndCacheThem(db);
      pathToFlags.forEach(function (fullFlags, path) {
        _this3._sourceCache.set(path, Promise.resolve(db));
      });
    })();
  }
}
