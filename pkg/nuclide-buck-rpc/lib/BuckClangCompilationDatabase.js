...
class BuckClangCompilationDatabaseHandler {
  ...
  getCompilationDatabase(file) {
    ...
    return this._sourceCache.getOrCreate(file, (0, _asyncToGenerator.default)(function* () {
      if ((0, (_utils || _load_utils()).isHeaderFile)(file)) {
        const source = yield (_nuclideClangRpc || _load_nuclideClangRpc()).getRelatedSourceOrHeader(file);
        if (source != null) {
  ...
  _loadCompilationDatabaseForBuckTarget(buckProjectRoot, target) {
    ...
    return (0, _asyncToGenerator.default)(function* () {
      ...
      const allArgs = _this4._params.args.length === 0 ? yield (_BuckServiceImpl || _load_BuckServiceImpl())._getFbRepoSpecificArgs(buckProjectRoot) : _this4._params.args;
