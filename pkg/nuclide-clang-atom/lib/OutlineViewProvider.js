...
function tokenizeType(type) {
  if (type.length > LONG_TYPE_LENGTH) {
    ...
    if (openIndex !== -1) {
      ...
      if (closeIndex !== -1) {
        return [(0, _nuclideTokenizedText.plain)(type.substring(0, openIndex + 1)), (0, _nuclideTokenizedText.string)('...'), (0, _nuclideTokenizedText.plain)(type.substring(closeIndex))];
      }
    }
  }
  return [(0, _nuclideTokenizedText.plain)(type)];
}

function tokenizeCursor(cursor) {
  if (cursor.children != null) {
    return [(0, _nuclideTokenizedText.keyword)(CLASS_KIND_NAMES[cursor.cursor_kind] || 'class'), (0, _nuclideTokenizedText.whitespace)(' '), (0, _nuclideTokenizedText.className)(cursor.name)];
  }
    ...
    var _ret = (function () {
      ...
      params.forEach(function (fparam) {
        if (paramTokens.length > 0) {
          paramTokens.push((0, _nuclideTokenizedText.plain)(', '));
        }
        paramTokens.push((0, _nuclideTokenizedText.param)(fparam));
      });
      var tparamTokens = [];
      if (tparams != null && tparams.length > 0) {
        tparamTokens.push((0, _nuclideTokenizedText.plain)('<'));
        tparams.forEach(function (tparam) {
          if (tparamTokens.length > 1) {
            tparamTokens.push((0, _nuclideTokenizedText.plain)(', '));
          }
          tparamTokens.push((0, _nuclideTokenizedText.plain)(tparam));
        });
        tparamTokens.push((0, _nuclideTokenizedText.plain)('>'));
      }
      return {
        v: [(0, _nuclideTokenizedText.method)(cursor.name)].concat(tparamTokens, [(0, _nuclideTokenizedText.plain)('(')], paramTokens, [(0, _nuclideTokenizedText.plain)(')')])
      };
    })();
  ...
  if (cursor.cursor_type != null) {
    return [].concat(_toConsumableArray(tokenizeType(cursor.cursor_type)), [(0, _nuclideTokenizedText.whitespace)(' '), (0, _nuclideTokenizedText.className)(cursor.name)]);
  }
  return [(0, _nuclideTokenizedText.plain)(cursor.name)];
}
...
function outlineFromClangOutline(outline) {
  return outline.map(function (cursor) {
    return {
      ...
      startPosition: new _atom.Point(cursor.extent.start.line, cursor.extent.start.column),
      endPosition: new _atom.Point(cursor.extent.end.line, cursor.extent.end.column),
...
var OutlineViewProvider = (function () {
  ...
  _createDecoratedClass(OutlineViewProvider, null, [{
    key: 'getOutline',
    decorators: [(0, _nuclideAnalytics.trackTiming)('nuclide-clang-atom:outline-view')],
    value: _asyncToGenerator(function* (editor) {
      var clangOutline = yield (0, _libclang.getOutline)(editor);
