...
function getSimulatorPlatform(buckRoot, ruleType, debuggerCallback) {
  return (_nuclideFbsimctl || _load_nuclideFbsimctl()).getFbsimctlSimulators().map(simulators => {
    let deviceGroups;
...
function getDevicePlatform(buckRoot, ruleType, debuggerCallback) {
  return (_nuclideFbsimctl || _load_nuclideFbsimctl()).getFbsimctlDevices().map(devices => {
    const deviceGroups = [];
