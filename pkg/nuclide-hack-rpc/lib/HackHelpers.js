...
async function callHHClient(args, errorStream, processInput, filePath) {
  ...
  const hackExecOptions = await (0, (_hackConfig || _load_hackConfig()).getHackExecOptions)(filePath);
  if (!hackExecOptions) {
    return null;
  }
  const { hackRoot, hackCommand } = hackExecOptions;

  return (0, (_nuclideAnalytics || _load_nuclideAnalytics()).trackTiming)(trackingIdOfHackArgs(args) + ':plus-queue', () => {
    if (!hhPromiseQueue) {
    ...
    return hhPromiseQueue.submit(async () => {
      ...
      execResult = await (0, (_nuclideAnalytics || _load_nuclideAnalytics()).trackTiming)(trackingIdOfHackArgs(args), () => {
