...
class HackProcess {
  ...
  async getAutocompleteSuggestions(fileVersion, position, activatedManually) {
    ...
    const unfilteredItems = await (await service).getCompletions(filePath, { line, column });
...
async function getHackProcess(fileCache, filePath) {
  const configDir = await (0, (_hackConfig || _load_hackConfig()).findHackConfigDir)(filePath);
  if (configDir == null) {
...
async function retryCreateHackProcess(fileCache, hackRoot) {
  ...
  while (hackProcess == null) {
    try {
      ...
      await (0, (_promise || _load_promise()).sleep)(waitTimeMs);
      waitTimeMs *= 2;
...
async function createHackProcess(fileCache, configDir) {
  const command = await (0, (_hackConfig || _load_hackConfig()).getHackCommand)();
  if (command === '') {
  ...
  try {
    await (0, (_process || _load_process()).runCommand)(command, ['start', configDir], {
      isExitError: ({ exitCode }) => !(exitCode === 0 || exitCode === HACK_SERVER_ALREADY_EXISTS_EXIT_CODE)
    }).toPromise();
  } catch (err) {
