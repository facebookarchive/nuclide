...
let HackProcess = class HackProcess extends (_nuclideRpc || _load_nuclideRpc()).RpcProcess {
  ...
  getBufferAtVersion(fileVersion) {
    ...
    return (0, _asyncToGenerator.default)(function* () {
      ...
      yield _this._fileVersionNotifier.waitForBufferAtVersion(fileVersion);

      if (!(buffer.changeCount === fileVersion.version)) {
        throw new Error('File changed waiting for edits to be sent to Hack');
      }
...
function editToHackEdit(editEvent) {
  ...
  const end = _editEvent$oldRange.end;
