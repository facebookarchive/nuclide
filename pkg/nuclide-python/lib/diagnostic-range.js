...
getDiagnosticRange = getDiagnosticRange;var _atom = require('atom');var _range;function _load_range() {return _range = require('nuclide-commons-atom/range');}var _log4js;function _load_log4js() {return _log4js = require('log4js');} /**
                                                                                                                                                                                                                                         ...
                                                                                                                                                                                                                                         * 
                                                                                                                                                                                                                                         * @format
                                                                                                                                                                                                                                         */const logger = (0, (_log4js || _load_log4js()).getLogger)('nuclide-python');function tokenizedLineForRow(editor, line) /* atom$TokenizedLine */{const tokenBuffer = editor.hasOwnProperty('displayBuffer') ? editor.displayBuffer.tokenizedBuffer : editor.tokenizedBuffer;return tokenBuffer.tokenizedLineForRow(line);} // Finds the range of the module name from a pyflakes F4XX message.
// Assumes that the module name exists.
// Ported from https://github.com/AtomLinter/linter-flake8
function getModuleNameRange(message, line, editor) {// Split on space or dot to get the basename or alias, i.e. retrieve <a> in
  // "from .. import <a>" or "from .. import .. as <a>".
  const match = /'([^']+)'/.exec(message);if (match == null) {return null;}const symbol = match[1].split(/\s|\./).pop();let foundImport = false;let lineNumber = line;for (;;) {let offset = 0;const tokenizedLine = tokenizedLineForRow(editor, lineNumber);if (!tokenizedLine) {break;}for (let i = 0; i < tokenizedLine.tokens.length; i++) {const token = tokenizedLine.tokens[i];if (foundImport && token.value === symbol) {return new _atom.Range([lineNumber, offset], [lineNumber, offset + token.value.length]);}if (token.value === 'import' && token.scopes.indexOf('keyword.control.import.python') >= 0) {foundImport = true;}offset += token.value.length;}lineNumber += 1;}logger.warn(`getModuleNameRange failed for message: ${message}`);} // Computes an appropriate underline range using the diagnostic type information.
...
function getDiagnosticRange(diagnostic, editor) {const buffer = editor.getBuffer(); // The diagnostic message's line index may be out of bounds if buffer contents
  ...
  const { code, line: unsafeLine, column, message } = diagnostic;const lastRow = buffer.getLastRow();const line = unsafeLine <= lastRow ? unsafeLine : lastRow;const lineLength = buffer.lineLengthForRow(line);const trimmedRange = (0, (_range || _load_range()).trimRange)(editor, buffer.rangeForRow(line, false));const trimmedStartCol = trimmedRange.start.column;const trimmedEndCol = trimmedRange.end.column;try {switch (code.slice(0, 2)) {// pep8 - indentation
      case 'E1':
      case 'E9':
      case 'W1':
