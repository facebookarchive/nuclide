...
function getModuleNameRange(message, line, editor) {
  ...
  var symbol = /'([^']+)'/.exec(message)[1].split(/\s|\./).pop();
  ...
  (0, (_assert2 || _assert()).default)(false, 'getModuleNameRange, module not found: ' + symbol);
}
...
function getDiagnosticRange(diagnostic, editor) {
  ...
  try {
    ...
    logger.error('Failed to find flake8 diagnostic range: ' + diagnosticAsString + ', Error: ' + e.message);
  }
