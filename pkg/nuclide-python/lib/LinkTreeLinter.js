...
function _load_debounced() {return _debounced = require('../../../modules/nuclide-commons-atom/debounced');}var _event;
...
function _load_shallowequal() {return _shallowequal = _interopRequireDefault(require('shallowequal'));}var _nuclideAnalytics;
...
const NUM_SUGGESTIONS = 3;class LinkTreeLinter {constructor() {this.
    _disposedPaths = new Set();} // Once the user interacts with a diagnostic, hide it forever.
  ...
  observeMessages() {
    return (0, (_debounced || _load_debounced()).observeActiveEditorsDebounced)(DEBOUNCE_INTERVAL).
    ...
    switchMap(editor => {
      ...
      !(_constants || _load_constants()).GRAMMAR_SET.has(editor.getGrammar().scopeName))
      {
      ...
      return _rxjsBundlesRxMinJs.Observable.fromPromise(pythonService.getBuildableTargets(path)).
      ...
      switchMap(targets => {
        ...
        const taskCompleted = (0, (_event || _load_event()).observableFromSubscribeFunction)(cb =>
        ...
        concat(_rxjsBundlesRxMinJs.Observable.never()).
        takeUntil(disposed).
        takeUntil(taskCompleted);
      }).
      takeUntil(
      (0, (_event || _load_event()).observableFromSubscribeFunction)(cb => editor.onDidDestroy(cb))).
