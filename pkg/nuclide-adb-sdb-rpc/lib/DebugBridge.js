...
class DebugBridge {
  ...
  runLongAdbCommand(device, command) {
    // TODO(T17463635)
    const deviceArg = device !== '' ? ['-s', device] : [];
    return (0, (_process || _load_process()).observeProcess)(this._adbPath, deviceArg.concat(command), { killTreeOnComplete: true, /* TODO(T17353599) */isExitError: () => false }).catch(error => _rxjsBundlesRxMinJs.Observable.of({ kind: 'error', error })); // TODO(T17463635)
  }
