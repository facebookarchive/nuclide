...
const bridge = new (_DebugBridge || _load_DebugBridge()).DebugBridge((0, (_Store || _load_Store()).createConfigObs)('sdb'));
...
class Sdb {
  ...
  static getDeviceList() {
    return bridge.getDevices().switchMap(devices => {
      return _rxjsBundlesRxMinJs.Observable.concat(...devices.map(name => {
        ...
        return _rxjsBundlesRxMinJs.Observable.forkJoin(sdb.getDeviceArchitecture().catch(() => _rxjsBundlesRxMinJs.Observable.of('')), sdb.getAPIVersion().catch(() => _rxjsBundlesRxMinJs.Observable.of('')), sdb.getDeviceModel().catch(() => _rxjsBundlesRxMinJs.Observable.of(''))).map(([architecture, apiVersion, model]) => ({
          name,
          architecture,
          apiVersion,
          model
  ...
  getPidFromPackageName(packageName) {
    ...
    return (0, _asyncToGenerator.default)(function* () {
      const pidLine = (yield _this2.runShortCommand('shell', 'ps', '|', 'grep', '-i', packageName).toPromise()).split(_os.default.EOL)[0];
      if (pidLine == null) {
