...
function getSuggestionForWord(textEditor, text, range) {
  ...
  var suggestion = {
    ...
    callback: function callback() {
      (_electron || _load_electron()).shell.openExternal(packageUrl);
    }
  };
...
function isPackageJson(textEditor) {
  ...
  return scopeName === 'source.json' && filePath != null && (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.basename(filePath) === 'package.json';
}
...
function getDependencyVersion(json, range) {
  var ast = (0, (_parsing || _load_parsing()).parseJSON)(json);
  if (ast == null) {
    // parse error
    return null;
  }
...
function getPathToNodeForRange(objectExpression, range) {
  ...
  for (var property of properties) {
    var propertyRange = (0, (_parsing || _load_parsing()).babelLocToRange)(property.loc);
    if (propertyRange.containsRange(range)) {
      var keyRange = (0, (_parsing || _load_parsing()).babelLocToRange)(property.key.loc);
