...
var DiagnosticStore = (function () {
  function DiagnosticStore() {
    ...
    this._fileToProviders = new (_commonsNodeCollection || _load_commonsNodeCollection()).MultiMap();
    this._providerToProjectDiagnostics = new Map();

    this._fileChanges = new (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Subject();
    this._projectChanges = new (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).BehaviorSubject([]);
    this._allChanges = new (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).BehaviorSubject([]);
  ...
  _createClass(DiagnosticStore, [{
    ...
    value: function _updateFileMessages(diagnosticProvider, newFilePathsToMessages) {
      ...
      newFilePathsToMessages.forEach(function (newMessagesForPath, filePath) {
        ...
        (0, (_assert || _load_assert()).default)(fileToMessages != null);
    ...
    value: function _invalidateSingleMessage(message) {
      ...
      for (var fileToMessages of this._providerToFileToMessages.values()) {
        ...
        if (fileMessages != null) {
          (0, (_commonsNodeCollection || _load_commonsNodeCollection()).arrayRemove)(fileMessages, message);
        }
      }
    ...
    value: function getFileMessageUpdates(filePath) {
      ...
      var initialObservable = (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.of({ filePath: filePath, messages: fileMessages });
      return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.concat(initialObservable, this._fileChanges.filter(function (change) {
    ...
    value: function onFileMessagesDidUpdate(callback, filePath) {
      return new (_commonsNodeUniversalDisposable || _load_commonsNodeUniversalDisposable()).default(this.getFileMessageUpdates(filePath).subscribe(callback));
    }
    ...
    value: function onProjectMessagesDidUpdate(callback) {
      return new (_commonsNodeUniversalDisposable || _load_commonsNodeUniversalDisposable()).default(this.getProjectMessageUpdates().subscribe(callback));
    }
    ...
    value: function onAllMessagesDidUpdate(callback) {
      return new (_commonsNodeUniversalDisposable || _load_commonsNodeUniversalDisposable()).default(this.getAllMessageUpdates().subscribe(callback));
    }
    ...
    value: function _getFileMessages(filePath) {
      ...
      for (var provider of relevantProviders) {
        ...
        (0, (_assert || _load_assert()).default)(_messages != null);
    ...
    value: function applyFixesForFile(file) {
      var messages = this._getFileMessages(file).filter(function (msg) {
        return msg.fix != null && msg.fix.speculative !== true;
      });
      this._applyFixes.apply(this, [file].concat(_toConsumableArray(messages)));
    }

    // Precondition: all messages have the given filePath
  }, {
    key: '_applyFixes',
    value: function _applyFixes(filePath) {
      for (var _len = arguments.length, messages = Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
        messages[_key - 1] = arguments[_key];
      }

      var messagesWithFixes = messages.filter(function (msg) {
        return msg.fix != null;
      });
      var fixes = [];
      for (var message of messagesWithFixes) {
        var fix = this._getUpdatedFix(message);
        if (fix == null) {
          notifyFixFailed();
          return;
        }
      ...
      var succeeded = (0, (_nuclideTextedit || _load_nuclideTextedit()).default).apply(undefined, [filePath].concat(fixes));
      if (succeeded) {
        for (var message of messagesWithFixes) {
    ...
    key: '_getUpdatedFix',
    value: function _getUpdatedFix(message) {
      var fix = message.fix;
      (0, (_assert || _load_assert()).default)(fix != null);
