...
var DiagnosticStore = (function () {
  function DiagnosticStore() {
    ...
    this._fileChanges = new (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Subject();
    this._projectChanges = new (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).BehaviorSubject([]);
    this._allChanges = new (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).BehaviorSubject([]);
  ...
  _createClass(DiagnosticStore, [{
    ...
    value: function getFileMessageUpdates(filePath) {
      ...
      var initialObservable = (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.of({ filePath: filePath, messages: fileMessages });
      return (_rxjsBundlesRxMinJs2 || _rxjsBundlesRxMinJs()).Observable.concat(initialObservable, this._fileChanges.filter(function (change) {
    ...
    value: function onFileMessagesDidUpdate(callback, filePath) {
      return new (_commonsNodeUniversalDisposable2 || _commonsNodeUniversalDisposable()).default(this.getFileMessageUpdates(filePath).subscribe(callback));
    }
    ...
    value: function onProjectMessagesDidUpdate(callback) {
      return new (_commonsNodeUniversalDisposable2 || _commonsNodeUniversalDisposable()).default(this.getProjectMessageUpdates().subscribe(callback));
    }
    ...
    value: function onAllMessagesDidUpdate(callback) {
      return new (_commonsNodeUniversalDisposable2 || _commonsNodeUniversalDisposable()).default(this.getAllMessageUpdates().subscribe(callback));
    }
    ...
    value: function _applyFixes(filePath) {
      ...
      var succeeded = (0, (_nuclideTextedit2 || _nuclideTextedit()).default).apply(undefined, [filePath].concat(fixes));
      if (succeeded) {
        for (var message of messagesWithFixes) {
