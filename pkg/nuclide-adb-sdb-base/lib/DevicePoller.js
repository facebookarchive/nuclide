...
class DevicePoller {
  ...
  observe(host) {
    ...
    observable = _rxjsBundlesRxMinJs.Observable.interval(3000).startWith(0).switchMap(() => this.fetch(host).map(devices => (_nuclideExpected || _load_nuclideExpected()).Expect.value(devices)).catch(() => _rxjsBundlesRxMinJs.Observable.of((_nuclideExpected || _load_nuclideExpected()).Expect.error(new Error(`Can't fetch ${this._getPlatform()} devices. Make sure that ${this._type} is in your $PATH and that it works properly.`))))).publishReplay(1).refCount();
