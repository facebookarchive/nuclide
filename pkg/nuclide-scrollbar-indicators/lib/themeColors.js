...
const COLOR_NAMES = ['backgroundColorHighlight', 'syntaxSelectionColor', 'syntaxCursorColor', 'syntaxGutterBackgroundColorSelected', 'syntaxTextColor'];

function getThemeChangeEvents() {
  return _RxMin.Observable.merge(atom.packages.hasActivatedInitialPackages() ? _RxMin.Observable.of(null) : _RxMin.Observable.empty(), (0, _event().observableFromSubscribeFunction)(cb => atom.packages.onDidActivateInitialPackages(cb)).mapTo(null), (0, _event().observableFromSubscribeFunction)(cb => atom.themes.onDidChangeActiveThemes(cb)) // TODO: It seems like the colors are not actually ready yet when
