...
var getHackRoot = _asyncToGenerator(function* (filePath) {
  return yield (_nuclideCommons2 || _nuclideCommons()).fsPromise.findNearestFile('.hhconfig', filePath);
});
...
var setRootDirectoryUri = _asyncToGenerator(function* (directoryUri) {
  ...
  var hackDummyRequestFilePath = (_path2 || _path()).default.join(hackRootDirectory ? hackRootDirectory : '', '/scripts/xdebug_includes.php');

  // Use hackDummyRequestFilePath if possible.
  if (yield (_nuclideCommons2 || _nuclideCommons()).fsPromise.exists(hackDummyRequestFilePath)) {
