...
class ArcBuildSystem {
  ...
  setProjectRoot(projectRoot, callback) {
    ...
    const storeReady = (0, (_event || _load_event()).observableFromSubscribeFunction)(this._model.onChange.bind(this._model)).map(() => this._model).startWith(this._model).filter(model =>
    // eslint-disable-next-line eqeqeq
    model.isArcSupported() !== null && model.getActiveProjectPath() === path);
