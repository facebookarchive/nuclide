...
var _commonsAtomOnWillDestroyTextBuffer;

function _load_commonsAtomOnWillDestroyTextBuffer() {
  return _commonsAtomOnWillDestroyTextBuffer = _interopRequireDefault(require('../../commons-atom/on-will-destroy-text-buffer'));
}
...
var ArcanistDiagnosticsProvider = (function () {
  function ArcanistDiagnosticsProvider(busySignalProvider) {
    ...
    this._subscriptions.add((0, (_commonsAtomOnWillDestroyTextBuffer || _load_commonsAtomOnWillDestroyTextBuffer()).default)(function (buffer) {
  ...
  _createDecoratedClass(ArcanistDiagnosticsProvider, [{
    ...
    key: '_runLint',
    decorators: [(0, (_nuclideAnalytics || _load_nuclideAnalytics()).trackTiming)('nuclide-arcanist:lint')],
    value: _asyncToGenerator(function* (textEditor) {
      var _this3 = this;

      var filePath = textEditor.getPath();
      (0, (_assert || _load_assert()).default)(filePath);
      try {
        var blacklistedLinters = (_commonsAtomFeatureConfig || _load_commonsAtomFeatureConfig()).default.get('nuclide-arcanist.blacklistedLinters');
        var result = yield this._requestSerializer.run((0, (_aggregateFindDiagnostics || _load_aggregateFindDiagnostics()).default)([filePath], blacklistedLinters));
        if (result.status === 'outdated') {
          return;
        }
        var diagnostics = result.result;
        var fileDiagnostics = diagnostics.map(function (diagnostic) {
          var range = new (_atom || _load_atom()).Range([diagnostic.row, diagnostic.col], [diagnostic.row, textEditor.getBuffer().lineLengthForRow(diagnostic.row)]);
          var text = undefined;
    ...
    value: function _getFix(diagnostic) {
      ...
      var _ref = (0, (_commonsNodeString || _load_commonsNodeString()).removeCommonSuffix)(diagnostic.original, diagnostic.replacement);
    ...
    value: function _getRangeForFix(startRow, startCol, originalText) {
      ...
      return new (_atom || _load_atom()).Range([startRow, startCol], [endRow, endCol]);
    }
  }, {
    key: 'onMessageUpdate',
