...
function replaceTargetsWithMatches(stringWithTargets, matches, projectRelativePath) {
  if (matches == null) {
    const basenameWithoutExtension = _path.default.basename(projectRelativePath, _path.default.extname(projectRelativePath));
  ...
  for (let i = 0; i < targets.length; i++) {
    if (i === targets.length - 1 && targets.length < matches.length) {
      replaced = replaced.replace('{}', _path.default.join(...matches.slice(i)));
    } else {
