...
exports.default = _asyncToGenerator(function* () {
  ...
  if (process.platform === 'darwin') {
    var result = yield (0, (_commonsNodeProcess || _load_commonsNodeProcess()).asyncExecute)('xcode-select', ['--print-path']);
    if (result.exitCode === 0) {
      libClangLibraryFile = result.stdout.trim();
      // If the user only has Xcode Command Line Tools installed, the path is different.
      if ((_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.basename(libClangLibraryFile) !== 'CommandLineTools') {
  ...
  var clangServerArgs = {
    ...
    pythonPathEnv: (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.join(__dirname, '../VendorLib')
  };
