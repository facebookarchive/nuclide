...
exports.findSourceFileForHeader = exports.getRelatedHeaderForSource = undefined;
...
let getRelatedHeaderForSourceFromFramework = (() => {
  var _ref2 = (0, _asyncToGenerator.default)(function* (src) {
    const frameworkStructure = getFrameworkStructure((_nuclideUri || _load_nuclideUri()).default.dirname(src));
...
let getRelatedHeaderForSource = exports.getRelatedHeaderForSource = (() => {
  ...
  return function getRelatedHeaderForSource(_x5) {
    return _ref3.apply(this, arguments);
  };
})();

let findSourceFileForHeader = exports.findSourceFileForHeader = (() => {
  var _ref4 = (0, _asyncToGenerator.default)(function* (header, projectRoot) {
    // Basic implementation: look at files in the same directory for paths
    // with matching file names.
    const source = yield searchFileWithBasename((_nuclideUri || _load_nuclideUri()).default.dirname(header), (0, (_utils || _load_utils()).getFileBasename)(header), (_utils || _load_utils()).isSourceFile);
    if (source != null) {
      return source;
    }
    // Try searching all subdirectories for source files that include this header.
    // Give up after INCLUDE_SEARCH_TIMEOUT.
    return findIncludingSourceFile(header, projectRoot).timeout(INCLUDE_SEARCH_TIMEOUT).catch(function () {
      return _rxjsBundlesRxMinJs.Observable.of(null);
    }).toPromise();
  });

  return function findSourceFileForHeader(_x6, _x7) {
...
function getFrameworkStructure(dir) {
  ...
  const frameworkPath = (_nuclideUri || _load_nuclideUri()).default.join(...paths.slice(sourcesIndex + 1).reverse());
