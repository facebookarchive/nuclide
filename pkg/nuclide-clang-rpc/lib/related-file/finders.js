...
function _load_buckFinder() {
  return _buckFinder = require('./buck-finder');
}
...
class RelatedFileFinder {
  constructor() {
    this._sourceForHeaderCache = new (_cache || _load_cache()).Cache({ keyFactory: key => JSON.stringify(key) });
  }
  ...
  getRelatedSourceForHeader(header, projectRoot) {
    ...
    return (0, _asyncToGenerator.default)(function* () {
      const { source, time } = yield _this._sourceForHeaderCache.getOrCreate({ header, projectRoot }, function () {
        return _this._getRelatedSourceForHeaderImpl(header, projectRoot).then(function (src) {
          return {
            source: src,
            time: Date.now()
          };
  ...
  _getRelatedSourceForHeaderImpl(header, projectRoot) {
    var _this2 = this;
    ...
    return (0, _asyncToGenerator.default)(function* () {
      ...
      source = yield (0, (_grepFinder || _load_grepFinder()).findIncludingSourceFile)(header, _this2._inferProjectRoot(header)).toPromise();
