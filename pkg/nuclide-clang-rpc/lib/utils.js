...
function isHeaderFile(filename) {
  return HEADER_EXTENSIONS.has((_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.extname(filename));
}

function isSourceFile(filename) {
  return SOURCE_EXTENSIONS.has((_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.extname(filename));
}
...
function processGrepResult(result, headerFile, includeRegex) {
  ...
  if (match[2] != null) {
    var includePath = (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.normalize((_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.join((_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.dirname(filename), match[1]));
...
function findIncludingSourceFile(headerFile, projectRoot) {
  var basename = (0, (_escapeStringRegexp || _load_escapeStringRegexp()).default)((_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.basename(headerFile));
  var relativePath = (0, (_escapeStringRegexp || _load_escapeStringRegexp()).default)((_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.relative(projectRoot, headerFile));
  ...
  var spawnGrepProcess = function spawnGrepProcess() {
    ...
    return (0, (_commonsNodeProcess || _load_commonsNodeProcess()).safeSpawn)('grep', ['-RE', // recursive, extended
    '--null', // separate file/match with \0
    pattern, (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.dirname(headerFile)]);
  };
  return (0, (_commonsNodeProcess || _load_commonsNodeProcess()).observeProcess)(spawnGrepProcess).flatMap(function (message) {
    switch (message.kind) {
      case 'stdout':
        var file = processGrepResult(message.data, headerFile, regex);
        return file == null ? (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.empty() : (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.of(file);
      case 'error':
