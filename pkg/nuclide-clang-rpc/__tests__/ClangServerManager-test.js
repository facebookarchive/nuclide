...
function _load_ClangServerManager() {
  return _ClangServerManager = _interopRequireDefault(require('../lib/ClangServerManager'));
}
...
describe('ClangServerManager', () => {
  ...
  let getFlagsSpy;
  beforeEach(() => {
    serverManager = new (_ClangServerManager || _load_ClangServerManager()).default();
  ...
  it('uses flags from manager if available', async () => {
    serverManager._flagsManager.getFlagsForSrc.mockReturnValue(Promise.resolve({ flags: ['a'] }));
  ...
  it('falls back to default flags with only flags file without flags', async () => {
    ...
    getFlagsSpy.mockImplementation(() => Promise.resolve({ flags: [], directory: '.', flagsFile: 'TARGET' }));
    const flagsResult = await serverManager._getFlags('test.cpp', emptyRequestSettings, serverSettings);
    expect(flagsResult).toEqual({
      flags: ['b'],
      usesDefaultFlags: true,
  ...
  it('enforces a memory limit', async () => {
    ...
    jest.spyOn(processModule, 'runCommand').mockImplementation((command, ...args) => {
      if (command === 'ps') {
        return _rxjsBundlesRxMinJs.Observable.of(serverManager._servers.values().map(server => server.getPID()).filter(Boolean).map(pid => `${pid}\t1000`).join('\n'));
      }
    ...
    server = serverManager.getClangServer('test.cpp', '', null, emptyServerSettings);
    ...
    await (0, (_waits_for || _load_waits_for()).default)(() => server2.isReady(), 'server2 to become ready');
    ...
    await (0, (_waits_for || _load_waits_for()).default)(() => server3.isReady(), 'server3 to become ready');
