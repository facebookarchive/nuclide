...
function syncPaneItemVisibility(panesStream, visibilityStream) {
  var activeItemsStream = panesStream.switchMap(function (panes_) {
    ...
    var activeItemChanges = panes.map(function (pane) {
      return (0, (_commonsNodeEvent || _load_commonsNodeEvent()).observableFromSubscribeFunction)(pane.observeActiveItem.bind(pane));
    });
    return (_Observable = (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable).merge.apply(_Observable, _toConsumableArray(activeItemChanges)).map(function () {
      return new Set((0, (_commonsNodeCollection || _load_commonsNodeCollection()).arrayCompact)(panes.map(function (pane) {
  ...
  (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.combineLatest(activeItemsStream, visibilityStream).subscribe(function (_ref) {
  ...
  activeItemsStream.pairwise().withLatestFrom(panesStream).subscribe(function (_ref3) {
    ...
    var deactivatedItems = (0, (_commonsNodeCollection || _load_commonsNodeCollection()).setDifference)(prev || new Set(), next || new Set());
    var panes = Array.from(panes_);
