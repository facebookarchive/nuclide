...
function _observeGrammarForTextEditors() {
  const data = _interopRequireDefault(require("../../commons-atom/observe-grammar-for-text-editors"));
  ...
  provideCodeActions() {
    ...
    return {
      ...
      async getCodeActions(editor, range, diagnostics) {
        ...
        if (provider == null || diagnostics.length > 0 || !(await (0, _passesGK().default)('nuclide_refactorizer_code_actions'))) {
          return [];
        }

        const refactors = await provider.refactorings(editor, range);
        return refactors.filter(refactor => {
          ...
          return {
            async apply() {
              return store.dispatch(Actions().inlinePickedRefactor(editor, range, provider, refactor));
            },
