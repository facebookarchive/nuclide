...
describe('BuckBuildSystem', () => {
  ...
  describe('_consumeEventStream', () => {
    ...
    it.skip('emits diagnostics', async () => {
      const spy = jest.fn();
      const subscription = buckBuildSystem.getDiagnosticProvider().updates.subscribe(spy);
      ...
      expect(spy.mock.calls.map(call => call[0])).toEqual([new Map([['a', [diagnostic]]]), new Map([['a', [diagnostic, Object.assign({}, diagnostic, { type: 'Warning' })]]]),
