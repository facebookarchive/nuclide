...
var getDevices = (0, (_lodashMemoize2 || _lodashMemoize()).default)(function () {
  return (0, (_nuclideCommons2 || _nuclideCommons()).observeProcess)(function () {
    return (0, (_nuclideCommons2 || _nuclideCommons()).safeSpawn)('xcrun', ['simctl', 'list', 'devices']);
  }).map(function (event) {
    // Throw errors.
    if (event.kind === 'error') {
      var error = new Error();
    ...
    return(
      ...
      error.name === 'XcrunError' ? (_rxjs2 || _rxjs()).Observable.of([]) : (_rxjs2 || _rxjs()).Observable.throw(error)
    );
  }).share();
});
