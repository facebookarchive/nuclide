  ...
  var eventStream = socketStream.flatMap(function (message) {
    switch (message.type) {
      case 'SocketConnected':
        return (_rxjsBundlesRxUmdMinJs2 || _rxjsBundlesRxUmdMinJs()).Observable.of({ type: 'socket-connected' });
      case 'ParseStarted':
...
function getDiagnosticEvents(events, buckRoot) {
  return events.flatMap(function (event) {
    ...
    if (event.type === 'log') {
      return (_rxjsBundlesRxUmdMinJs2 || _rxjsBundlesRxUmdMinJs()).Observable.fromPromise((0, (_getDiagnostics2 || _getDiagnostics()).default)(event.message, event.level, buckRoot)).map(function (diagnostics) {
