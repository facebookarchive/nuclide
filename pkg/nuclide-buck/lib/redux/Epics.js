...
function _load_PlatformService() {
  return _PlatformService = require('../PlatformService.js');
}
...
function setPlatforms(platforms) {
  return { type: (_Actions || _load_Actions()).SET_PLATFORMS, platforms };
}
...
function setRuleTypeEpic(actions, store) {
  return actions.ofType((_Actions || _load_Actions()).SET_RULE_TYPE).switchMap(action => {
    ...
    if (ruleType) {
      return (0, (_PlatformService || _load_PlatformService()).platformsForRuleType)(ruleType).map(platforms => setPlatforms(platforms));
    } else {
      return _rxjsBundlesRxMinJs.Observable.of(setPlatforms(null));
    }
  });
}