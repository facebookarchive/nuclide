'use strict';Object.defineProperty(exports, "__esModule", { value: true });exports.BuckTaskRunner = exports.CONSOLE_VIEW_URI = undefined;exports.
...
isDebugTask = isDebugTask;exports.



getBuckSubcommandForTaskType = getBuckSubcommandForTaskType;var _DeploymentTarget;function _load_DeploymentTarget() {return _DeploymentTarget = require('./DeploymentTarget');}var _PlatformService;function _load_PlatformService() {return _PlatformService = require('./PlatformService');}var _redux;function _load_redux() {return _redux = require('redux');}var _rxjsBundlesRxMinJs = require('rxjs/bundles/Rx.min.js');var _tasks;function _load_tasks() {return _tasks = require('../../commons-node/tasks');}var _nuclideArtillery;function _load_nuclideArtillery() {return _nuclideArtillery = require('../../nuclide-artillery');}var _BuckBuildSystem;function _load_BuckBuildSystem() {return _BuckBuildSystem = require('./BuckBuildSystem');}var _UniversalDisposable;function _load_UniversalDisposable() {return _UniversalDisposable = _interopRequireDefault(require('nuclide-commons/UniversalDisposable'));}var _reduxObservable;function _load_reduxObservable() {return _reduxObservable = require('nuclide-commons/redux-observable');}var _bindObservableAsProps;function _load_bindObservableAsProps() {return _bindObservableAsProps = require('nuclide-commons-ui/bindObservableAsProps');}var _log4js;function _load_log4js() {return _log4js = require('log4js');}var _Icon;function _load_Icon() {return _Icon = require('nuclide-commons-ui/Icon');}var _Actions;function _load_Actions() {return _Actions = _interopRequireWildcard(require('./redux/Actions'));}var _Epics;function _load_Epics() {return _Epics = _interopRequireWildcard(require('./redux/Epics'));}var _Reducers;function _load_Reducers() {return _Reducers = _interopRequireDefault(require('./redux/Reducers'));}var _BuckToolbar;function _load_BuckToolbar() {return _BuckToolbar = _interopRequireDefault(require('./BuckToolbar'));}var _react = _interopRequireWildcard(require('react'));var _collection;function _load_collection() {return _collection = require('nuclide-commons/collection');}var _shallowequal;function _load_shallowequal() {return _shallowequal = _interopRequireDefault(require('shallowequal'));}function _interopRequireWildcard(obj) {if (obj && obj.__esModule) {return obj;} else {var newObj = {};if (obj != null) {for (var key in obj) {if (Object.prototype.hasOwnProperty.call(obj, key)) newObj[key] = obj[key];}}newObj.default = obj;return newObj;}}function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}const TASKS = [{ type: 'build', label: 'Build', description: 'Build the specified Buck target', icon: 'tools' }, { type: 'run', label: 'Run', description: 'Run the specfied Buck target', icon: 'triangle-right' }, { type: 'test', label: 'Test', description: 'Test the specfied Buck target', icon: 'check' }, { type: 'debug', label: 'Debug', description: 'Debug the specfied Buck target', icon: 'nuclicon-debugger' }, { type: 'debug-attach', label: 'Attach & Debug', description: 'Attach and debug the specfied Buck target', icon: 'nuclicon-debugger' }]; // This must match URI defined in ../../nuclide-console/lib/ui/ConsoleContainer
/**
 ...
 * 
 * @format
 */const CONSOLE_VIEW_URI = exports.CONSOLE_VIEW_URI = 'atom://nuclide/console';function shouldEnableTask(taskType, ruleType) {switch (taskType) {case 'build':case 'test':return true;case 'run':return ruleType.endsWith('binary');case 'debug':case 'debug-attach':return ruleType.endsWith('binary') || ruleType.endsWith('test');default:return false;}}function isDebugTask(taskType) {return taskType.startsWith('debug');}function getBuckSubcommandForTaskType(taskType) {if (!(taskType === 'build' || taskType === 'run' || taskType === 'test')) {throw new Error('Invariant violation: "taskType === \'build\' || taskType === \'run\' || taskType === \'test\'"');}return taskType;}class BuckTaskRunner {
  ...
  getExtraUi() {
    if (this._extraUi == null) {
      ...
      const boundActions = {
        ...
        store.dispatch((_Actions || _load_Actions()).setBuildTarget(buildTarget)),
        setDeploymentTarget: deploymentTarget =>
        store.dispatch((_Actions || _load_Actions()).setDeploymentTarget(deploymentTarget)),
        setTaskSettings: settings =>
        store.dispatch((_Actions || _load_Actions()).setTaskSettings(settings)) };

      this._extraUi = (0, (_bindObservableAsProps || _load_bindObservableAsProps()).bindObservableAsProps)(
      // $FlowFixMe: type symbol-observable
      _rxjsBundlesRxMinJs.Observable.from(store).
      map(appState => Object.assign({ appState }, boundActions)).
      filter(props => props.appState.buckRoot != null), (_BuckToolbar || _load_BuckToolbar()).default);
  ...
  getIcon() {
    return () =>
    _react.createElement((_Icon || _load_Icon()).Icon, { icon: 'nuclicon-buck', className: 'nuclide-buck-task-runner-icon' });
  ...
  {
    // $FlowFixMe: type symbol-observable
    const storeReady = _rxjsBundlesRxMinJs.Observable.from(this._getStore()).
    ...
    const subscription = _rxjsBundlesRxMinJs.Observable.combineLatest(
    enabledObservable,
    tasksObservable).
  ...
  _getStore() {
    if (this._store == null) {if (!(
      this._serializedState != null)) {throw new Error('Invariant violation: "this._serializedState != null"');}
      const initialState = {
        platformGroups: [],
      ...
      (0, (_reduxObservable || _load_reduxObservable()).combineEpics)(...epics)(actions, store)
      // Log errors and continue.
      .catch((err, stream) => {
        (0, (_log4js || _load_log4js()).getLogger)('nuclide-buck').error(err);
        return stream;
      });
      this._store = (0, (_redux || _load_redux()).createStore)((_Reducers || _load_Reducers()).default,

      initialState,
      (0, (_redux || _load_redux()).applyMiddleware)((0, (_reduxObservable || _load_reduxObservable()).createEpicMiddleware)(rootEpic)));
  ...
  runTask(taskType) {
    ...
    const {
      ...
      taskSettings } =
    state;if (!(
    buckRoot != null)) {throw new Error('Invariant violation: "buckRoot != null"');}if (!
    buildRuleType) {throw new Error('Invariant violation: "buildRuleType"');}

    const deploymentTargetString = (0, (_DeploymentTarget || _load_DeploymentTarget()).formatDeploymentTarget)(
    selectedDeploymentTarget);

    const deploymentString =
    deploymentTargetString === '' ? '' : ` on "${deploymentTargetString}"`;

    const task = (0, (_tasks || _load_tasks()).taskFromObservable)(
    _rxjsBundlesRxMinJs.Observable.concat(
    (0, (_tasks || _load_tasks()).createMessage)(
