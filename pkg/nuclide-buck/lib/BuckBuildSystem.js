...
var BuckBuildSystem = (function () {
  function BuckBuildSystem(initialState) {
    ...
    this._diagnosticUpdates = new (_rxjsBundlesRxUmdMinJs2 || _rxjsBundlesRxUmdMinJs()).Subject();
    this._diagnosticInvalidations = new (_rxjsBundlesRxUmdMinJs2 || _rxjsBundlesRxUmdMinJs()).Subject();
  ...
  _createClass(BuckBuildSystem, [{
    ...
    value: function _runTaskType(taskType) {
      ...
      return (_rxjsBundlesRxUmdMinJs2 || _rxjsBundlesRxUmdMinJs()).Observable.fromPromise(buckProject.getHTTPServerPort()).catch(function (err) {
        ...
        socketEvents == null ? (_rxjsBundlesRxUmdMinJs2 || _rxjsBundlesRxUmdMinJs()).Observable.empty() : socketEvents.filter(function (event) {
          return event.type === 'socket-connected';
        }).take(1).timeout(SOCKET_TIMEOUT, Error('Timed out connecting to Buck server.')).ignoreElements(), _this3._consumeEventStream((_rxjsBundlesRxUmdMinJs2 || _rxjsBundlesRxUmdMinJs()).Observable.merge(mergedEvents, (_commonsAtomFeatureConfig2 || _commonsAtomFeatureConfig()).get('nuclide-buck.compileErrorDiagnostics') ? (0, (_BuckEventStream2 || _BuckEventStream()).getDiagnosticEvents)(mergedEvents, buckRoot) : (_rxjsBundlesRxUmdMinJs2 || _rxjsBundlesRxUmdMinJs()).Observable.empty(), isDebug && subcommand === 'install' ? (0, (_LLDBEventStream2 || _LLDBEventStream()).getLLDBInstallEvents)(processMessages, buckProject) : (_rxjsBundlesRxUmdMinJs2 || _rxjsBundlesRxUmdMinJs()).Observable.empty(), isDebug && subcommand === 'build' ? (0, (_LLDBEventStream2 || _LLDBEventStream()).getLLDBBuildEvents)(processMessages, buckProject, buildTarget, settings.runArguments || []) : (_rxjsBundlesRxUmdMinJs2 || _rxjsBundlesRxUmdMinJs()).Observable.empty())));
      }).finally(function () {
    ...
    value: function _consumeEventStream(events) {
      ...
      return (0, (_commonsNodeStream2 || _commonsNodeStream()).compact)(events.do(function (event) {
