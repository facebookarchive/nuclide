  ...
  _createClass(BuckBuildSystem, [{
    ...
    value: function _runTaskType(taskType) {
      ...
      return (_rxjsBundlesRxUmdMinJs2 || _rxjsBundlesRxUmdMinJs()).Observable.fromPromise(buckProject.getHTTPServerPort()).catch(function (err) {
        ...
        var buckObservable = (_rxjsBundlesRxUmdMinJs2 || _rxjsBundlesRxUmdMinJs()).Observable.fromPromise(_this2._runBuckCommand(buckProject, buildTarget, subcommand, taskType === 'debug'));
        return socketStream.merge(buckObservable).takeUntil(buckObservable);
      }).share();
    }
  }, {
    key: '_runBuckCommand',
    value: _asyncToGenerator(function* (buckProject, buildTarget, subcommand, debug) {
