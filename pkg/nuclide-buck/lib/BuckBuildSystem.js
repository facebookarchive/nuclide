...
class BuckBuildSystem {
  ...
  _getBuckTargetType(buckService, buckRoot, buildTarget, buildArguments) {
    return (0, _asyncToGenerator.default)(function* () {
      const output = yield buckService.showOutput(buckRoot, buildTarget, buildArguments);
      if (output.length === 1) {
  ...
  runSubcommand(buckRoot, subcommand, buildTarget, taskSettings, isDebug, udid) {
    ...
    return _rxjsBundlesRxMinJs.Observable.fromPromise(this._getBuckTargetType(buckService, buckRoot, targetString, buildArguments)).flatMap(targetType => {
