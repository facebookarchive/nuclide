...
let BuckBuildSystem = exports.BuckBuildSystem = class BuckBuildSystem {
  ...
  observeTaskList(cb) {
    if (this._tasks == null) {
      ...
      this._tasks = (0, (_event || _load_event()).observableFromSubscribeFunction)(store.subscribe.bind(store)).startWith(null)
      // Wait until we're done loading the buck project.
      .filter(() => !store.isLoadingBuckProject()).map(() => this.getTaskList());
    }
