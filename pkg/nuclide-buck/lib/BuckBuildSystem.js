...
class BuckBuildSystem {
  ...
  runSubcommand(buckRoot, subcommand, buildTarget, taskSettings, isDebug, udid, processEventCallback, skipLaunchAfterInstall = false) {
    ...
    return _RxMin.Observable.fromPromise(Promise.all([buckService.getHTTPServerPort(buckRoot), (0, _passesGK().default)('nuclide_buck_superconsole')])).switchMap(([httpPort, useSuperconsole]) => {
  ...
  */
  ...
  _processStatusEvent(event) {
    ...
    const result = event.reset ? _RxMin.Observable.of({
      type: 'status',
      status: {
        type: 'bulletin',
        bulletin: {
