...
class BuckBuildSystem {
  ...
  getExtraUi() {
    if (this._extraUi == null) {
      ...
      const boundActions = {
        ...
        setDevice: device => store.dispatch((_Actions || _load_Actions()).setDevice(device)),
  ...
  runTask(taskType) {
    ...
    const resultStream = this._runTaskType(taskType, state.buckRoot, fullTargetName, state.taskSettings[taskType] || {}, isInstallableRule(taskType, state.buildRuleType), udid);
