'use strict';Object.defineProperty(exports, "__esModule", { value: true });exports.BuckBuildSystem = undefined;
...
function _load_nuclideUri() {return _nuclideUri = _interopRequireDefault(require('../../../modules/nuclide-commons/nuclideUri'));}var _tasks;
function _load_tasks() {return _tasks = require('../../commons-node/tasks');}var _log4js;




function _load_log4js() {return _log4js = require('log4js');}var _nuclideRemoteConnection;
function _load_nuclideRemoteConnection() {return _nuclideRemoteConnection = require('../../nuclide-remote-connection');}var _featureConfig;
function _load_featureConfig() {return _featureConfig = _interopRequireDefault(require('../../../modules/nuclide-commons-atom/feature-config'));}var _BuckEventStream;
function _load_BuckEventStream() {return _BuckEventStream = require('./BuckEventStream');}function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };} /**
...
class BuckBuildSystem {constructor() {this.
    _diagnosticUpdates = new _rxjsBundlesRxMinJs.Subject();this.
    _diagnosticInvalidations =
...

  {
    ...
    return _rxjsBundlesRxMinJs.Observable.fromPromise(buckService.getHTTPServerPort(buckRoot)).
    catch(err => {
      (0, (_log4js || _load_log4js()).getLogger)('nuclide-buck').warn(
      `Failed to get httpPort for ${(_nuclideUri || _load_nuclideUri()).default.getPath(buckRoot)}`,
      err);
    ...
    switchMap(httpPort => {
      ...
      if (httpPort > 0) {
        socketEvents = (0, (_BuckEventStream || _load_BuckEventStream()).getEventsFromSocket)(
      ...
      if (socketEvents == null) {
        ...
        httpRecommendation = (0, (_tasks || _load_tasks()).createMessage)(
        'For better logs, set httpserver.port in your Buck config and restart Nuclide.',
        'info');

      } else {
        mergedEvents = (0, (_BuckEventStream || _load_BuckEventStream()).combineEventStreams)(
        subcommand,
        socketEvents,
        processEvents).
        share();
      ...
      mergedEvents,
      (_featureConfig || _load_featureConfig()).default.get('nuclide-buck.compileErrorDiagnostics') ?
      (0, (_BuckEventStream || _load_BuckEventStream()).getDiagnosticEvents)(mergedEvents, buckRoot) :
      _rxjsBundlesRxMinJs.Observable.empty(),
  ...
  buckRoot)
  {
    ...
    events.flatMap(event => {
      if (event.type === 'progress') {
        ...
        return _rxjsBundlesRxMinJs.Observable.concat(
        (0, (_tasks || _load_tasks()).createMessage)(`Target: ${target}`, 'log'),
        (0, (_tasks || _load_tasks()).createMessage)(`Output: ${path}`, 'log'),
        (0, (_tasks || _load_tasks()).createMessage)(`Success type: ${successType}`, 'log'),
        (0, (_tasks || _load_tasks()).createResult)(Object.assign({},
        event.output, {
