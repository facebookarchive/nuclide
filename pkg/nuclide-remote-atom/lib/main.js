...
class Activation {
  ...
  constructor() {
    ...
    this._commands = {
       ...
       * programmatically.
       */
      getClientConnections(hostname) {
        return (0, _asyncToGenerator.default)(function* () {
          return [{
            rootFolders: atom.project.getPaths().filter(function (uri) {
              return (_nuclideUri || _load_nuclideUri()).default.isRemote(uri) && (_nuclideUri || _load_nuclideUri()).default.getHostname(uri) === hostname;
            })
          }];
        })();
      },
