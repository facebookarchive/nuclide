...
function syncFolder(from, to) {
  return (0, _observable().splitStream)((0, _process().observeProcessRaw)('rsync', ['-rtvuc', '--delete', '--progress', '--info=progress2', from, to]).concatMap(msg => msg.kind === 'stdout' ? _RxMin.Observable.of(msg.data) : _RxMin.Observable.empty()).map(data => data.replace(/\r/g, '\n'))).concatMap(line => {
