...
var _commonsAtomObserveLanguageTextEditors;

function _load_commonsAtomObserveLanguageTextEditors() {
  return _commonsAtomObserveLanguageTextEditors = _interopRequireDefault(require('../../commons-atom/observe-language-text-editors'));
}
...
var ObjectiveCBracketBalancer = (function () {
  ...
  _createClass(ObjectiveCBracketBalancer, [{
    ...
    value: function enable() {
      ...
      this._languageListener = (0, (_commonsAtomObserveLanguageTextEditors || _load_commonsAtomObserveLanguageTextEditors()).default)(GRAMMARS, function (textEditor) {
    ...
    value: function _enableInTextEditor(textEditor) {
      var insertTextSubscription = textEditor.onDidInsertText(function (event) {
        (0, (_nuclideAnalytics || _load_nuclideAnalytics()).trackOperationTiming)('objc:balance-bracket', function () {
    ...
    value: function getOpenBracketInsertPosition(buffer, closeBracketPosition) {
      ...
      if (stringLiteralMatch) {
        return (_atom || _load_atom()).Point.fromObject([closeBracketPosition.row, stringLiteralMatch.index]);
        ...
        if (currentRowPlusOne !== null && currentRowPlusOne !== closeBracketPosition.row) {
          ...
          if (targetLine[targetMatch.index] === '[') {
            ...
            return (_atom || _load_atom()).Point.fromObject([currentRowPlusOne, targetMatch.index]);
          }
        } else {
          ...
          return (_atom || _load_atom()).Point.fromObject([closeBracketPosition.row, column]);
        }
      } else {
        return null;
      }
