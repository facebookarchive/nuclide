...
var Bridge = (function () {
  function Bridge(debuggerModel) {
    ...
    this._disposables = new _atom.CompositeDisposable(debuggerModel.getBreakpointStore().onChange(this._handleBreakpointStoreChange.bind(this)));
  ...
  _createClass(Bridge, [{
    ...
    value: function setWebviewElement(webview) {
      ...
      this._cleanupDisposables.add(new _atom.Disposable(function () {
    ...
    value: function _setSelectedCallFrameLine(nullableOptions) {
      ...
      if (nullableOptions) {
        (function () {
          ...
          var path = _nuclideRemoteUri2.default.uriToNuclideUri(options.sourceURL);
    ...
    value: function _addBreakpoint(location) {
      var path = _nuclideRemoteUri2.default.uriToNuclideUri(location.sourceURL);
      // only handle real files for now.
      if (path) {
        try {
      ...
      if (webview && !this._suppressBreakpointSync) {
        (function () {
          ...
          _this2._debuggerModel.getBreakpointStore().getAllBreakpoints().forEach(function (line, key) {
            results.push({
              sourceURL: _nuclideRemoteUri2.default.nuclideUriToUri(key),
              lineNumber: line
            });
          });
