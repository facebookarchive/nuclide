...
function registerConsoleExecutor(watchExpressionStore, registerExecutor) {
  ...
  var send = function send(expression) {
    disposables.add(new (_commonsNodeStream2 || _commonsNodeStream()).DisposableSubscription(watchExpressionStore.evaluateWatchExpression(expression).subscribe(rawOutput)));
  };
  var output = rawOutput.filter(function (result) {
    return result != null;
  }).map(function (result) {
