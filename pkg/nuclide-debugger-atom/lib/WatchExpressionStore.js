...
var WatchExpressionStore = (function () {
  ...
  _createClass(WatchExpressionStore, [{
    ...
    value: function _requestActionFromBridge(subject, callback) {
      return new (_commonsNodeStream2 || _commonsNodeStream()).DisposableSubscription((_rxjs2 || _rxjs()).default.Observable.fromPromise(callback()).merge((_rxjs2 || _rxjs()).default.Observable.never()).subscribe(subject));
    }
  }, {
    ...
    value: function _requestExpressionEvaluation(expression, subject) {
      ...
      var evaluationDisposable = this._requestActionFromBridge(subject, function () {
        return _this._bridge.evaluateOnSelectedCallFrame(expression);
      });
      this._previousEvaluationSubscriptions.add(evaluationDisposable);
    }
  }, {
    key: '_requestPropertiesForObjectId',
    value: function _requestPropertiesForObjectId(objectId, subject) {
      var _this2 = this;

      this._requestActionFromBridge(subject, function () {
        return _this2._bridge.getProperties(objectId);
      });
    }
