...
function _load_UniversalDisposable() {return _UniversalDisposable = _interopRequireDefault(require('nuclide-commons/UniversalDisposable'));}var _nuclideRemoteConnection;
function _load_nuclideRemoteConnection() {return _nuclideRemoteConnection = require('../../nuclide-remote-connection');}var _nuclideProjectionist;
function _load_nuclideProjectionist() {return _nuclideProjectionist = _interopRequireDefault(require('../../nuclide-projectionist'));}var _event;
...
class ProjectionistFileFamilyProvider {
  ...
  constructor(cwdApis) {
    ...
    (0, (_event || _load_event()).observableFromSubscribeFunction)(cwdApi.observeCwd.bind(cwdApi))).
    ...
    _rxjsBundlesRxMinJs.Observable.of([null, cwd]) :
  ...
  getRelatedFiles(path) {var _this = this;return (0, _asyncToGenerator.default)(function* () {
      ...
      (_nuclideUri || _load_nuclideUri()).default.relative(cwd, path));


      const files = new Map([
      [path, { labels: new Set() }],
      ...alternates.map(function (alternate) {
        const type = projectionist.getType(alternate);
        return [
        (_nuclideUri || _load_nuclideUri()).default.resolve(cwd, alternate),
        {
