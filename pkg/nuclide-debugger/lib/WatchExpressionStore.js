...
var WatchExpressionStore = (function () {
  function WatchExpressionStore(dispatcher, bridge) {
    ...
    // `this._disposables`.
    this._previousEvaluationSubscriptions = new (_commonsNodeUniversalDisposable || _load_commonsNodeUniversalDisposable()).default();
  ...
  _createClass(WatchExpressionStore, [{
    ...
    value: function getProperties(objectId) {
      ...
      return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.fromPromise(getPropertiesPromise);
    }
  }, {
    ...
    value: function _evaluateExpression(expression, supportRepl) {
      if (!supportRepl && this._watchExpressions.has(expression)) {
        ...
        (0, (_assert || _load_assert()).default)(cachedResult);
        return cachedResult;
      }
      var subject = new (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).BehaviorSubject();
    ...
    value: function _requestExpressionEvaluation(expression, subject, supportRepl) {
      ...
      var evaluationDisposable = new (_commonsNodeUniversalDisposable || _load_commonsNodeUniversalDisposable()).default((_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.fromPromise(evaluationPromise).merge((_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.never()) // So that we do not unsubscribe `subject` when disposed.
      .subscribe(subject));
    ...
    value: _asyncToGenerator(function* (command) {
      ...
      var deferred = new (_commonsNodePromise || _load_commonsNodePromise()).Deferred();
