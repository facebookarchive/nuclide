...
let BreakpointDisplayController = class BreakpointDisplayController {
  ...
  constructor(delegate, breakpointStore, editor, debuggerActions) {
    ...
    this._gutter = gutter;
    this._registerGutterMouseHandlers(atom.views.getView(editor).component.gutterContainerComponent.getDomNode());

    this._disposables.add(gutter.onDidDestroy(this._handleGutterDestroyed.bind(this)), this._breakpointStore.onNeedUIUpdate(this._handleBreakpointsChanged.bind(this)), this._editor.onDidDestroy(this._handleTextEditorDestroyed.bind(this)));
    this._update();
  }

  _registerGutterMouseHandlers(gutterView) {
