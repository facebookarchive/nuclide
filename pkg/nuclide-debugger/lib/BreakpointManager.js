...
var BreakpointManager = (function () {
  function BreakpointManager(store, debuggerActions) {
    ...
    this._disposables = new (_atom || _load_atom()).CompositeDisposable(atom.workspace.observeTextEditors(this._handleTextEditor.bind(this)));
  }
  ...
  _createClass(BreakpointManager, [{
    ...
    value: function _handleTextEditor(editor) {
      if (!this._displayControllers.has(editor)) {
        var controller = new (_BreakpointDisplayController || _load_BreakpointDisplayController()).default(this, this._breakpointStore, editor, this._debuggerActions);
