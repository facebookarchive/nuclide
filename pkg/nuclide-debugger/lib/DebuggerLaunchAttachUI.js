...
class DebuggerLaunchAttachUI extends _react.default.Component {
  ...
  constructor(props) {
    ...
    this._setConfigValid = this._setConfigValid.bind(this);
  ...
  render() {
    ...
    if (tabs.length > 0) {
      const selectedTab = this.state.selectedProviderTab != null ? this.state.selectedProviderTab : tabs[0].name;
      const provider = this.state.enabledProviders.find(p => p.debuggerName === selectedTab);
