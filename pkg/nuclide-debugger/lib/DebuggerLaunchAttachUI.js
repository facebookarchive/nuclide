  ...
  _createClass(DebuggerLaunchAttachUI, [{
    ...
    value: function render() {
      var connectionItems = this.state.connections.map(function (connection, index) {
        return {
          label: (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.isRemote(connection) ? (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.getHostname(connection) : connection,
          value: index
        };
      });
    ...
    value: function _resetElement(provider, action) {
      ...
      if (element != null) {
        element = (_reactForAtom || _load_reactForAtom()).React.cloneElement(element, { key: provider.getUniqueKey() });
      }
      this.setState({
        element: element
      });
