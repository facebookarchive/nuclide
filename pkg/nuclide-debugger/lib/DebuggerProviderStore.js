  ...
  _createClass(DebuggerProviderStore, [{
    ...
    value: function _handlePayload(payload) {
      switch (payload.actionType) {
        case (_DebuggerDispatcher2 || _DebuggerDispatcher()).ActionTypes.ADD_DEBUGGER_PROVIDER:
          if (this._debuggerProviders.has(payload.data)) {
            return;
          }
          this._debuggerProviders.add(payload.data);
          break;
        case (_DebuggerDispatcher2 || _DebuggerDispatcher()).ActionTypes.REMOVE_DEBUGGER_PROVIDER:
          if (!this._debuggerProviders.has(payload.data)) {
            return;
          }
          this._debuggerProviders.delete(payload.data);
          break;
        case (_DebuggerDispatcher2 || _DebuggerDispatcher()).ActionTypes.UPDATE_CONNECTIONS:
