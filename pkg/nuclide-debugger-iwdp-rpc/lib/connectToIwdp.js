...
function connectToIwdp() {
  return (0, (_process || _load_process()).observeProcess)(() => {
    ...
    if (message.kind === 'stdout') {
      ...
      if (matches != null) {
        ...
        return _rxjsBundlesRxMinJs.Observable.interval(POLLING_INTERVAL).switchMap(() => {
          return _rxjsBundlesRxMinJs.Observable.fromPromise(fetchDeviceData(port));
        });
      }
  ...
  }).mergeMap(deviceInfos => _rxjsBundlesRxMinJs.Observable.from(deviceInfos)).distinctKey('webSocketDebuggerUrl');
}