...
class TerminalView {
  ...
  constructor(paneUri) {
    ...
    const terminal = this._terminal = new (_xterm || _load_xterm()).Terminal({
      ...
      fontFamily: (_featureConfig || _load_featureConfig()).default.get(FONT_FAMILY_CONFIG),
      fontSize: (_featureConfig || _load_featureConfig()).default.get(FONT_SIZE_CONFIG),
      lineHeight: (_featureConfig || _load_featureConfig()).default.get(LINE_HEIGHT_CONFIG)
    });
    ...
    }), atom.commands.add(div, ADD_ESCAPE_COMMAND, this._addEscapePrefix.bind(this)), atom.commands.add(div, 'nuclide-terminal:clear', this._clear.bind(this)), (_featureConfig || _load_featureConfig()).default.observeAsStream(CURSOR_STYLE_CONFIG).skip(1).subscribe(cursorStyle => terminal.setOption('cursorStyle', cursorStyle)), (_featureConfig || _load_featureConfig()).default.observeAsStream(CURSOR_BLINK_CONFIG).skip(1).subscribe(cursorBlink => terminal.setOption('cursorBlink', cursorBlink)), (_featureConfig || _load_featureConfig()).default.observeAsStream(SCROLLBACK_CONFIG).skip(1).subscribe(scrollback => terminal.setOption('scrollback', scrollback)), (_featureConfig || _load_featureConfig()).default.observeAsStream(FONT_SIZE_CONFIG).skip(1).subscribe(fontSize => terminal.setOption('fontSize', fontSize)), (_featureConfig || _load_featureConfig()).default.observeAsStream(FONT_FAMILY_CONFIG).skip(1).subscribe(fontFamily => terminal.setOption('fontFamily', fontFamily)), (_featureConfig || _load_featureConfig()).default.observeAsStream(LINE_HEIGHT_CONFIG).skip(1).subscribe(lineHeight => terminal.setOption('lineHeight', lineHeight)), _rxjsBundlesRxMinJs.Observable.merge(_rxjsBundlesRxMinJs.Observable.fromEvent(this._terminal, 'focus'), _rxjsBundlesRxMinJs.Observable.fromEvent(window, 'resize'), new (_observableDom || _load_observableDom()).ResizeObservable(this._div)).subscribe(this._fitAndResize.bind(this)));
