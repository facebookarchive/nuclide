...
function computeAllParentDirectories(directories, modifiedPath, pathPrefixToSkip, operation) {
  ...
  var stopPrefix = pathPrefixToSkip == null ? '' : (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.ensureTrailingSeparator(pathPrefixToSkip);
  do {
    current = (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.ensureTrailingSeparator((_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.dirname(current));
  ...
  } while (!(_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.isRoot(current));
}
