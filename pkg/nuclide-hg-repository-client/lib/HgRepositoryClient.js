...
function filterForOnlyNotIgnored(code) {
  return code !== (_nuclideHgRpcLibHgConstants || _load_nuclideHgRpcLibHgConstants()).StatusCodeId.IGNORED;
}
...
var HgRepositoryClient = (function () {
  function HgRepositoryClient(repoPath, hgService, options) {
    ...
    this._serializedRefreshStatusesCache = (0, (_commonsNodeDebounce || _load_commonsNodeDebounce()).default)((0, (_commonsNodePromise || _load_commonsNodePromise()).serializeAsyncCall)(this._refreshStatusesOfAllFilesInCache.bind(this)), STATUS_DEBOUNCE_DELAY_MS);
    ...
    var serializedUpdateChangedPaths = (0, (_commonsNodeDebounce || _load_commonsNodeDebounce()).default)((0, (_commonsNodePromise || _load_commonsNodePromise()).serializeAsyncCall)(function () {
    ...
    var shouldRevisionsUpdate = (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.merge(fileChanges, repoStateChanges, activeBookmarkChanges, allBookmarChanges);
  ...
  _createClass(HgRepositoryClient, [{
    ...
    value: function getDirectoryStatus(directoryPath) {
      ...
      var directoryPathWithSeparator = (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.normalizeDir(directoryPath);
      if (this._modifiedDirectoryCache.has(directoryPathWithSeparator)) {
        return (_nuclideHgRpcLibHgConstants || _load_nuclideHgRpcLibHgConstants()).StatusCodeNumber.MODIFIED;
      }
      return (_nuclideHgRpcLibHgConstants || _load_nuclideHgRpcLibHgConstants()).StatusCodeNumber.CLEAN;
    }
    ...
    value: _asyncToGenerator(function* (paths, options) {
      ...
      paths.forEach(function (filePath) {
        ...
        if (statusId) {
          ...
          statusMap.set(filePath, (_nuclideHgRpcLibHgConstants || _load_nuclideHgRpcLibHgConstants()).StatusCodeIdToNumber[statusId]);
        } else {
      ...
      if (pathsWithCacheMiss.length) {
        ...
        newStatusInfo.forEach(function (status, filePath) {
          statusMap.set(filePath, (_nuclideHgRpcLibHgConstants || _load_nuclideHgRpcLibHgConstants()).StatusCodeIdToNumber[status]);
        });
      }
      return statusMap;
    })
    ...
    value: _asyncToGenerator(function* (filePaths, options) {
      ...
      statusMapPathToStatusId.forEach(function (newStatusId, filePath) {
        ...
        if (oldStatus && oldStatus !== newStatusId || !oldStatus && newStatusId !== (_nuclideHgRpcLibHgConstants || _load_nuclideHgRpcLibHgConstants()).StatusCodeId.CLEAN) {
          statusChangeEvents.push({
            path: filePath,
            pathStatus: (_nuclideHgRpcLibHgConstants || _load_nuclideHgRpcLibHgConstants()).StatusCodeIdToNumber[newStatusId]
          });
          if (newStatusId === (_nuclideHgRpcLibHgConstants || _load_nuclideHgRpcLibHgConstants()).StatusCodeId.CLEAN) {
            ...
            if (newStatusId === (_nuclideHgRpcLibHgConstants || _load_nuclideHgRpcLibHgConstants()).StatusCodeId.MODIFIED) {
      ...
      if (hgStatusOption === (_nuclideHgRpcLibHgConstants || _load_nuclideHgRpcLibHgConstants()).HgStatusOption.ONLY_IGNORED) {
        queriedFiles.forEach(function (filePath) {
          if (_this3._hgStatusCache[filePath] === (_nuclideHgRpcLibHgConstants || _load_nuclideHgRpcLibHgConstants()).StatusCodeId.IGNORED) {
            delete _this3._hgStatusCache[filePath];
          }
        });
      } else if (hgStatusOption === (_nuclideHgRpcLibHgConstants || _load_nuclideHgRpcLibHgConstants()).HgStatusOption.ALL_STATUSES) {
        ...
        queriedFiles.forEach(function (filePath) {
          ...
          delete _this3._hgStatusCache[filePath];
          if (cachedStatusId === (_nuclideHgRpcLibHgConstants || _load_nuclideHgRpcLibHgConstants()).StatusCodeId.MODIFIED) {
            _this3._removeAllParentDirectoriesFromCache(filePath);
          }
        });
      } else {
        queriedFiles.forEach(function (filePath) {
          var cachedStatusId = _this3._hgStatusCache[filePath];
          if (cachedStatusId !== (_nuclideHgRpcLibHgConstants || _load_nuclideHgRpcLibHgConstants()).StatusCodeId.IGNORED) {
            delete _this3._hgStatusCache[filePath];
            if (cachedStatusId === (_nuclideHgRpcLibHgConstants || _load_nuclideHgRpcLibHgConstants()).StatusCodeId.MODIFIED) {
              _this3._removeAllParentDirectoriesFromCache(filePath);
            }
    ...
    value: function _addAllParentDirectoriesToCache(filePath) {
      (0, (_utils || _load_utils()).addAllParentDirectoriesToCache)(this._modifiedDirectoryCache, filePath, this._projectDirectory.getParent().getPath());
    }
  }, {
    key: '_removeAllParentDirectoriesFromCache',
    value: function _removeAllParentDirectoriesFromCache(filePath) {
      (0, (_utils || _load_utils()).removeAllParentDirectoriesFromCache)(this._modifiedDirectoryCache, filePath, this._projectDirectory.getParent().getPath());
    }
    ...
    */
    ...
    value: _asyncToGenerator(function* (changedPaths) {
      ...
      if (relevantChangedPaths.length === 0) {
        ...
        yield this._updateStatuses(relevantChangedPaths, { hgStatusOption: (_nuclideHgRpcLibHgConstants || _load_nuclideHgRpcLibHgConstants()).HgStatusOption.ALL_STATUSES });
    ...
    value: _asyncToGenerator(function* () {
      ...
      // previously modified.
      yield this._updateStatuses([this.getProjectDirectory()], { hgStatusOption: (_nuclideHgRpcLibHgConstants || _load_nuclideHgRpcLibHgConstants()).HgStatusOption.ONLY_NON_IGNORED });
    ...
    key: 'refreshStatus',
    value: _asyncToGenerator(function* () {
      var repoRoot = this.getWorkingDirectory();
      var repoProjects = atom.project.getPaths().filter(function (projPath) {
        return projPath.startsWith(repoRoot);
      });
      yield this.getStatuses(repoProjects, {
        hgStatusOption: (_nuclideHgRpcLibHgConstants || _load_nuclideHgRpcLibHgConstants()).HgStatusOption.ONLY_NON_IGNORED
      });
    })
