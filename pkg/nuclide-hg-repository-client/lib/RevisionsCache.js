 ...
 *  strict-local
 * @format
 */const FETCH_REVISIONS_TIMEOUT_MS = 50 * 1000;const FETCH_REVISIONS_RETRY_COUNT = 2; // The revisions haven't changed if the revisions' ids are the same.
...
function isEqualRevisions(revisionsFetched1, revisionsFetched2) {const { revisions: revisions1, fromFilesystem: fromFilesystem1 } =
  revisionsFetched1;
  const {
  ...
  return (
    ...
    (0, (_collection || _load_collection()).arrayEqual)(revisions1, revisions2, (revision1, revision2) => {
      return (
        revision1.id === revision2.id &&
        revision1.isHead === revision2.isHead &&
        (0, (_collection || _load_collection()).arrayEqual)(revision1.tags, revision2.tags) &&
        (0, (_collection || _load_collection()).arrayEqual)(revision1.bookmarks, revision2.bookmarks));
...
class RevisionsCache {
  ...
  constructor(hgService) {
    ...
    .let((0, (_observable || _load_observable()).fastDebounce)(FETCH_REVISIONS_DEBOUNCE_MS)).
    switchMap(() =>
    ...
    _rxjsBundlesRxMinJs.Observable.defer(() => this._fetchSmartlogRevisions()).
