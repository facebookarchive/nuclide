...
async function createCqueryService(params) {
  ...
  const cqueryFactory = async projectRoot => {
    ...
    const lsp = new (_CqueryLanguageClient().CqueryLanguageClient)(logger, fileCache, host, command, process.execPath, [require.resolve("./child/main-entry"), _nuclideUri().default.ensureTrailingSeparator(projectRoot), logFile, recordFile, String(params.enableLibclangLogs)], spawnOptions, projectRoot, EXTENSIONS, {
