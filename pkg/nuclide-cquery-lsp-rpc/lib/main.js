...
function findNearestCompilationDbDir(source) {
  return (0, _CompilationDatabaseFinder().findNearestCompilationDbDir)(source);
}
...
async function createCqueryService(params) {
  ...
  const cqueryFactory = async projectRoot => {
    ...
    const initializationOptions = (0, _CqueryInitialization().getInitializationOptions)(cacheDirectory, projectRoot, params.defaultFlags);
    ...
    const lsp = new (_CqueryLanguageClient().CqueryLanguageClient)(logger, fileCache, host, command, process.execPath, [require.resolve("./child/main-entry"), _nuclideUri().default.ensureTrailingSeparator(projectRoot), logFile, recordFile, String(params.enableLibclangLogs)], spawnOptions, projectRoot, EXTENSIONS, initializationOptions, 5 * 60 * 1000, // 5 minutes
  ...
  multiLsp.initialize(logger, fileCache, forkedHost, ['.buckconfig', _CompilationDatabaseFinder().COMPILATION_DATABASE_FILE], 'nearest', EXTENSIONS, cqueryFactory);
  return multiLsp;
}