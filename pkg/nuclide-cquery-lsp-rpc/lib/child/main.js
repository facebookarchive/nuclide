...
const DEFAULT_MEMORY_LIMIT = 30; // Time between checking cquery memory usage, in millseconds.
...
const libclangLogging = process.argv[5] === 'true'; // client reader/writer reads/writes to Nuclide.
...
function onChildSpawn(childProcess) {
  ...
  const messageHandler = new (_MessageHandler().MessageHandler)(projectRoot, serverWriter, clientWriter);
