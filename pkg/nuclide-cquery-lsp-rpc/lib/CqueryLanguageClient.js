...
class CqueryLanguageClient extends _LspLanguageService().LspLanguageService {
  ...
  async requestLocationsCommand(methodName, path, point) {
    const position = (0, _convert().atomPoint_lspPosition)(point);
    const response = await this._lspConnection._jsonRpcConnection.sendRequest(methodName, {
      textDocument: {
        uri: (0, _convert().localPath_lspUri)(path)
      },
      position
    });
    return response == null ? [] : response.map(({
      uri,
      range
    }) => ({
      uri: (0, _convert().lspUri_localPath)(uri),
