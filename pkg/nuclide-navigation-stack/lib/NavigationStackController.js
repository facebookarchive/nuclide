  ...
  _createClass(NavigationStackController, [{
    ...
    value: function _updateStackLocation(editor) {
      ...
      if (previousEditor === editor) {
        ...
        (0, (_assert || _load_assert()).default)(previousLocation != null && previousLocation.type === 'editor');
    ...
    value: function updatePosition(editor, newBufferPosition) {
      log('updatePosition ' + newBufferPosition.row + ', ' + (newBufferPosition.column + ' ' + (0, (_commonsNodeString || _load_commonsNodeString()).maybeToString)(editor.getPath())));
    ...
    value: function updateScroll(editor, scrollTop) {
      log('updateScroll ' + scrollTop + ' ' + (0, (_commonsNodeString || _load_commonsNodeString()).maybeToString)(editor.getPath()));
    ...
    value: function onCreate(editor) {
      log('onCreate ' + (0, (_commonsNodeString || _load_commonsNodeString()).maybeToString)(editor.getPath()));
    ...
    value: function onDestroy(editor) {
      log('onDestroy ' + (0, (_commonsNodeString || _load_commonsNodeString()).maybeToString)(editor.getPath()));
    ...
    value: function onOpen(editor) {
      log('onOpen ' + (0, (_commonsNodeString || _load_commonsNodeString()).maybeToString)(editor.getPath()));
    ...
    value: function onActivate(editor) {
      log('onActivate ' + (0, (_commonsNodeString || _load_commonsNodeString()).maybeToString)(editor.getPath()));
    ...
    value: function onActiveStopChanging(editor) {
      log('onActivePaneStopChanging ' + (0, (_commonsNodeString || _load_commonsNodeString()).maybeToString)(editor.getPath()));
      this._inActivate = false;
    }
  }, {
    key: 'onOptInNavigation',
    value: function onOptInNavigation(editor) {
      log('onOptInNavigation ' + (0, (_commonsNodeString || _load_commonsNodeString()).maybeToString)(editor.getPath()));
    ...
    value: function removePath(removedPath, remainingDirectories) {
      ...
      this._navigationStack.filter(function (location) {
        var uri = (0, (_Location || _load_Location()).getPathOfLocation)(location);
        return uri == null || !(_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.contains(removedPath, uri) || remainingDirectories.find(function (directory) {
          return (_commonsNodeNuclideUri || _load_commonsNodeNuclideUri()).default.contains(directory, uri);
        }) != null;
      });
    }
  }, {
    ...
    value: _asyncToGenerator(function* (location) {
      (0, (_assert || _load_assert()).default)(!this._isNavigating);
      if (location == null) {
        return;
      }
      ...
      try {
        var editor = yield (0, (_Location || _load_Location()).editorOfLocation)(location);
        ...
        (0, (_commonsAtomTextEditor || _load_commonsAtomTextEditor()).setPositionAndScroll)(editor, location.bufferPosition, location.scrollTop);
      } finally {
        this._isNavigating = false;
      }
    })
    ...
    key: 'observeStackChanges',
    value: function observeStackChanges() {
      return (_rxjsBundlesRxMinJs || _load_rxjsBundlesRxMinJs()).Observable.of(this._navigationStack).concat(this._navigationStack.observeChanges());
    }
