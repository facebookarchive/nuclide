var doGetBlameForEditor = _asyncToGenerator(function* (editor) {
  ...
  var repo = (0, (_common || _load_common()).hgRepositoryForEditor)(editor);
  if (!repo) {
  ...
  var useShortName = !(_commonsAtomFeatureConfig || _load_commonsAtomFeatureConfig()).default.get('nuclide-blame-provider-hg.showVerboseBlame');
...
function getBlameForEditor(editor) {
  return (0, (_nuclideAnalytics || _load_nuclideAnalytics()).trackOperationTiming)('blame-provider-hg:getBlameForEditor', function () {
    return doGetBlameForEditor(editor);
  });
}

function formatBlameInfo(rawBlameData, useShortName) {
  var extractAuthor = useShortName ? (_nuclideVcsLog || _load_nuclideVcsLog()).shortNameForAuthor : identity;
