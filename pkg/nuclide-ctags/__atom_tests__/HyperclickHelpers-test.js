...
function _load_HyperclickHelpers() {
  return _HyperclickHelpers = _interopRequireDefault(require('../lib/HyperclickHelpers'));
}
...
describe('HyperclickHelpers', () => {
  ...
  beforeEach(() => {
    ...
    goToLocationSpy = jest.spyOn(require('../../../modules/nuclide-commons-atom/go-to-location'), 'goToLocation');
  ...
  it('works with multiple tag results', async () => {
    await (async () => {
      ...
      const result = await (_HyperclickHelpers || _load_HyperclickHelpers()).default.getSuggestionForWord(fakeEditor, 'A', new _atom.Range([0, 0], [0, 1]));
  ...
  it('directly jumps for single results', async () => {
    await (async () => {
      ...
      const result = await (_HyperclickHelpers || _load_HyperclickHelpers()).default.getSuggestionForWord(fakeEditor, 'test', new _atom.Range([0, 0], [0, 1]));
