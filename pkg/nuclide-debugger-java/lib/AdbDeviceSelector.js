...
exports.AdbDeviceSelector = exports.DEFAULT_ADB_PORT = undefined;
...
class AdbDeviceSelector extends _react.Component {
  ...
  componentDidMount() {
    this._disposables.add((0, (_DevicePoller || _load_DevicePoller()).observeAndroidDevicesX)(this.props.targetUri).startWith((_expected || _load_expected()).Expect.pendingValue([])).distinctUntilChanged((a, b) => {
  ...
  _getDeviceItems() {
    ...
    return this.state.deviceList.value.map(device => ({
      value: device,
      label: device.port === DEFAULT_ADB_PORT ? device.displayName : `${device.displayName} on ADB port ${device.port}`
    }));
  }
