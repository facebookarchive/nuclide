...
function _HackDiagnosticsProvider() {
  return _HackDiagnosticsProvider2 = require('./HackDiagnosticsProvider');
}
...
function activate() {
  ...
  if ((0, (_config2 || _config()).getConfig)().useIdeConnection) {
...
function provideDiagnostics() {
  if (!hackDiagnosticsProvider) {
    ...
    hackDiagnosticsProvider = new (_HackDiagnosticsProvider2 || _HackDiagnosticsProvider()).HackDiagnosticsProvider(false, busyProvider);
  }
...
function provideObservableDiagnostics() {
  if (observableDiagnosticsProvider == null) {
    observableDiagnosticsProvider = new (_HackDiagnosticsProvider2 || _HackDiagnosticsProvider()).ObservableDiagnosticProvider();
  }
