...
function _load_isPlainObject() {
  return _isPlainObject2 = _interopRequireDefault(require('lodash/isPlainObject'));
}
  ...
  atom.project.replace = projectSpecification => {
    if (projectSpecification == null) {
      ...
      if (!Array.isArray(projectSpecification.paths)) {
        projectSpecification.paths = [(_nuclideUri || _load_nuclideUri()).default.dirname(projectSpecification.originPath)];
      }
      // $FlowIgnore
...
const updateConfigGet = () => {
  ...
  atom.config.getRawValue = (keyPath, options = {}) => {
    ...
    if (value != null) {
      ...
      if ((0, (_isPlainObject2 || _load_isPlainObject()).default)(value) && (0, (_isPlainObject2 || _load_isPlainObject()).default)(defaultValue)) {
        // $FlowIgnore
  ...
  atom.config.set = (...args) => {
    ...
    if (
    // $FlowFixMe(>=0.68.0) Flow suppress (T27187857)
    source === atom.config.mainSource && shouldSave &&
    // $FlowIgnore
  ...
  atom.config.getRawScopedValue = (scopeDescriptor, keyPath, options) => {
    ...
    // $FlowIgnore
    const result = atom.config.scopedSettingsStore.getPropertyValue(newScopeDescriptor.getScopeChain(), keyPath, options);
    // $FlowIgnore
  ...
  atom.config.getAll = (keyPath, options = {}) => {
    ...
    if (scope != null) {
      ...
      // $FlowIgnore
      result = atom.config.scopedSettingsStore.getAll(scopeDescriptor.getScopeChain(), keyPath, options);
      // $FlowIgnore
...
const withoutEmptyObjects = o => {
  const object = o;
  let resultObject;
  if ((0, (_isPlainObject2 || _load_isPlainObject()).default)(object)) {
