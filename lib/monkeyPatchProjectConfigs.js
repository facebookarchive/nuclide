...
exports.default = monkeyPatchProjectConfigs;

function _isPlainObject2() {
  const data = _interopRequireDefault(require("lodash/isPlainObject"));

  _isPlainObject2 = function () {
    return data;
  };
...
function monkeyPatchProjectConfigs() {
  ...
  if (replaceProject != null) {
    atom.project.replace = projectSpecification => {
      let formatted = Object.assign({}, projectSpecification);

      if (formatted.atom != null) {
        formatted = Object.assign({}, formatted, formatted.atom);
  ...
  atom.project.replace = projectSpecification => {
    if (projectSpecification == null) {
      ...
      if (!Array.isArray(projectSpecification.paths)) {
        projectSpecification.paths = [_nuclideUri().default.dirname(projectSpecification.originPath)];
      } // $FlowIgnore


      atom.config.resetProjectSettings(projectSpecification.config, projectSpecification.originPath); // $FlowIgnore
...
const updateConfigGet = () => {
  // $FlowIgnore
  atom.config.projectFile = null; // $FlowIgnore

  atom.config.projectSettings = {}; // $FlowIgnore

  atom.config.getRawValue = (keyPath, options = {}) => {
    let value;

    if (!options.excludeSources || // $FlowIgnore
    !options.excludeSources.includes(atom.config.mainSource)) {
      // $FlowIgnore
      value = getValueAtKeyPath(atom.config.settings, keyPath); // $FlowFixMe(>=0.68.0) Flow suppress (T27187857)

      if (atom.config.projectFile != null) {
        const projectValue = getValueAtKeyPath( // $FlowIgnore
        atom.config.projectSettings, keyPath);
    ...
    if (value != null) {
      ...
      if ((0, _isPlainObject2().default)(value) && (0, _isPlainObject2().default)(defaultValue)) {
        // $FlowIgnore
  ...
  atom.config.get = (...args) => {
    ...
    if (scope != null) {
      // $FlowIgnore
      const value = atom.config.getRawScopedValue(scope, keyPath, options); // $FlowIgnore
  ...
  atom.config.set = (...args) => {
    // eslint-disable-next-line
    let [keyPath, value, options = {}] = args; // $FlowIgnore
    ...
    const shouldSave = options.save != null ? options.save : true; // $FlowIgnore
    ...
    if (scopeSelector != null) {
      ...
      // $FlowIgnore
      atom.config.setRawValue(keyPath, value, {
        source
      });
    }

    if ( // $FlowFixMe(>=0.68.0) Flow suppress (T27187857)
    source === atom.config.mainSource && shouldSave && // $FlowIgnore
  ...
  atom.config.getRawScopedValue = (scopeDescriptor, keyPath, options) => {
    const newScopeDescriptor = ScopeDescriptor.fromObject(scopeDescriptor); // $FlowIgnore

    const result = atom.config.scopedSettingsStore.getPropertyValue(newScopeDescriptor.getScopeChain(), keyPath, options); // $FlowIgnore

    const legacyScopeDescriptor = atom.config.getLegacyScopeDescriptorForNewScopeDescriptor(newScopeDescriptor);

    if (result != null) {
      return result;
    } else if (legacyScopeDescriptor) {
      // $FlowIgnore
      return atom.config.scopedSettingsStore.getPropertyValue(legacyScopeDescriptor.getScopeChain(), keyPath, options);
    }
  }; // $FlowIgnore
  ...
  atom.config.getAll = (keyPath, options = {}) => {
    ...
    if (scope != null) {
      const scopeDescriptor = ScopeDescriptor.fromObject(scope); // $FlowIgnore

      result = atom.config.scopedSettingsStore.getAll(scopeDescriptor.getScopeChain(), keyPath, options); // $FlowIgnore

      const legacyScopeDescriptor = atom.config.getLegacyScopeDescriptorForNewScopeDescriptor(scopeDescriptor);

      if (legacyScopeDescriptor) {
        result.push(...Array.from( // $FlowIgnore
        atom.config.scopedSettingsStore.getAll(legacyScopeDescriptor.getScopeChain(), keyPath, options) || []));
      }
    } else {
      result = [];
    }

    const globalValue = atom.config.getRawValue(keyPath, options); // $FlowIgnore
  ...
  atom.config.legacyScopeAliases = new Map(); // $FlowIgnore

  atom.config.setLegacyScopeAliasForNewScope = (languageId, legacyScopeName) => {
    // $FlowIgnore
  ...
  atom.config.getLegacyScopeDescriptorForNewScopeDescriptor = scopeDescriptor => {
    ...
    const legacyAlias = atom.config.legacyScopeAliases.get( // $FlowIgnore
...
const addConfigResetProjectSettings = () => {
  ...
  atom.config.resetScopedSettings = (newScopedSettings, options = {}) => {
    const source = // $FlowIgnore
    options.source == null ? atom.config.mainSource : options.source; // $FlowIgnore

    const priority = atom.config.priorityForSource(source); // $FlowIgnore

    atom.config.scopedSettingsStore.removePropertiesForSource(source);

    for (const scopeSelector in newScopedSettings) {
      let settings = newScopedSettings[scopeSelector]; // $FlowIgnore
      ...
      if (validatedSettings[scopeSelector] != null) {
        // $FlowIgnore
        atom.config.scopedSettingsStore.addProperties(source, validatedSettings, {
          priority
        });
      }
    } // $FlowIgnore
  ...
  atom.config.resetUserScopedSettings = atom.config.resetScopedSettings; // $FlowIgnore

  atom.config._resetSettings = (ns, options = {}) => {
    let newSettings = ns;
    const source = options.source; // eslint-disable-next-line
    ...
    if (newSettings['*'] != null) {
      ...
      delete scopedSettings['*']; // $FlowIgnore

      atom.config.resetScopedSettings(scopedSettings, {
        source
      });
    } // $FlowIgnore
    ...
    return atom.config.transact(() => {
      // $FlowIgnore
      atom.config._clearUnscopedSettingsForSource(source); // $FlowIgnore
  ...
  atom.config.resetProjectSettings = (newSettings, projectFile) => {
    ...
    // eslint-disable-next-line
    newSettings = Object.assign({}, newSettings); // $FlowIgnore

    const oldProjectFile = atom.config.projectFile; // $FlowIgnore
    ...
    if (atom.config.projectFile != null) {
      ...
      // $FlowIgnore
      atom.config.scopedSettingsStore.removePropertiesForSource(oldProjectFile); // $FlowIgnore
...
const getValueAtKeyPath = (o, keyPath) => {
  ...
  for (let i = 0, len = keys.length; i < len; i++) {
    const key = keys[i]; // eslint-disable-next-line
