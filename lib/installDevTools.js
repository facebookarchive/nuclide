 ...
 * @format
 */
// $FlowFixMe Flow is convinced that perf_hooks is Not A Thing
  ...
  for (const extensionID of extensionIDsToLoad) {
    ...
    if (latest != null && latestManifest != null) {
      try {
        ...
        log(`Successfully loaded Chrome extension "${latestManifest.name} in ${_perf_hooks.performance.now() - start}ms - ${latest.dirname}"`);
