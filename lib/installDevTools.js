...
const log = console.debug || console.log;let installed;function installDevTools() {const disposable = new (_UniversalDisposable || _load_UniversalDisposable()).default(new (_ReactPerfMonitor || _load_ReactPerfMonitor()).default());if (installed) {return disposable;}const chromeHome = getChromeHomeDir();const profileDirs = getProfileDirs(chromeHome);(0, (_nullthrows || _load_nullthrows()).default)(_electron.default.webFrame).registerURLSchemeAsBypassingCSP('chrome-extension');
  ...
  for (const extensionID of extensionIDsToLoad) {
    ...
    const namesWithTimes = extensionVersions.map(dirname => Object.assign({},
    _fs.default.statSync(dirname), {
      dirname }));
    ...
    try {
      ...
      (_nuclideUri || _load_nuclideUri()).default.join(latest.dirname, 'manifest.json'),
      'utf-8'));
...
function getChromeHomeDir() {
  switch (process.platform) {
    ...
    case 'win32':if (!(
      process.env.LOCALAPPDATA != null)) {throw new Error('Invariant violation: "process.env.LOCALAPPDATA != null"');}
      return (_nuclideUri || _load_nuclideUri()).default.join(
...
extensionID)
{
  for (const profileDir of profileDirs) {
    const extensionPath = (_nuclideUri || _load_nuclideUri()).default.join(
    profileDir,
    'Extensions',
    extensionID);
