...
function isNuclideInCallStack(callStack) {
  const ignoreCallSitesFromThisFile = callSite => callSite.getFileName() && !callSite.getFileName().includes('installErrorReporter.js');
...
function handleAtomNotification(notification) {
  ...
  if (isNuclideInCallStack(callStack)) {
    (0, (_log4js || _load_log4js()).getLogger)('atom-error-notification').error(notification.getMessage(), callStack);
  }
}