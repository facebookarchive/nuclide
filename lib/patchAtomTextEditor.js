...
exports.default = patchAtomTextEditor;
 ...
 * @format
 */
// TODO: Remove once fixed upstream in Atom
...
function patchAtomTextEditor() {
  if (isPatched) {
    return;
  }

  isPatched = true;
  const atomTextEditor = atom.workspace.buildTextEditor(); // getElement initializes the AtomTextEditor's component property if it does
  // not exist yet

  atomTextEditor.getElement(); // $FlowFixMe component is added by getElement() call if it doesn't exist

  if (!(atomTextEditor.component != null)) {
    throw new Error("Invariant violation: \"atomTextEditor.component != null\"");
  }

  const atomTextEditorComponentPrototype = Object.getPrototypeOf(atomTextEditor.component);
