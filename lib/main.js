...
const baseConfig = {
  ...
  // TODO(hansonw): Enable these for everyone.
  useLocalRpc: {
    default: __DEV__,
    description:
  ...
  useLocalRpcServer: {
    default: __DEV__,
    description:
      '(Experimental) Spawn a separate process for local RPC services.',
    title: 'Spawn local RPC server',
    type: 'boolean',
  },
};

// `setUseLocalRpc` can only be called once, so it's set out here during load.
const useLocalRpc = atom.config.get('nuclide.useLocalRpc');
const useLocalRpcServer = atom.config.get('nuclide.useLocalRpcServer');
// Config defaults aren't initialized until the package is loaded!
// Manually check the default value here.
setUseLocalRpc(
  useLocalRpc == null ? baseConfig.useLocalRpc.default : Boolean(useLocalRpc),
  useLocalRpcServer == null
    ? baseConfig.useLocalRpcServer.default
    : Boolean(useLocalRpcServer),
);
