...
const UPLoader = {
  ...
  activate() {
    ...
    const insertIndex = findIndex(template, item => item.role === 'window' || item.role === 'help');
    if (insertIndex !== -1) {
      const nuclideIndex = findIndex(template, item => item.label === 'Nuclide');
      const menuItem = template.splice(nuclideIndex, 1)[0];
...
function findIndex<T>(array: Array<T>, predicate: (item: T) => boolean): number {
