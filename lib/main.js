...
var _pkgCommonsAtomFeatureConfig;

function _load_pkgCommonsAtomFeatureConfig() {
  return _pkgCommonsAtomFeatureConfig = _interopRequireDefault(require('../pkg/commons-atom/featureConfig'));
}
...
var _pkgNuclideRemoteConnectionLibServiceManager;

function _load_pkgNuclideRemoteConnectionLibServiceManager() {
  return _pkgNuclideRemoteConnectionLibServiceManager = require('../pkg/nuclide-remote-connection/lib/service-manager');
}
...
function _load_packageJson() {
  return _packageJson = _interopRequireDefault(require('../package.json'));
}
...
(0, (_pkgNuclideRemoteConnectionLibServiceManager || _load_pkgNuclideRemoteConnectionLibServiceManager()).setUseLocalRpc)(_shouldUseLocalRpc);

var runningNuclideVersion = (_packageJson || _load_packageJson()).default.version;

// Nuclide packages for Atom are called "features"
var FEATURES_DIR = (_path || _load_path()).default.join(__dirname, '../pkg');
var features = {};
 ...
 */
(_fs || _load_fs()).default.readdirSync(FEATURES_DIR).forEach(function (item) {
  // Optimization: Our directories don't have periods - this must be a file
  if (item.indexOf('.') !== -1) {
    return;
  }
  var dirname = (_path || _load_path()).default.join(FEATURES_DIR, item);
  var filename = (_path || _load_path()).default.join(dirname, 'package.json');
  try {
    var stat = (_fs || _load_fs()).default.statSync(filename);
    (0, (_assert || _load_assert()).default)(stat.isFile());
  } catch (err) {
    if (err && err.code === 'ENOENT') {
      return;
    }
  }
  var src = (_fs || _load_fs()).default.readFileSync(filename, 'utf8');
  ...
  if (pkg.nuclide && pkg.nuclide.packageType === 'Atom') {
    (0, (_assert || _load_assert()).default)(pkg.name);
    features[pkg.name] = {
      pkg: pkg,
      dirname: dirname,
...
var initialLoadDisposable = atom.packages.onDidLoadPackage(function (pack) {
  ...
  (0, (_assert || _load_assert()).default)(initialLoadDisposable != null);
...
function activate() {
  ...
  (0, (_assert || _load_assert()).default)(nuclidePack != null);
  ...
  var pkgJsonPath = (_path || _load_path()).default.join(nuclidePack.path, 'package.json');
  var installedPkg = JSON.parse((_fs || _load_fs()).default.readFileSync(pkgJsonPath, 'utf8'));
  ...
  if ((_pkgCommonsAtomFeatureConfig || _load_pkgCommonsAtomFeatureConfig()).default.get('installRecommendedPackages')) {
