...
import ReactMountRootElement from '../pkg/nuclide-ui/lib/ReactMountRootElement';
import fs from 'fs';
...
const runningNuclideVersion = require('../package.json').version;
...
export function activate() {
  ...
  // method.
  const modelsToElements = new WeakMap();
  disposables.add(
    atom.views.addViewProvider(model => {
      if (typeof model.getReactElement !== 'function') { return; }
      let element = modelsToElements.get(model);
      if (element != null) { return element; }
      const reactElement = model.getReactElement();
      element = new ReactMountRootElement();
  ...
  Promise.all(atom.packages.activatePackages(loaded)).then(() => {
    // nothing to do...
  });
...
function safeDeactivate(name) {
  try {
    const pkg = atom.packages.getActivePackage(name);
    if (pkg != null) {
