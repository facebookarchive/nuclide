...
// *nix.
const MULTILINE_CHAR = (os.platform() === 'win32') ? '^' : '\\';
  ...
  if (outdatedAvailablePackages.length > 0) {
    ...
    atom.notifications.addWarning('Found Outdated Nuclide Packages', {
      ...
      description: 'Outdated Nuclide packages have been disabled, and it is recommended you'
        + ' uninstall them before using Atom+Nuclide. To uninstall them, run the following command'
        + ` from the command line and then restart Atom: <pre>${uninstallCommand}</pre>`,
      dismissable: true,
    });
  }
}
